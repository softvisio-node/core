...
export default class {
    ...
    #walletPayApi; // eslint-disable-line no-unused-private-class-members
    ...
    updateFields ( fields ) {
        if ( "wallet_pay_storage_api_key" in fields ) {
            this.#setWalletPayStorageApiKey( this.app.crypto
                .decrypt( fields.wallet_pay_storage_api_key, {
                    "encoding": "base64url",
                } )
                ?.toString( "latin1" ) );
        }
    ...
    createWalletPayWebAppButton ( ctx, orderId ) {
        return {
            ...
            "web_app": {
                "url": ctx.createWebAppUrl( "wallet-pay", {
                    orderId,
                } ),
            },
        };
    ...
    async setWalletPayStorageApiKey ( walletPaystorageApiKey ) {
        ...
        const res = await this.dbh.do( SQL.setWalletPayStorageApiKey, [
            await this.app.crypto.encrypt( walletPaystorageApiKey, {
