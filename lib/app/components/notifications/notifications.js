...
export default class {
    ...
    async sendNotification ( type, users, subject, body, options = {} ) {
        ...
        for ( const userId of users ) {
            ...
            if ( defaultChannels.internal.enabled && ( userChannels?.internal ?? defaultChannels.internal.default ) ) {
                ...
                if ( subject instanceof LocaleTemplate ) subject = subject.toString( { "domain": user.locale } );
                if ( body instanceof LocaleTemplate ) body = body.toString( { "domain": user.locale } );
    ...
    async sendInternalNotification ( users, subject, body, { maxAge } = {} ) {
        ...
        const res = await this.dbh.begin( async dbh => {
            ...
            res = await dbh.do( sql`INSERT INTO notification_internal_user`.VALUES( users.map( userId => {
                return {
                    "user_id": userId,
