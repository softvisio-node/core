...
export default class {
    ...
    async sendNotification ( type, users, subject, body, options = {} ) {
        ...
        for ( const userId of users ) {
            ...
            // internal
            if ( defaultChannels.internal.enabled && ( defaultChannels.internal.editable ? userChannels?.internal ?? defaultChannels.internal.active : defaultChannels.internal.active ) ) {
                internalUsers.push( user );
            }

            // email
            if ( defaultChannels.email.enabled && ( defaultChannels.email.editable ? userChannels?.email ?? defaultChannels.email.active : defaultChannels.email.active ) ) {
                emailUsers.push( user );
            }

            // telegram
            if ( defaultChannels.telegram.enabled && ( defaultChannels.telegram.editable ? userChannels?.telegram ?? defaultChannels.telegram.active : defaultChannels.telegram.active ) ) {
                telegramUsers.push( user.id );
            }

            // push
            if ( defaultChannels.push.enabled && ( defaultChannels.push.editable ? userChannels?.push ?? defaultChannels.push.active : defaultChannels.push.active ) ) {
                pushUsers.push( user );
            }
        }
        ...
        for ( const notification in this.#types ) {
            ...
            for ( const channel of Object.keys( this.#channels ) ) {
                notificationProfile.channels[channel] = {
                    ...
                    "active": defaultChannels[channel].editable ? userChannels?.[channel] ?? defaultChannels[channel].active : defaultChannels[channel].active,
                };
            }
        }
    ...
    async setUserNotificationActive ( { userId, notification, channel, active } = {} ) {
        if ( channel ) {
