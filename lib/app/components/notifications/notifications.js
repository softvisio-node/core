...
export default class {
    ...
    async configure () {
        ...
        for ( const component of this.app.components ) {
            if ( component.name === "notifications" ) continue;
    ...
    async sendInternalNotification ( users, subject, body, { maxAge } = {} ) {
        ...
        if ( maxAge ) {
            var expires = Duration.new( maxAge ).toDate();
        }
        ...
        for ( const user of users ) {
            ...
            locales[user.locale] ??= {
                "users": [],
                "subject": Translation.toString( subject, { "localeDomain": user.locale } ),
                "body": Translation.toString( body, { "localeDomain": user.locale } ),
            };
    ...
    async sendEmailNotification ( users, subject, body, options = {} ) {
        ...
        for ( const user of users ) {
            ...
            this.sendEmail(
                ...
                //
                user.email,
                Translation.toString( subject, { "localeDomain": user.locale } ),
                Translation.toString( body, { "localeDomain": user.locale } ),
                options
    ...
    async sendTelegramNotification ( users, subject, body, options ) {
        ...
        for ( const telegramUser of users ) {
            ...
            if ( subject ) {
                text.push( "<b>" + Translation.toString( subject, { "localeDomain": telegramUser.locale } ) + "</b>" );
            }

            if ( body ) {
                text.push( Translation.toString( body, { "localeDomain": telegramUser.locale } ) );
            }

            await telegramUser.send( "sendMessage", {
                "text": text.join( "\n\n" ),
                "parse_mode": "HTML",
            } );
        }
    ...
    async sendEmail ( to, subject, body, options = {} ) {
        ...
        // filter fake emails
        to.filter( email => !this.app.emailIsFake( email ) );
    ...
    async getLinkedTelegramUser ( userId ) {
        if ( !this.channels.telegram.supported ) return null;

        return this.#telegramBot.users.getByApiUserId( userId );
    }
