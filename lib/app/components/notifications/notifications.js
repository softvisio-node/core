...
export default class {
    ...
    async init () {
        ...
        if ( this.config.types ) {
            for ( const [type, typeConfig] of Object.entries( this.config.types ) ) {
                this.#types[type] = {
                ...
                if ( typeConfig.channels ) {
                    for ( const [channel, channelConfig] of Object.entries( typeConfig.channels ) ) {
                        this.#types[type].channels[channel] = { ...channelConfig };
                    }
                }
            }
        }
    ...
    async sendInternalNotification ( users, subject, body, { maxAge } = {} ) {
        ...
        const res = await this.dbh.begin( async dbh => {
            ...
            this.app.publishApi( "/notifications/update/", userId, { "inbox": true, "done": false } );
        } );
...
` );
        ...
        if ( res.data ) {
            for ( const row of res.data ) {
                ...
                this.app.publishApi( "/notifications/update/" + userId, row );
            }
        }
