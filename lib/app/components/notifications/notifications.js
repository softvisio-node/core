...
const SQL = {
    ...
    "setUserNotificationSubscribed": sql`INSERT INTO user_notification ( user_id, notification_id, internal, email, telegram, push ) VALUES ( ?, get_notification_id( ? ), ?, ?, ?, ? ) ON CONFLICT ( user_id, notification ) DO UPDATE SET internal = EXCLUDED.internal, email = EXCLUDED.email, telegram = EXCLUDED.telegram, push = EXCLUDED.push`.prepare(),

    "setChannelSubscribed": {
        "internal": sql`INSERT INTO user_notification ( user_id, notification_id, internal ) VALUES ( ?, get_notification_id( ? ), ? ) ON CONFLICT ( user_id, notification ) DO UPDATE SET internal = EXCLUDED.internal`.prepare(),

        "email": sql`INSERT INTO user_notification ( user_id, notification_id, email ) VALUES ( ?, get_notification_id( ? ), ? ) ON CONFLICT ( user_id, notification ) DO UPDATE SET email = EXCLUDED.email`.prepare(),

        "telegram": sql`INSERT INTO user_notification ( user_id, notification_id, telegram ) VALUES ( ?, get_notification_id( ? ), ? ) ON CONFLICT ( user_id, notification ) DO UPDATE SET telegram = EXCLUDED.telegram`.prepare(),

        "push": sql`INSERT INTO user_notification ( user_id, notification_id, push ) VALUES ( ?, get_notification_id( ? ), ? ) ON CONFLICT ( user_id, notification ) DO UPDATE SET push = EXCLUDED.push`.prepare(),
    },
};
...
export default class {
    ...
    async sendTelegramNotification ( users, subject, body, options ) {
        ...
        users = await this.#telegramBot.users.getUsersByApiUserId( users );
        if ( !users ) return result( [500, `Unable to get users`] );
