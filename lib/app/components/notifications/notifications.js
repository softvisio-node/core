...
export default class {
    ...
    async sendInternalNotification ( users, subject, body, { maxAge } = {} ) {
        ...
        for ( const user of users ) {
            ...
            locales[user.locale] ??= {
                "users": [],
                "subject": Translation.translate( subject, { "domain": user.locale } ),
                "body": Translation.translate( body, { "domain": user.locale } ),
            };
    ...
    async sendEmailNotification ( users, subject, body, options = {} ) {
        ...
        for ( const user of users ) {
            ...
            this.sendEmail(
                ...
                //
                user.email,
                Translation.translate( subject, { "domain": user.locale } ),
                Translation.translate( body, { "domain": user.locale } ),
                options
    ...
    async sendTelegramNotification ( users, subject, body, options ) {
        ...
        for ( const telegramUser of users ) {
            const msg = Translation.translate( subject, { "domain": telegramUser.locale } ) + "\n\n" + Translation.translate( body, { "domain": telegramUser.locale } );
