...
const SQL = {
    ...
    "setChannelEnabled": {
        ...
        "email": sql`INSERT INTO user_notification_profile ( user_id, notification, email ) VALUES ( ?, ?, ? ) ON CONFLICT ( user_id, notification ) DO UPDATE SET internal = EXCLUDED.email`.prepare(),

        "telegram": sql`INSERT INTO user_notification_profile ( user_id, notification, telegram ) VALUES ( ?, ?, ? ) ON CONFLICT ( user_id, notification ) DO UPDATE SET internal = EXCLUDED.telegram`.prepare(),

        "push": sql`INSERT INTO user_notification_profile ( user_id, notification, push ) VALUES ( ?, ?, ? ) ON CONFLICT ( user_id, notification ) DO UPDATE SET internal = EXCLUDED.push`.prepare(),
    },
};
...
export default class {
    ...
    get internalNotificationsEnabled () {
        return this.#channels.internal.enabled;
    }
    ...
    async configure () {
        ...
        this.#channels.internal.enabled = this.config.internalNotificationsEnabled;

        // email notifications
        if ( this.config.smtp ) {
            this.#channels.email.supported = true;
            this.#channels.email.enabled = this.config.emailNotificationsEnabled;
        ...
        if ( this.config.telegramBot.apiKey ) {
            ...
            this.#channels.telegram.enabled = this.config.telegramNotificationsEnabled;
        }
        else {
        ...
        if ( this.config.firebase?.serviceAccount ) {
            ...
            this.#channels.push.enabled = this.config.pushNotificationsEnabled;
    ...
    // XXX rename types -> notifications
    async getUserNotificationsProfile ( userId ) {
        ...
        if ( this.telegramSupported ) {
            profile.telegramBotUrl = "https://t.me/" + this.telegramBotUsername;
            profile.linkedTelegramUsername = await this.getLinkedTelegramUsername( userId );
        }
        ...
        for ( const type in this.#types ) {
            ...
            for ( const channel of Object.keys( this.#channels ) ) {
                typeProfile.channels[channel] = {
                    "supported": defaultChannels[channel].enabled, // XXX ??? enabled???
    ...
    async setUserNotificationChannelEnabled ( userId, notification, channel, enabled ) {
        ...
        return this.dbh.do( SQL.setChannelEnabled[channel], [userId, notification, enabled] );
    }

    // XXX
    async subscribeUserNotification ( userId, notification ) {}

    // XXX
    async unsubscribeUserNotification ( userId, notification ) {}
