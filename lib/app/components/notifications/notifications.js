...
export default class {
    ...
    async sendNotification ( type, users, subject, body, options = {} ) {
        ...
        // email
        if ( emails.length ) {
            promises.push( this.sendEmailNotification( emails, subject, body, options.email ) );
        }
    ...
    async sendEmailNotification ( users, subject, body, options = {} ) {
        ...
        for ( const userId of users ) {
            ...
            this.sendEmail(
                ...
                //
                user.email,
                subject instanceof LocaleTemplate ? subject.toString( { "domain": user.locale } ) : subject,
                body instanceof LocaleTemplate ? body.toString( { "domain": user.locale } ) : body,
                options
            ).then( res => counter.dev() );
        }
    ...
    async sendPushNotification ( targets, subject, body, options = {} ) {
        ...
        for ( const target of targets ) {
            ...
            if ( !PUSH_NOTIFICATION_GROUPS.has( target ) ) {
                const user = target instanceof User ? user : await this.app.users.getUserById( target );
                ...
                this.#googleCloudMessagingApi
                    .send( {
                        ...message,
                        "topic": this.#pushNotificationsPrefix + "users." + target,
                        "notification": {
                            "title": subject instanceof LocaleTemplate ? subject.toString( { "domain": user.locale } ) : subject,
