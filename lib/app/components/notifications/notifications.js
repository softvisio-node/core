...
export default class {
    ...
    async sendInternalNotification ( users, subject, body, { maxAge } = {} ) {
        ...
        for ( const user of users ) {
            ...
            locales[user.locale] ??= {
                "users": [],
                "subject": Translation.toString( subject, { "domain": user.locale } ),
                "body": Translation.toString( body, { "domain": user.locale } ),
            };
    ...
    async sendEmailNotification ( users, subject, body, options = {} ) {
        ...
        for ( const user of users ) {
            ...
            this.sendEmail(
                ...
                //
                user.email,
                Translation.toString( subject, { "domain": user.locale } ),
                Translation.toString( body, { "domain": user.locale } ),
                options
    ...
    async sendTelegramNotification ( users, subject, body, options ) {
        ...
        for ( const telegramUser of users ) {
            ...
            if ( subject ) {
                text.push( "<b>" + Translation.toString( subject, { "domain": telegramUser.locale } ) + "</b>" );
            }

            if ( body ) {
                text.push( Translation.toString( body, { "domain": telegramUser.locale } ) );
            }
