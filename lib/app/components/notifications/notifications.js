    ...
    async sendNotification ( type, users, subject, body, options = {} ) {
        ...
        for ( const userId of users ) {
            ...
            for ( const channel in defaultChannels ) {
                ...
                const subscribed = defaultChannels[channel].editable ? userChannels?.[channel] ?? defaultChannels[channel].subscrined : defaultChannels[channel].subscrined;

                if ( !subscribed ) continue;

                channelUsers[channel.channel] ??= [];
                channelUsers[channel.channel].push( user );
            }
        }
        ...
        if ( channelUsers.internal?.length ) {
            promises.push( this.sendInternalNotification( channelUsers.internal.length, subject, body, options.internal ) );
        }

        // email
        if ( channelUsers.email?.length ) {
            promises.push( this.sendEmailNotification( channelUsers.email.length, subject, body, options.email ) );
        }

        // telegram
        if ( channelUsers.telegram?.length ) {
            promises.push( this.sendTelegramNotification( channelUsers.telegram.length, subject, body, options.telegram ) );
        }

        // push
        if ( channelUsers.push?.length ) {
            promises.push( this.sendPushNotification( channelUsers.push.length, subject, body, options.push ) );
        }
