...
const SQL = {
    ...
    "setUserNotificationSubscribed": sql`INSERT INTO user_notification ( user_id, notification, internal, email, telegram, push ) VALUES ( ?, ?, ?, ?, ?, ? ) ON CONFLICT ( user_id, notification ) DO UPDATE SET internal = EXCLUDED.internal, email = EXCLUDED.email, telegram = EXCLUDED.telegram, push = EXCLUDED.push`.prepare(),

    "setChannelSubscribed": {
        "internal": sql`INSERT INTO user_notification ( user_id, notification, internal ) VALUES ( ?, ?, ? ) ON CONFLICT ( user_id, notification ) DO UPDATE SET internal = EXCLUDED.internal`.prepare(),

        "email": sql`INSERT INTO user_notification ( user_id, notification, email ) VALUES ( ?, ?, ? ) ON CONFLICT ( user_id, notification ) DO UPDATE SET email = EXCLUDED.email`.prepare(),

        "telegram": sql`INSERT INTO user_notification ( user_id, notification, telegram ) VALUES ( ?, ?, ? ) ON CONFLICT ( user_id, notification ) DO UPDATE SET telegram = EXCLUDED.telegram`.prepare(),

        "push": sql`INSERT INTO user_notification ( user_id, notification, push ) VALUES ( ?, ?, ? ) ON CONFLICT ( user_id, notification ) DO UPDATE SET push = EXCLUDED.push`.prepare(),
    },
};
...
export default class {
    ...
    // public
    async configure () {
        this.#channels = JSON.parse( JSON.stringify( this.config.channels ) );
    ...
    async sendNotification ( type, users, subject, body, options = {} ) {
        ...
        for ( const userId of users ) {
            ...
            // internal
            if ( defaultChannels.internal.enabled && ( defaultChannels.internal.editable ? userChannels?.internal ?? defaultChannels.internal.subscribed : defaultChannels.internal.subscribed ) ) {
                internalUsers.push( user );
            }

            // email
            if ( defaultChannels.email.enabled && ( defaultChannels.email.editable ? userChannels?.email ?? defaultChannels.email.subscribed : defaultChannels.email.subscribed ) ) {
                emailUsers.push( user );
            }

            // telegram
            if ( defaultChannels.telegram.enabled && ( defaultChannels.telegram.editable ? userChannels?.telegram ?? defaultChannels.telegram.subscribed : defaultChannels.telegram.subscribed ) ) {
                telegramUsers.push( user );
            }

            // push
            if ( defaultChannels.push.enabled && ( defaultChannels.push.editable ? userChannels?.push ?? defaultChannels.push.subscribed : defaultChannels.push.subscribed ) ) {
                pushUsers.push( user );
            }
        }
        ...
        if ( telegramUsers.length ) {
            promises.push( this.sendTelegramNotification( telegramUsers, subject, body, options.telegram ) );
        }
    ...
    async sendTelegramNotification ( users, subject, body, options ) {
        ...
        for ( let user of users ) {
            if ( !( user instanceof User ) ) user = await this.app.users.getUserById( user );

            // user not found or disabled
            if ( !user?.isEnabled ) continue;

            const telegramUser = await this.#telegramBot.users.getByApiUserId( user.id );

            if ( !telegramUser ) continue;

            const msg = LocaleTranslation.translate( subject, { "domain": telegramUser.locale } ) + "\n\n" + LocaleTranslation.translate( body, { "domain": telegramUser.locale } );

            await telegramUser.sendMessage( msg );
        }
        ...
        for ( const notification in this.#types ) {
            ...
            for ( const channel of Object.keys( this.channels ) ) {
                notificationProfile.channels[channel] = {
                    ...
                    "subscribed": defaultChannels[channel].editable ? userChannels?.[channel] ?? defaultChannels[channel].subscribed : defaultChannels[channel].subscribed,
                };
            }
        }
    ...
    async setUserNotificationSubscribed ( { userId, notification, channel, subscribed } = {} ) {
        if ( !notification ) {
            ...
            return this.dbh.do( sql`INSERT INTO user_notification`.VALUES( values ).sql`ON CONFLICT ( user_id, notification ) DO UPDATE`.SET( set ) );
        }
        else if ( channel ) {
    ...
    async unlinkTelegram ( userId ) {
        if ( !this.channels.telegram.supported ) {
