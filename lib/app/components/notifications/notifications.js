...
export default class {
    ...
    async sendInternalNotification ( users, subject, body, { maxAge } = {} ) {
        ...
        if ( maxAge ) {
            var expires = Interval.new( maxAge ).toDate();
        }
        ...
        for ( const user of users ) {
            ...
            locales[ user.locale ] ??= {
                "users": [],
                "subject": LocaleTemplate.toString( subject, { "localeDomain": user.locale } ),
                "body": LocaleTemplate.toString( body, { "localeDomain": user.locale } ),
            };
    ...
    async sendEmailNotification ( users, subject, body, options = {} ) {
        ...
        for ( const user of users ) {
            ...
            this.sendEmail(
                ...
                //
                user.email,
                LocaleTemplate.toString( subject, { "localeDomain": user.locale } ),
                LocaleTemplate.toString( body, { "localeDomain": user.locale } ),
                options
    ...
    // private
    get #telegramBot () {
        return this.app.telegram.bots.getBot( this.#telegramBotId );
    }
