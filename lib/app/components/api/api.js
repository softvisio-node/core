...
export default class AppApi extends Api {
    ...
    get avatarMaxAgeDuration () {
        this.#avatarMaxAgeDuration ??= Duration.new( this.config.avatarMaxAge );

        return this.#avatarMaxAgeDuration;
    }

    get avatarCacheControl () {
        this.#avatarCacheControl ??= "public, max-age=" + this.avatarMaxAgeDuration.toSeconds();
    ...
    async #initRootUser () {
        ...
        res = await this.app.users.createUser( this.config.root.email, {
        ...
        if ( process.cli?.command === "/reset-root-password" ) {
            ...
            // root user already exists
                ...
                res = await this.app.users.setUserPassword( constants.rootUserId, rootPassword );
    ...
    async #downloadAvatar ( req ) {
        ...
        if ( this.app.emailIsFake( email ) ) return this.downloadDefaultAvatar( req );
