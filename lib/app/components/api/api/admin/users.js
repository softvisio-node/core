...
export default Super =>
    class extends mixins( Read, Super ) {
        ...
        async API_setPassword ( ctx, userId, password ) {
            ...
            if ( this.api.userIsRoot( userId ) && !ctx.user.isRoot ) return result( [400, `You can't change root user password`] );

            return this.api.setUserPassword( userId, password );
        }

        async API_getRoles ( ctx, userId ) {
            var roles = await this.api.getUserRoles( userId, ctx.user.roles );
        ...
        async API_getSessions ( ctx, userId ) {
            return this.api.getUserSessions( userId, { "currentSessionId": ctx.token.id } );
        }

        async API_signoutSession ( ctx, userId, sessionId ) {

            // user can't set root password
            if ( this.api.userIsRoot( userId ) && !ctx.user.isRoot ) return result( [400, `You can't manage root user sessions`] );
        ...
        async API_signoutAllSessions ( ctx, userId ) {
            ...
            return this.api.deleteUserSessions( userId, { "excludeSessionId": ctx.token.id } );
        }
    };
