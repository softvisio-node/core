...
export default Super =>
    class extends Super {
        ...
        async API_setPassword ( ctx, password ) {
            ...
            if ( res.ok ) {
                ...
                this.app.notifications.sendNotification(
                    ...
                    "security",
                    ctx.user.id,
                    locale.i18nt( locale => locale.i18n( "Password changed" ) ),
                    locale.i18nt( locale => locale.i18n( "Your password was just changed." ) )
                );
            }
        ...
        async API_sendEmailChangeToken ( ctx, email ) {
            ...
            const token = await this.app.actionTokens.createActionToken( ctx.user.id, constants.tokenTypeEmailChange, { "data": { email } } );
...
`;
        ...
        async API_setEmailByToken ( ctx, token ) {
            const res = await this.dbh.begin( async dbh => {
                const res = await this.app.actionTokens.activateActionToken( token, constants.tokenTypeEmailChange, { "userId": ctx.user.id, dbh } );
                if ( !res.ok ) throw res;
            ...
            if ( res.ok ) {
                ...
                this.app.notifications.sendNotification(
                    "security",
                    ctx.user.id,
                    locale.i18nt( locale => locale.i18n( "Email changed" ) ),
                    locale.i18nt( locale =>
                        locale.i18n( msgid`Your email address was changed.
