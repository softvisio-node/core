...
` )
        ...
        async API_setEmailByToken ( ctx, token ) {
            ...
            const res = await this.dbh.begin( async dbh => {
                ...
                const res1 = await this.api.users.setUserEmail( userId, res.data.data.email, {
                    "emailConfirmad": true,
                    dbh,
                } );
            ...
            if ( res.ok ) {
                const subjet = this.app.locale.l10nt( locale => locale.l10n( "Email changed" ) );
...
` ) );
                ...
                this.app.notifications.sendEmail( oldEmail, subjet, body );

                this.app.notifications.sendNotification( "security", userId, subjet, body );
            }
