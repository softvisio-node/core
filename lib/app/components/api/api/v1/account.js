...
export default Super =>
    class extends Super {
        ...
        async API_sendEmailChangeToken ( ctx, email ) {
            ...
            const subject = this.app.locale.l10nt( "Email address change" ).toString( { "domain": ctx.user.locale } );

            const text = this.app.locale
                .l10nt( msgid`Use the following confirmation token to change your email address on ${this.api.config.frontendUrl}:
...
` )
                .toString( { "domain": ctx.user.locale } );

            return this.app.notifications.sendEmail( email, subject, text );
        }
...
` );
                ...
                this.app.notifications.sendEmail( oldEmail, subjet.toString( { "domain": ctx.yser.locale } ), body.toString( { "domain": ctx.yser.locale } ) );
