...
export default Super =>
    class extends Super {
        ...
        async API_sendEmailChangeToken ( ctx, email ) {
            ...
            const token = await this.api.sessions.createActionToken( ctx.user.id, constants.tokenTypeEmailChange, { "data": { email } } );
...
`;
        ...
        async API_setEmailByToken ( ctx, token ) {
            const res = await this.dbh.begin( async dbh => {
                const res = await this.api.actionTokens.activateActionToken( token, constants.tokenTypeEmailChange, { "userId": ctx.user.id, dbh } );
                if ( !res.ok ) throw res;
