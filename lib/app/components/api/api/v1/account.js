...
export default Super =>
    class extends Super {
        ...
        async API_sendEmailChangeToken ( ctx, email ) {
            ...
            const url = `${this.api.config.frontendUrl}#/change-email/${token.data.token}`;

            const subject = this.app.locale.l10nt( "Email address change" );

            const body = this.app.locale.l10nt( msgid`Use the following lunk to change your email address on ${this.api.config.frontendUrl} to ${email}:
...
` );
            ...
            return this.app.notifications.sendEmail( email, subject.toString( { "domain": ctx.user.locale } ), body.toString( { "domain": ctx.user.locale } ) );
        }

        // XXX
        async API_setEmailByToken ( ctx, token ) {
            var oldEmail = ctx.user.email,
                newEmail;
            ...
            const res = await this.dbh.begin( async dbh => {
                ...
                // user id is not valud
                if ( res.data.user_id !== ctx.user.id ) {
                    throw res( [400, `Token is not valid`] );
                }

                newEmail = res.data.data.email;

                const res1 = await this.api.users.setUserEmail( ctx.user.id, newEmail, {
                    "emailConfirmad": true,
                    dbh,
                } );
            ...
            if ( res.ok ) {
                const subjet = this.app.locale.l10nt( locale => locale.l10n( "Your email address was changed" ) );

                const body = this.app.locale.l10nt( msgid`Your email address was changed.
...
` );
                ...
                this.app.notifications.sendEmail( oldEmail, subjet.toString( { "domain": ctx.user.locale } ), body.toString( { "domain": ctx.user.locale } ) );

                this.app.notifications.sendNotification( "security", ctx.user.id, subjet, body );
            }
