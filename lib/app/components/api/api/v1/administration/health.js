...
SELECT
    series.date,
    calls,
    ( duration::numeric / calls )::float4 AS duration_per_call,
    ( exceptions::numeric / calls )::float4 AS exceptions_per_call
FROM
    series
...
export default Super =>
    class extends Super {
        ...
        async API_getLatestStats ( ctx, methodId ) {
            return this.dbh.select( SQL.getLatestStats, [methodId] );
        }
