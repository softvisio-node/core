...
`.WHERE( where );
        ...
        async API_setReadAll ( ctx ) {
            const res = await this.dbh.do( SQL.setReadAll, [ctx.user.id] );
        ...
        async API_setUnreadAll ( ctx ) {
            const res = await this.dbh.do( SQL.setUnreadAll, [ctx.user.id] );
        ...
        async API_setDoneAll ( ctx ) {
            const res = await this.dbh.do( SQL.setDoneAll, [ctx.user.id] );
        ...
        async API_setUndoneAll ( ctx ) {
            const res = await this.dbh.do( SQL.setUndoneAll, [ctx.user.id] );
        ...
        async API_deleteAll ( ctx ) {
            const res = await this.dbh.do( SQL.deleteAll, [ctx.user.id] );
