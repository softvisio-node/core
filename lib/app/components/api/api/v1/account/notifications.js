...
` );
        ...
        async API_getLinkTelegramAccountUrl ( ctx ) {
            ...
            const linkUrl = new URL( "tg://resolve" );
            linkUrl.searchParams.set( "domain", this.api.notifications.telegramBotUsername );
            linkUrl.searchParams.set( "start", "link-user:" + res.data.token );

            return result( 200, linkUrl );
        }
    };
