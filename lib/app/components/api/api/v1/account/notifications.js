    ...
    class extends Super {
        async API_getUserNotificationsProfile ( ctx, { "acl_id": aclId } = {} ) {
            ...
            if ( profile.telegramSupported ) {
                ...
                const linkedTelegramUser = await notifications.getLinkedTelegramUser( ctx.user.id );

                profile.linkedTelegramUser = {
                    "username": linkedTelegramUser.username,
                    "avatar_url": linkedTelegramUser.avatarUrl,
                };
            }
...
` );
        ...
        async API_getTelegramLinkUrl ( ctx ) {
            if ( !this.app.notifications.channels.telegram.supported ) {
                return result( [500, `Telegram is not supported`] );
            }

            const linkedTelegramUser = await this.app.notifications.getLinkedTelegramUser( ctx.user.id );
            ...
            if ( linkedTelegramUser ) {
                ...
                const res = await this.app.actionTokens.createActionToken( ctx.user.id, constants.linkTelegramAccountToken.id, {
                    "length": constants.linkTelegramAccountToken.length,
                    "maxAge": constants.linkTelegramAccountToken.maxAge,
                } );

                if ( !res.ok ) return res;

                const start = this.app.telegram.encodeCallback( "link-api-user", res.data.token );
