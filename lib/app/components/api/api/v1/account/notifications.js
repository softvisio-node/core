...
export default Super =>
    class extends mixins( Read, Super ) {
        async API_getNotificationsSettings ( ctx ) {
            const profile = await this.api.backend.notifications.getUserNotificationsProfileByUserId( ctx.user.id );
            ...
            for ( const type in profile.notifications ) {
                ...
                data.push( {
                    ...
                    type,
                    "name": this.config.notifications[type].name,
                    "description": this.config.notifications[type].description,
