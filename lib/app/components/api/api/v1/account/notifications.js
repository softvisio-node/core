...
export default Super =>
    class extends mixins( Read, Super ) {
        async API_getNotificationsSettings ( ctx ) {
            const types = await this.api.notifications.getUserNotificationsTypes( ctx.user.id );

            return result( 200, types );
        }

        async API_setUserNotificationChannel ( ctx, type, channel, enabled ) {
            return this.api.notifications.setUserNotificationTypeChannelEnabled( ctx.user.id, type, channel, enabled );
        }
