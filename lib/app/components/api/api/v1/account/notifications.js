...
` );
        ...
        async API_getTelegramLinkUrl ( ctx ) {
            ...
            if ( linkedTelegramUsername ) {
                ...
                const res = await this.app.actionTokens.createActionToken( ctx.user.id, constants.tokenTypeLinkTelegramAccount, { "length": 8, "data": { "userId": ctx.user.id } } );

                if ( !res.ok ) return res;

                const start = Buffer.from( JSON.stringify( { "apiUserToken": res.data.token } ) ).toString( "base64url" );

                const localUrl = new URL( "tg://resolve" );
                localUrl.searchParams.set( "domain", this.app.notifications.telegramBotUsername );
                localUrl.searchParams.set( "start", start );

                const remoteUrl = new URL( "https://t.me/" + this.app.notifications.telegramBotUsername );
                remoteUrl.searchParams.set( "start", start );
