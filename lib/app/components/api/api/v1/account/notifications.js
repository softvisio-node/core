...
`.WHERE( where );
        ...
        async API_update ( ctx, options ) {
            if ( options.id ) {
                ...
                return this.dbh.do( sql`UPDATE user_internal_notification`.SET( options ).sql`WHERE user_id = ${ctx.user.id} AND id = ${notificationId}` );
            }
            else {
                return this.dbh.do( sql`UPDATE user_internal_notification`.SET( options ).sql`WHERE user_id = ${ctx.user.id}` );
            }
        }
