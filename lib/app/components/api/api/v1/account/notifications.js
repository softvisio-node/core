...
export default Super =>
    class extends Super {
        async API_getUserNotificationsProfile ( ctx, { "acl_id": aclId } = {} ) {
            ...
            if ( aclId ) {
                userNotifications = await this.app.acl.getUserNotificationsProfile( aclId, ctx.user.id );
            }
            else {
            ...
            const profile = {
                "internalNotificationsEnabled": notifications.isChannelEnabled( "internal" ),
                "emailNotificationsEnabled": notifications.isChannelEnabled( "email" ),
                "telegramNotificationsEnabled": notifications.isChannelEnabled( "telegram" ),
                "pushNotificationsEnabled": notifications.isChannelEnabled( "push" ),
                "telegramSupported": notifications.isChannelSupported( "telegram" ),
            };

            if ( profile.telegramSupported ) {
                profile.telegramBotUrl = "https://t.me/" + notifications.telegramBotUsername;
                profile.linkedTelegramUsername = await notifications.getLinkedTelegramUsername( ctx.user.id );
            }
        ...
        async API_setUserNotificationActive ( ctx, { "acl_id": aclId, notification, channel, active } = {} ) {
            if ( aclId ) {
                if ( channel ) {
                    return this.app.acl.setAclUserNotificationChannelActive( aclId, ctx.user.id, notification, channel, active );
                }
                else {
                    return this.app.acl.setAclUserNotificationActive( aclId, ctx.user.id, notification, active );
                }
            }
            else {
                return this.app.notifications.setUserNotificationActive( {
                    "userId": ctx.user.id,
                    notification,
                    channel,
                    active,
...
` );
        ...
        async API_getTelegramLinkUrl ( ctx ) {
            if ( !this.app.notifications.isChannelSupported( "telegram" ) ) {
