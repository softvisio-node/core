...
export default Super =>
    class extends mixins( Super ) {
        async API_getAclPermissions ( ctx, aclId, aclObjectType ) {
            return this.api.getAclPermissions( aclId, ctx.user.id, aclObjectType );
        }

        async API_getUsers ( ctx, aclId ) {
            return this.api.getAclUsers( aclId, { "editorUserId": ctx.user.id } );
        }

        async API_getRoles ( ctx, aclId ) {
            return this.api.getAclRoles( aclId, { "editorUserId": ctx.user.id } );
        }
        ...
        async API_addUser ( ctx, aclId, userId, { roles, enabled } = {} ) {
            ...
            return this.api.setAclUserRoles( aclId, userId, { enabled, roles, "editorUserId": ctx.user.id } );
        }
        ...
        async API_setRoleEnabled ( ctx, aclId, userId, role, enabled ) {
            ...
            return this.api.setAclUserRoleEnabled( aclId, userId, role, enabled, { "editorUserId": ctx.user.id } );
        }
