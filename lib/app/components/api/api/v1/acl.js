...
export default Super =>
    class extends mixins( Super ) {
        ...
        async API_setScopeEnabled ( ctx, aclId, userId, scope, enabled ) {
            if ( ctx.user.id === userId ) return result( [400, `You are unable to change own scope`] );

            return this.api.acl.setAclUserScopeEnabled( aclId, userId, scope, enabled, { "editorUserId": ctx.user.id } );
        }
