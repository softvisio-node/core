...
export default Super =>
    class extends Super {
        ...
        async API_signIn ( ctx, { email, password, "oauth_provider": oauthProvider, "oauth_code": oauthCode, "oauth_redirect_uri": oauthRedirectUri } ) {
            ...
            // create user session
            const session = await this.api.sessions.createUserSession( user.id, {
        ...
        async API_signOut ( ctx ) {
            ...
            return await this.api.sessions.deleteUserSession( ctx.token.id, { "userId": ctx.user.id } );
        }
        ...
        async API_sendConfirmationEmail ( ctx ) {
            const token = await this.api.actionTokens.createUserActionToken( ctx.user.id, constants.tokenTypeEmailConfirmation );
        ...
        async API_confirmEmailByToken ( ctx, token ) {
            return this.api.actionTokens.activateUserActionToken( token, constants.tokenTypeEmailConfirmation );
        }
        ...
        async API_sendPasswordRecoveryEmail ( ctx, email ) {
            ...
            const token = await this.api.actionTokens.createUserActionToken( user.data.id, constants.tokenTypePasswordReset );
            if ( !token.ok ) return token;
        ...
        async API_setPasswordByToken ( ctx, token, password ) {
            return this.dbh.begin( async dbh => {
                const res = await this.api.actionTokens.activateUserActionToken( token, constants.tokenTypePasswordReset, { dbh } );
                if ( !res.ok ) throw res;
