...
export default Super =>
    class extends Super {
        ...
        async API_sendConfirmationEmail ( ctx ) {
            const token = await this.api.actionTokens.createActionToken( ctx.user.id, constants.tokenTypeEmailConfirmation );
        ...
        async API_confirmEmailByToken ( ctx, token ) {
            return this.api.actionTokens.activateActionToken( token, constants.tokenTypeEmailConfirmation );
        }
        ...
        async API_sendPasswordRecoveryEmail ( ctx, email ) {
            ...
            const token = await this.api.actionTokens.createActionToken( user.data.id, constants.tokenTypePasswordReset );
            if ( !token.ok ) return token;
        ...
        async API_setPasswordByToken ( ctx, token, password ) {
            return this.dbh.begin( async dbh => {
                const res = await this.api.actionTokens.activateActionToken( token, constants.tokenTypePasswordReset, { dbh } );
                if ( !res.ok ) throw res;
