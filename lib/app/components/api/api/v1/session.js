    ...
    class extends Super {
        ...
        async API_checkAuthorization ( ctx ) {
            ...
            if ( ctx.isAuthenticated ) {
                permissions = await this.api.acl.getAclUserPermissions( constants.defaultAclId, ctx.user.id );
        ...
        async API_signIn ( ctx, { email, password, "oauth_provider": oauthProvider, "oauth_code": oauthCode, "oauth_redirect_uri": oauthRedirectUri } ) {
            ...
            if ( oauthProvider ) {
                user = await this.api.user.authenticateUserOauth( oauthProvider, oauthCode, oauthRedirectUri );
            }
            else {
                user = await this.api.user.authenticateUserPassword( email, password );
            }
            ...
            // create user session
            const session = await this.api.userSessions.createUserSession( user.id, {
        ...
        async API_authorize ( ctx, { password, "oauth_provider": oauthProvider, "oauth_code": oauthCode, "oauth_redirect_uri": oauthRedirectUri } = {} ) {
            ...
            if ( oauthProvider ) {
                ...
                user = await this.api.user.authenticateUserPassword( ctx.user.email, password );
            }

            // not authenticated
            if ( !user ) return result( 401 );

            return this.api.userSessions.updateSession( ctx.token.id, ctx.remoteAddress + "", ctx.userAgent );
        }

        async API_signOut ( ctx ) {
            if ( !ctx.token.isUserSessionToken ) return result( 404 );

            return await this.api.userSessions.deleteUserSession( ctx.token.id, { "userId": ctx.user.id } );
        }
        ...
        async API_signUp ( ctx, email, fields ) {
            ...
            if ( typeof email === "object" ) {
                res = await this.api.user.createUserOauth( email.oauth_provider, email.oauth_code, email.oauth_redirect_uri, fields );
            }

            // email
            else {
                res = await this.api.user.createUser( email, fields );
            }
        ...
        async API_sendConfirmationEmail ( ctx ) {
            const token = await this.api.userActionTokens.createUserActionToken( ctx.user.id, constants.tokenTypeEmailConfirmation );
        ...
        async API_confirmEmailByToken ( ctx, token ) {
            return this.api.userActionTokens.activateUserActionToken( token, constants.tokenTypeEmailConfirmation );
        }
        ...
        async API_sendPasswordRecoveryEmail ( ctx, email ) {
            ...
            const token = await this.api.userActionTokens.createUserActionToken( user.data.id, constants.tokenTypePasswordReset );
            if ( !token.ok ) return token;
        ...
        async API_setPasswordByToken ( ctx, token, password ) {
            return this.dbh.begin( async dbh => {
                const res = await this.api.userActionTokens.activateUserActionToken( token, constants.tokenTypePasswordReset, { dbh } );
                if ( !res.ok ) throw res;

                const res1 = await this.api.user.setUserPassword( ctx.user.id, password, { dbh } );
        ...
        async API_registerPushNotificationsToken ( ctx, token ) {
            return this.api.notifications.registerPushNotificationsToken( token, ctx.user.id );
        }
...
`;
            ...
            return this.api.notifications.sendEmail( email, "Confirm your email", text );
        }
...
`;
            ...
            return this.api.notifications.sendEmail( email, "Password reset link", text );
        }

        async _sendNewSigninNotification ( userId, { userAgent, remoteAddress } ) {
            return this.api.notifications.sendNotification(
                "security",
                userId,
                "New sign in",
        ...
        async #getAppSettings () {
            if ( !this.#settings ) {
                this.#settings = {
                    ...
                    "internal_notifications_enabled": this.api.notifications.internalNotificationsEnabled,
                    "push_notifications_supported": this.api.notifications.pushNotificationsSupported,
                    "push_notifications_prefix": this.api.notifications.pushNotificationsPrefix,
