    ...
    class extends Super {
        ...
        async API_setPasswordByToken ( ctx, token, password ) {
            return this.dbh.begin( async dbh => {
                ...
                res = await this.api.users.setUserPassword( userId, password, { dbh } );
                if ( !res.ok ) throw res;

                this.app.templates["notification/password-change/subject"] ??= this.app.locale.l10nt( `Password changed` );

                this.app.templates["notification/password-change/body"] ??= this.app.locale.l10nt( `Your password was just changed.` );
                ...
                this.app.notifications.sendNotification(
                    ...
                    "security",
                    userId,
                    this.app.templates["notification/password-change/subject"],
                    this.app.templates["notification/password-change/body"]
                );
        ...
        async #sendConfirmationEmail ( email, token, userLocale ) {
            this.app.templates["confirm-email/subject"] ??= this.app.locale.l10nt( "Confirm your email address" );

            this.app.templates["confirm-email/body"] ??= this.app.locale.l10nt( ( locale, { data } ) =>
                locale.l10n( msgid`Use the following link to confirm your email on ${data.frontendUrl}:

${data.url}

This link is valid till ${data.token.data.expires.toISOString()}.
...
` ) );
            ...
            return this.app.notifications.sendEmail(
                email,
                this.app.templates["confirm-email/subject"].toString( { "domain": userLocale } ),
                this.app.templates["confirm-email/body"]
                    .createTemplate( {
                        "frontendUrl": this.api.config.frontendUrl,
                        "url": `${this.api.config.frontendUrl}#/confirm-email/${token.data.token}`,
                        token,
                    } )
                    .toString( { "domain": userLocale } )
            );
        }

        async #sendPasswordRecoveryEmail ( email, token, userLocale ) {
            this.app.templates["password-recovery-email/subject"] ??= this.app.locale.l10nt( "Password reset link" );

            this.app.templates["password-recovery-email/body"] ??= this.app.locale.l10nt( ( locale, data ) =>
                locale.l10n( msgid`Use the following link to reset your password on ${data.frontendUrl}.
...
` ) );
            ...
            return this.app.notifications.sendEmail(
                email,
                this.app.templates["password-recovery-email/subject"].toString( { "domain": userLocale } ),
                this.app.templates["password-recovery-email/body"]
                    .createTemplate( {
                        "frontendUrl": this.api.config.frontendUrl,
                        "url": `${this.api.config.frontendUrl}#/reset-password/${token.data.token}`,
                        token,
                    } )
                    .toString( { "domain": userLocale } )
            );
        ...
        async #sendNewSigninNotification ( userId, { userAgent, remoteAddress } ) {
            this.app.templates["NewSigninNotification/subject"] ??= this.app.locale.l10nt( locale => locale.l10n( "New device" ) );

            this.app.templates["NewSigninNotification/body"] ??= this.app.locale.l10nt( locale =>
...
` ) );
            ...
            return this.app.notifications.sendNotification( "security", userId, this.app.templates["NewSigninNotification/subject"], this.app.templates["NewSigninNotification/body"] );
        }
    };
