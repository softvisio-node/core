...
export default Super =>
    class extends Super {
        ...
        async API_sendConfirmationEmail ( ctx ) {
            ...
            const res = await this._sendConfirmationEmail( token.data.email, token, ctx.user.locale );
        ...
        async API_sendPasswordRecoveryEmail ( ctx, email ) {
            ...
            return this._sendPasswordRecoveryEmail( email, token, user.locale );
        }
        ...
        async API_setPasswordByToken ( ctx, token, password ) {
            return this.dbh.begin( async dbh => {
                ...
                res = await this.api.users.setUserPassword( res.data.user_id, password, { dbh } );
                if ( !res.ok ) throw res;
        ...
        // protected
        async _sendConfirmationEmail ( email, token, locale ) {
            const url = `${this.api.config.frontendUrl}#/confirm-email/${token.data.token}`;

            const subject = this.app.locale.l10nt( "Confirm your email address" ).toString( { "domain": locale } );

            const text = this.app.locale
                .l10nt( msgid`Use the following link to confirm your email on ${this.api.config.frontendUrl}:
...
` )
                .toString( { "domain": locale } );

            return this.app.notifications.sendEmail( email, subject, text );
        }

        async _sendPasswordRecoveryEmail ( email, token, locale ) {
            const url = `${this.api.config.frontendUrl}#/reset-password/${token.data.token}`;

            const subject = this.app.locale.l10nt( "Password reset link" ).toString( { "domain": locale } );

            const text = this.app.locale
                .l10nt( msgid`Use the following link to reset your password on ${this.api.config.frontendUrl}.
