...
export default Super =>
    class extends Super {
        ...
        async API_sendConfirmationEmail ( ctx ) {
            ...
            const res = await this._sendConfirmationEmail( token.data.email, token );
        ...
        async API_confirmEmailByToken ( ctx, token ) {
            ...
            if ( res.ok ) {
                this.app.publishToApi( "/session/email/confirm/", ctx.user.id );
            }
        ...
        async API_setPasswordByToken ( ctx, token, password ) {
            return this.dbh.begin( async dbh => {
                let res = await this.app.actionTokens.activateActionToken( token, constants.passwordResetToken.id, { dbh } );
                if ( !res.ok ) throw res;

                // XXX use token user id
