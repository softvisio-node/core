...
export default Super =>
    class extends Super {
        ...
        async API_initSession ( ctx, { locale, locales, defaultLocale, detectLocaleByClientIpAddress } = {} ) {
            var permissions;
        ...
        async API_signIn ( ctx, { email, password, "oauth_provider": oauthProvider, "oauth_code": oauthCode, "oauth_redirect_uri": oauthRedirectUri, "telegram_bot_id": telegramBotId, "telegram_initial_data": telegramInitialData } ) {
            var user;
            ...
            if ( oauthProvider ) {
                ...
                const bot = this.app.telegram?.getBut( telegramBotId );

                if ( bot ) {
                    user = await bot.authenticateWebApp( telegramInitialData );
                }
            }
            else {
