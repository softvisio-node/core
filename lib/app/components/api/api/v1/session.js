    ...
    class extends Super {
        ...
        async #sendConfirmationEmail ( email, token, userLocale ) {
            ...
            this.app.templates["confirm-email/body"] ??= this.app.locale.l10nt( ( locale, { data } ) =>
                locale.l10n( msgid`Use the following link to confirm your email on ${this.api.config.frontendUrl}:
...
` ) );
        ...
        // XXX
        async #sendPasswordRecoveryEmail ( email, token, locale ) {
            const url = `${this.api.config.frontendUrl}#/reset-password/${token.data.token}`;

            const subject = this.app.locale.l10nt( "Password reset link" );

            const body = this.app.locale.l10nt( msgid`Use the following link to reset your password on ${this.api.config.frontendUrl}.
...
` );
            ...
            return this.app.notifications.sendEmail( email, subject.toString( { "domain": locale } ), body.toString( { "domain": locale } ) );
        }
