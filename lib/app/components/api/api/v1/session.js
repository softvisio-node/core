...
export default Super =>
    class extends mixins( Super ) {
        ...
        async API_signin ( ctx, { email, password, "oauth_provider": oauthProvider, "oauth_code": oauthCode, "oauth_redirect_uri": oauthRedirectUri } ) {
            ...
            if ( oauthProvider ) {
                auth = await this.api.user.authenticateUserOauth( oauthProvider, oauthCode, oauthRedirectUri );
            }
            else {
                auth = await this.api.user.authenticateUserPassword( email, password );
            }
            ...
            // create user session
            const session = await this.api.userSessions.createUserSession( auth.user.id, {
        ...
        async API_authorize ( ctx, { password, "oauth_provider": oauthProvider, "oauth_code": oauthCode, "oauth_redirect_uri": oauthRedirectUri } = {} ) {
            ...
            if ( oauthProvider ) {
                ...
                auth = await this.api.user.authenticateUserPassword( ctx.user.email, password );
            }
