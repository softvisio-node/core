...
export default Super =>
    class extends mixins( Super ) {
        ...
        async API_signin ( ctx, { email, password, "oauth_provider": oauthProvider, "oauth_code": oauthCode, "oauth_redirect_uri": oauthRedirectUri } ) {
            ...
            if ( oauthProvider ) {
                auth = await this.api.backend.user.authenticateUserOauth( oauthProvider, oauthCode, oauthRedirectUri );
            }
            else {
                auth = await this.api.backend.user.authenticateUserPassword( email, password );
            }
            ...
            // create user session
            const session = await this.api.backend.userSessions.createUserSession( auth.user.id, {
        ...
        async API_authorize ( ctx, { password, "oauth_provider": oauthProvider, "oauth_code": oauthCode, "oauth_redirect_uri": oauthRedirectUri } = {} ) {
            ...
            if ( oauthProvider ) {
                ...
                if ( auth && auth.user.email !== ctx.user.email ) return result( 401 );
            }
            else {
                auth = await this.api.authenticateUserCredentials( ctx.user.email, password );
            }

            // not authenticated
            if ( !auth ) return result( 401 );

            return this.api.updateSession( ctx.token.id, ctx.remoteAddress + "", ctx.userAgent );
        }

        async API_signout ( ctx ) {
            if ( !ctx.token.isUserSessionToken ) return result( 404 );

            return await this.api.deleteUserSession( ctx.token.id, { "userId": ctx.user.id } );
        }
        ...
        async API_sendConfirmationEmail ( ctx ) {
            const token = await this.api.createUserActionToken( ctx.user.id, constants.tokenTypeEmailConfirmation );
        ...
        async API_setPasswordByToken ( ctx, token, password ) {
            return this.dbh.begin( async dbh => {
                ...
                const res1 = await this.api.setUserPassword( ctx.user.id, password, { dbh } );
        ...
        async API_registerPushNotificationsToken ( ctx, token ) {
            return this.api.backend.notifications.registerPushNotificationsToken( token, ctx.user.id );
        }
...
`;
            ...
            return this.api.backend.notifications.sendEmail( email, "Confirm your email", text );
        }
...
`;
            ...
            return this.api.backend.notifications.sendEmail( email, "Password reset link", text );
        }

        async _sendNewSigninNotification ( userId, { userAgent, remoteAddress } ) {
            return this.api.backend.notifications.sendNotification(
                "security",
                userId,
                "New sign in",
        ...
        async #getAppSettings ( ctx ) {
            if ( !this.#settings ) {
                this.#settings = {
                    ...
                    "push_notifications_supported": this.api.backend.notifications.pushEnabled,
