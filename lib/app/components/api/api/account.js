...
export default Super =>
    class extends mixins( Super ) {
        ...
        async API_sendEmailChangeToken ( ctx, email ) {
            ...
            const token = await this.api.createUserActionToken( ctx.userId, constants.tokenTypeEmailChange, { "data": { email } } );
...
`;
        ...
        async API_setEmailByToken ( ctx, token ) {
            const res = await this.dbh.begin( async dbh => {
                const res = await this.api.activateUserActionToken( token, constants.tokenTypeEmailChange, { "userId": ctx.userId, dbh } );
                if ( !res.ok ) throw res;

                const res1 = await this.api.setUserEmail( ctx.userId, res.data.email, { dbh } );
...
`
        ...
        async API_setTelegramUsername ( ctx, username ) {
            ...
            const res = await this.dbh.do( QUERIES.setTelegramUsername, [username, ctx.userId, username] );
...
`
        ...
        async API_getSessions ( ctx ) {
            return this.api.getUserSessions( ctx.userId, { "currentSessionId": ctx.token.id } );
        }
        ...
        async API_signoutAllSessions ( ctx ) {
            return this.api.deleteUserSessions( ctx.userId, { "excludeSessionId": ctx.token.id } );
        }
    };
