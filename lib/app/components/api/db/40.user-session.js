...
-- after update
CREATE FUNCTION user_session_after_update_trigger() RETURNS TRIGGER AS $$
BEGIN
    PERFORM pg_notify( 'api/user-session/update', json_build_object(
        'id', NEW.id::text,
...
CREATE TRIGGER user_session_last_authorized_after_update AFTER UPDATE OF last_authorized, remote_address, user_agent ON user_session FOR EACH ROW EXECUTE FUNCTION user_session_after_update_trigger();

-- after delete
CREATE FUNCTION user_session_after_delete_trigger() RETURNS TRIGGER AS $$
BEGIN
    PERFORM pg_notify( 'api/user-session/delete', json_build_object(
        'id', OLD.id::text
    )::text );
