...
export default class PostgreSql {
    ...
    async init () {
        ...
        // init primary
        if ( this.isPrimary ) {
            res = await this.#cluster.initPrimary();
            if ( !res.ok ) return res;
        }

        // init standby
        else {
            res = await this.#cluster.initStandby();
            if ( !res.ok ) return res;
        }
    ...
    async upgrade ( oldClusterVersion, clusterName ) {
        ...
        const cluster = new PostgreSqlCluster( this, process.env.POSTGRESQL_VERSION, clusterName );

        const res = await cluster.upgrade( oldClusterVersion, clusterName );
