...
export default class PostgreSql {
    ...
    async migrate ( oldVersion ) {
        ...
        const oldBinDir = `/usr/lib/postgresql/${oldVersion}/bin`,
            oldDataDir = path.join( this.#dataRoot, oldVersion + "", this.config.postgresClusterName );
        ...
        // chdir to the old cluster data dir, because postgres required write permissions to the cwd
        process.chdir( oldDataDir );
...
` );
        ...
        res = childProcess.spawnSync(
            ...
            [
                ...
                `--new-bindir=/usr/lib/postgresql/${this.config.postgresVersion}/bin`,
...
Cluster migrated to the version: ${this.config.postgresVersion}.
...
` );
    ...
    async #updatePrimary () {
        ...
        // run server
        const proc = childProcess.spawn( "postgres", ["-D", this.#dataDir, "-k", this.#unixSocketDirectories, "-h", "127.0.0.1"], {
            "uid": this.#uid,
            "gid": this.#gid,
            "stdio": "inherit",
            "detached": true,
