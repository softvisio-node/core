...
` );
    ...
    writeReBackupConfig () {
        ...
        if ( this.isPrimary && this.config.backup.enabled ) {
            ...
            config.push( "wal_level = " + this.#quoteConfigValue( "replica" ) );
            config.push( "summarize_wal = " + this.#quoteConfigValue( true ) );
            config.push( "# wal_summary_keep_time = " + this.#quoteConfigValue( 14_400 ) ); // minutes, 10 days

            config.push( "archive_mode = " + this.#quoteConfigValue( true ) );

            // XXX
            config.push( "archive_command = " + this.#quoteConfigValue( "test ! -f /var/local/backup/wal/%f && cp %p /var/local/backup/wal/%f" ) );
    ...
    writeReRecoveryConfig () {
        ...
        if ( this.isPrimary && this.config.backup.enabled ) {
            ...
            // XXX
            config.push( "restore_command = " + this.#quoteConfigValue( "cp /var/local/backup/wal/%f %p" ) );

            // XXX recovery_target_time = 'YYYY-MM-DD HH:MM:SS' # Or recovery_target_xid, recovery_target_name, etc.
            config.push( "recovery_target_time = " + this.#quoteConfigValue() );
            config.push( "recovery_target_inclusive = " + this.#quoteConfigValue( true ) );

            fs.writeFileSync( this.#cluster.recoveryConfigPath, config.join( "\n" ) );
