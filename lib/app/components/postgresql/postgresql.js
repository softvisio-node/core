...
export default class PostgreSql {
    ...
    async migrate ( oldVersion ) {
        ...
        const oldCluster = path.join( this.#dataRoot, oldVersion + "", this.config.postgresClusterName );

        if ( !fs.existsSync( oldCluster ) ) {
        ...
        res = childProcess.spawnSync(
            ...
            [
                ...
                "--check",
                `-b`,
                `/usr/lib/postgresql/${oldVersion}/bin`,
                `-B`,
                `/usr/lib/postgresql/${this.config.postgresVersion}/bin`,
                `-d`,
                `${oldCluster}`,
                `-D`,
                `${this.#dataDir}`,
