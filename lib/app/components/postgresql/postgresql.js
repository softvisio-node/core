...
` );
    ...
    async smartShutDown () {
        ...
        return this.#cluster.smartShutDown();
    }
    ...
    async fastShutDown () {
        ...
        return this.#cluster.fastShutDown();
    }
    ...
    async immediateShutDown () {
        ...
        return this.#cluster.immediateShutDowns();
    }
    ...
    // XXX
    async #updatePrimary () {
        var res = await this.#cluster.start( { "network": false } );
        if ( !res.ok ) return res;

        // run server
        const proc = childProcess.spawn(
            `${this.#cluster.binDir}/postgres`,
            [

                //
                "-D",
                this.#cluster.dataDir,
                "-k",
                this.#unixSocketDirectories,
                "-h",
                "127.0.0.1",
            ],
            {
