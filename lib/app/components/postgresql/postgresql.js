...
` );
    ...
    #writePostgresConfig () {
        ...
        if ( !fs.existsSync( this.#cluster.dataDir + "/conf.d" ) ) {
            ...
            fs.chownSync( this.#cluster.baseConfigPath, uid, gid );
    ...
    #writeReplicationsConfig () {
        ...
        // sync stanby
            ...
            replicationConfig.push( `primary_conninfo = 'host=${ this.replicationHostname } port=${ this.config.replication.port } user=${ this.config.replication.username } password=${ this.config.replication.password } application_name=${ this.#cluster.id }'` );

            // replication slot
            replicationConfig.push( `primary_slot_name = '${ this.#cluster.id }'` );
        }
