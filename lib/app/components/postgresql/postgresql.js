...
export default class PostgreSql {
    ...
    async migrate ( oldVersion ) {
        ...
        // check old cluster data dir exitsts
        if ( !fs.existsSync( oldDataDir ) ) {
            return result( [500, `Old postgres cluster not found`] );
        }

        // check new cluster not exists
        if ( fs.existsSync( this.#dataDir + "/PG_VERSION" ) ) {
            return result( [500, `New cluster data directory "${this.#dataDir}" already exists. Remove it manually and try again,`] );
        }
