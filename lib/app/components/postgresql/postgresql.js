...
export default class PostgreSql {
    ...
    async #updatePrimary () {
        ...
        // run server
        const proc = childProcess.spawn( "postgres", ["-D", this.#dataDir, "-k", this.#unixSocketDirectories, "-h"], {
            "encoding": "utf8",
            "uid": this.#uid,
            "gid": this.#gid,
            "stdio": "inherit",
...
` );
    ...
    #checkReady () {
        const res = childProcess.spawnSync( "pg_isready", ["--timeout", 60], { "encoding": "utf8", "stdio": "inherit" } );

        return result( res.status ? 500 : 200 );
    }
