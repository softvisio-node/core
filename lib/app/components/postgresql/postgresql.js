...
import PostgreSqlCluster from "./postgresql/cluster";
...
export default class PostgreSql {
    ...
    constructor ( app, config ) {
        ...
        this.#cluster = new PostgreSqlCluster( this.#dataRoot, this.config.clusterName, process.env.POSTGRESQL_VERSION );
    ...
    async upgrade ( oldVersion ) {
        ...
        if ( fs.existsSync( this.#cluster.dataDir + "/PG_VERSION" ) ) {
            console.log( `
========================================
...
` );
        ...
        if ( fs.existsSync( oldDataDir + "/postgresql.auto.conf" ) ) {
            fs.copyFileSync( oldDataDir + "/postgresql.auto.conf", this.#cluster.dataDir + "/postgresql.auto.conf" );
        }
...
` );
    ...
    #writePostgresConfig () {
        ...
        if ( !fs.existsSync( this.#cluster.dataDir + "/conf.d" ) ) {
            ...
            fs.chownSync( this.#cluster.baseConfigPath, this.#uid, this.#cluster.gid );

            fs.writeFileSync( this.#cluster.dataDir + "/postgresql.conf", "include_dir = 'conf.d'" );
            fs.chownSync( this.#cluster.dataDir + "/postgresql.conf", this.#uid, this.#gid );
        }
    ...
    async #backup () {
        ...
        fs.chownSync( this.#cluster.backupsDir, this.#uid, this.#gid );
            ...
            childProcess
                .spawn(
                    ...
                    [
                        ...
                        //
                        "--label=" + backupId,
                        "--pgdata=" + path.join( this.#cluster.backupsDir, backupId ),
                        "--format=tar",
                        "--gzip",
                        "--username=" + this.#config.replication.username,
