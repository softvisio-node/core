...
` );
        ...
        oldCluster.delereReplicationConfig();
...
` );
    ...
    async start () {
        ...
        res = await this.#cluster.start( {
            "stanbdby": !this.isPrimary,
        } );
        if ( !res.ok ) return res;

        this.#cluster.on( "shitdown", this.#onClusterShutdown.bind( this ) );
    ...
    #init () {
        ...
        fs.mkdirSync( this.#unixSocketDir, { "recursive": true } );
        fs.chownSync( this.#unixSocketDir, uid, gid );
    ...
    async #initStandby () {
        ...
        // base backup
        if ( !this.#cluster.isExists ) {
            const res = await this.#makeBaseBackup();
    ...
    async #updatePrimary () {
        ...
        this.#cluster.delereReplicationConfig();

        res = await this.#cluster.start( { "network": false } );
        if ( !res.ok ) return res;
...
` );
        ...
        // shutdown server
        await this.#cluster.shutDownFast();
    ...
    #writeReplicationsConfig () {
        ...
        if ( fs.existsSync( this.#cluster.autoConfigPath ) ) {
            let config = fs.readFileSync( this.#cluster.autoConfigPath, "utf8" );
            ...
            fs.writeFileSync( this.#cluster.autoConfigPath, config );
        }
    }
