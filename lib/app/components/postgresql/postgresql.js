...
export default class PostgreSql {
    ...
    get standbyId () {
        this.#standbyId ??= this.app.env.instanceId.replaceAll( "-", "_" );
...
` );
    ...
    #writeReplicationsConfig () {
        ...
        // sync stanby
            ...
            replicationConfig.push( `primary_conninfo = 'host=${this.replicationHostname} port=${this.config.replication.port} user=${this.config.replication.username} password=${this.config.replication.password} application_name=${this.standbyId}'` );

            // replication slot
            replicationConfig.push( `primary_slot_name = '${this.standbyId}'` );
        }

        // async standby
        else {
            replicationConfig.push( `primary_conninfo = 'host=${this.replicationHostname} port=${this.config.replication.port} user=${this.config.replication.username} password=${this.config.replication.password}'` );
