...
export default class PostgreSqlBaclup {
    ...
    constructor ( postgresql, cluster ) {
        ...
        this.backupsDir = `${this.#postgresql.dataRootDir}/backups-${this.#cluster.version}-${this.#cluster.name}`;
    }
    ...
    async #uploadBackup ( source, name ) {
        fs.mkdirSync( this.backupsDir, { "recursive": true } );

        return fs.promises.copyFile( source + "", this.#backupsDir + "/" + name );
    }

    async #deleteBackups ( backups ) {
        if ( backups ) {
            for ( const file of backups ) {
                fs.rmSync( this.backupsDir + "/" + file, {
                    "force": true,
                    "recursive": true,
                } );
            }
