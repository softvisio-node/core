    ...
    async #getBackups () {
        ...
        if ( res.data ) {
            for ( const row of res.data ) {
                const exists = await this.app.storage.isFileExists( row.path, { "checkImage": true } );

                if ( !exists ) {
                    if ( exists === false ) {
                        await this.app.storage.deleteFile( row.path );
                    }

                    continue;
                }

                const file = row.path.substring( this.#storageLocation.length );
