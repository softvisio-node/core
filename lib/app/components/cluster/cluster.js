...
export default class Cluster {
    ...
    publish ( name, ...args ) {
        ...
        if ( typeof name === "object" ) {
            ( { name, users, "arguments": args } = name );
        }
        ...
        if ( name.endsWith( "/" ) ) {
            ...
            for ( const user of users ) {
                this.#api.voidCall( "cluster/publish", `${this.#id}/${name}${user}`, msg );
            }
        }
        else {
            this.#api.voidCall( "cluster/publish", `${this.#id}/${name}`, JSON.stringify( args ) );
        }
    }
}
