        ...
        this.#hub.forwardSubscriptions( this.#api, {
            "on": ( name, listener ) => {
                if ( RESERVED.has( name ) ) {
                    this.#api.on( name, listener );
                }
                else {
                    this.#api.on( `${this.#prefix}/${name}`, listener );
                }
            },
            "off": ( name, listener ) => {
                if ( RESERVED.has( name ) ) {
                    this.#api.off( name, listener );
                }
                else {
                    this.#api.off( `${this.#prefix}/${name}`, listener );
                }
            },
            "listener": ( name, ...args ) => {
                if ( RESERVED.has( name ) ) {
                    ...
                    this.#hub.publish( name, ...JSON.parse( ...args ) );
                }
            },
        } );
    }
