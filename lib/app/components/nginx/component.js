    ...
    class extends Super {
        ...
        async _configure () {
            if ( this.config.acme.enabled ) {
                ...
                if ( !this.config.acme.httpAuthorizationEnabled && !cloudFlareEnabled ) {
                    return result( [ 500, `Nginx ACME httpAuthorizationEnabled or cloudFlareApiToken are required` ] );
                }
            }
