...
export default class Nginx {
    ...
    get unixSocketsPath () {
        if ( !this.#unixSocketsPath ) {
            this.#unixSocketsPath = "/var/run/nginx." + uuid();

            fs.mkdirSync( this.#unixSocketsPath );
        }
    ...
    async start () {
        ...
        if ( this.app.privateHttpServer ) {
            ...
            res = await this.addServer( "_private-http-server", {
                ...component.config.nginx,
                "httpPort": component.config.port,
                "upstreamHttpPort": this.app.privateHttpServer.port,
        ...
        if ( this.app.publicHttpServer ) {
            ...
            res = await this.addServer( "_public-http-server", {
                ...component.config.nginx,
                "httpPort": component.config.port,
                "upstreamHttpPort": this.app.publicHttpServer.port,
