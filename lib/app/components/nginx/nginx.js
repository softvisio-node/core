    ...
    async #updateConfig () {
        ...
        while ( true ) {
            ...
            for ( const server of config.servers ) {
                ...
                server
                    .writeConfig( {
                        "listenSocket": config.isRouterPort( server.port ),
                    } )
                    .then( () => counter.value-- );
            }
        ...
        // generate routers
        for ( const options of config.routers ) {
            this.#writeRouterConfig( options );
        }

        // write nginx config
        this.#writeNginxConfig( {
            "listenSocket": config.isRouterPort( 443 ),
        } );
