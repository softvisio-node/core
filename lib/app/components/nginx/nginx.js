...
export default class Nginx {
    ...
    async addServer ( id, options ) {
        ...
        if ( server.isDefaultServer ) {
            if ( this.#defaultServers[server.httpPort] ) {
                return result( [400, `Nginx default server for http port ${server.httpPort} already exists`] );
            }

            this.#defaultServers[server.httpPort] = server;
        }
        ...
        if ( !res.ok ) {
            ...
            if ( server.isDefaultServer ) delete this.#defaultServers[server.httpPort];
    ...
    defaultServer ( httpPort ) {
        return this.#defaultServers[httpPort];
    }
