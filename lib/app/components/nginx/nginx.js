...
export default class Nginx {
    ...
    #configsDir;
    #configPath;
    #httpSetRealIpFromPath;
    #streamSetRealIpFromPath;
    ...
    constructor ( app, config ) {
        ...
        this.#httpSetRealIpFromPath = this.#configsDir + "/set-real-ip-from.http.nginx.conf";
        this.#streamSetRealIpFromPath = this.#configsDir + "/set-real-ip-from.stream.nginx.conf";
    ...
    get httpSetRealIpFromPath () {
        return this.#httpSetRealIpFromPath;
    }

    get streamSetRealIpFromPath () {
        return this.#streamSetRealIpFromPath;
    }
    ...
    #writeSetRealIpFromConfig () {
        ...
        const http = [ ...setRealIpFrom ];

        if ( this.#config.httpSetRealIpFrom ) {
            http.push( "# custom\n" + this.#config.httpSetRealIpFrom.map( cidr => `set_real_ip_from    ${ cidr };\n` ).join( "" ) );
        }

        fs.writeFileSync( this.#httpSetRealIpFromPath, http.join( "\n" ) );

        const stream = [ ...setRealIpFrom ];

        if ( this.#config.streamSetRealIpFrom ) {
            stream.push( "# custom\n" + this.#config.streamSetRealIpFrom.map( cidr => `set_real_ip_from    ${ cidr };\n` ).join( "" ) );
        }

        fs.writeFileSync( this.#streamSetRealIpFromPath, stream.join( "\n" ) );
    }
