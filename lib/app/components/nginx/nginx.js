...
export default class Nginx {
    ...
    getHttpsSocketPath ( port ) {
        return this.unixSocketsDir + "/_https." + port + ".socket";
    }
    ...
    async #updateConfig () {
        ...
        var config, ports, proxies, servers;
        ...
        while ( true ) {
            ...
            for ( const proxy of this.#proxies.values() ) {
                ...
                if ( !proxy.hasUpstreams ) continue;

                for ( const server of proxy.servers.values() ) {
                    config.addServer( server );

                    if ( ports[server.port] ) {
                        if ( ports[server.port].ssl !== !!server.ssl ) continue;

                        if ( ports[server.port].proxyProtocol !== server.proxyProtocol ) continue;

                        // http
                        if ( server.isHttp ) {
                            if ( server.ssl ) {
                                if ( ports[server.port].types.has( "udp" ) ) continue;
                            }
                            else {
                                if ( ports[server.port].types.has( "tcp" ) || ports[server.port].types.has( "udp" ) ) continue;
                            }
                        }
                    ...
                    // register port
                    ports[server.port] ||= {};
                    ports[server.port].types ||= new Set();
                    ports[server.port].types.add( server.type );
                    ports[server.port].ssl = !!server.ssl;
                    ports[server.port].proxyProtocol = server.proxyProtocol;
                    ...
                    // register proxy
                    proxies[proxy.id] ||= {};
                    proxies[proxy.id].proxy = proxy;
                    if ( server.isHttp ) {
                        proxies[proxy.id].hasHttpServers = true;
                    }
                    else {
                        proxies[proxy.id].hasStreamServers = true;
                    }
                }
            }
            ...
            for ( const server of servers ) {
                ...
                server
                    .writeConfig( {
                        ...
                        // XXX
                        "listenSocket": ports[server.port].types.size > 1,
                    } )
                    .then( () => counter.value-- );
            }
        ...
        this.#writeNginxConfig( {
            "listenSocket": ports["443"].types.size > 1,
        } );
