...
export default class Nginx {
    ...
    async init () {
        ...
        if ( this.#acme && this.app.privateHttpServer ) {
            const location = "/.well-known/acme-challenge/";

            this.app.privateHttpServer.get( location + "*", async req => {
                this.app.storage.downloadFile( req, req.path.substring( location.length ), {
                    "cwd": this.#acmeChallengesStorageLocation,
                } );
            } );

            this.app.privateHttpServer.head( "/.well-known/acme-challenge/test", async req => {
                req.end( {
                    "status": 200,
                    "headers": {
                        "x-acme-test": this.#acme.signHost( req.headers.host ),
                    },
                } );
            } );
        }
