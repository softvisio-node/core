...
export default class Nginx {
    ...
    // XXX
    getSocketPath ( name, port ) {
        const path = this.app.env.unixSocketsDir + `/nginx-${name}.${port}.socket`;

        // XXX https://forum.nginx.org/read.php?21,298674
        // XXX delete after bug will be closed
        return "/var/run/" + crypto.createHash( "md5" ).update( path ).digest( "base64url" ) + ".socket";
    }
    ...
    async #updateConfig () {
        ...
        this.#writeNginxConfig( {
            "useRouter": config.useRouter( 443 ),
        } );
