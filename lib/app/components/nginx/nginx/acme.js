...
export default class {
    ...
    async getCertificate ( domains ) {
        ...
        // init acme
        if ( this.#acme == null ) await this.#init();
    ...
    async #checkDomain ( { name, isWildcard, domain, dnsTxtRecordName, resolved } ) {
        ...
        if ( resolved ) {
            ...
            while ( attempts ) {
                ...
                if ( res.ok && res.headers.get( "x-acme-test" ) === id ) return true;
    ...
    async #getStoredCertificate ( { certificatePath, keyPath, checkLastModified } ) {
        ...
        if ( checkLastModified ) {
            const info = new crypto.X509Certificate( certificateBuffer );

            const expires = new Date( info.validTo );
