...
`,
    ...
    "getCertificate": sql`SELECT certificate, key FROM nginx_certificate WHERE id = ? AND test = ? AND expires > CURRENT_TIMESTAMP`,

    "upsertCertificate": sql`
INSERT INTO nginx_certificate
    ( id, test, domains, expires, certificate, key )
VALUES
    ( ?, ?, ?, ?, ?, ? )
ON CONFLICT ( id, test ) DO UPDATE SET
...
export default class {
    ...
    async getCertificate ( domains ) {
        ...
        const id = crypto.createHash( "md5" ).update( JSON.stringify( domains ) ).digest( "hex" );
    ...
    async #getStoredCertificate ( id, { checkRenewInterval } = {} ) {
        ...
        if ( this.#nginx.config.acmeuseLocalFilesStorage ) {
            const certificatePath = this.#dataDir + "/certificates/" + id + "-" + ( this.#nginx.config.acmeTest ? "test" : "production" ) + ".json";
    ...
    async #uploadCertificate ( { id, domains, certificate, privateKey, expires } ) {
