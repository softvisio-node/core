...
export default class {
                ...
                res = await this.app.storage.uploadFile(
                    ...
                    new File( {
                        "buffer": JSON.stringify( {
                            "accountKey": crypto
                                .createPrivateKey( this.#acme.accountKey )
                                .export( {
                                    "type": "pkcs8",
                                    "format": "der",
                                } )
    ...
    async #checkDomain ( { name, isWildcard, domain, dnsTxtRecordName, resolved } ) {
        ...
        // XXX
        console.log( "---", { name, isWildcard, domain, dnsTxtRecordName, resolved } );
        ...
        if ( resolved ) {
            ...
            TEST: while ( attempts ) {
                ...
                // XXX
                console.log( "---", domain, res + "", res.headers.get( "x-acme-test" ), res.headers.get( "x-acme-test" ) === hostHash );
