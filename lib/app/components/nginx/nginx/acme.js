...
export default class {
    ...
    constructor ( nginx ) {
        ...
        if ( this.#nginx.config.clodFlare?.apiKey && this.#nginx.config.clodFlare?.email ) {
            ...
            this.#acmeAccountKeyPath = path.posix.join( this.#nginx.config.storageLocation, "acme-account.key" );
        }
    }
    ...
    async #createAcme () {
        ...
        const res = await this.#nginx.app.storage.getFile( this.#acmeAccountKeyPath );
        if ( res.ok ) {
            this.#acmeAccountKey = await res.file.buffer();
        }
    ...
    // XXX
    async #storeAccountKey ( acmeAccountKey ) {
        if ( this.#acmeAccountKey === acmeAccountKey ) return;

        this.#acmeAccountKey = acmeAccountKey;

        await await this.#nginx.app.storage.uploadFile(
            new File( {
                "buffer": acmeAccountKey,
            } ),
