    ...
    async init () {
        ...
        try {
            ...
            this.#acme = new Acme( {
                ...
                "accountKey": acmeAccount?.accountKey ? Buffer.from( acmeAccount.accountKey, "hex" ) : null,
                ...
                res = await this.#nginx.app.storage.uploadFile(
                    ...
                    new File( {
                        "buffer": JSON.stringify( {
                            "accountKey": this.#acme.accountKey.toString( "hex" ),
            ...
            this.#id = crypto.createHash( "md5" ).update( this.#acme.accountKey ).digest( "base64url" );
    ...
    async #checkDomain ( { name, isWildcard, domain, dnsTxtRecordName, resolved } ) {
        ...
        if ( resolved ) {
            ...
            while ( attempts ) {
                ...
                if ( res.ok && res.headers.get( "x-acme-test" ) === this.#id ) return true;
