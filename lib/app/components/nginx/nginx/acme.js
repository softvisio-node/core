...
export default class {
    ...
    async #getStoredCertificate ( id, { checkRenewInterval } = {} ) {
        ...
        if ( this.#nginx.config.acmeuseFiles ) {
            ...
            certificate = this.app.crypto.decrypt( fs.readFileSync( certificatePath ) );

            privateKey = this.app.crypto.decrypt( fs.readFileSync( keyPath ) );
        }
        else {
        ...
        const certtificateTmpFile = this.app.env.tmpFile(),
            privateKeyTmpFile = this.app.env.tmpFile();
