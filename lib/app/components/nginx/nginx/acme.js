...
export default class {
    ...
    async #checkDomain ( { name, isWildcard, domain, dnsTxtRecordName, resolved } ) {
        ...
        if ( resolved ) {
            ...
            while ( attempts ) {
                ...
                if ( res.ok && res.headers.get( "x-acme-test" ) === sign ) return true;
    ...
    async #uploadCertificate ( { certificate, certificatePath, key, privateKeyPath, expires } ) {
        var res;
        ...
        if ( name === "test" ) {
            ...
            if ( !host ) {
                ...
                return req.end( {
                    "status": 200,
                    "headers": {
                        "x-acme-test": this.#signHost( host ),
                    },
                } );
            }
        }
            ...
            return this.app.storage.downloadFile( req, name, {
                "cwd": this.app.acmeChallengesStorageLocation,
            } );
        }
    }
}
