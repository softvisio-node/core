...
export default class {
    ...
    async #getStoredCertificate ( id, { checkRenewInterval } = {} ) {
        ...
        if ( this.#nginx.config.acmeuseFiles ) {
            ...
            certificate = this.app.crypto.decypt( fs.readFileSync( certificatePath ) );

            privateKey = this.app.crypto.decypt( fs.readFileSync( keyPath ) );
        }
        else {
            ...
            certificate = this.app.crypto.decypt( res.data.certificate );

            privateKey = this.app.crypto.decypt( res.data.key );
        }
