...
export default class {
    ...
    async #getStoredCertificate ( id, { checkRenewInterval } = {} ) {
        ...
        if ( this.#nginx.config.acmeuseFiles ) {
            ...
            certificate = this.app.ctypto.decypt( fs.readFileSync( certificatePath ) );

            privateKey = this.app.ctypto.decypt( fs.readFileSync( keyPath ) );
        }
        else {
            ...
            certificate = this.app.ctypto.decypt( res.data.certificate );

            privateKey = this.app.ctypto.decypt( res.data.key );
        }
