...
export default class {
    ...
    constructor ( nginx ) {
        ...
        this.#dataDir = this.#dataDir = this.#nginx.dataDir + "/acme-" + ( this.#nginx.config.acme.test ? "test" : "production" );
    ...
    async #createChallenge ( { type, domain, dnsTxtRecordName, httpLocation, token, content } ) {
        ...
        // http
        if ( type === "http-01" ) {
            if ( !this.#nginx.config.acme.httpEnabled ) return false;

            if ( this.#nginx.config.acme.useLocalStorage ) {
                ...
                const res = await this.dbh.do( SQL.insertChallenge, [
                    ...
                    token,
                    content,
                    new Interval( this.#nginx.config.acme.challengeMaxAge ).toDate(),
                ] );
