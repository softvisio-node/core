...
export default class NginxServer {
    ...
    constructor ( nginx, id, { type, port, serverName, upstreamDefaultPort, maxBodySize, cacheEnabled, cacheBypass, sslEnabled = true } = {} ) {
        this.#nginx = nginx;
        this.#id = id;
        this.#type = type;
        ...
        // upstream port
        if ( !upstreamDefaultPort && this.isHttp ) upstreamDefaultPort = 80;
        this.#upstreamDefaultPort = this.#validatePort( upstreamDefaultPort );
        ...
        // stream
            ...
            if ( this.#port === 443 && this.#serverName.length ) {
                this.#mapConfigPath = this.#nginx.configsDir + `/map/${this.#id}.443.nginx.conf`;
            }
        }
    }
    ...
    #writeConfig ( config ) {
        ...
        if ( this.#mapConfigPath ) {
            ...
            fs.writeFileSync( this.#mapConfigPath, this.#serverName.map( serverName => `"${serverName}" "${this.#id}"` ).join( "\n" ) );
        }
