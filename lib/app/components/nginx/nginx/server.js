...
export default class NginxServer {
    ...
    constructor ( nginx, id, { type, port, serverNames, defaultUpstreamPort, upstreams, maxBodySize, cacheEnabled, cacheBypass, sslEnabked = true } = {} ) {
        this.#nginx = nginx;
        this.#id = id;
        this.#type = type;
        ...
        this.#names = [...new Set( serverNames )].filter( name => name ).sort();
        ...
        this.#cacheEnabled = this.#cacheEnabled ? !!cacheEnabled : false;
        this.#cacheBypass = !!cacheBypass;

        this.#sslEnabked = !!sslEnabked;
    ...
    async reload () {
        ...
        // XXX
        // if ( !this.#nginx.isStarted ) return;
    ...
    async #updateCertificates () {
        try {
            if ( !this.#sslEnabked ) throw false;

            if ( !this.#names.length ) throw false;

            // certs already exists
            if ( this.#sslCertificateHash ) return;

            const res = await this.#nginx.acme.getCertificate( [...this.#names] );
            if ( !res.ok ) {
