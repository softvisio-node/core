...
export default class NginxServer {
    ...
    async addUpstreams ( upstreams ) {
        ...
        for ( const upstream of added ) {
            ...
            await fetch( `http://127.0.0.1:${this.#nginx.config.dynamicUpstreamsPort}/dynamic-upstream?upstream=${this.id}-${this.#nginx.config.httpPort}&add=&server=${server}` );
        }
    }
    ...
    async deleteUpstreams ( upstreams ) {
        ...
        for ( const upstream of deleteed ) {
            ...
            await fetch( `http://127.0.0.1:${this.#nginx.config.dynamicUpstreamsPort}/dynamic-upstream?upstream=${this.id}-${this.#nginx.config.httpPort}&remove=&server=${server}` );
        }
    }
}
