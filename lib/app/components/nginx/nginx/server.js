...
export default class NginxServer {
    ...
    constructor ( nginx, id, { httpPort, upstreams, serverNames, streamPorts, upstreamHttpPort, clientMaxBodySize, cacheEnabled, cacheStatus, cacheBypass } = {} ) {
        this.#nginx = nginx;
        this.#id = id;
    ...
    async updateNginxConfigs () {
        ...
        const wasInstalled = this.isInstalled,
            defaultServer = this.#serverNames.has( "*" ),
            defaultServerInstalled = defaultServer && this.#httpConfigInstalled;
        ...
        try {
            ...
            if ( this.#upstreams.size && this.#serverNames.size ) {
                ...
                if ( defaultServer ) {
                    ...
                    // other default server is installed
                    if ( !defaultServerInstalled && !this.#nginx.defaultServerInstalled( this.#httpPort ) ) {
                        throw result( [400, `Nginx default server for port ${this.#httpPort} already exists`] );
                    }
                    else {
                        this.#nginx.deleteDefaultServer( this.#httpPort );
                    }
                }
            ...
            if ( !this.isInstalled ) {
                ...
                // config is not valid
                    ...
                    if ( defaultServer ) this.#nginx.installDefaultServer( this.#httpPort );
