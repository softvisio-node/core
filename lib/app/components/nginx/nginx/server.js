...
export default class NginxServer {
    ...
    async #syncUpstreams ( operation, upstreams ) {
        ...
        for ( const upstream of upstreams ) {
            ...
            promises.push( fetch( `http://127.0.0.1:${this.#nginx.config.httpPort}/?upstream=${this.id}-${this.#nginx.config.httpPort}&${operation}=&server=${server}`, {
                "headers": {
                    "host": "dynamic-upstreams",
                },
            ...
            for ( const streamPort of this.#streamPorts ) {
                ...
                promises.push( fetch( `http://127.0.0.1:${this.#nginx.config.httpPort}/?upstream=${this.id}-${streamPort}&${operation}=&server=${server}&stream=`, {
                    "headers": {
                        "host": "dynamic-upstreams",
                    },
                } ).then( res => console.log( `Server ${this.id}, add upstream: ${server} ... ${res}` ) ) );
