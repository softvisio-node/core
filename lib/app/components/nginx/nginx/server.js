...
export default class NginxServer {
    ...
    async #syncUpstreams ( operation, upstreams ) {
        ...
        for ( const upstream of upstreams ) {
            ...
            fetch( `http://127.0.0.1:${this.#nginx.config.dynamicUpstreamsPort}/dynamic-upstream?upstream=${this.id}-${this.#nginx.config.httpPort}&${operation}=&server=${server}` ).then( res => console.log( `Server ${this.id}, add upstream: ${server} ... ${res}` ) );

            for ( const streamPort of this.#streamPorts ) {
                const server = upstream + ":" + streamPort;

                fetch( `http://127.0.0.1:${this.#nginx.config.dynamicUpstreamsPort}/dynamic-upstream?upstream=${this.id}-${streamPort}&${operation}=&server=${server}&stream=` ).then( res => console.log( `Server ${this.id}, add upstream: ${server} ... ${res}` ) );
            }
        }
