    ...
    constructor ( proxy, { port, type, serverName, sslEnabled, maxBodySize, cacheEnabled, cacheBypass, proxyProtocol } = {} ) {
        ...
        if ( this.isHttp ) {
            if ( this.#serverName.length ) {
                ...
                this.#configPath += "http-servers-default";
            }
        }
        else {
        ...
        if ( this.isHttp ) {
            if ( this.ssl ) {
                this.#socketPath = this.nginx.getSocketPath( "https", this.port );
            }
            else {
                this.#socketPath = this.nginx.getSocketPath( "http", this.port );
            }
        }
        else {
            if ( this.ssl ) {
                this.#socketPath = this.nginx.getSocketPath( this.proxy.id, this.port );
            }
            else {
                this.#socketPath = this.nginx.getSocketPath( this.proxy.id + "-ssl", this.port );
            }
        }
    }
