...
export default class NginxProxyServer {
    ...
    async updateConfig ( ports ) {
        ...
        if ( !this.#proxy.hasUpstreams ) return;

        // check conflicted ports
        if ( this.port !== 80 && this.port !== 443 ) {
            let id = this.type;
            if ( this.ssl ) id += "/ssl";
            if ( this.proxyProtocol ) id += "/proxy-protocol";

            if ( this.isHttp ) {
                if ( ports[this.port] && ports[this.port] !== id ) return;
            }
            else {
                if ( ports[this.port] ) return;
            }
