...
export default class DockerService {
    ...
    async #update ( labels ) {
        ...
        if ( labels.nginx ) {
            for ( let proxy of labels.nginx.split( /[,\n]/ ) ) {
                ...
                const serverId = `_docker-${this.id}-${options.upstreamPort}`;
    ...
    #onUpstreamsAdd ( addresses ) {
        this.#nginxProxy?.addUpstreams( addresses );
    }

    async #onUpstreamsDelete ( addresses ) {
        this.#nginxProxy?.deleteUpstreams( addresses );
    }
}
