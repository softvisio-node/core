...
export default class DockerService {
    ...
    async #update ( labels ) {
        ...
        if ( labels.nginx ) {
            for ( let proxy of labels.nginx.split( /[,\n]/ ) ) {
                ...
                const serverId = `_docker_${this.id}_${options.type}_${options.port}`;
    ...
    async #onUpstreamsAdd ( addresses ) {
        for ( const server of this.#servers.values() ) {
            console.log( "-- add upstream", server.nginx.id, addresses );

            const res = await server.nginx.addUpstreams( addresses );
