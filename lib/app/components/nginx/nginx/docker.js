...
export default class Docker {
    ...
    #addService ( data ) {
        var service = this.#services[data.ID];

        if ( service ) return this.#updateService( data );

        // XXX
        data.Spec.Labels["nginx.http.server-names"] = "devel,*";

        service = new DockerService( this.#nginx, data.ID, data.Spec.Name, data.Spec.Labels );

        this.#services[service.id] = service;
    }

    #updateService ( data ) {
        const service = this.#services[data.ID];

        if ( !service ) return;

        service.update( data.Spec.Labels );
    }
