export default class NginxConfig {
            ...
            for ( const [ port, options ] of Object.entries( this.#ports ) ) {
                ...
                if ( options.servers[ "http" ] && !options.hasDefaultHttpServer ) {
                    servers.push( {
                        ...
                        "localAddress": this.#useRouter( port )
                            ? this.#getLocalAddress( "http", port, false )
                            : null,
                        "acmeLocation": Boolean( +port === 80 && this.nginx.privateHrrpServerUpstream && this.nginx.app.acme ),
                    } );
                }
                ...
                if ( options.servers[ "http-ssl" ] && !options.hasDefaultHttpSslServer ) {
                    servers.push( {
                        ...
                        "proxyProtocol": options.httpProxyProtocol,
                        "localAddress": this.#useRouter( port )
                            ? this.#getLocalAddress( "http", port, true )
                            : null,
                    } );
                }

                // tcp+ssl
                // if ( options.servers[ "tcp-ssl" ] && !options.hasDefaultTcpSslServer ) {
                //     servers.push( {
                //         "port": +port,
                //         "ssl": true,
                //         "proxyProtocol": options.httpProxyProtocol,
                //         "localAddress": this.#useRouter( port )
                //             ? this.#getLocalAddress( "tcp", port, true )
                //             : null,
                //     } );
                // }
            }
    ...
    #build () {
        ...
        this.#servers = this.#servers.map( server => {
            const options = {
                "localAddress": this.#useRouter( server.port )
                    ? this.#getServerLocalAddress( server )
                    : null,
            };
    ...
    #addServer ( server ) {
        ...
        if ( port ) {
            try {
                ...
                if ( server.port === 80 && !( server.isHttp || server.ssl ) ) throw `Port 80 must be HTTP, non-SSL `;
        ...
        port.servers[ server.type + ( server.ssl
            ? "-ssl"
            : "" ) ] ??= 0;
        port.servers[ server.type + ( server.ssl
            ? "-ssl"
    ...
    #getServerLocalAddress ( server ) {
        ...
        else if ( server.isTcp && server.ssl ) {
            return this.#getLocalAddress( "tcp", server.port, !!server.ssl );
        }
        else {
