export default class NginxConfig {
    ...
    constructor ( nginx ) {
        ...
        if ( this.#nginx.config.localAddress === "unix:" ) {
            ...
            [ this.#localAddress, this.#localAddressPort ] = this.#nginx.config.localAddress.split( ":" );

            this.#localAddressPort = +this.#localAddressPort;
        }
    }
    ...
    #addServer ( server ) {
        ...
        if ( server.ssl ) {
            if ( server.isDefaultServer ) {
                port.router.defaultSslServer = this.#getServerLocalAddress( server );
            }
            else {
                for ( const serverName of server.serverName ) {
