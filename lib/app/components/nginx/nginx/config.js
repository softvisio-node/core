export default class NginxConfig {
    ...
    addServer ( server ) {
        ...
        if ( port ) {
            try {
                ...
                // http
                if ( server.isHttp ) {
                    if ( !server.serverName.length && port.hasDefaultHttpServer ) throw true;
        ...
        if ( server.isHttp ) {
            port.hasHttpServer = true;

            if ( !server.serverName.length ) port.hasDefaultHttpServer = true;
        }
    ...
    getDefaultHttpServers () {
        ...
        for ( const [port, options] of Object.values( this.#ports ) ) {
            if ( !options.hasHttpServer ) continue;
