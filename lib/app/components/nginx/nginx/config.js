var USE_LOCAL_SOCKET = false, // XXX
...
export default class NginxConfig {
    ...
    getRouters () {
        ...
        for ( const [ port, options ] of Object.entries( this.#ports ) ) {
            ...
            if ( options.router.defaultLocalAddress ) {
                ...
                router.defaultLocalAddress = this.nginx.getLocalAddress( "http", port, false );
            }
    ...
    #addServer ( server ) {
        ...
        if ( server.ssl ) {
            if ( server.isDefaultServer ) {
                port.router.defaultLocalSslAddress = server.localAddress;
            }
            else {
                for ( const serverName of server.serverName ) {
                    port.router.serverName[ serverName ] ||= server.localAddress;
                }
            }
        }
        else {
            port.router.defaultLocalAddress = server.localAddress;
        }
    ...
    #getLocalAddress ( name, port, ssl ) {
        return this.#nginx.getLocalAddress( name, port, ssl );
    }

    // XXX
    #getLocalAddress1 ( name, port, ssl ) {
        if ( !USE_LOCAL_SOCKET ) {
            ...
            if ( !address ) {
                address = LISTEN_ADDRESS + ":" + this.#localAddressPort++;
