export default class NginxConfig {
    ...
    getRouters () {
        ...
        for ( const [ port, options ] of Object.entries( this.#ports ) ) {
            ...
            if ( options.router.defaultSocket ) {
                ...
                router.defaultSocket = this.nginx.getLocalAddress( "http", port, false );
            }
    ...
    #addServer ( server ) {
        ...
        if ( server.ssl ) {
            if ( server.isDefaultServer ) {
                port.router.defaultSslSocket = server.localAddress;
            }
            else {
                for ( const serverName of server.serverName ) {
            ...
            port.router.defaultSocket = server.localAddress;
        }
