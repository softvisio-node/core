export default class NginxConfig {
            ...
            try {
                ...
                if ( server.ssl && !server.isDefaultServer ) {
                    ...
                    for ( const serverName of server.serverName ) {
                        if ( !port.router.serverName[serverName] ) hasUniqueServerName = true;
                    }

                    if ( !hasUniqueServerName ) return;
                }
            }
            catch ( e ) {
    ...
    getRouters () {
        ...
        for ( const [port, options] of Object.entries( this.#ports ) ) {
            ...
            else if ( options.server["http"] ) {
                router.defaultSocket = this.nginx.getSocketPath( "http", port, false );
            }

            if ( options.router.defaultSslSocket ) {
                router.defaultSslSocket = options.router.defaultSslSocket;
            }
            else if ( options.server["http-ssl"] ) {
                router.defaultSslSocket = this.nginx.getSocketPath( "http", port, true );
            }
