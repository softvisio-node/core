...
CREATE FUNCTION acl_has_user ( _acl_id int8, _user_id int8 ) RETURNS bool STABLE AS $$
BEGIN
    IF _user_id = ${constants.rootUserId} THEN
...
CREATE FUNCTION acl_user_editable ( _acl_id int8, _acl_user_id int8, _parent_user_id int8 ) RETURNS bool STABLE AS $$
BEGIN
        ...
        RETURN NOT EXISTS (
            WITH acl_user_permissions AS (
                ...
                WHERE
                    ...
                    AND acl_user_role.acl_role_id = acl_role.id
                    AND acl_user.acl_id = acl_user_role.acl_id AND acl_user.user_id = acl_user_role.user_id
                    AND acl_user.acl_id = _acl_id AND acl_user.user_id = _acl_user_id AND acl_user.enabled
            ),
            ...
            WHERE
                ...
                AND acl_user_role.acl_role_id = acl_role.id
                AND acl_user.acl_id = acl_user_role.acl_id AND acl_user.user_id = acl_user_role.user_id
                AND acl_user.acl_id = _acl_id AND acl_user.user_id = _parent_user_id AND acl_user.enabled
        )
        SELECT FROM
            acl_user_permissions
        WHERE
            permission NOT IN ( SELECT permission FROM parent_user_permissions )
        );
    END IF;
