        ...
        res = await this.dbh.begin( async dbh => {
            ...
            // delete types
            {
                ...
                if ( deletedTypes.length ) {
                    ...
                    // XXX disable roles
                    // XXX disable notifications

                    // delete permissions
                    res = await dbh.do( sql`UPDATE acl_notifications SER enabled = FALSE WHERE acl_type_id`.IN( deletedTypes ) );
                    if ( !res.ok ) throw res;
            ...
            for ( const type of Object.values( types ) ) {
                ...
                if ( !index[type.type] ) {
                    res = await dbh.selectRow( sql`INSERT INTO acl_type ( id, type ) VALUES ( ?, ? ) RETURNING id`, [
                        ...
                        //
                        type.type === constants.defaultAclType ? constants.defaultAclId : null,
                        type.type,
                    ] );
                    if ( !res.ok ) throw res;
