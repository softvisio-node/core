...
SELECT
    ...
    acl_role.role,
    acl_role.name AS role_name,
    acl_role.description AS role_description,
...
export default class Acl {
    ...
    async updateTypes ( types ) {
        ...
        res = await this.dbh.begin( async dbh => {
            ...
            res = await dbh.select( SQL.loadTypes );
            if ( !res.ok ) throw res;
            ...
            if ( res.data ) {
                for ( const row of res.data ) {
                    ...
                    index[row.type].roles[row.role].name = row.role_name;
                    index[row.type].roles[row.role].description = row.role_description;
...
` );
    ...
    // XXX load notifications// private
    async #loadTypes () {
        ...
        if ( res.data ) {
            for ( const row of res.data ) {
                if ( !row.type_enabled || !row.role_enabled ) continue;
                ...
                types[row.type].roles[row.role].permissions.push( row.permission );
            }
        }
