...
export default class Acl {
    ...
    async resolveAclPermissions ( userId, aclResolvers ) {
        ...
        // user is root
            ...
            return this.getAclUserFullPermissionss( [ ...aclIds ], userId );
        }
    }
...
` );
    ...
    // XXX
    async getAclUserFullPermissions ( aclId, userId ) {
        if ( !userId ) {
            GUESTS_PERMISSIONS;
        }
        else if ( this.app.userIsRoot( userId ) ) {
            ...
            const id = constants.mainAclId + "/" + aclId;

            if ( !this.#aclUserPermissionsCache[ userId ]?.[ id ] ) {
                const permissions = [];

                for ( const _aclId of [ constants.mainAclId.aclId ] ) {
                    const user = await this.getAclUser( _aclId, userId );
