...
` );
    ...
    async getAclUserRoles ( aclId, userId, { parentUserId } = {} ) {
        ...
        var user, parentUserPermissions;

        // get parent user
        if ( parentUserId && !this.app.userIsRoot( parentUserId ) ) {
            parentUserPermissions = await this.getAclUserPermissions( [ constants.mainAclId, aclId ], parentUserId );

            if ( !parentUserPermissions ) return result( [ 400, `Unable to get ACL roles` ] );
        }
        ...
        for ( const role of Object.values( this.#types[ aclType ].roles ) ) {
            ...
            else if ( userId === parentUserPermissions?.userId ) {
                readonly = true;
            }
    ...
    async getAclUserPermissions ( aclIds, userId ) {
        if ( !userId ) {
            GUESTS_PERMISSIONS;
        }
        else if ( this.app.userIsRoot( userId ) ) {
            const aclType = await this.#getAclType( aclIds );
