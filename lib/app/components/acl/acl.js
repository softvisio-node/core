...
export default class Acl {
    ...
    async deleteAclUser ( aclId, userId, { parentUserId, dbh } = {} ) {
        ...
        // check parent user
        if ( parentUserId ) {
            const parentUserPermissions = await this.getAclUserPermissions( [ constants.mainAclId, aclId ], parentUserId );
    ...
    async addAclUserRoles ( aclId, userId, roles, { parentUserId, dbh } = {} ) {
        ...
        // check parent user
        if ( parentUserId ) {
            var parentUserPermissions = await this.getAclUserPermissions( [ constants.mainAclId, aclId ], parentUserId );

            if ( !parentUserPermissions ) return result( [ 400, `Unable to get parnet user` ] );
        }
...
` );
    ...
    async getAclUserRoles ( aclId, userId, { parentUserId } = {} ) {
        ...
        for ( const role of Object.values( this.#types[ aclType ].roles ) ) {
            ...
            const enabled = user ? user.hasRoles( role.role ) : false;
