...
`,
    ...
    "setAllChannelsEnabled": sql`
INSERT INTO
    acl_user_notification
( alc_id, user_id, acl_notification_id, internal, email, telegram, push )
VALUES ( ?, ?, ?, ?, ?, ?, ? )
...
WHERE
    acl_id = ?
    AND user_id = ?
    AND acl_notification_id = ( SELECT id FROM acl_notification WHERE acl_type_id = ? AND notification = ? )
ON CONFLICT ( acl_id, user_id, acl_notification_id ) DO UPDATE SET
    internal = EXCLUDED.internal
`.prepare(),
...
WHERE
    ...
    AND acl_notification_id = ( SELECT id FROM acl_notification WHERE acl_type_id = ? AND notification = ? )
ON CONFLICT ( acl_id, user_id, acl_notification_id ) DO UPDATE SET
    push = EXCLUDED.internal
`.prepare(),
    },

    "getUserNotifications": sql`
SELECT
    json_agg_object(
...
` );
    ...
    async getAclUserNotificationsProfile ( aclId, userId ) {
        ...
        const res = await this.dbh.select( SQL.getUserNotifications, [aclId, userId] );
        ...
        for ( const notification of this.#types[user.aclType].notifications ) {
            ...
            for ( const channel of Object.values( notification.channels ) ) {
                p.channels[channel.channel] = {
                    ...
                    "enabled": !channel.enabled ? false : userNotifications[notification.notification][channel.channel] ?? channel.default,
                };
            }
        }
    ...
    async setAclUserNotificationChannelEnabled ( aclId, userId, notification, channel, enabled ) {
        const user = await this.#getAclUser( aclId, userId );

        if ( !user ) return result( [500, `ACL user not found`] );

        if ( !this.#types[user.aclType]?.notifications?.[notification] ) {
            return result( [400, `Invalid ACL notification`] );
        }

        if ( !this.#types[user.aclType].notifications[notification][channel].enabled ) {
            return result( [400, `ACL notification channel is disabled`] );
        }

        if ( !this.#types[user.aclType].notifications[notification][channel].editable ) {
            return result( [400, `ACL notification channel is read-only`] );
        }

        return this.dbh.do( SQL.setChannelEnabled[channel], [
    ...
    async subscribeAclUserNotification ( aclId, userId, notification ) {
        const user = await this.#getAclUser( aclId, userId );

        if ( !user ) return result( [500, `ACL user not found`] );

        const notificationSpec = this.#types[user.acltype]?.notifications?.[notification];

        if ( !notificationSpec ) {
            return result( [400, `Invalid ACL notification`] );
        }

        return this.dbh.do( SQL.setAllChannelsEnabled, [
    ...
    async unsubscribeAclUserNotification ( aclId, userId, notification ) {
        const user = await this.#getAclUser( aclId, userId );
                    ...
                    if ( !notification.enabled ) {
                        ...
                        for ( const channel of ["internal", "email", "telegram", "push"] ) {
                            ...
                            if ( notification.channels[channel].enabled ) {
                                notification.channels[channel].default ??= true;
                                notification.channels[channel].editable ??= true;
                            }
                            else {
                                notification.channels[channel].default = false;
