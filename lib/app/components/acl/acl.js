        ...
        // user is root
            ...
            if ( aclResolvers ) {
                for ( const aclResolver of aclResolvers ) {
                    ...
                    aclId.push( resolvedAclId );
                }
            }

            return this.getAclUserFullPermissionss( aclId, userId );
        }
    }
...
` );
    ...
    async getAclUserFullPermissions ( aclIds, userId ) {
        if ( !userId ) {
            ...
            if ( !aclIds ) {
                aclIds = [ constants.mainAclId ];
            }
            else if ( !Array.isArray( aclIds ) ) {
                aclIds = [ aclIds ];
            }
            ...
            if ( aclIds.length === 1 ) {
                if ( aclIds[ 0 ] === constants.mainAclId ) {
                    id = constants.mainAclId;
                }
                else {
                    aclIds.unshift( constants.mainAclId );

                    id = constants.mainAclId + "/" + aclIds[ 0 ];
                }
            }
            else {
                aclIds = [ constants.mainAclId, ...new Set( aclIds ) ].sort();
