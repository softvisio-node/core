...
export default class Acl {
    ...
    async resolveAclPermissions ( userId, aclResolvers ) {
        ...
        // user is root
            ...
            const aclIds = new Set( [ constants.mainAclId ] );
            ...
            return this.getAclUserPermissions( [ ...aclIds ], userId );
        }
    }
    ...
    async addAclUser ( aclId, userId, { enabled = true, roles, parentUserId, dbh } = {} ) {
        ...
        if ( parentUserId && !this.app.userIsRoot( parentUserId ) ) {
            ...
            if ( !parentUser?.isEnabled ) return result( [ 400, `Unable to get parnet user` ] );
        }
    ...
    async deleteAclUser ( aclId, userId, { parentUserId, dbh } = {} ) {
        ...
        if ( parentUserId && !this.app.userIsRoot( parentUserId ) ) {
            ...
            if ( !parentUser.permissions.hasAll( user.permissions ) ) {
                return result( [ 400, `Unable to delete ACL user` ] );
            }
        }
    ...
    async addAclUserRoles ( aclId, userId, roles, { parentUserId, dbh } = {} ) {
        ...
        for ( const role of roles ) {
            ...
            if ( parentUser && !parentUser.permissions.hasAll( this.#types[ aclType ].roles[ role ].permissions ) ) {
...
` );
    ...
    async getAclUserRoles ( aclId, userId, { parentUserId } = {} ) {
        ...
        if ( parentUserId && !this.app.userIsRoot( parentUserId ) ) {
            ...
            if ( !parentUser?.isEnabled ) return result( [ 400, `Unable to get ACL roles` ] );
        }
        ...
        for ( const role of Object.values( this.#types[ aclType ].roles ) ) {
            ...
            else if ( userId === parentUser?.id ) {
                readonly = true;
            }
            else if ( parentUser ) {
                readonly = !parentUser.permissions.hasAll( role.permissions );
            }
            else {
                readonly = false;
            }

            const enabled = userIsRoot ? true : user ? user.hasRoles( role.role ) : false;
    ...
    async getAclUserPermissions ( aclId, userId ) {
        ...
        else if ( Array.isArray( aclId ) ) {
            const id = userId + "/" + aclId.sort().join( "/" );

            if ( !this.#aclUserPermissionsCache[ id ] ) {
                const permissions = [];

                var member;

                for ( const _aclId of aclId ) {
                    const aclUser = await this.getAclUser( _aclId, userId );

                    // XXX do not cache on error

                    // acl user not found of disabled
                    if ( !aclUser?.isEnabled ) continue;

                    permissions.push( ...aclUser.permissions );

                    member = true;
                }

                this.#aclUserPermissionsCache[ id ] = new Permissions( userId, permissions, { member } );
            }

            return this.#aclUserPermissionsCache[ id ];
        }
        else {
    ...
    async #sendAclNotification ( aclId, notificationType, subject, body, options = {} ) {
        ...
        for ( const user of users ) {
            for ( const channel of Object.values( notification.channels ) ) {
                ...
                const subscribed = channel.editable ? user[ channel.channel ] ?? channel.subscribed : channel.subscribed;
