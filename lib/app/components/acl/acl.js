...
export default class Acl {
    ...
    async deleteAclUser ( aclId, userId, { parentUserId, dbh } = {} ) {
        ...
        if ( parentUserId && !this.app.userIsRoot( parentUserId ) ) {
            ...
            if ( !this.#comparePermissions( user.permissions, parentUser.permissions ) ) {
                return result( [400, `Unable to delete ACL user`] );
            }
        }
    ...
    async addAclUserRoles ( aclId, userId, roles, { parentUserId, dbh } = {} ) {
        ...
        for ( const role of roles ) {
            ...
            if ( parentUser ) {
                for ( const permission of this.#types[aclType].roles[role].permissions ) {
                    if ( !parentUser.permissions.has( permission ) ) {
    ...
    async #setAclUserRoles ( aclId, userId, roles, { parentUserId, dbh } = {} ) {
        ...
        for ( const role of roles ) {
            ...
            if ( parentUser ) {
                ...
                if ( !this.#comparePermissions( this.#types[aclType].roles[role].permissions, parentUser.permissions ) ) {
...
` );
    ...
    #comparePermissions ( permissions, parentPermisiions ) {
        for ( const permission of permissions ) {
            if ( !parentPermisiions.has( permission ) ) return false;
        }
