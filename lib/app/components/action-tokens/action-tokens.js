...
const SQL = {
    ...
    "insertToken": sql`INSERT INTO action_token ( user_id, type, expires, data ) VALUES ( ?, ?, ?, ? ) RETURNING id, ( SELECT email FROM "user" WHERE id = user_id ) AS email`.prepare(),

    "insertHash": sql`INSERT INTO action_token_hash ( action_token_id, fingerprint, hash ) VALUES ( ?, ?, ? )`.prepare(),
...
export default class {
    ...
    async createActionToken ( userId, tokenType, { length, maxAge, data, dbh } = {} ) {
        dbh ||= this.dbh;
        ...
        return dbh.begin( async dbh => {
            ...
            const expires = Duration.new( maxAge ).toDate();
            ...
            // generate token
            const token = Token.generate( this.app, tokenType, tokenId, { length } );
    ...
    async activateActionToken ( token, tokenType, { userId, confirmEmail, dbh } = {} ) {
