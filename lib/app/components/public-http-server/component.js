...
export default class extends Component {
    ...
    async _afterAppStarted () {
        ...
        if ( this.#nginxEnabled ) {
            const res = await this.app.nginx.addServer( "_public-http-server", {
                ...
                "upstreamHttpPort": this.instance.port,
                "upstreams": this.instance.address,
            } );
    ...
    async #registerNginx () {
        ...
        const options = objectOmit( this.config.nginx, ["serverId", "apiUrl"] );
        options.upstreamHttpPort = this.config.port;

        while ( true ) {
            const res = await api.call( {
                "methpd": "nginx/register",
                "argumants": [this.config.nginx.serverId, options],
                signal,
            } );
