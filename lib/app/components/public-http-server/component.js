...
export default class extends Component {
    ...
    async _configureInstance () {
        ...
        if ( this.#nginxEnabled ) {
            this.#address = `${this.app.nginx.unixSocketsPath}/_public-http-server_http.${this.config.port}.socket`;
            this.#port = null;
    ...
    // private
    async #registerNginx () {
        if ( !this.config.nginxServerId ) return;
        ...
        while ( true ) {
            const res = await api.call( {
                ...
                "argumants": [this.config.nginxServerId, options],
                signal,
            } );
