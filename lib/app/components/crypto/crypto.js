...
export default class Env {
    ...
    encrypt ( data, { inputEncoding } = {} ) {
        return this.#encrypt( data, { inputEncoding } );
    }

    decrypt ( data, { inputEncoding } = {} ) {
        return this.decrypt( data, { inputEncoding } );
    }
    ...
    #encrypt ( data, { key, inputEncoding } = {} ) {
    ...
    #decrypt ( data, { inputEncoding } = {} ) {
        ...
        if ( data instanceof stream.Readable ) {
                    ...
                    if ( !decipher ) {
                        ...
                        if ( buffer.length < 2 ) {
                            ...
                            if ( buffer.length < size + 2 ) {
                                ...
                                try {
                                    const header = msgpack.decode( buffer.subarray( 2, size + 2 ) ),
                                        key = header.key === MASTER_KEY_ID
                                            ? this.#key
                                            : this.#keys[ header.key ];

                                    decipher = crypto.createDecipheriv( header.cipher, key.key, header.iv );
            ...
            const size = data.readUInt16BE(),
                header = msgpack.decode( data.subarray( 2, size + 2 ) ),
                key = header.key === MASTER_KEY_ID
                    ? this.#key
                    : this.#keys[ header.key ];

            const decipher = crypto.createDecipheriv( header.cipher, key.key, header.iv );
