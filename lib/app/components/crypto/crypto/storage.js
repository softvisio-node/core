...
import { decrypt, encrypt, getCipherOptions, hash } from "#lib/crypto";
...
export default class CryptoStorage {
    ...
    constructor ( crypto, masterKey ) {
        ...
        this.#cipherOptions = getCipherOptions( this.#crypto.algorithm );
    }
    ...
    async _rewrapKey ( encryptedKey, masterKey ) {
        ...
        res = await this._wrapKey( unencryptedKey, { masterKey } );
        if ( !res.ok ) return res;
    ...
    #parseKey ( key ) {
        try {
            ...
            if ( key.type !== "secret" || key.symmetricKeySize !== this.#cipherOptions.keyLength ) {
                return result( [ 400, `AES key ${ this.#cipherOptions.keyLength } bytes length is required` ] );
            }
    ...
    async #generateKey () {
        return new Promise( resolve => {
            crypto.generateKey(
                "aes",
                {
                    "length": this.#cipherOptions.keyLength * 8,
                },
                ( e, key ) => resolve( e
                    ? result( [ 500, e ] )
                    : result( 200, key ) )
    ...
    async #initEncryption ( { buffer, key } = {} ) {
        ...
        // create header
            ...
            const iv = await new Promise( ( resolve, reject ) => {
                crypto.randomBytes( this.#cipherOptions.ivLength, ( e, buffer ) => {
                    if ( e ) {
                        reject( e );
                    }
                    else {
            ...
            return {
                "algorithm": this.#cipherOptions.algorithm,
                "key": key.key,
                iv,
                "header": msgpack.encode( [ this.#cipherOptions.algorithm, key.id, iv ] ),
            };
        }
    }
}
