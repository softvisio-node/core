...
const defaultServerTemplate = fs.readFileSync( new URL( "resources/default.http.nginx.conf", import.meta.url ), "utf8" );
...
export default class Nginx {
    ...
    async start () {
        ...
        const conf = ejs.render( fs.readFileSync( new URL( "resources/nginx.conf", import.meta.url ), "utf8" ), {
            ...
            "cacheMinFree": this.config.cacheMinFree,
            "cacheInactive": this.config.cacheInactive,
        } );
        ...
        this.installDefaultServer( this.config.httpPort );
    ...
    async addServer ( id, options ) {
        ...
        if ( !server.serverNames.size && !server.streamPorts.size ) {
            return result( [400, `Nginx server ${server.id} can't be added`] );
        }
    ...
    // XXX
    defaultServerInstalled ( httpPort ) {
        return this.#defaultServerInstalled[httpPort];
    }

    // XXX
    installDefaultServer ( httpPort ) {
        const defaultServerConfigPath = this.#configsDir + `/_default.${httpPort}.http.nfinx.conf`;

        this.#defaultServerInstalled[httpPort] = true;
    ...
    // XXX
    deleteDefaultServer ( httpPort ) {
        if ( !this.#defaultServerInstalled[httpPort] ) return;

        this.#defaultServerInstalled[httpPort] = false;
