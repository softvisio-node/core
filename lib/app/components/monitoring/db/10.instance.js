...
CREATE TABLE monitoring_instance (
    ...
    UNIQUE ( cluster_id, package_name, service_name, instance_id )
);
...
CREATE FUNCTION insert_monitoring_instance ( _cluster_id text, _package_name text, _service_name text, _instance_id uuid ) RETURNS int8 AS $$
DECLARE
    _id int8;
BEGIN

    SELECT id FROM monitoring_instance WHERE instance_id = _instance_id INTO _id;

    IF _id IS NULL THEN
        INSERT INTO monitoring_instance ( cluster_id, package_name, service_name, instance_id ) VALUES ( _cluster_id, _package_name, _service_name, _instance_id ) RETURNING id INTO _id;
    END IF;
