...
CREATE INDEX monitoring_instance_package_service_instance_id_idx ON monitoring_instance ( package, service, instance_id );
...
CREATE FUNCTION insert_monitoring_instance ( _package text, _service text, _instance_id uuid ) RETURNS int8 AS $$
DECLARE
    _id int8;
BEGIN
    ...
    IF _id IS NULL THEN
        INSERT INTO
            monitoring_instance
        ( package, service, instance_id )
        VALUES
        ( _package, _service, _instance_id )
