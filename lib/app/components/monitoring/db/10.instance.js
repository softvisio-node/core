...
CREATE TABLE monitoring_instance_stats (
    ...
    cpu_user_delta int53,
    cpu_system_delta int53,
    memory_free int53,
    memory_free_percent number2,
    memory_used int53,
    memory_used_percent number2,
    memory_rss int53,
    memory_rss_percent number2,
    fs_free int53,
    fs_free_percent number2,
    fs_used int53,
    fs_used_percent number2,
    UNIQUE ( monitoring_instance_id, date )
);

CREATE FUNCTION insert_monitoring_instance ( _package text, _service text, _instance uuid, _memory_total int53, _fs_total int53 ) RETURNS int8 AS $$
DECLARE
    _id int8;
BEGIN
