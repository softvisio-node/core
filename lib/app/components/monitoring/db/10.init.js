...
CREATE FUNCTION insert_monitoring_method ( _component text, _method text ) RETURNS int8 STABLE AS $$
DECLARE
    _id int8;
BEGIN

    SELECT id FROM monitoring_method WHERE component = _component AND method = _method INTO _id;

    IF _id IS NULL THEN
        INSERT INTO monitoring_method ( component, method ) VALUES ( _component, _method ) RETURNING id INTO _id;
    END IF;
