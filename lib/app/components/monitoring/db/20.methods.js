...
CREATE TABLE monitoring_method (
    ...
    UNIQUE ( package_name, component, method )
);
...
CREATE FUNCTION insert_monitoring_method ( _package_name text, _component text, _method text ) RETURNS int8 AS $$
DECLARE
    _id int8;
BEGIN

    SELECT id FROM monitoring_method WHERE package_name = _package_name AND component = _component AND method = _method INTO _id;

    IF _id IS NULL THEN
        INSERT INTO monitoring_method ( package_name, component, method ) VALUES ( _package_name, _component, _method ) RETURNING id INTO _id;
    END IF;
