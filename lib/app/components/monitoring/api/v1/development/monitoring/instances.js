...
stats AS (
    SELECT
        ...
        avg( cpu_user_delta ) AS cpu_user_delta,
        avg( cpu_system_delta ) AS cpu_system_delta,
        avg( memory_free ) AS memory_free,
        avg( memory_rss ) AS memory_rss,
        avg( fs_free ) AS fs_free
    FROM
...
SELECT
    series.date,
    coalesce( cpu_user_delta::float8, 0 ) AS cpu_user_delta,
    coalesce( cpu_system_delta::float8, 0 ) AS cpu_system_delta,
    coalesce( memory_free::float8, 0 ) AS memory_free,
    coalesce( memory_rss::float8, 0 ) AS memory_rss,
    coalesce( fs_free::float8, 0 ) AS fs_free
FROM
    series
