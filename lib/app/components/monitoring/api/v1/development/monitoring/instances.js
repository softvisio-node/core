...
SELECT
    ...
    coalesce( memory_free / 1_048_576, 0 )::int53 AS memory_free,
    coalesce( memory_rss / 1_048_576, 0 )::int53 AS memory_rss,
    coalesce( fs_free / 1_048_576, 0 )::int53 AS fs_free
FROM
    series
...
stats AS (
    SELECT
        ...
        ( avg( memory_free ) / 1_048_576)::int53 AS memory_free,
        ( avg( memory_rss ) / 1_048_576)::int53 AS memory_rss,
        ( avg( fs_free ) / 1_048_576)::int53 AS fs_free
    FROM
