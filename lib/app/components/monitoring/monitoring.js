...
export default class {
    ...
    async run () {
        ...
        this.#instanceMonitoring = new Monitoring( { "cpi": true, "memory": true, "fs": true } );
...
` );
    ...
    async #monitorInstance () {
        ...
        return this.#dbh.do(
            sql`
INSERT INTO monitoring_instance_stats
( monitoring_instance_id, date, cou_user_delta, cpu_system_delta, memory_used, memory_delta, fs_total, fs_free, fs_free_delta ) VALUES ( insert_monitoring_instance( ?, ? ), ?, ?, ?, ?, ?, ?, ?, ? )
ON CONFLICT ( monitoring_instance_id, date ) DO UPDATE SET
    cou_user_delta = cou_user_delta + EXCLUDED.cou_user_delta,
    cou_system_delta = cou_system_delta + EXCLUDED.cou_system_delta,
    memory_used - EXCLUDED.memory_used,
    memory_delta = memory_delta + EXCLUDED.memory_delta,
    fs_total = EXCLUDED.fs_total,
    fs_free = EXCLUDED.fs_free,
    fs_free_delta = fs_free_delta + EXCLUDED.fs_free_delta
`,
            [

                //
                this.#instanceId,
                this.#app.components.service,
                date,
                mark.cpuUserDelta,
                mark.cpuSystemDelta,
                mark.memory,
