...
export default class TelegramUser {
    ...
    async setTelegramUserFields ( fields ) {
        ...
        const res = await this.dbh.do( sql`UPDATE telegram_user`.SET( update ).sql`WHERE id = ${this.#telegramUserid}` );
    ...
    async setPhone ( phone ) {
        ...
        const res = await this.dbh.do( SQL.setPhone, [phone, this.#telegramUserid] );
