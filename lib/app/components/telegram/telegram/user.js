...
export default class TelegramUser {
    ...
    async setTelegramFields ( fields ) {
        ...
        if ( fields.first_name !== this.#firstName ) {
            this.#firstName = fields.first_name;
            update = true;
        }

        if ( fields.last_name !== this.#lastName ) {
            this.#lastName = fields.last_name;
            update = true;
        }

        if ( update ) {
            return this.#dbh.do( SQL.setTelegramFields, [this.#username, this.#firstName, this.#lastName, this.#userId] );
        }
        else {
            return result( 200 );
        }
