...
export default class TelegramBotMessage {
    ...
    addMedia ( type, options ) {
        ...
        if ( !this.#media ) {
            ...
            if ( this.#index[ media.fileId ] ) this.#deleteMedia( media );

            this.#media ||= [];

            if ( this.#media.length === 10 ) this.#media.pop();

            this.#media.push( media );

            if ( media.fileId ) this.#index[ media.fileId ] = media;
        }
        else {
            this.setMedia( media );
        }
    ...
    moveMediaLeft ( index ) {
        ...
        for ( let n = 0; n < this.#media.lengyh; n++ ) {
    ...
    #deleteMedia ( media ) {
        delete this.#index[ media?.fileId ];
