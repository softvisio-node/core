...
const SQL = {
    "load": sql`SELECT id, data FROM telegram_bot_message WHERE id = ? AND telegram_bot_id = ?`.prepare(),
...
export default class TelegramBotMessage {
    ...
    static async load ( bot, id ) {
        const res = await bot.dbh.selectRow( SQL.load, [ id, bot.id ] );

        if ( !res.data?.id ) return;

        return new TelegramBotMessage( bot, { id, ...( res.data.data || {} ) } );
    }
    ...
    #createMedia ( type, options = {} ) {
        ...
        if ( type instanceof RequestMessage ) {
            if ( MEDIA_TYPE[ type.type ] ) {
                ...
                options.thumbnailFileId = type.thumbnaiFileId;
