...
export default class {
    ...
    async sendCommandsList ( ctx, { level } = {} ) {
        ...
        return ctx.user.sendText( commandsList );
    }
        ...
        if ( !this.#commands[ id ] ) {
            ...
            if ( !commands ) {
                ...
                for ( const commandInstance of commands ) {
                    if ( !commandInstance.checkPermissions( ctx ) ) continue;

                    addedCommands.add( commandInstance.id );

                    data.data.push( {
                        "command": commandInstance.id,
                        "description": this.#translate( ctx, commandInstance.getTitle( ctx ) ),
                    } );
                }

                // add parent command
                if ( parentCommand && !addedCommands.has( parentCommand.id ) ) {
                    if ( parentCommand.checkPermissions( ctx ) ) {
                        addedCommands.add( parentCommand.id );

                        data.data.unshift( {
                            "command": parentCommand.id,
                            "description": this.#translate( ctx, parentCommand.getTitle( ctx ) ),
                        } );
                    }
                }
        ...
        if ( this.#commandsList[ id ] === undefined ) {
            ...
            if ( level === 1 ) {
                for ( const commandInstance of this.childCommands ) {
                    ...
                    commands.push( `/${ commandInstance.id } - ${ this.#translate( ctx, commandInstance.getTitle( ctx ) ) }` );
                }
            }
            ...
            // level 2
                ...
                for ( const commandInstance of this.childCommands ) {
                    ...
                    for ( const groupCommand of commandInstance.childCommands ) {
                        if ( !groupCommand.checkPermissions( ctx ) ) continue;

                        groupCommands.push( `/${ groupCommand.id } - ${ this.#translate( ctx, groupCommand.getTitle( ctx ) ) }` );
                    }
                    ...
                    commands.push( this.#translate( ctx, commandInstance.getTitle( ctx ) ) );
            ...
            if ( commands.length ) {
                commands.unshift( this.#translate( ctx, this.l10nt( `List of available commands:\n` ) ) );
            }
