...
export default class {
                ...
                if ( parentCommand && !addedCommands.has( parentCommand.id ) ) {
                    if ( parentCommand.checkPermissions( ctx ) ) {
                        ...
                        data.data.unshift( {
                            ...
                            "description": parentCommand.getTitle( ctx ),
                        } );
                    }
                }
    ...
    #getCommandsList ( ctx, level ) {
        ...
        // level 1
        if ( level === 1 ) {
            if ( this.#commandsList1[ ctx.permissions.hash ] === undefined ) {
                const commands = [];

                for ( const commandInstance of this.childCommands ) {
                    if ( !commandInstance.checkPermissions( ctx ) ) continue;

                    commands.push( `/${ commandInstance.id } - ${ commandInstance.getTitle( ctx ) }` );
                }

                this.#commandsList1[ ctx.permissions.hash ] = commands.join( "\n" );
            }

            return this.#commandsList1[ ctx.permissions.hash ];
        }

        // level 2
        else {
            if ( this.#commandsList2[ ctx.permissions.hash ] === undefined ) {
                const commands = [];

                this.#commandsList2[ ctx.permissions.hash ] = commands.join( "\n" );
            }

            return this.#commandsList2[ ctx.permissions.hash ];
        }
    }
}
