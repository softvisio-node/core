...
export default class TelegramBotFiles {
    ...
    async downloadAvatar ( req, telegramBotUserId ) {
        ...
        res = await this.dbh.selectRow( sql`SELECT telegram_bot_file_id FROM telegram_bot_user_profile_photo WHERE telegram_bot_user_id = ? AND index = ?`, [telegramBotUserId] );
