...
export default class TelegramBotFiles {
    ...
    async getFile ( id ) {
        ...
        if ( file.id ) {
            ...
            res1 = await this.dbh.do( SQL.updateStorageFileId, [
    ...
    async downloadFile ( req, id, fileUniqueId ) {
        ...
        if ( file.storage_file_id ) return this.app.storage.downloadFile( req, { "path": file.storage_file_id } );
        ...
        if ( res.data?.storage_file_id ) {
            let res1 = await this.app.storage.copyFile( res.data.storage_file_id, this.bot.telegram.config.storageLocation + "/" + file.id );

            if ( !res1.ok ) return req.end( res1.status );
            ...
            return this.app.storage.downloadFile( req, { "path": res1.data.id } );
        }
        ...
        if ( !res1.ok ) return req.end( res.status );
