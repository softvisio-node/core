...
SELECT
    *,
    CASE
        ...
        ELSE ( SELECT t.storage_file_id FROM telegram_bot_file AS t WHERE t.file_unique_id = telegram_bot_file.file_unique_id AND t.storage_file_id IS NOT NULL LIMIT 1 )
...
export default class TelegramBotFiles {
    ...
    async downloadAvatar ( req, telegramBotUserId ) {
        ...
        if ( !user.avatarFileId ) {
            ...
            return this.app.api.downloadDefaultAvatar( req );
        }
        else {
            return this.#downloadFile( req, user.avatarFileId, {
                "maxAge": null,
                "cacheContrpl": this.app.users.config.avatarCacheControl,
            } );
        }
    }
