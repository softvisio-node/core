...
export default class TelegramBotFiles {
    ...
    async getFile ( id ) {
        ...
        // store file
        if ( file.id ) {
            res = await this.app.storage.uploadFile( res.data.file, this.bot.telegram.config.storageLocation + "/" + file.id );
    ...
    async downloadFile ( req, id, fileUniqueId ) {
        ...
        if ( res.data?.storage_file_id ) {
            ...
            return this.app.storage.downloadFile( req, { "path": storageFileId } );
        }
