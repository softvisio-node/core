...
`,
        ...
        res = await this.dbh.do( sql`INSERT INTO telegram_bot_user_profile_photo`.VALUES( res.data.photos.map( photos => {
            const photo = photos.pop();

            return {
                "telegram_bot_user_id": telegramBotUserId,
                "index": index++,
                "telegram_bot_file_id": sql`create_telegram_bot_file(
    ...
    _content_type => 'image/jpg'
        ...
        } ) ).aql`ON CONFLICT ( telegram_bot_user_id, index ) DO UPDATE SET telegram_bot_file_id = EXCLUDED` );
    }
