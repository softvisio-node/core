    ...
    async getFile ( id ) {
        ...
        if ( file.id ) {
            res = await this.dbh.begin( async dbh => {
                ...
                res = await this.app.storage.uploadFile( this.bot.telegram.config.storageLocation + "/" + file.id, tmpFile, { dbh } );
                if ( !res.ok ) throw res;
    ...
    async downloadFile ( req, fileUniqueId ) {
        ...
        if ( file.downloaded_storage_file_id ) {
            ...
            res = await this.dbh.begin( async dbh => {
                ...
                res = await this.app.storage.copyFile( file.downloaded_storage_file_id, this.bot.telegram.config.storageLocation + "/" + file.id, { dbh } );
