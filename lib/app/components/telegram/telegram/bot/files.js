...
export default class TelegramBotFiles {
    ...
    async getFile ( id, { fileUniqueId } = {} ) {
        ...
        res = await this.#getFile( file.file_id, { "fileUniqueId": file.file_unique_id } );
        if ( !res.ok ) return res;
    ...
    async #getFile ( fileId, { fileUniqueId } = {} ) {
        return this.bot.telegramApi.getFile( fileId, { fileUniqueId } );
    }
}
