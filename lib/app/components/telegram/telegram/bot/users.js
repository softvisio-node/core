...
export default class TelegramBotUsers {
    ...
    constructor ( bot ) {
        ...
        this.#bot.app.users.maxListeners++;

        this.#usersEvents = new Events().link( this.#bot.app.users ).on( "userLocaleChange", apiUser => {
    ...
    async #getUser ( query, id ) {
        ...
        if ( res.data ) {
            ...
            if ( user.apiUserId ) {
                const apiUser = await this.#bot.app.users.getUserById( user.apiUserId );
