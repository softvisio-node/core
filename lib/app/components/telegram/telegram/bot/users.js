...
export default class TelegramBotUsers {
    ...
    // public
    async getByTelegramId ( telegramUserId, { dbh } = {} ) {
        var user = this.#cache.get( telegramUserId );

        user ??= await this.#getUser( "getByTelegramId", telegramUserId, dbh );

        return user;
    }

    async getByBotUserId ( botUserId, { dbh } = {} ) {
        var user = this.#botUserIdIndex[botUserId];

        user ??= await this.#getUser( "getByBotUserId", botUserId, dbh );
    ...
    #cacheUser ( user ) {
        ...
        this.#botUserIdIndex[user.id] = user;
    ...
    #deleteUser ( user ) {
        ...
        delete this.#botUserIdIndex[user.id];
