...
export default class TelegramBotUsers {
    ...
    constructor ( bot ) {
        ...
        this.#cache = new CacheLru( { "maxSize": this.bot.config.usersCacheMaxSize || this.bot.telegram.config.usersCacheMaxSize } );
        ...
        this.#usersEvents = new Events().link( this.app.users ).on( "userLocaleChange", apiUser => {
            ...
            user?.setApiUserLocale( apiUser.locale );
        } );
    }
    ...
    async getUsersByApiUserId ( users ) {
        ...
        if ( select.length ) {
            const res = await this.dbh.select( SQL.getUsersByApiUserId, [Object.keys( select )] );
            ...
            if ( res.data ) {
                for ( const row of res.data ) {
                    ...
                    user.setApiUserLocale( select[user.apiUserId].locale );
    ...
    async #getUser ( query, id ) {
        ...
        if ( res.data ) {
            ...
            if ( user.apiUserId ) {
                const apiUser = await this.app.users.getUserById( user.apiUserId );
