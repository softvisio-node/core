...
export default class {
    ...
    async updateBotDetails ( values ) {
        ...
        if ( values.name != null ) {
            res = this.#bot.telegramBotApi.setMyName( {
                "name": values.name,
            } );
            if ( !res.ok ) return res;
        }
        ...
        if ( values.short_description != null ) {
            res = this.#bot.telegramBotApi.setMyShortDescription( {
        ...
        if ( values.description != null ) {
            res = this.#bot.telegramBotApi.setMyDescription( {
                "description": values.description,
            } );
            if ( !res.ok ) return res;
        }

        res = await this.dbh.do( sql`UPDATEE telegram_bot`.VALUES( values ).WHERE( { "id": this.#bot.id } ) );
        if ( !res.ok ) return res;
