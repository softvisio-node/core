    ...
    async updateBotDetails ( values ) {
        ...
        if ( values.name != null ) {
            res = await this.dbh.begin( async dbh => {
                res = await dbh.do( sql`UPDATE telegram_bot SET name = ? WHERE id = ? AND name != ?`, [
        ...
        if ( values.short_description != null ) {
            res = await this.dbh.begin( async dbh => {
                res = await dbh.do( sql`UPDATE telegram_bot SET short_description = ? WHERE id = ? AND short_description != ?`, [
        ...
        if ( values.description != null ) {
            res = await this.dbh.begin( async dbh => {
                res = await dbh.do( sql`UPDATE telegram_bot SET description = ? WHERE id = ? AND description != ?`, [
