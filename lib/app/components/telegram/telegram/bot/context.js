export default class TelegramBotContext {
    ...
    async call ( module, req ) {
        ...
        // check redirect
        const redirectModule = this.#bot.modules.get( "start" ).redirectCall( this.#clone( "start" ) );
        ...
        if ( module !== this.#userModule ) {
            ...
            if ( currentModule ) {
                ...
                await currentModule.beforeExit( ctx );
            }
    ...
    createWebAooUrl ( data ) {
        ...
        const url = new URL( this.bot.telegram.config.telegram.webAppUrl + "?locale=" + this.#user.locale );
