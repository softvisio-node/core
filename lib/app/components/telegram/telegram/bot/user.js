...
export default class TelegramBotUser extends TelegramUser {
    ...
    async send ( method, data ) {
        ...
        if ( mediaMethods.has( method ) ) {
            if ( data.caption ) {
                data.caption = LocaleTranslation.toString( data.caption, { "localeDomain": this.locale } );
            }
        }
            ...
            for ( let item of data.media ) {
                if ( media.caption || media.title ) {
                    ...
                    if ( media.caption ) {
                        media.caption = LocaleTranslation.toString( media.caption, { "localeDomain": this.locale } );
                    }

                    if ( media.title ) {
                        media.title = LocaleTranslation.toString( media.title, { "localeDomain": this.locale } );
                    }
                }
