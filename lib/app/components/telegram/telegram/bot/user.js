...
export default class TelegramBotUser extends TelegramUser {
    ...
    async getPermissions () {
        ...
        if ( !this.isAuthenticated ) {
            return new Permissions( null, false );
        }

        // user is root
        else if ( this.app.userIsRoot( this.id ) ) {
            return this.app.acl.getAclUserPermissions( this.bot.aclId, this.id );
        }
        else {
            let user = await this.app.users.getUserById( this.id );

            if ( !user?.isEnabled ) return new Permissions( null, false );

            const permissions = [];

            user = await this.app.acl.getAclUser( constants.mainAclId, this.id );

            if ( user?.isEnabled ) permissions.push( ...user.permissions );

            user = await this.app.acl.getAclUser( this.bot.aclId, this.id );

            if ( user?.isEnabled ) permissions.push( ...user.permissions );

            return new Permissions( permissions, false );
        }
    }
}
