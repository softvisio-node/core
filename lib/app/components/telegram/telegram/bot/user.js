...
export default class TelegramBotUser extends TelegramUser {
    ...
    async setApiUserId ( apiUserId ) {
        ...
        if ( !apiUserId ) {
            ...
            this.#bot.app.notifications.sendNotification(
                ...
                "security",
                oldApiUserId,
                this.#bot.app.telegram.locale.i18nt( `Telegram account removed` ),
                this.#bot.app.telegram.locale.i18nt( `Your Telegram account removed from your user profile.` )
            );
        }
        ...
        // link
            ...
            const apiUser = await this.#bot.app.users.getUserById( apiUserId );
            this.setApiUserLocale( apiUser.locale );

            this.#bot.app.publishToApi( "/notifications/telegram-linked/", apiUserId, this.username );
            ...
            this.#bot.app.notifications.sendNotification(
                ...
                this.#bot.locale.i18nt( `Your Telegram account linked to your user profile.` )
            );
        }
    ...
    // private
    #defineLocale () {
        if ( this.#botLocale && this.#bot.isLocaleValid( this.#botLocale ) ) {
            this.#locale = this.#botLocale;

            this.#localeIsSet = true;
        }
        else if ( this.#apiUserLocale && this.#bot.isLocaleValid( this.#apiUserLocale ) ) {
