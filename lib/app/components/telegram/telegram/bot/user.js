...
export default class TelegramBotUser extends TelegramUser {
    ...
    updateBotUserFields ( fields ) {
        ...
        if ( "locale" in fields ) this.#locale = fields.locale;

        this.#defineLocale();
    }

    async setTelegramFields ( fields ) {
        const updateLocale = fields.language_code !== this.languageCode;

        const res = await super.setTelegramFields( fields );

        if ( res.ok && updateLocale ) this.#defineLocale();
    ...
    async setLocale ( locale ) {
        ...
        if ( !this.#bot.app.locales.isLocaleValid( locale ) ) return result( [400, `Locale is not valid`] );
    ...
    // private
    #defineLocale () {
        if ( this.#locale && this.#bot.app.locales.isLocaleValid( this.#locale ) ) return;

        this.#locale = this.#bot.app.locales.defineLocale( {
