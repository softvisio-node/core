...
export default class TelegramBotUser extends TelegramUser {
    ...
    async downloadAvatar ( req ) {
        const storagePath = `/private/telegram/user/${this.telegramId}]/profile-photo/0`;

        const fileExists = await this.app.storage.isFileExists( storagePath );
        ...
        if ( !fileExists ) {
            ...
            if ( !res.ok ) return req.end( 500 );

            res = await this.#bot.telegramBotApi.getFile( res.data.photos[0][0].file_id );

            if ( !res.ok ) return req.end( 500 );

            res.data.file.name = res.data.file_path;

            res = await this.app.storage.uploadFile( res.data.file, storagePath, {
