...
export default class TelegramBotUser extends TelegramUser {
    ...
    async getPermissions () {
        ...
        // user is root
            ...
            // user is disabled
            if ( !user?.isEnabled ) return new Permissions();

            const permissions = [];

            for ( const aclId of [ constants.mainAclId, this.bot.aclId ] ) {
                const user = await this.app.acl.getAclUser( aclId, this.apiUserId );

                if ( user?.isEnabled ) permissions.push( ...user.permissions );
            }

            return new Permissions( this.apiUserId, permissions );
        }
    }
}
