            ...
            CONFIRM: if ( ctx.state.email ) {
                ...
                if ( req.decodedCommandData?.args[0].toString( "hex" ) === token ) {
                    const res = await this.dbh.begin( async dbh => {
                        ...
                        const user = await this.app.users.getByEmail( ctx.state.email );
                        ...
                        const res = await this.dbh.do( sql`UPDATE telegram_user SET api_user_id = ? WHERE telegram_id = ? AND ( api_user_id IS NULL OR api_user_id = ? )`, [
