...
`.prepate(),
    ...
    "setApiUserId": sql`UPDATE telegram_user SET api_user_id = ? WHERE telegram_id = ? AND ( api_user_id IS NULL OR api_user_id = ? )`.prepate(),
};
            ...
            CONFIRM: if ( ctx.state.email ) {
                ...
                if ( req.decodedCommandData?.args[0].toString( "hex" ) === token ) {
                    const res = await this.dbh.begin( async dbh => {
                        ...
                        // XXX
                        // this.app.publishToApi( "/notifications/telegram/update/", userId, this );
