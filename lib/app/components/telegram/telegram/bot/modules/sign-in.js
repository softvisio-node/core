...
` ),
        ...
        async #sendConfirmationEmail ( ctx ) {
            const token = crypto
                ...
                .update( ctx.user.telegramId + "/" + ctx.state.email )
                .digest();
            ...
            return ctx.user.sendText( this.l10nt( msgid`I just sent link to sign inl to your emaul address ${ctx.state.email}.
Please, use this link to finish sign in.` ) );
        }
