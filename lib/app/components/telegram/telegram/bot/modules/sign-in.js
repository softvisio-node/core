...
export default Super =>
    class extends Super {
            ...
            CONFIRM: if ( ctx.state.email ) {
                ...
                if ( token.toString( "hex" ) === validToken ) {
                    ...
                    const res = await this.dbh.begin( async dbh => {
                        ...
                        const user = await this.app.users.getUserByEmail( ctx.state.email, { dbh } );
                        ...
                        // create user
                            ...
                            const res = await this.#createApiUser( ctx, ctx.state.email, {
                                "emailConfirmed": true,
                                dbh,
                            } );
        ...
        async #signUpAutomatically ( ctx ) {
            ...
            const apiUser = await this.app.users.getUserByEmail( email );
            ...
            await this.#setApiUser( ctx, apiUserId );
...
If you didn't received emaul in several minutes you can send it again.` ),
        ...
        async #setApiUser ( ctx, apiUserId, { dbh } = {} ) {
            ...
            return dbh.do( SQL.setApiUserId, [
