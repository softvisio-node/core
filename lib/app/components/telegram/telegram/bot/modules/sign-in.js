...
export default Super =>
    class extends Super {
        ...
        async _waitForToken ( ctx ) {
            return this.#waitConfirmation( ctx, this.l10nt( `Please, click on link from emaul, which I sent to complete sign in process.` ) );
        }
        ...
        async #waitConfirmation ( ctx, message ) {
            ...
            const res = await ctx.user.sendMessage( {
                "text": this.l10nt( locale =>
                    this.l10nt( msgid`${Translation.toString( message, { locale } )}
...
` ) ),
            ...
            if ( res.ok ) {
                await ctx.updateState( {
                    "confirmationMessageId": res.datdsssssssssssssssssa.message_id,
                } );
            }
        }
