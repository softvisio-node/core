...
export default Super =>
    class extends Super {
        ...
        async startRequest ( ctx, req ) {
            ...
            if ( req.command ) {
                ...
                if ( req.command === "start" && req.decodedCommandData ) {
                    const linkId = req.decodedCommandData.method;

                    if ( linkId ) await this.#addLink( ctx, linkId );

                    if ( req.decodedCommandData.args?.length ) {
                        const method = req.decodedCommandData.args.shift();

                        return ctx.runCallback( {
                            method,
                            "args": req.decodedCommandData.args,
                        } );
                    }
                    else {
        ...
        async #addLink ( ctx, linkId ) {
            const linkGuid = uuidFromBuffer( linkId );
