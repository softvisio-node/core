        ...
        async processRequest ( ctx, req ) {
            ...
            if ( req.command ) {
                ...
                // command is valid
                    ...
                    return this.#call( ctx, req.command, req );
                }
            ...
            // run module
                ...
                return this.#call( ctx, ctx.userModule, req );
            }
        }

        // XXX
        async beforeCall ( ctx, module, req ) {}

        // private
        async #call ( ctx, module, req ) {
            if ( await this.#checkLocale( ctx, module, req ) ) return;

            if ( await this.#checkSignin( ctx, module, req ) ) return;

            return ctx.call( module, req );
        }

        async #checkLocale ( ctx, module, req ) {
            ...
            if ( module !== "locale" ) req = null;

            await ctx.call( "locale", req );

            return true;
        }

        async #checkSignin ( ctx, module, req ) {
            if ( !this.bot.modules.has( "sign_in" ) ) return;

            if ( ctx.user.apiUserId ) return;

            if ( module !== "sign_in" ) req = null;

            await ctx.call( "sign_in", req );
