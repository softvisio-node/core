...
export default class TelegramBot {
    ...
    async #onTelegramUpdate ( req ) {
        ...
        // inline query
        if ( req.isInlineQuery || req.is.isChosenInlineResult ) {
            return;
        }
    ...
    async #privateChatUpdate ( req ) {
        ...
        if ( req.isMessage ) {
            ...
            if ( start?.method === "link-api-user" ) {
                await this.#linkUser( user, start.args[0] );
            }
        ...
        // callbacl query
            ...
            try {
                return this.#modules[module][method]( ctx, ...callbackData.args );
            }
            catch ( e ) {
        ...
        // navigation
        return ctx.runModule( user.state.module, req );
    }
