...
const SQL = {
    ...
    "createTelegramUser": sql`INSERT INTO telegram_user ( api_user_id, telegram_id, is_bot, username, first_name, last_name ) VALUES ( ?, ?, ?, ?, ?, ? ) ON CONFLICT ( telegram_id ) DO UPDATE SET username = EXCLUDED.username RETURNING id`.prepare(),
...
export default class TelegramBot {
    ...
    async #privateChatUpdate ( req ) {
        ...
        if ( req.command === "/start" && req.commandData ) {
            ...
            if ( start?.method === "link-api-user" ) {
                var linkApiUserToken = start.args[0];
            }
        }
        ...
        if ( req.isMessage ) {
            ...
            // link api user
            if ( linkApiUserToken ) {
                await this.#linkUser( user, linkApiUserToken );
            }
    ...
    async #createUser ( options ) {
        ...
        const res = await this.dbh.begin( async dbh => {
            ...
            if ( options.linkApiUserToken ) {
                res = await this.app.actionTokens.activateActionToken( options.linkApiUserToken, constants.linkTelegramAccountToken.id, {
                    dbh,
                } );

                if ( res.ok ) {
                    var apiUserId = res.data.user_id;
                }
            }
