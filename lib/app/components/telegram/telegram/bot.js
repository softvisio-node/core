...
export default class TelegramBot {
    ...
    async #privateChatUpdate ( update, signal ) {
        ...
        if ( update.isMessage && update.data.text?.startsWith( "/start " ) ) {
            var [startMethod, startArgs] = this.telegram.decodeCallback( update.data.text.substring( 7 ) );
        }
        ...
        if ( !user ) {
            ...
            await this.#createUser( {
                ...update.data.from,
                "adCampaignGuid": startMethod === "ad-campaign" ? startArgs[0] : null,
            } );
        ...
        if ( update.isMessage ) {
            ...
            // link api user
            if ( startMethod === "link-api-user" ) {
                await this.#linkUser( ctx.user, startArgs[0] );
            }
        ...
        // callbacl query
            ...
            const [path, args] = this.telegram.decodeCallback( update.data.data );
            ...
            const module = path.substring( 0, idx ),
                method = "API_" + path.substring( idx + 1 );

            // module method os not found
            if ( typeof this.#modules[module]?.[method] !== "function" ) return;

            try {
                return this.#modules[module][method]( ctx, ...args );
            }
            catch ( e ) {
