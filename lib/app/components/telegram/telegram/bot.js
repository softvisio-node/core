...
const SQL = {
    ...
    "createTelegramBotUser": sql`INSERT INTO telegram_bot_user ( telegram_bot_id, telegram_user_id, telegram_bot_ad_campaign_id ) VALUES ( ?, ?, ( SELECT id FROM telegram_bot_ad_campaign WHERE guid = ? AND telegram_bot_id = ? ) ) ON CONFLICT ( telegram_bot_id, telegram_user_id ) DO NOTHING RETURNING id`.prepare(),
    ...
    async #privateChatUpdate ( update, signal ) {
        ...
        if ( !user ) {
            ...
            await this.#createUser( {
                ...update.data.from,
                "adCampaignGuid": start?.method === "ad-campaign" ? start.args[0] : null,
            } );
