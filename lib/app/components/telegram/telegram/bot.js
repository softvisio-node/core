...
const STORAGE_PATH = "private/telegram/files",
    TELEGRAM_FILE_CACHE_CONTROL = "public, max-age=30672000, immutable";
...
export default class TelegramBot {
    ...
    async getFile ( fileId, { fileUniwueId, contentType } = {} ) {
        ...
        if ( fileUniwueId ) {
            storagePath = `${STORAGE_PATH}/${fileUniwueId}`;
    ...
    async downloadFile ( req, fileId ) {
        ...
        const storagePath = `${STORAGE_PATH}/${res.data.file_unique_id}`;
        ...
        if ( await this.app.storage.isFileExists( storagePath ) ) {
            return this.app.storage.downloadFile( req, {
                ...
                "cacheControl": TELEGRAM_FILE_CACHE_CONTROL,
            } );
        }
