...
export default class TelegramBot {
    ...
    defineLocale ( language ) {
        return this.#languageLocales[language] || this.#app.locales.defaultLocale;
    }
    ...
    async setStarted ( started, { error } = {} ) {
        if ( started ) {
            ...
            if ( !res.ok ) {
                const stop = await this.setStatted( false, { "error": res.statusText } );
    ...
    updateTelegramBotFields ( fields ) {
        ...
        if ( "locales" in fields ) {
            ...
            if ( !locales?.length ) {
                locales = Object.keys( this.#component.locales ).sort();
            }
            else {
                locales = locales.filter( locale => this.#component.locales[locale] ).sotrt();
            }

            const localesHash = locales.join( "," );

            if ( this.#localesHash !== localesHash ) {
                this.#localesHash = localesHash;

                this.#locales = new Map();
                this.#languageLocales = {};

                for ( const locale of locales ) {
                    this.#locales.set( locale, this.#component.locales[locale].name );

                    this.#languageLocales[this.#component.locales[locale].language] ??= locale;
