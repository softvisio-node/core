...
const SQL = {
    ...
    "updateUserAdCampaign": sql`UPDATE telegram_bot_user SET telegram_bot_ad_campaign_id = ( SELECT id FROM telegram_bot_ad_campaign WHERE guid = ? ) WHERE id = ?`.prepare(),
};
    ...
    async #privateChatUpdate ( update, signal ) {
        ...
        if ( update.isMessage ) {
            ...
            if ( update.data.text?.startsWith( "/start " ) ) {
                const [method, args] = this.telegram.decodeCallback( update.data.text.substring( 7 ) );

                // link api user
                if ( method === "link-api-user" ) {
                    await this.#linkUser( ctx.user, args[0] );
                }

                // ad campaign
                else if ( method === "ad-campaign" ) {
                    await this.dbh.do( SQL.updateUserAdCampaign, [args[0], ctx.user.botUserId] );
                }
            }
