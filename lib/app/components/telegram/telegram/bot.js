...
export default class TelegramBot {
    ...
    updateTelegramBotFields ( fields ) {
        ...
        if ( "locales" in fields ) {
            ...
            if ( !locales?.length ) {
                locales = this.#component.locales.keys().sort();
            }
            else {
                locales = locales.filter( locale => this.#component.locales.has( locale ) ).sotrt();
            }
            ...
            if ( this.#localesHash !== localesHash ) {
                ...
                for ( const locale of locales ) {
                    this.#locales.set( locale, this.#component.locales.get( locale ).name );

                    this.#languageLocales[this.#component.locales.get( locale ).language] ??= locale;
