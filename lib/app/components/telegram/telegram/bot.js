...
import { tokenTypeLinkTelegramAccount } from "#lib/app/constants";
...
export default class TelegramBot {
    ...
    async start () {
        if ( this.isListening ) return result( 200 );

        if ( !this.#isStarted ) return result( [500, `Bot is disabled`] );

        if ( this.#isStopping ) return result( [500, `Bot is stopping`] );
    ...
    async linkUser ( token, userId, telegramUserId ) {
        ...
        // verify token
        var res = await this.app.api.userActionToken.activateUserActionToken( token, tokenTypeLinkTelegramAccount, { userId } );
    ...
    async #getUpdates () {
        ...
        while ( true ) {
            ...
            // bot was disabled
            if ( this.#isStopping ) return result( 200 );
            ...
            for ( const update of updates.data ) {
                ...
                await this.#setTelegramNextUpdateId( updateId + 1 );
            }
        }
    }

    async #afterStart ( res ) {
