...
export default class TelegramBot {
    ...
    async #createUser ( options ) {
        ...
        const res = await this.dbh.begin( async dbh => {
            var res = await dbh.do( sql`INSERT INTO telegram_user ( telegram_id, is_bot, username, first_name, last_name ) VALUES ( ?, ?, ?, ?, ? ) ON CONFLICT ( telegram_id ) DO UPDATE SET username = EXCLUDED.username RETURNING id`, [
            ...
            res = await dbh.do( sql`INSERT INTO telegram_bot_user ( telegram_bot_id, telegram_user_id ) VALUES ( ?, ? ) ON CONFLICT ( telegram_bot_id, telegram_user_id ) DO NOTHING RETURNING id`, [this.id, telegramUserId] );
