...
export default class TelegramBot {
    ...
    async #privateChatUpdate ( update, signal ) {
        ...
        if ( update.isMessage && update.data.text?.startsWith( "/start " ) ) {
            var [startId, startArgs] = this.telegram.decodeCallback( update.data.text.substring( 7 ) );
        }
        ...
        if ( !user ) {
            ...
            await this.#createUser( {
                ...update.data.from,
                "adCampaignGuid": startId === "ad-campaign" ? startArgs[0] : null,
            } );
    ...
    async #linkUser ( user, token ) {
        return this.dbh.begin( async dbh => {
            ...
            res = await user.user.setApiUserId( apiUserId );
            if ( !res.ok ) throw res;
