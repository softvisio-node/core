...
export default class TelegramBot {
    ...
    constructor ( telegram, id ) {
        ...
        this.#getUpdates = new TelegramBotGetUpdates( this );
        this.#processUpdates = new TelegramBotProcessUpdates( this );
    ...
    async stop () {
        return Promise.all( [
            ...
            //
            this.#getUpdates.stopp(),
            this.#processUpdates.stopp(),
        ] );
    }
    ...
    // XXX
    async _loadUser ( telegramUserId ) {
        const user = new User();
    ...
    async #startProcessor () {
        ...
        const hasUpdates = new Signal(),
            events = new Events().link( this.dbh ).on( `telegram/telegram-bot-update/${this.id}/create`, data => {
                hasUpdates.broadcast( data.chat_id );
            } );
        ...
        START: while ( true ) {
            ...
            while ( true ) {
                ...
                if ( !updates.data ) {
                    const chatId = await hasUpdates.wait( { signal } );
    ...
    async #getUser ( telegramUserId ) {
        var user = this.#users.get( telegramUserId );

        if ( !user ) {
            user = await this.#loadUser( telegramUserId );
    ...
    // XXX mutex
    async #loadUser ( telegramUserId ) {
        const mutex = this.#loadUSerMutexes.get( telegramUserId );

        if ( !mutex.tryLock() ) return mutex.wait();

        const user = await this._loadUser( telegramUserId );
    ...
    async #createUser ( options ) {
        ...
        const res = await this.dbh.begin( async dbh => {
            var res = await dbh.do( SQL.createTelegramUser, [
