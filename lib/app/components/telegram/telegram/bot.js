...
const SQL = {
    ...
    "createTelegramUser": sql`INSERT INTO telegram_user ( telegram_id, is_bot, username, first_name, last_name ) VALUES ( ?, ?, ?, ?, ? ) ON CONFLICT ( telegram_id ) DO UPDATE SET username = EXCLUDED.username RETURNING id`.prepare(),
...
export default class TelegramBot {
    ...
    get detectLocaleByClientLanguage () {
        return this.#detectLocaleByClientLanguage;
    }
    ...
    updateTelegramBotFields ( fields ) {
        if ( "telegram_api_token" in fields ) this.#telegramBotApi.apiToken = fields.telegram_api_token;
        ...
        if ( "locales" in fields ) {
            ...
            this.#detectLocaleByClientLanguage = fields.detect_locale_by_client_language;
    ...
    async #linkUser ( user, token ) {
        return this.dbh.begin( async dbh => {
            ...
            const apiUserId = res.data.user_id;
