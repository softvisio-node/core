...
export default class TelegramBot {
    ...
    async start () {
        ...
        const stopSignal = this.#updatesAbortController.signal();
        ...
        START: while ( true ) {
            ...
            // can't start bot
            if ( !( await this._canStart() ) ) break START;

            // lock mutex
            await this.#startMutex.lock();
            const mutexSignal = this.#startMutex.signal;
            mutexSignal.on( "abort", () => this.#updatesAbortController.abort() );

            let res = await this.#updateTelegramData();
            if ( !res.ok ) break START;
            ...
            if ( mutexSignal.aborted ) continue START;
