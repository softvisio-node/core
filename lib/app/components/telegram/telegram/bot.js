    ...
    async #privateChatUpdate ( update, signal ) {
        ...
        if ( update.isMessage ) {
            ...
            if ( update.data.text?.startsWith( "/start " ) ) {
                const [method, args] = this.telegram.decodeTelegramCallback( update.data.text.substring( 7 ) );

                // link api user
                if ( method === "/link-api-user" ) {
                    await this.#linkUser( update, args[0] );
                }
            }
        ...
        // callbacl query
            ...
            const [path, args] = this.telegram.decodeTelegramCallback( update.data.data );
            ...
            try {
                ...
                console.log( `Telegram callback query error: ${e}` );
