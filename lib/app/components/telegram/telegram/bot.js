...
export default class TelegramBot {
    ...
    async #linkUser ( update, token ) {
        const res = await this.dbh.begin( async dbh => {
            const res = await this.app.api.userActionTokens.activateUserActionToken( token, constants.tokenTypeLinkTelegramAccount, { dbh } );

            if ( !res.ok ) throw result( [400, `Token is not valid. Please, try agin.`] );

            const res1 = await dbh.do( SQL.linkUser, [res.data.userId, this.id, update.data.from.id] );

            if ( !res1.meta.rows ) throw result( [500, `Internal server error. Please, try agin.`] );

            this.app.publish( "/api/notifications/telegram-linked/", res.data.userId, true );

            // XXX send security notifications
