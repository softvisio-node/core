    ...
    async #onTelegramUpdate ( update ) {
        ...
        if ( update.type === "message" ) {
            if ( update.data.text.startsWith( "/start link-user:" ) ) {
                ...
                const a = await this.app.api.userActionTokens.createUserActionToken( -1, constants.tokenTypeLinkTelegramAccount, { "data": { "userId": -1 } } );

                const res = await this.app.api.userActionTokens.activateUserActionToken( a.data.token, constants.tokenTypeLinkTelegramAccount );

                if ( !res.ok ) {
                    this.sendMessage( update.chatId, `Token is not valid. Please, try agin.` );
                }
                else {
                    const res1 = await this.dbh.do( SQL.linkUser, [res.data.userId, this.id, update.data.from.id] );

                    if ( !res1.meta.rows ) {
                        this.sendMessage( update.chatId, `Some error occured. Please, try agin.` );
                    }
                    else {
