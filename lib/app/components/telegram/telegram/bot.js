...
export default class TelegramBot {
    ...
    async #onTelegramUpdate ( update, signal ) {
        const req = new TelegramBotUpdate( this, signal.update );
    ...
    async #privateChatUpdate ( req ) {
        ...
        // decode start params
        if ( req.isMessage && req.data.text?.startsWith( "/start " ) ) {
            var start = this.telegram.decodeCallbackData( req.data.text.substring( 7 ) );
        }
        ...
        // get user
        user = await this.#users.getByTelegramId( req.data.from.id );
        ...
        // update user
            ...
            await user.setTelegramFields( req.data.from );
        ...
        // create context
        const ctx = new TelegramBotContext( this, {
            req,
            user,
            newUser,
        } );
        ...
        if ( req.isMessage ) {
            ...
            // contact
            else if ( req.data.contact ) {
                await ctx.user.updateContact( req.data.contact );
            }
        }
        ...
        // callbacl query
            ...
            const callback = this.telegram.decodeCallbackData( req.data.data );
    ...
    async #linkUser ( user, token ) {
        return this.dbh.begin( async dbh => {
            ...
            res = await user.setApiUserId( apiUserId, { dbh } );
            if ( !res.ok ) throw res;
