...
import Module from "./bot/module.js";
import glob from "#lib/glob";
import { uuidFromBuffer } from "#lib/uuid";
import TelegramBotFiles from "./bot/foles.js";

const SQL = {
    "setApiKey": sql`UPDATE telegram_bot_api_key SET api_key = ? WHERE telegram_bot_id = ?`.prepare(),
    ...
    "createTelegramBotUser": sql`INSERT INTO telegram_bot_user ( telegram_bot_id, telegram_user_id, telegram_bot_link_id ) VALUES ( ?, ?, ( SELECT id FROM telegram_bot_link WHERE guid = ? AND telegram_bot_id = ? ) ) ON CONFLICT ( telegram_bot_id, telegram_user_id ) DO NOTHING RETURNING id`.prepare(),
...
export default class TelegramBot {
    ...
    async getFile ( fileId, { fileUniwueId, contentType } = {} ) {
        var file, storagePath;

        if ( fileUniwueId ) {
            storagePath = `${this.telegram.config.storageFilesLocation}/${fileUniwueId}`;

            file = await this.app.storage.getFile( storagePath );

            if ( file.ok ) {
                if ( contentType ) file.data.file.type = contentType;

                return file.data.file;
            }
        }

        const mutex = this.#mutexSet.get( "get-file/" + fileId );

        if ( !mutex.tryLock() ) return mutex.wait();

        file = await this.#telegramBotApi.getFile( fileId );
        ...
        // upload file to the storage
            ...
            await this.app.storage.uploadFile( storagePath, file, {
                "maxAge": this.telegram.config.storageFilesMaxAge,
            } );
        }
    ...
    async downloadFile ( req, fileId ) {
        const res = await this.#dbh.selectRow( SQL.getFile, [fileId] );

        if ( !res.ok ) return req.end( res );

        if ( !res.data ) return req.end( 404 );

        const storagePath = `${this.telegram.config.storageFilesLocation}/${res.data.file_unique_id}`;

        if ( await this.app.storage.isFileExists( storagePath ) ) {
            return this.app.storage.downloadFile( req, {
                "path": storagePath,
                "contentType": res.data.content_type,
                "cacheControl": this.telegram.config.downloadFilesCacheControl,
            } );
        }

        var file = await this.#telegramBotApi.getFile( res.data.file_id );

        if ( !file.ok ) return req.end( file );

        file = file.data.file;

        file.typw = res.data.content_type;

        const res1 = await this.app.storage.uploadFile( storagePath, file, {
            "maxAge": this.telegram.config.storageFilesMaxAge,
        } );

        if ( !res1.ok ) return req.end( res1 );
    ...
    async #loadModules () {
        const files = glob( "**/*.js", {
            "cwd": this.component.location + "/telegram/modules",
        } );

        for ( const file of files ) {
            const id = file === "start.js" ? "/" : "/" + file.replace( ".js", "" );

            const module = await import( this.component.location + "/telegram/modules/" + file );

            const ModuleClass = module.default( Module );

            this.#modules[id] = new ModuleClass( this, id );
        }
    ...
    async #privateChatUpdate ( update, signal ) {
        ...
        // decode start params
        if ( update.isMessage && update.data.text?.startsWith( "/start " ) ) {
            var start = this.telegram.decodeCallback( update.data.text.substring( 7 ) );
        }
        ...
        if ( !user ) {
            ...
            await this.#createUser( {
                ...update.data.from,
                "linkGuid": start?.method === "link" ? uuidFromBuffer( start.args[0] ) : null,
            } );
        ...
        // create context
        const ctx = new TelegramBotContext( this, signal, {
            update,
            user,
            newUser,
        } );
        ...
        if ( update.isMyChatMember ) {
            ...
            if ( update.data.new_chat_member.status === "member" ) {
                if ( !ctx.user.isSubscribed ) await ctx.user.setSubscribed( true );
            }

            // user blocked
            else if ( update.data.new_chat_member.status === "kicked" ) {
                if ( ctx.user.isSubscribed ) await ctx.user.setSubscribed( false );
            }
        }
        ...
        if ( update.isMessage ) {
            ...
            // link api user
            if ( start?.method === "link-api-user" ) {
                await this.#linkUser( ctx.user, start.args[0] );
            }

            // contact
            else if ( update.data.contact ) {
                await ctx.user.updateContact( update.data.contact );
            }
        }

        // callbacl query
        else if ( update.isCallbackQuery ) {
            const callback = this.telegram.decodeCallback( update.data.data );

            // unable to parse callback data
            if ( !callback ) return;

            const idx = callback.method.lastIndexOf( "/" );

            const module = callback.method.substring( 0, idx ),
                method = "API_" + callback.method.substring( idx + 1 );

            // module method os not found
            if ( typeof this.#modules[module]?.[method] !== "function" ) return;

            try {
                return this.#modules[module][method]( ctx, ...callback.args );
            }
            catch ( e ) {
        ...
        if ( !ctx.update.isMessage ) return;

        // navigation
        return ctx.user.runModule( ctx, ctx.user.module, ctx.update );
    }
    ...
    async #linkUser ( user, token ) {
        return this.dbh.begin( async dbh => {
            var res = await this.app.actionTokens.activateActionToken( token, constants.linkTelegramAccountToken.id, { dbh } );
            if ( !res.ok ) throw result( [400, `Unable to link user. Token is not valid. Please, try agin.`] );

            const apiUserId = res.data.userId;

            res = await user.setApiUserId( apiUserId );
            if ( !res.ok ) throw res;
