    ...
    class extends Super {
        ...
        async _sendContacts ( ctx ) {
            ...
            if ( config.phoneEnabled ) {
                if ( contact.phone ) {
                    res = await ctx.sendMessage( {
                        "text": l10nt( locale => locale.l10n( `Phone` ) + ": " + locale.l10n( `not set` ) ),
                        "parse_node": "HTML",
                        "reply_markup": {
                            "inline_keyboard": [
                                [
            ...
            if ( config.emailEnabled ) {
                if ( contact.email ) {
                    res = await ctx.sendMessage( {
                        "text": l10nt( locale => locale.l10n( `Email` ) + ": " + locale.l10n( `not set` ) ),
                        "parse_node": "HTML",
                        "reply_markup": {
                            "inline_keyboard": [
                                [
            ...
            if ( config.notesEnabled ) {
                if ( contact.notes ) {
                    ...
                    res = await ctx.sendMessage( {
                        "text": l10nt( locale => locale.l10n( `Notes` ) + ": " + locale.l10n( `not set` ) ),
                        "parse_node": "HTML",
                        "reply_markup": {
                            "inline_keyboard": [
                                [
