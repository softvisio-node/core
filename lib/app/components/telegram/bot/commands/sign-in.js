...
export default Super =>
    class extends Super {
        ...
        // public
        getDescription ( ctx ) {
            return global.l10nt( `sign in to your account` );
        }
        ...
        async API_signIn ( ctx, token ) {
            ...
            // confirmation failed
            await ctx.sendText( global.l10nt( `Confirmation link is not valid. Please, try again.` ) );
        ...
        async #requestEmail ( ctx, requestMessage ) {
            EMAIL: if ( requestMessage?.text ) {
                ...
                if ( !validateEmail( email ).ok ) {
                    await ctx.sendText( global.l10nt( `Email address is not valid` ) );
                }
                ...
                // email is valid
                    ...
                    if ( !this.bot.config.telegram.signupEnabled ) {
                        ...
                        if ( !user ) {
                            await ctx.sendText( global.l10nt( `This email address is not registered` ) );
                    ...
                    if ( res.data?.id ) {
                        await ctx.sendText( global.l10nt( `This email address is already used by the other user` ) );
                    }
            ...
            return ctx.sendMessage( {
                "text": global.l10nt( `To sign in, please, enter yoour email address` ),
            } );
        }
        ...
        async #requestEmailConfirmation ( ctx ) {
            ...
            const res = await ctx.sendMessage( {
                "text": global.l10nt( `To complete sign in, please, click on link which I just sent to your email address.
If you didn't received emaul in several minutes you can send it again.` ),
                "reply_markup": {
                    "inline_keyboard": [
                        [
                            {
                                "text": global.l10nt( `Send confirmation email again` ),
                                "callback_data": this.encodeCallbackData( "resendConfirmatioEmail" ),
                            },
                        ],
                        [
                            {
                                "text": global.l10nt( `Change email address` ),
        ...
        async #signinComplete ( ctx ) {
            ...
            return ctx.sendMessage( {
                "text": global.l10nt( msgid`You are signed in as: ${ apiUser.email }` ),
                "reply_markup": {
                    "inline_keyboard": [
                        [
                            {
                                "text": global.l10nt( `Back to the start` ),
