    ...
    class extends Super {
        ...
        async _showMessage ( ctx, message, { canEdit } = {} ) {
            ...
            if ( !message?.text ) {
                ...
                buttons.push( [
                    {
                        "text": global.l10nt( `Change text` ),
                        "callback_data": this.encodeCallbackData( "addText" ),
                    },
                    {
                        "text": global.l10nt( `Delete text` ),
            ...
            if ( message?.isMedia ) {
                buttons[ 1 ].push( {
                    "text": global.l10nt( `Edit media` ),
                    "callback_data": this.encodeCallbackData( "editMedia" ),
                } );
            }

            res = await ctx.send( "sendMessage", {
                "text": global.l10nt( `Please, choose what you want to dp` ),
                "reply_markup": {
        ...
        async _showEmptyMessage ( ctx ) {
            return ctx.sendText( global.l10nt( `Ypir message has no content. You need to add text and media.` ) );
        }

        async _showAddTextPrompt ( ctx ) {
            const text = global.l10nt( `Send me a text to add to the message.
...
||text under spoiler||: <span class="tg-spoiler">text under spoiler</span>` );
        ...
        async _showAddMediaPrompt ( ctx ) {
            const text = global.l10nt( `Send me medfia files ( photos, videos, audios, documents ) to add to the message.
Photos / videos, audios and documents can be grouped to the album. up to 10 files in the album.` );
        ...
        async _showEditMediaPrompt ( ctx, message ) {
                ...
                const res = await media.send( ctx, {
                    ...
                    "reply_markup": {
                        "inline_keyboard": [
                            [
                                {
                                    "text": global.l10nt( `Delete media file` ),
            ...
            return ctx.send( "sendMessage", {
                "text": global.l10nt( `Press button when you will finish edit media:` ),
                "reply_markup": {
                    "inline_keyboard": [
                        [
                            {
