...
const SIGNIN_START_CALLBACK_ID = "signin";
...
export default Super =>
    class extends Super {
        ...
        // protected
        async _init () {
            const res = this._registerStartCallback( SIGNIN_START_CALLBACK_ID, "signIn" );
        ...
        async #sendConfirmationEmail ( ctx ) {
            ...
            await this.app.notifications.sendEmail(
                ...
                this.app.templates.get( "telegram/sign-in-email/body" ).toString( {
                    "localeDomain": ctx.user.id,
                    "data": {
                        "url": this.createStartUrl( SIGNIN_START_CALLBACK_ID, token ),
                        "telegramBotName": this.bot.name,
                    },
                } )
            );
