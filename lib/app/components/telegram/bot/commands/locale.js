...
export default Super =>
    class extends Super {
        ...
        // public
        getDescription ( ctx ) {
            return global.l10nt( `change communication language` );
        }
        ...
        async API_setLocale ( ctx, locale ) {
            ...
            if ( !res.ok ) {
                await ctx.sendText( global.l10nt( `Some error occured, pleast try again.` ) );
        ...
        async #sendKeyboard ( ctx ) {
            ...
            res = await ctx.sendMessage( {
                "text": global.l10nt( `Please, select your communication language:` ),
                "parse_mode": "HTML",
                "reply_markup": {
        ...
        async #onLocaleSet ( ctx ) {
            ...
            await ctx.sendText( global.l10nt( locale => `Communication language set to: ` + new Locale( ctx.user.locale ).name ) );
