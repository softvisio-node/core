...
const SQL = {
    "setStatus": sql`UPDATE telegram_bot_user SET status = ? WHERE telegram_bot_id = ? AND telegram_user_id = ?`.prepare(),
...
export default class TelegramBotUser extends Chat( TelegramUser ) {
    ...
    // XXX kicked - when user blocks bot
    // XXX member - when uuser unblock bot
    async setStatus ( value ) {
        if ( value === this.#status ) return result( 200 );

        const res = await this.dbh.do( SQL.setStatus, [ value, this.bot.id, this.id ] );
    ...
    async setSubscribed ( value ) {
        ...
        const res = await this.dbh.do( SQL.setSubscribed, [ value, this.bot.id, this.id ] );
