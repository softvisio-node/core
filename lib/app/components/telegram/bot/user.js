...
export default class TelegramBotUser extends TelegramUser {
    ...
    // static
    static async getByTelegramUserId ( dbh, bot, telegramUserId ) {
        return dbh.selectRow( SQL.getByTelegramUserId, [ telegramUserId, bot.id ] );
    }

    static async getByTelegramBotUserId ( dbh, bot, telegramBotUserId ) {
        return dbh.selectRow( SQL.getByTelegramBotUserId, [ telegramBotUserId, bot.id ] );
    }

    static async getByApiUserId ( dbh, bot, apiUserId ) {
        if ( Array.isArray( apiUserId ) ) {
            return dbh.select( SQL.getUsersByApiUserId, [ bot.id, apiUserId ] );
        }
        else {
            return dbh.selectRow( SQL.getByApiUserId, [ apiUserId, bot.id ] );
        }
    }
    ...
    async getPermissions () {
        ...
        // not authemticated
        if ( !this.apiUserId ) {
            return this.app.acl.guestsPermissions;
        }

        // user is root
        else if ( this.app.userIsRoot( this.apiUserId ) ) {
            return this.app.acl.rootPermissions;
        }
        else {
            const user = await this.getApiUser();

            // user is disabled
            if ( !user?.isEnabled ) return this.app.acl.guestsPermissions;

            const permissions = await this.app.acl.getAclUserFullPermissions( this.bot.aclId, this.apiUserId );

            return permissions || this.app.acl.guestsPermissions;
        }
    }
}
