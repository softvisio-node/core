...
const SQL = {
    "setSubscribed": sql`UPDATE telegram_bot_user SET subscribed = ? WHERE id = ? AND telegram_bot_id = ?`.prepare(),

    "setBanned": sql`UPDATE telegram_bot_user SET banned = ? WHERE id = ? AND telegram_bot_id = ?`.prepare(),

    "setState": sql`UPDATE telegram_bot_user SET state = ? WHERE id = ? AND telegram_bot_id = ?`.prepare(),

    "setLocale": sql`UPDATE telegram_bot_user SET locale = ? WHERE id = ? AND telegram_bot_id = ?`.prepare(),
};
...
export default class TelegramBotUser extends Chat( TelegramUser ) {
    ...
    async setSubscribed ( value ) {
        ...
        const res = await this.dbh.do( SQL.setSubscribed, [ value, this.id, this.bot.id ] );
    ...
    async setBanned ( value ) {
        ...
        const res = await this.dbh.do( SQL.setBanned, [ value, this.id ], this.bot.id );
    ...
    async updateState ( state, { dbh } = {} ) {
        ...
        const res = await dbh.do( SQL.setState, [ stateJson, this.id, this.bot.id ] );
    ...
    async setLocale ( locale, { dbh } = {} ) {
        ...
        const res = await dbh.do( SQL.setLocale, [ locale, this.id, this.bot.id ] );
