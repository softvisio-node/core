...
export default class TelegramBotUsers {
    ...
    async getUserByApiUserId ( apiUserId, { dbh } = {} ) {
        var user = this.#apiUserIdIndex[ apiUserId ];

        user ??= await this.#getUser( "_getUserByApiUserId", apiUserId, dbh );
    ...
    async getUsersByApiUserId ( users, { dbh } = {} ) {
        ...
        if ( select.size ) {
            const res = await this._getUserByApiUserId( dbh || this.dbh, [ ...select ] );
    ...
    async _getUserByApiUserId ( dbh, apiUserId ) {
        if ( Array.isArray( apiUserId ) ) {
            return dbh.select( SQL.getUsersByApiUserId, [ apiUserId, this.#bot.id ] );
        }
        else {
            return dbh.selectRow( SQL.getUserByApiUserId, [ apiUserId, this.#bot.id ] );
        }
    }
