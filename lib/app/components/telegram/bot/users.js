...
FROM
    ...
    telegram_bot_user
WHERE
    telegram_user.id = telegram_bot_user.id
...
WHERE
    ...
    AND telegram_user.id = telegram_bot_user.id
    AND telegram_bot_user.telegram_bot_id = ?
`.prepare(),
...
FROM
    ...
    telegram_bot_user
WHERE
    telegram_user.api_user_id IN ( SELECT json_array_elements_text( ? )::int53 )
    AND telegram_user.id = telegram_bot_user.id
    AND telegram_bot_user.telegram_bot_id = ?
`.prepare(),
};
...
export default class TelegramBotUsers {
    ...
    // public
    async getById ( telegramId, { dbh } = {} ) {
        var user = this.#cache.get( telegramId );

        user ??= await this.#getUser( "_getById", telegramId, dbh );
    ...
    // protected
    async _getById ( dbh, id ) {
        return dbh.selectRow( SQL.getById, [ id, this.#bot.id ] );
    }
