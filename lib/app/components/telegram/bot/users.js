...
export default class TelegramBotUsers {
    ...
    // public
    async getTelegramBotUserByTelegramId ( telegramId, { dbh } = {} ) {
        var user = this.#cache.get( telegramId );

        user ??= await this.#getUser( "_getByTelegramId", telegramId, dbh );
    ...
    // protected
    async _getByTelegramId ( dbh, telegramId ) {
        return dbh.selectRow( SQL.getByTelegramId, [ telegramId, this.#bot.id ] );
    }
