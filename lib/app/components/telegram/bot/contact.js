...
const SQL = {
    "create": sql`INSERT INTO telegram_bot_contact ( telegram_bot_contact ) VALUES ( ? ) RETURNINF id`.prepare(),
...
export default class {
    ...
    constructor ( contacts, id, fields ) {
        ...
        if ( fields ) this.updateFields( fields?.data );
    }

    // static
    async new ( contacts, { dbh } = {} ) {
        dbh ||= contacts.dbh;

        const res = await dbh.selectRow( SQL.create, [ contacts.bot.id ] );

        if ( !res.ok ) throw res;

        return new this( contacts, res.data.id );
    }
