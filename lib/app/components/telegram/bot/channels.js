...
export default class TelegramBotGroups {
    ...
    // public
    async getById ( telegramGroupId, { dbh } = {} ) {
        var group = this.#cache.get( telegramGroupId );

        group ??= await this.#getGroup( "_getById", telegramGroupId, dbh );
    ...
    // protected
    async _getById ( dbh, telegramGroupId ) {
        return dbh.selectRow( SQL.getById, [ this.#bot.id, telegramGroupId ] );
    }
