...
export default class TelegramBotChannels {
    ...
    // public
    async getById ( telegramChannelId, { dbh } = {} ) {
        var channel = this.#cache.get( telegramChannelId );

        channel ??= await this.#getChannel( "_getById", telegramChannelId, dbh );
    ...
    // protected
    async _getById ( dbh, telegramChannelId ) {
        return dbh.selectRow( SQL.getById, [ this.#bot.id, telegramChannelId ] );
    }
