...
( telegram_bot_id, api_key ) VALUES ( ?, ? )
ON CONFLICT ( telegram_bot_id ) DO UPDATE SET api_key = EXCLUDED.api_key
`.prepare(),
...
export default class Telegram {
    ...
    async init () {
        ...
        this.app.publicHttpServer?.get( this.config.progilePhotosLocation + "/*", this.#downloadProfilePhoto.bind( this ) );
    ...
    async #createBot ( options, isStatic ) {
        ...
        return this.dbh.begin( async dbh => {
            ...
            // upsert api key
            res = await dbh.do( SQL.upsertApiKey, [id, options.apiKey] );
            if ( !res.ok ) throw res;

            if ( options.ownerUserd ) {
                res = await this.app.acl.addAclUser( aclId.options.ownerUserd, {
                    "enabled": true,
                    "roles": ["owner"],
                    dbh,
                } );
    ...
    async #downloadProfilePhoto ( req ) {
        const [botId, telegramBotUserId, profilePhotoIndex] = req.path.split( "/" ).slice( -3 );

        if ( !botId || !telegramBotUserId || !profilePhotoIndex ) return req.end( 404 );

        const bot = this.getBot( botId );

        if ( !bot ) return req.end( 404 );

        return bot.files.downloadProfilePhoto( req, telegramBotUserId, profilePhotoIndex );
    }
}
