...
export default class Telegram {
    ...
    // XXX watch dbh connect, update bots
    async run () {
        ...
        for ( const { id, type } of bots.data || [] ) {
            const res = await this.#newBot( id, type );
        ...
        this.dbh.on( "telegram/telegram-bot/create", async data => {
            await this.#newBot( data.id, data.type );
        } );

        this.dbh.on( "telegram/telegram-bot/update", data => this.getBot( data.id )?.updateTelegramBotFields( data ) );

        this.dbh.on( "telegram/telegram-bot/delete", data => {
            this.getBot( data.id )?.stop();
    ...
    async shutDown () {
        return Promise.all( Object.values( this.#bots ).map( bot => bot.shutDown() ) );
    }
    ...
    // private
    // XXX update API key for static bot
