...
export default class Telegram {
    ...
    async init () {
        ...
        this.app.publicHttpServer?.get( this.config.storageLocation + "/*", this.#downloadFile.bind( this ) );
    ...
    // XXX
    async setApiUser ( apiUserId, { telrgramUserId, dbh } = {} ) {
        telrgramUserId ||= null;

        // api user id must be defined
        if ( !apiUserId ) return result( 400 );

        var res, apiUser, oldTelegramUserId, oldApiUser;

        dbh ||= this.dbh;

        apiUser = await this.app.users.get( apiUserId, { dbh } );
        if ( !apiUser ) return result( [400, `API user not found`] );

        res = await dbh.selectRow( sql`SELECT telegram_user_id FROM telegram_user WHERE api_user_id = ?`, [apiUserId] );
        if ( !res.pk ) return res;

        oldTelegramUserId = res.data?.telegram_user_id;

        // not changed
        // XXX
        if ( oldTelegramUserId === telrgramUserId ) {
            return result( 200, {
                oldTelegramUserId,
                "newTelegramUserId": telrgramUserId,

                "oldApiUserId": apiUser.id,
                "oldApiUserEmail": apiUser.email,

                "newApiUserId": apiUser.id,
                "newApiUserEmail": apiUser.email,
            } );
        }

        if ( telrgramUserId ) {
            res = await dbh.selectRow( sql`SELECT api_user_id FROM telegram_user WHERE telegram_user_id = ?`, [telrgramUserId] );
            if ( !res.pk ) return res;

            const oldApiUserId = res.data?.api_user_id;

            if ( oldApiUserId ) {
                oldApiUser = await this.app.users.get( oldApiUserId );
                if ( !oldApiUser ) return result( [400, `API user not found`] );
            }
        }

        return dbh.begin( async dbh => {

            // api user is linked to the other telegram account
            if ( oldTelegramUserId ) {

                // unlink
                res = await dbh.do( sql`UPDATE telegram_user SET api_user_id = NULL WHERE telegram_user_id = ?`, [oldTelegramUserId] );
                if ( !res.pk ) throw res;

                // XXX update users cache
                // XXX notify
            }
            ...
            if ( telrgramUserId ) {
                ...
                // link
                res = await dbh.selectRow( sql`UPDATE telegram_user SET api_user_id = ? WJERE telegram_user_id = ?`, [
    ...
    async #createBot ( options, isStatic ) {
        ...
        return this.dbh.begin( async dbh => {
            ...
            // upsert api token
            res = await dbh.do( SQL.upsertApiToken, [id, options.apiToken] );
            if ( !res.ok ) throw res;
    ...
    async #downloadFile ( req ) {
        const [botId, fileUniqueId] = req.path.split( "/" ).slice( -2 );

        if ( !botId || !fileUniqueId ) return req.end( 404 );

        const bot = this.getBot( botId );

        if ( !bot ) return req.end( 404 );

        return bot.files.downloadFile( req, fileUniqueId );
    }

    async #downloadAvatar ( req ) {
        const path = req.path.substring( this.config.avatarUrl.length );

        const [botId, telegramBotUserId] = path.split( "/" );
        ...
        return bot.files.downloadAvatar( req, telegramBotUserId );
    }
}
