...
export default class TelegramGroup extends Events {
    ...
    async setTelegramGroupFields ( fields ) {
        if ( this.#title === fields.title && ( this.#username || null ) === ( fields.username || null ) && this.#isForum === fields.is_forum ) return result( 200 );

        const res = await this.dbh.do( SQL.setFields, [ fields.title, fields.username, fields.is_forum, this.#id ] );
