...
import { uuid, uuidToBuffer } from "#lib/uuid";
...
SELECT
   id,
    ...
    description,
    created,
    'https://t.me/' || ( SELECT telegram_username FROM telegram_bot WHERE id = telegram_bot_id ) || ? || start_param AS url,
    last_user_created,
    total_users,
...
SELECT
    ...
    description,
    created,
    'https://t.me/' || ( SELECT telegram_username FROM telegram_bot WHERE id = telegram_bot_id ) || ${"?start="} || start_param AS url,
    last_user_created,
    total_users,
    total_subscribed_users,
    total_unsubscribed_users,
...
`.WHERE( where );
        ...
        async API_createLink ( ctx, botId, { name, description } ) {
            ...
            return this.dbh.do( SQL.create, [
                ...
                botId,
                guid,
                this.app.telegram.encodeCallback( "link", uuidToBuffer( guid ) ),
                name,
                description,
            ] );
        }
        ...
        async API_updateLink ( ctx, linkId, values ) {
        ...
        async API_deleteLink ( ctx, linkId ) {
            const res = await this.dbh.do( SQL.delete, [linkId] );
