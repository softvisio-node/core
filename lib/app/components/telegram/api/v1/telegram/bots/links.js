...
SELECT
    ...
    description,
    created,
    'https://t.me/' || ( SELECT telegram_username FROM telegram_bot WHERE id = telegram_bot_id ) || ${"?start="} || start_param AS link,
    last_user_created,
    total_users,
    total_subscribed_users,
    total_unsubscribed_users,
...
`.WHERE( where );
        ...
        async API_createLink ( ctx, botId, { name, description } ) {
            ...
            return this.dbh.do( SQL.create, [
                ...
                botId,
                guid,
                this.app.telegram.encodeCallbackData( "link", uuidToBuffer( guid ) ),
                name,
                description,
            ] );
        }
