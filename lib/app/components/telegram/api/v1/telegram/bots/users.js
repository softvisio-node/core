...
SELECT
    ...
    CASE
        WHEN telegram_user.api_user_id IS NOT NULL THEN ${ this.app.api.config.avatarUrl } || telegram_user.api_user_id
        ELSE NULL
    END AS api_user_avatar_url
FROM
    telegram_bot_user,
    telegram_user
    LEFT JOIN "user" ON ( telegram_user.api_user_id = "user".id )
`.WHERE( where );

            return this._read( ctx, query, { options } );
        }

        async API_setUserBanned ( ctx, telegramBotId, telegramUserId, banned ) {
            const bot = this.app.telegram.bots.getBot( telegramBotId );
            if ( !bot ) return result( 404 );

            const user = await bot.users.getById( telegramUserId );
