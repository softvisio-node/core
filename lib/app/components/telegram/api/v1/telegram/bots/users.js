...
export default Super =>
    class extends Super {
        async API_getUsersList ( ctx, options ) {
            ...
            if ( options.search ) {
                where.and( `username ILIKE ${options.search[1]} OR first_name ILIKE ${options.search[1]} OR last_name ILIKE ${options.search[1]} OR phone ILIKE ${options.search[1]}` );
...
SELECT
    ...
    '/api/telegram/bots/users/avatar/' || telegram_bot_user.telegram_bot_id || '/' || telegram_bot_user.id AS avatar_url
FROM
    telegram_user,
    telegram_bot_user
`.WHERE( where );
