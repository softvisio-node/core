...
`.WHERE( where );
        ...
        async API_getRegisteredComponents ( ctx ) {
            return result(
                200,
                [...this.app.telegram.telegramComponents.values()].map( component => component.name )
            );
        }

        async API_checkBotApiToken ( ctx, apiToken ) {
            const telegramBotApi = new TelegramBotApi( apiToken );

            var res = await telegramBotApi.getMe();
            if ( !res.ok ) return res;

            const bot = res.data;

            res = await this.dbh.selectRow( sql`SELECT id FROM telegram_bot WHERE telegram_id = ?`, [bot.id] );
            if ( !res.ok ) return res;
        ...
        async API_createBot ( ctx, apiToken, type ) {
            return this.app.telegram.createBot( {
                apiToken,
                type,
                "ownerUserId": ctx.user.isRoot ? null : ctx.user.id,
            } );
        }
