...
export default class TelegramBot {
    ...
    constructor ( telegram, component, id, type, data ) {
        ...
        this.#users = new this.component.Users( this );
    ...
    updateTelegramBotFields ( fields ) {
        ...
        if ( "start_message_id" in fields ) {
            this.#startMessageId = fields.start_message_id;
        }
    }
    ...
    async getStartMessage ( { dbh } = {} ) {
        ...
        if ( !this.#startMessageId ) {
            ...
            const res = await dbh.begin( async dbh => {
                ...
                res = await dbh.do( sql`UPDATE telegram_bot SET start_message_id = ? WHERE id = ?`, [ message.id, this.id ] );
                if ( !res.ok ) throw res;
            } );
        ...
        return this.#messages.getMessageById( this.#startMessageId, { dbh } );
    }
