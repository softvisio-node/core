...
export default class TelegramBot {
    ...
    async #runSupergroupRequest ( req ) {
        ...
        if ( req.isMyChatMember ) {
            ...
            // XXX
            // await group.setSubscribed( req.data.new_chat_member.status === "member" );
        }
        else {

            // XXX
            // if ( !group.isSubscribed ) await user.setSubscribed( true );
        }
    ...
    async #createGroup ( options ) {
        const mutex = this.#mutexSet.get( "create-group/" + options.id );
        ...
        const res = await this.dbh.begin( async dbh => {
            ...
            res = await this._createGroup( dbh, options.id );
