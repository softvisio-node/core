...
const SQL = {
    ...
    // XXX
    "getStorageKey": sql`SELECT AS key FROM telegram_client WHERE id = ?`.prepare(),

    // XXX
    "setStorageKey": sql`UPDATE telegram_client SET WHERE id = ?`.prepare(),
};
...
export default class TelegramClient extends Events {
    ...
    async #getStorageKey ( key ) {
        ...
        if ( !res.data?.key ) {
            ...
            return this.app.crypto.decrrypt( !res.data.key, { "encoding": "base64url" } );
        }
    }
    ...
    async #setStorageKey ( key, value ) {
        if ( value != null ) {
            value = this.app.crypto.encrrypt( value ).toString( "base64url" );

            await this.dbh.do( SQL.setStorageKey, [ key, value, this.id ] );
        }
    }
