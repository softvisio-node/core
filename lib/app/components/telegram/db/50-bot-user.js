...
BEGIN
    ...
    ( date, telegram_bot_id, subscribed_users, unsubscribed_users ) VALUES (
        ...
        _user.telegram_bot_id,
        CASE WHEN _user.subscribed THEN 1 ELSE 0 END,
        CASE WHEN _user.subscribed THEN 0 ELSE 1 END
    )
    ON CONFLICT ( date, telegram_bot_id ) DO UPDATE SET
        subscribed_users = CASE
            WHEN _user.subscribed THEN t.subscribed_users + 1
            ELSE t.subscribed_users
            END,
        unsubscribed_users = CASE
            WHEN _user.subscribed THEN t.unsubscribed_users
            ELSE t.unsubscribed_users + 1
            END;

    -- ad campaign
    IF _user.telegram_bot_ad_campaign_id THEN
