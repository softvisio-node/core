...
CREATE FUNCTION _telegram_bot_user_subscribe_update( _bot_id int8, _telegram_bot_ad_campaign_id int8, _subscribed bool, _new_user bool ) RETURNS VOID AS $$
BEGIN
    ...
    SET
       total_users = CASE
        ...
        total_subscribed_users = CASE
            WHEN _subscribed THEN total_subscribed_users + 1
            ELSE total_subscribed_users
            END,
        total_returned_users = CASE
            WHEN _new_user THEN total_returned_users
            WHEN _subscribed THEN total_returned_users + 1
    ...
    INSERT INTO
        telegram_bot_subscription_stats AS t
    ( date, telegram_bot_id, subscribed_users, unsubscribed_users ) VALUES (
    ...
    ON CONFLICT ( date, telegram_bot_id ) DO UPDATE SET
        subscribed_users = CASE
            WHEN _subscribed THEN t.subscribed_users + 1
            ELSE t.subscribed_users
            END,
        unsubscribed_users = CASE
            WHEN _subscribed THEN t.unsubscribed_users
            ELSE t.unsubscribed_users + 1
            END;
    ...
    IF _telegram_bot_ad_campaign_id THEN
        ...
        SET
           total_users = CASE
            ...
            total_subscribed_users =
                CASE WHEN _subscribed THEN total_subscribed_users + 1
                ELSE total_subscribed_users
                END,
            total_returned_users = CASE
                WHEN _new_user THEN total_returned_users
        ...
        INSERT INTO
            telegram_bot_ad_capmaign_stats AS t
        ( date, telegram_bot_ad_campaign_id, subscribed_users, unsubscribed_users ) VALUES (
            date_trunc( 'hour', CURRENT_TIMESTAMP ),
            _telegram_bot_ad_campaign_id,
            CASE WHEN _subscribed THEN 1 ELSE 0 END,
            CASE WHEN _subscribed THEN 0 ELSE 1 END
        )
        ON CONFLICT ( date, telegram_bot_ad_campaign_id ) DO UPDATE SET
            subscribed_users = CASE
                WHEN _subscribed THEN t.subscribed_users + 1
                ELSE t.subscribed_users
                END,
...
BEGIN
    ...
    PERFORM _telegram_bot_user_subscribe_update(
        NEW.telegram_bot_id,
...
BEGIN
    ...
    IF NEW.subscribed != OLD.subscribed THEN
        ...
        NEW.returned = CASE WHEN NEW.subscribed THEN TRUE ELSE FALSE END;
...
BEGIN
    ...
    IF NEW.subscribed != OLD.subscribed THEN
        ...
        UPDATE
            telegram_bot
        SET
            total_banned_users = CASE WHEN NEW.banned THEN total_banned_users + 1 ELSE total_banned_users - 1 END
        WHERE
            id = NEW.telegram_bot_id;

        IF NEW.telegram_bot_ad_campaign_id THEN
            ...
            SET
                total_banned_users = CASE WHEN NEW.banned THEN total_banned_users + 1 ELSE total_banned_users - 1 END
            WHERE
                id = NEW.telegram_bot_ad_campaign_id;
