...
CREATE SEQUENCE telegram_bot_id_seq AS int8 MAXVALUE ${ Number.MAX_SAFE_INTEGER };

CREATE TABLE telegram_bot (
    id int53 PRIMARY KEY DEFAULT nextval( 'telegram_bot_id_seq' ),
    ...
    telegram_user_id int53 NOT NULL UNIQUE,
    ...
    start_message_id int53 -- REFERENCES telegram_bot_message ( id ) ON DELETE SET RESTRICT
);

ALTER SEQUENCE telegram_bot_id_seq OWNED BY telegram_bot.id;
...
BEGIN
    ...
    PERFORM pg_notify( 'telegram/telegram-bot/update', json_build_object(
        ...
        'start_message_id', NEW.start_message_id
    )::text );
...
CREATE TRIGGER telegram_bot_locales_after_update AFTER UPDATE OF locales, default_locale, start_message_id ON telegram_bot FOR EACH ROW EXECUTE FUNCTION telegram_bot_locales_after_update_trigger();
