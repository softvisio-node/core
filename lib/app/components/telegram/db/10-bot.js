...
BEGIN
    ...
    PERFORM pg_notify( 'telegram-bot/create', json_build_object(
        'id', NEW.id::text
    )::text
...
BEGIN
    ...
    PERFORM pg_notify( 'telegram-bot/delete', json_build_object(
        'id', NEW.id::text,
...
CREATE TRIGGER telegram_bot_after_update AFTER UPDATE ON telegram_bot FOR EACH ROW EXECUTE FUNCTION telegram_bot_after_update_trigger();
