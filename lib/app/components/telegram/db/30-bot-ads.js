...
CREATE TABLE telegram_bot_ad_campaign_stats (
    ...
    PRIMARY KEY ( telegram_bot_ad_campaign_id, date )
);

CREATE FUNCTION telegram_bot_ad_campaign_after_update_stats_trigger() RETURNS TRIGGER AS $$
BEGIN

    INSERT INTO
        telegram_bot_ad_campaign_stats AS t
    (
        date,
...
CREATE TRIGGER telegram_bot_ad_campaign_after_update_stats AFTER UPDATE OF total_users, total_subscribed_users, total_unsubscribed_users, total_returned_users ON telegram_bot_ad_campaign FOR EACH ROW EXECUTE FUNCTION telegram_bot_ad_campaign_after_update_stats_trigger();
