...
CREATE SEQUENCE telegram_bot_user_id_seq AS int8 MAXVALUE ${ Number.MAX_SAFE_INTEGER };

CREATE TABLE telegram_bot_user (
    id int53 PRIMARY KEY DEFAULT nextval( 'telegram_bot_user_id_seq' ),
...
ALTER SEQUENCE telegram_bot_user_id_seq OWNED BY telegram_bot_user.id;

CREATE OR REPLACE VIEW telegram_bot_user_view AS
SELECT
    telegram_bot_user.telegram_bot_id AS telegram_bot_id,
    telegram_bot_user.id AS telegram_bot_user_id,
    telegram_user_view.*,
    json_build_object(
        'id', telegram_bot_user.id,
        'telegram_bot_id', telegram_bot_user.telegram_bot_id,
        'subscribed', telegram_bot_user.subscribed,
        'banned', telegram_bot_user.banned,
        'state', telegram_bot_user.state,
        'locale', telegram_bot_user.locale
    ) AS telegram_bot_user
FROM
    telegram_bot_user,
    telegram_user_view
WHERE
    telegram_bot_user.telegram_user_id = telegram_user_view.telegram_user_id;
