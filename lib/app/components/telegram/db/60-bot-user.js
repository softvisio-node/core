...
CREATE TABLE telegram_bot_user_link (
    ...
    FOREIGN KEY ( telegram_bot_id, telegram_user_id ) REFERENCES telegram_bot_user ( telegram_bot_id, id ) ON DELETE CASCADE,
    PRIMARY KEY ( telegram_user_id, telegram_bot_id, telegram_bot_link_id)
);
...
BEGIN
    ...
    IF NEW.subscribed != OLD.subscribed THEN
        ...
        WHERE
            ...
            AND telegram_bot_user_link.telegram_user_id = NEW.id
    ...
    IF NEW.banned != OLD.banned THEN
        ...
        WHERE
            telegram_bot_user_link.telegram_bot_id = NEW.telegram_bot_id
            AND telegram_bot_user_link.telegram_user_id = MEW.id
            AND telegram_bot_user_link.telegram_bot_link_id = telegram_bot_link.id;
