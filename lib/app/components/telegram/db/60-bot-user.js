...
CREATE TABLE telegram_bot_user (
    ...
    locale text,
    avatar_telegram_bot_file_id int8 REFERENCES telegram_bot_file ( id ) ON DELETE RESTRICT,
...
CREATE FUNCTION telegram_bot_user_avatar_after_update_trigger() RETURNS TRIGGER AS $$
BEGIN

    -- subscribed changed
    IF ( OLD.avatar_telegram_bot_file_id IS NOT NULL AND NEW.avatar_telegram_bot_file_id != OLD.avatar_telegram_bot_file_id ) THEN

        DELETE FROM telegram_bot_file WHERE id = OLD.avatar_telegram_bot_file_id;
...
CREATE TRIGGER telegram_bot_user_avatar_after_update AFTER UPDATE OF avatar_telegram_bot_file_id ON telegram_bot_user FOR EACH ROW EXECUTE FUNCTION telegram_bot_user_avatar_after_update_trigger();
