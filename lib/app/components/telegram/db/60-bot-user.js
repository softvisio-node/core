...
CREATE TABLE telegram_bot_user (
    ...
    locale text,
    avatar_file_id int8 REFERENCES telegram_bot_file ( id ) ON DELETE RESTRICT,
...
BEGIN
    ...
    IF ( OLD.avatar_file_id IS NOT NULL AND NEW.avatar_file_id != OLD.avatar_file_id ) THEN
        DELETE FROM telegram_bot_file WHERE id = OLD.avatar_file_id;
    END IF;
    ...
    PERFORM pg_notify( 'telegram/telegram-bot-user/' || NEW.telegram_bot_id || '/update', json_build_object(
        ...
        'locale', NEW.locale,
        'avatar_file_id', CASE
            WHEN NEW.avatar_file_id IS NOT NULL THEN NEW.avatar_file_id::text
            ELSE NULL
        END,
        'avatar_updated', NEW.avatar_updated
    )::text );
...
CREATE TRIGGER telegram_bot_user_after_update AFTER UPDATE OF api_user_id, subscribed, banned, locale, avatar_file_id ON telegram_bot_user FOR EACH ROW EXECUTE FUNCTION telegram_bot_user_after_update_trigger();
