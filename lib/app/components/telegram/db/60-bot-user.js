...
CREATE TABLE telegram_bot_user (
    ...
    locale text,
    profile_photo_updated timestamptz,
    UNIQUE ( telegram_bot_id, telegram_user_id ),
    UNIQUE ( telegram_bot_id, api_user_id )
);

CREATE TABLE telegram_bot_user_profile_photo (
    telegram_bot_user_id int8 REFERENCES TABLE telegram_bot_user ( id ) ON DELETE CASCADE,
    index int4 NOT NULL,
    telegram_bot_file_id int8 NOT NULL REFERENCES telegram_bot_file ( id ) ON DELETE RESTRICT,
    PRIMARY KEY ( telegram_bot_user_id, index )
);
...
CREATE FUNCTION telegram_bot_user_profile_photo_after_update_trigger() RETURNS TRIGGER AS $$
BEGIN

    -- subscribed changed
    IF ( OLD.telegram_bot_file_id IS NOT NULL AND NEW.telegram_bot_file_id != OLD.telegram_bot_file_id ) THEN

        DELETE FROM telegram_bot_file WHERE id = OLD.telegram_bot_file_id;
...
CREATE TRIGGER telegram_bot_user_profile_photo_after_update AFTER UPDATE ON telegram_bot_user_profile_photo FOR EACH ROW EXECUTE FUNCTION telegram_bot_user_profile_photo_after_update_trigger();

CREATE FUNCTION telegram_bot_user_profile_photo_after_delete_trigger() RETURNS TRIGGER AS $$
BEGIN

    -- subscribed changed
    IF ( OLD.telegram_bot_file_id IS NOT NULL ) THEN
...
CREATE TRIGGER telegram_bot_user_profile_photo_after_udelete AFTER DELETE ON telegram_bot_user_profile_photo FOR EACH ROW EXECUTE FUNCTION telegram_bot_user_profile_photo_after_delete_trigger();
