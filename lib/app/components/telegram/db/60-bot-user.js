...
CREATE TYPE telegram_bot_user_status AS ENUM ( 'member', 'kicked' );
...
CREATE TABLE telegram_bot_user (
    ...
    status telegram_bot_user_status NOT NULL DEFAULT 'member',
...
BEGIN
    ...
    IF NEW.banned != OLD.banned THEN
        ...
        WHERE
            telegram_bot_user_link.telegram_bot_id = NEW.telegram_bot_id
            AND telegram_bot_user_link.telegram_user_id = MEW.telegram_user_id
            AND telegram_bot_user_link.telegram_bot_link_id = telegram_bot_link.id;
...
CREATE TRIGGER telegram_bot_user_after_update AFTER UPDATE OF status, subscribed, banned, locale ON telegram_bot_user FOR EACH ROW EXECUTE FUNCTION telegram_bot_user_after_update_trigger();
