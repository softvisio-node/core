...
CREATE TABLE telegram_bot_user (
    id int53 NOT NULL REFERENCES telegram_user ( id ) ON DELETE RESTRICT,
    ...
    state text,
    locale text,
    PRIMART KEY ( id, telegram_bot_id )
);

CREATE TABLE telegram_bot_user_link (
    telegram_bot_user_id int53 NOT NULL,
    ...
    FOREIGN KEY ( telegram_bot_user_id, telegram_bot_id ) REFERENCES telegram_bot_user ( id, telegram_bot_id ) ON DELETE CASCADE,
    PRIMARY KEY ( telegram_bot_user_id, telegram_bot_id, telegram_bot_link_id)
);
...
BEGIN
    ...
    IF NEW.subscribed != OLD.subscribed THEN
        ...
        WHERE
            ...
            AND telegram_bot_user_link.telegram_bot_id = NEW.telegram_bot_id
