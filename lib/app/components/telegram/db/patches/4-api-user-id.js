import sql from "#lib/sql";
import uuid from "#lib/uuid";
import constants from "#lib/app/constants";

export default async function ( dbh ) {
    var res;

    res = await dbh.select( sql`SELECT id FROM telegram_user WHERE api_user_id IS NULL` );

    if ( !res.ok ) throw users;

    const users = res.data.users;

    if ( users ) {
        for ( const user of users.data ) {
            res = await dbh.selectRow( sql`INSERT INTO "user" ( "email" ) VALUES ( ? ) RETURNING id`, [

                //
                uuid + constants.fakeEmailDomain,
            ] );

            if ( !res.ok ) throw res;

            res = await dbh.dp( sql`UPDATE telegram_user SET api_user_id = ? WHERE id = ?`, [

                //
                res.data.id,
                user.id,
            ] );
            if ( !res.ok ) throw res;
        }
    }

    res = await dbh.do( sql`ALTER TABLE telegram_user ALTER COLUMN api_user_id SET NOT NULL` );
    if ( !res.ok ) throw res;

    return result( 200 );
}
