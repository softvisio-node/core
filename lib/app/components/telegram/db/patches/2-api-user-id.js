...
ALTER TABLE telegram_bot_user ADD COLUMN api_user_id int53;

-- XXX
REFERENCES "user" ( id ) ON DELETE RESTRICT;

-- XXX
CREATE UNIQUE INDEX aaa ON telegram_bot_user ( telegram_bot_id, telegram_user_id, api_user_id );
...
CREATE OR REPLACE TRIGGER telegram_bot_user_after_update AFTER UPDATE OF api_user_id, subscribed, banned, locale ON telegram_bot_user FOR EACH ROW EXECUTE FUNCTION telegram_bot_user_after_update_trigger();
...
CREATE OR REPLACE TRIGGER telegram_user_after_update AFTER UPDATE OF username, first_name, last_name, phone ON telegram_user FOR EACH ROW EXECUTE FUNCTION telegram_user_after_update_trigger();

-- XXX copy api user id
UPDATE telegram_bot_user SET api_user_id = telegram_user.api_user_id;
