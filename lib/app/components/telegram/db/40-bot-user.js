...
CREATE TABLE telegram_bot_subscription_stat (
    ...
    subsctibed_users int4 NOT NULL DEFAULT 0,
    unsubsctibed_users int4 NOT NULL DEFAULT 0,
    PRIMARY KEY ( date, telegram_bot_id ),
    UNOQUE ( date, telegram_bot_id, telegram_bot_ad_campaign_id )
}
...
CREATE FUNCTION telegram_bot_user_subscribe_update( _bot_id int8, _bot_user_id int8, _telegram_bot_ad_campaign_id int8, _subscribed bool, _new_user bool ) RETURNS NULL AS $$
BEGIN
UPDATE
        telegram_bot
    SET
    ...
    INSERT INTO
        telegram_bot_subscription_stat
    ( date, telegram_bot_id, telegram_bot_ad_campaign_id, subscribed, unsubscribed ) VALUES (
        date_trunc( 'hour', CURRENT_TIMESTAMP ),
        _telegram_bot_id,
        _telegram_bot_ad_campaign_id,
        CASE _subscribed THEN 1 ELSE 0 END,
        CASE _subscribed THEN 0 ELSE 1 END
    )
    ON CONFLICT ( date, telegram_bot_id, telegram_bot_ad_campaign_id ) DO UPDATE SET
        subscribed = CASE _subscribed THEN subscribed + 1 ELSE subscribed END,
        unsubscribed = CASE _subscribed THEN unsubscribed ELSE unsubscribed + 1 END
    ;
