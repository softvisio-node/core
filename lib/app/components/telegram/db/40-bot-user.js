...
CREATE FUNCTION telegram_bot_user_subscribe_update( _bot_id int8, _bot_user_id int8, _telegram_bot_ad_campaign_id int8, _subscribed bool, _new_user bool ) RETURNS VOID AS $$
BEGIN
UPDATE
        telegram_bot
    SET
    ...
    ON CONFLICT ( date, telegram_bot_id, telegram_bot_ad_campaign_id ) DO UPDATE SET
        subscribed = CASE WHEN _subscribed THEN subscribed + 1 ELSE subscribed END,
        unsubscribed = CASE WHEN _subscribed THEN unsubscribed ELSE unsubscribed + 1 END
    ;
