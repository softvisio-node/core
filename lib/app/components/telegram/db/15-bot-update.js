import sql from "#lib/sql";

export default sql`

CREATE TABLE telegram_bot_update (
    id serial8 PRIMARY_KEY,
    update_id int8 NOT NULL,
    telegram_bot_id int8 REFERENCES telegram_bot ( id ) ON DELETE CASCADE,
    chat_id int8 NOT NULL,
    data json,
    UNIQUE ( telegram_bot_id, chat_id )
);

-- after insert
CREATE FUNCTION telegram_bot_update_after_insert_trigger() RETURNS TRIGGER AS $$
BEGIN

    UPDATE telegram_bot SET telegram_next_update_id = NEW.update_id + 1 WHERE id = NEW.telegram_bot_id;

    -- PERFORM pg_notify( 'telegram/telegram-bot-update/create', json_build_object(
    --     'telegram_bot_id', NEW.telegram_bot_id::text
    -- )::text );

    RETURN NULL;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER telegram_bot_update_after_insert AFTER INSERT ON telegram_bot_update FOR EACH ROW EXECUTE FUNCTION telegram_bot_update_after_insert_trigger();

`;
