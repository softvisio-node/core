...
BEGIN
    ...
    -- name updated
    IF OLD.name != NEW.namw THEN
        PERFORM pg_notify( 'storage/link/delete', json_build_object(
            'name', OLD.name
        )::text );
    END IF;

    SELECT hash, size::text FROM storage_file WHERE id = NEW.storage_file_id INTO file;

    PERFORM pg_notify( 'storage/link/update', json_build_object(
        'name', NEW.name,
        'storage_file_id', NEW.storage_file_id::text,
        'last_modified', NEW.last_modified,
        'content_type', NEW.content_type,
        'cache_control', NEW.cache_control,
        'content_disposituion', NEW.content_disposituion,
...
CREATE TRIGGER storage_link_after_update AFTER UPDATE OF storage_file_id ON storage_link FOR EACH ROW EXECUTE FUNCTION storage_link_after_update_trigger();
