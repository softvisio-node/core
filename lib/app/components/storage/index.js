...
import LocalStorage from "./static-files/local.js";
...
`.prepare(),
    ...
    "unlock": sql`SELECT pg_advisory_unlock( get_lock_id( 'static-files/clear' ) )`.prepare(),
};
...
export default class StaticFiles {
    ...
    constructor ( dbh, url, { clearFilesLimit } = {} ) {
        this.#dbh = dbh;
        ...
        if ( url.protocol === "file:" ) {
            this.#storage = new LocalStorage( this, url );
        }
        else {
            throw Error( `Static files storage url is invalid` );
        }
    ...
    async add ( file, { name, headers, dbh } = {} ) {
        ...
        const res = await dbh.begin( async dbh => {
            ...
            if ( !+link.data.file_links_count ) {
                const res = await this.#storage.add( file, link.data.file_guid );
                if ( !res.ok ) throw res;
            }
    ...
    async get ( guid ) {
        ...
        var file = await this.#storage.get( res.data.guid );
        if ( !file.ok ) return res;
        ...
        const res = await this.dbh.lock( async dbh => {
            ...
            while ( true ) {
                ...
                for ( const file of files.data ) {
                    const res = await this.#storage.clear( file.guid );
