import path from "node:path";

export default class {
    #storage;
    #location;
    #encrypt;
    #deduplicate;

    constructor ( storage, location, encrypt, deduplicate ) {
        this.#storage = storage;
        this.#location = location;
        this.#encrypt = !!encrypt;
        this.#deduplicate = !!deduplicate;
    }

    // properties
    get app () {
        return this.#storage.app;
    }

    get storage () {
        return this.#storage;
    }

    get location () {
        return this.#location;
    }

    get encrypt () {
        return this.#encrypt;
    }

    get deduplicate () {
        return this.#deduplicate;
    }

    // public
    createImagePath ( filePath, hash ) {
        if ( !this.deduplicate ) return filePath;

        return path.posix.join( this.location, hash.substring( 0, 2 ), hash.substring( 2, 4 ), hash );
    }

    async uploadImage ( imagePath, file, { encrypt } = {} ) {
        if ( encrypt && !this.app.crypto ) return result( [ 500, `Unable to encrypt file` ] );

        return this._uploadImage( imagePath, file, { encrypt } );
    }

    async getFile ( file ) {
        if ( file.isEncrypted && !this.app.crypto ) return result( [ 500, `Unable to decrypt file` ] );

        return this._getFile( file );
    }

    async getBuffer ( file ) {
        if ( file.isEncrypted && !this.app.crypto ) return result( [ 500, `Unable to decrypt file` ] );

        return this._getBuffer( file );
    }

    async downloadFile ( req, imagePath, headers, isEncrypted ) {
        if ( isEncrypted && !this.app.crypto ) return req.end( 404 );

        return this._downloadFile( req, imagePath, headers, isEncrypted );
    }
}
