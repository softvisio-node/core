...
export default class GoogleStorage extends Storage {
    ...
    async _uploadImage ( _path, file ) {
        return this.#storage.uploadFile( this.#bucket + _path, file );
    }

    async _deleteImage ( _path ) {
        const res = await this.#storage.deleteFile( this.#bucket + _path );
    ...
    async _getFile ( file ) {
        const res = await this.#storage.getFile( this.#bucket + file.path );
    ...
    async _downloadFile ( req, _path, headers ) {
        req.end( {
            ...
            "body": async () => {
                ...
                const res = await this.#storage.getFileHttpResponse( this.#bucket + _path, {
                    "method": req.method,
                    "headers": reqHeaders,
                } );
