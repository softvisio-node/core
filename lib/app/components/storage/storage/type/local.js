...
export default class LocalStorage extends Storage {
    ...
    async _uploadImage ( _path, file ) {
        await fs.promises.copyFile( file.path, path.join( this.#location, _path ) );

        return result( 200 );
    }

    async _deleteImage ( _path ) {
        await fs.promises.rm( path.join( this.#location, _path ), { "force": true } );
    ...
    async _getFile ( file ) {
        ...
        await fs.promises.copyFile( path.join( this.#location, file.oath ), tmp.path );
    ...
    async _downloadFile ( req, _path, headers ) {
        const file = new File( {
            "path": path.join( this.#location, _path ),
        } );
