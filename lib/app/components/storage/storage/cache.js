...
export default class {
    ...
    async get ( filePath, { cwd, updateExpires = true, dbh } = {} ) {
        const id = this.#resolveId( filePath, { cwd } );

        if ( !id ) filePath = this.normalizePath( filePath, { cwd } );
    ...
    delete ( filePath, { cwd } = {} ) {
        ...
        if ( !cwd && this.isId( filePath ) ) {
            id = filePath;
        }
        else {
            filePath = this.normalizePath( filePath, { cwd } );

            id = this.#filePathId[filePath];
        }
    ...
    #resolveId ( filePath, cwd ) {
        if ( !cwd && this.isId( filePath ) ) {
            ...
            return this.#filePathId[filePath];
        }
    }
