...
export default class extends Bucket {
    ...
    async _getFile ( file, { dbh } = {} ) {
        ...
        try {
            if ( file.isEncrypted ) {
                await stream.promises.pipeline( await this.app.crypto.decrypt( readStream ), writeStream );
            }
            else {
    ...
    async _getStream ( file, { offset, length, dbh } = {} ) {
        ...
        if ( file.isEncrypted ) {
            out = stream.pipeline( await this.app.crypto.decrypt( await dbh.largeObject.createReadStream( file.imageOid ) ), new StreamSlice( { offset, length } ), e => {} );
        }
        else {
            out = await dbh.largeObject.createReadStream( file.imageOid, { offset, length } );
        }
