...
export default class extends Bucket {
    ...
    async _uploadImage ( image, file, { encrypt, dbh } = {} ) {
        if ( encrypt ) {
            ...
            try {
                await stream.promises.pipeline( await this.app.crypto.encrypt( file.stream() ), fs.createWriteStream( tmpFile.path ) );
        ...
        return this.#api.uploadFile( this.#buildImagePath( image.path ), file );
    }
