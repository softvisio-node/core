...
export default class Storage {
        ...
        if ( await mutex.tryLock() ) {
            while ( true ) {
                ...
                for ( const image of images.data ) {
                    ...
                    this.#deleteImagesThreads.runThread( this.#deleteImage.bind( this, image ) ).then( () => counter.value-- );
                }
    ...
    async uploadFile ( filePath, file, { cwd, lastModified, contentType, cacheControl, contentDisposition, expires, maxAge, inactiveMaxAge, encrypt, dbh } = {} ) {
        ...
        try {
            ...
            if ( location.deduplicate ) {
                const imgeExists = await bucket.imageExists( imagePath, { dbh } );

                if ( !imgeExists ) {
                    res = await bucket.uploadImage( imagePath, file, { encrypt, dbh } );
                    if ( !res.ok ) throw res;
                }
            }
            else {
    ...
    async getFileMeta ( filePath, { cwd, checkImage, dbh } = {} ) {
        ...
        if ( checkImage ) {
            ...
            const imageExists = await bucket.imageExists( file.imagePath, { dbh } );
    ...
    async #getHash ( file ) {
        return hash( HASH_ALGORYTM, file, {
