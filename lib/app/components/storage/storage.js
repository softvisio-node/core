...
INSERT INTO storage_file
    ( name, storage_image_id, last_modified, content_type, cache_control, content_disposition )
    VALUES
    ( ?, ?, ?, ?, ?, ? )
...
`,
    ...
    "deleteFile": sql`DELETE FROM storage_file WHERE name = ?`.prepare(),
};
...
export default class Storage {
    ...
    async uploadFile ( name, file, { lastModified, contentType, cacheControl, contentDisposition } = {} ) {
        ...
        // upsert file link
        return this.#dbh.do( SQL.upsertLink, [name, imageId, lastModified || new Date(), contentType, cacheControl, contentDisposition] );
    }

    async deleteFile ( name ) {
        return this.#dbh.do( SQL.deleteFile, [name] );
    }
    ...
    async #getFile ( req ) {
        const name = req.path.substring( this.#location.length + 1 );

        const file = await this.#cache.get( name );

        if ( !file ) return req.end( 404 );

        return this._downloadFile( req, name, file );
    }
}
