...
export default class Storage {
    ...
    async init () {
        ...
        // init buckets
        for ( const [location, config] of Object.entrues( this.config.buckets ) ) {
            let bucket;
    ...
    async start () {
        this.#clearCron = new Cron( this.config.clearCron ).on( "tick", this.clear.bind( this ) ).unref().start();
        ...
        try {
            ...
            while ( true ) {
                ...
                try {
                    ...
                    // deleteeee image
                    res = await this.deleteImage( image.data.path );
                    if ( !res.ok ) throw res;

                    res = await this.#dbh.do( SQL.deleteImage, [image.data.path] );
                    if ( !res.ok ) throw res;
                }
                catch ( e ) {}
    ...
    async uploadFile ( path, file, { lastModified, contentType, cacheControl, contentDisposition, expires, maxAge, inactiveMaxAge, dbh } = {} ) {
        ...
        try {
            ...
            // create image
            res = await dbh.do( SQL.createImage, [path] );
            if ( !res.ok ) throw res;

            // upload image
            res = await this.uploadImage( path, file );
            if ( !res.ok ) throw res;
    ...
    async isFileExists ( id, { checkImage, dbh } = {} ) {
        const file = await this.#cache.get( id, { "updateExpires": false, dbh } );

        if ( !file ) return false;

        if ( checkImage ) {
            return this.imageExists( file.path );
        }
        else {
            return true;
        }
    ...
    async downloadFile ( req, id, { contentType, cacheControl, contentDisposition, dbh } = {} ) {
        ...
        if ( !file ) {
            ...
            return this.downloadFile( req, file.path, headers );
        }
    }
