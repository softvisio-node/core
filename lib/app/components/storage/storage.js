...
`.prepare(),
    ...
    "deleteFileById": sql`DELETE FROM storage_file WHERE id = ? RETURNING storage_image_id`.prepare(),
...
export default class Storage {
    ...
    async deleteFile ( patterns, { cwd, dbh } = {} ) {
        ...
        if ( this.#cache.isId( patterns ) ) {
            const res = await dbh.selectRow( SQL.deleteFileById, [patterns] );
            ...
            if ( !Array.isArray( patterns ) && !GlobPatterns.isGlobPattern( patterns ) ) {
                ...
                const res = await dbh.selectRow( sql`DELETE FROM storage_file`.WHERE( where ).sql`RETURNING storage_image_id` );
            ...
            // glob
                ...
                const res = await dbh.do( sql`DELETE FROM storage_file`.WHERE( where ) );

                if ( res.meta.rows ) this.cleat();
