...
export default class Storage {
    ...
    async configure () {
        ...
        for ( const component of this.app.components ) {
            ...
            const locations = component.storageConfig;
        ...
        for ( const location in this.config.locations ) {
            ...
            this.config.locations[location].virtual ??= true;
        }
    ...
    async init () {
        ...
        for ( const [location, config] of Object.entries( this.config.locations ) ) {
            if ( !config.private ) {
                publicHttpServer?.get( `${this.#location}/${location}/*`, this.downloadFile.bind( this ) );

                privateHttpServer?.get( `${this.#location}/${location}/*`, this.downloadFile.bind( this ) );
            }
            else {
                publicHttpServer?.get( `${this.#location}/${location}/*`, this.#privateLocation.bind( this ) );

                privateHttpServer?.get( `${this.#location}/${location}/*`, this.#privateLocation.bind( this ) );
            }
        }
    ...
    async uploadFile ( path, file, { lastModified, contentType, cacheControl, contentDisposition, expires, maxAge, inactiveMaxAge, callback } = {} ) {
        ...
        if ( path.startsWith( VIRTUAL_LOCATION ) ) return result( [400, `Location is not valid`] );
