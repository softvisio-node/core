...
export default class Storage {
    ...
    async init () {
        ...
        this.#locations = new Locations( this, this.config.locations );
        ...
        for ( const location of this.#locations.getLocations() ) {
            let httpLocation = this.#cache.normalizePath( this.config.location + "/" + location.location );
            if ( httpLocation === "/" ) httpLocation = "";

            if ( !location.isPrivate ) {
                publicHttpServer?.get( location.httpLocation + "/*", this.#downloadFile.bind( this ) );

                privateHttpServer?.get( location.httpLocation + "/*", this.#downloadFile.bind( this ) );
            }
            else {
                publicHttpServer?.get( location.httpLocation + "/*", this.#privateLocation.bind( this ) );

                privateHttpServer?.get( location.httpLocation + "/*", this.#privateLocation.bind( this ) );
            }
        }
    ...
    async uploadFile ( filePath, file, { cwd, lastModified, contentType, cacheControl, contentDisposition, expires, maxAge, inactiveMaxAge, encrypt, dbh } = {} ) {
        ...
        const bucket = this.#getBucket( filePath ),
            ...
            location = this.#locations.getLocation( filePath );

        // encrypt
        encrypt = !!( encrypt ?? location.encrypt );
        if ( encrypt && !this.app.crypto ) return result( [ 400, `Unable to encrypt file` ] );

        const imagePath = bucket.createImagePath( filePath, hash, encrypt );
    ...
    async getFileMeta ( filePath, { cwd, checkImage, dbh } = {} ) {
        ...
        if ( checkImage ) {
            ...
            const imageExists = await bucket.imageExists( file.imageExists );
    ...
    async downloadFile ( req, filePath, { cwd, contentType, cacheControl, contentDisposition, dbh } = {} ) {
        ...
        if ( !file ) {
            ...
            const headers = file.getHeaders(),
                location = this.#locations.getLocation( file.path );
