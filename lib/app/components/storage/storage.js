...
INSERT INTO storage_file
    ( path, storage_image_id, last_modified, content_type, cache_control, content_disposition, expires )
    VALUES
    ( ?, ?, ?, ?, ?, ?, ? )
...
export default class Storage {
    ...
    constructor ( app, config ) {
        ...
        this.#shutdownSignal = global.shutdown.signal( "storage" ).on( "shutdown", this.#shutdown.bind( this ) );
    }
    ...
    // maxAge: seconds
    async uploadFile ( path, file, { lastModified, contentType, cacheControl, contentDisposition, expires, maxAge } = {} ) {
        ...
        if ( maxAge ) expires = new Date( Date.now() + maxAge * 1000 );

        // upsert file link
        return this.#dbh.do( SQL.upsertLink, [this.#normalizePath( path ), imageId, lastModified || new Date(), contentType, cacheControl, contentDisposition, expires] );
    }
