...
export default class Storage {
    ...
    async init () {
        ...
        for ( var [location, config] of Object.entries( this.config.locations ) ) {
            ...
            if ( location.startsWith( VIRTUAL_LOCATION ) ) return result( [500, `Storage location "${location}" is not valid`] );
        ...
        this.sortedLocations = Object.kwys( this.#locations ).sort( ( a, b ) => b.length - a.length );
    ...
    async uploadFile ( path, file, { lastModified, contentType, cacheControl, contentDisposition, expires, maxAge, inactiveMaxAge, callback } = {} ) {
        ...
        // XXX
        // if ( path.startsWith( VIRTUAL_LOCATION ) )
