...
export default class Storage {
    ...
    async isFileExists ( path ) {
        const file = await this.#cache.get( path, { "updateExpires": false } );
    ...
    async copyFile ( from, to, { lastModified, cacheControl, contentDisposition, expires, maxAge, inactiveMaxAge, dbh } = {} ) {
        ...
        return dbh.selectRow( SQL.createFile, [
    ...
    async deleteFile ( path ) {
        ...
        if ( id ) {
            return this.#dbh.do( SQL.deleteFileById, [id] );
        }
        else {
            return this.#dbh.do( SQL.deleteFileByPath, [this.#cache.normalizePath( path )] );
        }
    }
