...
export default class Storage {
    ...
    getFileUrl ( filePath, { cwd } = {} ) {
        return this.#cache.normalizePath( filePath, { "cwd": this.location + "/" + ( cwd || "" ) } );
    }
    ...
    async uploadFile ( filePath, file, { cwd, lastModified, contentType, cacheControl, contentDisposition, expires, maxAge, inactiveMaxAge, dbh } = {} ) {
    ...
    // XXX cwd
    async downloadFile ( req, id, { cwd, contentType, cacheControl, contentDisposition, dbh } = {} ) {
    ...
    async deleteFile ( patterns, { cwd, dbh } = {} ) {
        ...
        // id
        if ( !cwd && this.#cache.isId( patterns ) ) {
