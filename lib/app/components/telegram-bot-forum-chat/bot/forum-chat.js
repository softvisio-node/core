...
`.prepare(),
    ...
    "setUserTopic": sql`INSERT INTO telegram_bot_forum_chat_group_topic ( telegram_group_id, topic_id, telegram_user_id ) VALUES ( ?, ?, ? )`.prepare(),
...
export default class {
    ...
    async getUserTopic ( user ) {
        ...
        res = await this.dbh.do( SQL.setUserTopic, [ group.id, topicId, user.id ] );
        if ( !res.ok ) return;
