...
export default class {
    ...
    // XXX
    async setGroup ( telegramGroupUrl ) {
        const url = new URL( telegramGroupUrl );
        ...
        if ( labels[ 0 ] === "c" ) {
            const groupId = labels[ 1 ];

            newGroup = await this.#bot.groups.getById( groupId );

            if ( !newGroup ) {
                const res = await this.bot.send( "getChatMember", {
                    "chat_id": groupId,
                    "user_id": this.bot.id,
                } );
            ...
            // XXX
            newGroup = await this.#bot.groups.getByUsername( labels[ 0 ] );
        }
