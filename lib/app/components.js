...
export default class Components {
    ...
    #validateComponentConfig ( component, schema, config ) {
        if ( !this.#ajvCache[ component.id ] ) {
            ...
            if ( fs.existsSync( schemaPath ) ) {
                try {
                    ...
                    this.#ajvCache[ component.id ].config = configAjv;

                    if ( configAjv.getSchema( "env" ) ) {
                        const envAjv = new Ajv( {
                            "coerceTypes": true,
                        } ).addSchema( schema );

                        this.#ajvCache[ component.id ].env = envAjv;
                    }
                }
                catch ( e ) {
        ...
        try {
            if ( schema ) {
                if ( ajv?.conig.getSchema( schema ) && !ajv.config.validate( schema, config ) ) {
