...
export default class Components {
    ...
    async create ( app, publicConfig ) {
        ...
        for ( const spec of Object.values( this.#components ) ) {
            try {
                ...
                const res = this.#validateComponentConfig( spec, "public-config", publicConfig?.[spec.name] );
                if ( !res.ok ) return res;
                ...
                const config = utils.mergeObjects( [
                    ...
                    spec.appConfig, // component private config
                    publicConfig?.[spec.name], // component public config
                ] );
    ...
    *[Symbol.iterator] () {
    ...
    #validateComponentConfig ( component, schema, config ) {
        var ajv = this.#componentsValidatorCache[component.name];

        if ( !ajv ) {
            const schemaPath = component.location + "/config.schema.yaml";

            if ( fs.existsSync( schemaPath ) ) {
                this.#componentsValidatorCache[component.name] = ajv = new Ajv().addSchema( readConfig( schemaPath ) );
            }
        }
