        ...
        try {
            ...
            const sort = ( componentName, required ) => {
                ...
                // process component
                    ...
                    const isEnabled = appConfig.components?.[componentName] !== false && runtimeConfig?.[componentName] !== false;
        ...
        for ( const spec of Object.values( this.#components ) ) {
            try {
                ...
                const component = new Class( {
                    ...
                    "required": this.#requiredComponents.has( spec.name ),
        ...
        const usedComponents = new Set(),
            checkDependencies = component => {
                ...
                for ( let dependency of component.dependencies ) {
                    ...
                    if ( !dependency ) {
                        return result( [500, `Component "${dependency.name}" required by "${component.name}" is not enabled`] );
                    }

                    const res = checkDependencies( dependency );
                    if ( !res.ok ) return res;
                }

                for ( let dependency of component.optionalDependencies ) {
                    dependency = this.get( dependency );
