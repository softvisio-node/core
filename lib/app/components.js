...
export default class Components {
    ...
    async create ( app, runtimeConfig ) {
        ...
        for ( const spec of Object.values( this.#specs ) ) {
            ...
            const component = new Class( this, spec.name, spec.location, utils.mergeObjects( {}, spec.config.config, spec.appConfig, runtimeConfig?.[spec.name] ) );
    ...
    getApiSchema () {
        return this.#createApiSchema( "api" );
    }

    getRpcSchema () {
        return this.#createApiSchema( "rpc" );
    }
    ...
    #createApiSchema ( type ) {
        ...
        for ( const location of [...Object.values( this.#specs ).map( component => component.location ), this.#location] ) {
            const schemaLocation = path.join( location, type );

            if ( !fs.existsSync( schemaLocation ) ) continue;

            const res = schema.addSchema( schemaLocation );
