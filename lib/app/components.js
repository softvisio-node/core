    ...
    async configure () {
        ...
        for ( const component of Object.values( this.#components ).reverse() ) {
            ...
            if ( component.isEnabled ) {
                ...
                // check, that all component required dependencies are enabled
                for ( const dependency of component.dependencies ) {
                    if ( !this.get( dependency ).isEnabled == null ) {
                        return result( [500, `Component "${dependency}" required by "${component.name}" is disabled`] );
                    }
                }
            ...
            else if ( this.#requiredComponents.has( component.name ) ) {
                return result( [500, `Required component "${component.name}" is not enabled`] );
            }
        }

        console.log( `Application components:`, enabledComponents.sort().join( ", " ) || "-" );
