...
export default class Components {
    ...
    async configureInstances () {
        ...
        this.#componentsValidatorCache = null;
    ...
    *[Symbol.iterator] () {
    ...
    #validateComponentConfig ( component ) {
        ...
        // validate env
        validator = this.#componentsValidatorCache[component.name + "/env"];

        if ( !validator ) {
            const schemaPath = component.location + "/env.schema.yaml";

            if ( fs.existsSync( schemaPath ) ) {
                this.#componentsValidatorCache[component.name + "/env"] = validator = new Ajv().compile( readConfig( schemaPath ) );
            }
        }

        if ( validator && !validator( process.env ) ) {
            return result( [400, `Component "${component.name}" env is not valid:\n` + validator.errors] );
        }

        // validate config
        validator = this.#componentsValidatorCache[component.name + "/config"];

        if ( !validator ) {
            const schemaPath = component.location + "/config.schema.yaml";

            if ( fs.existsSync( schemaPath ) ) {
                this.#componentsValidatorCache[component.name + "/config"] = validator = new Ajv().compile( readConfig( schemaPath ) );
            }
        }

        if ( validator && !validator( component.config ) ) {
