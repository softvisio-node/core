...
import { BULLET } from "#lib/unicode";
...
export default class Components {
    ...
    load ( service ) {
        ...
        try {
            ...
            const checkDeps = component => {
                if ( !component.required ) return;

                if ( !component.config.dependencies ) return;

                for ( const name of component.config.dependencies ) {
                    components[name].required = true;

                    checkDeps( components[name] );
                }
            };
    ...
    async create ( publicConfig ) {
        ...
        for ( const spec of Object.values( this.#components ) ) {
            try {
                ...
                // custom public config validation
                const res = await component.validatePublicConfig( utils.mergeObjects(
                    {},
    ...
    async configure () {
        ...
        console.log( BULLET + ` Used components:`, Object.keys( this.#components ).sort().join( ", " ) || "-" );
    ...
    async configureInstances () {
        ...
        for ( const component of Object.values( this.#components ).reverse() ) {
            ...
            // custom validate component config
            res = await component.validateConfig();
            if ( !res.ok ) return res;
