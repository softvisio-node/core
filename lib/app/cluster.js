...
export default class Cluster {
    ...
    constructor ( clusterId, api ) {
        ...
        this.#hub.forwardSubscriptions( this.#api, {
            ...
            "listener": ( name, ...args ) => {
                ...
                else this.#hub.publish( name, ...JSON.parse( ...args ) );
            },
        } );
    }
    ...
    publish ( name, ...args ) {
        ...
        // other cluster
        if ( name.startsWith( "//" ) ) {
            this.#api.callVoid( "cluster/publish", `cluster:${name}`, JSON.stringify( args ) );
        }

        // own cluster
        else {
            this.#api.callVoid( "cluster/publish", `${this.#prefix}/${name}`, JSON.stringify( args ) );
        }
    }
}
