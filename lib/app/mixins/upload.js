...
class Upload {
    ...
    constructor ( api, auth, args, onHash, onFinish ) {
        this.id = uuidv4();
    ...
    async start ( onStart, idleTimeout ) {
        if ( onStart ) {
    ...
    class extends Super {
            ...
            async _upload ( auth, args, onStart, onFinish, onHash ) {
                ...
                if ( !args.id ) {
                    ...
                    const upload = new Upload( this, auth, args, onHash, onFinish );

                    const res = await upload.start( onStart, this.uploadIdleTimeout );
