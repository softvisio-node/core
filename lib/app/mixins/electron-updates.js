...
/** class: ElectronUpdates
         ...
         *   - admin
         */
    class extends mix( Read, Upload, Super ) {
            readMaxLimit = 100;
             ...
             *       properties:
             *         id: {type: number}
             *         where: {type: object, properties: {search: {type: array}}, additionalProperties: false}
             *         order_by: {type: array}
             *         limit: {type: number}
             *         offset: {type: number}
            ...
            _onUploadStart ( upload ) {
                if ( !upload.data.version.match( /^\d+[.]\d+[.]\d+$/ ) ) return [[400, "Version is invalid"]];
            ...
            async _onUploadHash ( upload ) {
                ...
                if ( !res.data ) {
                    ...
                    return [[400, "Release is already uploaded."]];
                }
            }
             ...
             *       type: object
             *       properties:
             *         platform: {'type:string': ~, enum: [win32, linux, darwin]}
             *         arch: {type: string, enum: [x64, x32]}
             *         version: {type: string}
             *       required:
             *         - platform
             *         - arch
