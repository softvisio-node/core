...
const { TOKEN_TYPE_SESSION, TOKEN_TYPE_EMAIL_CONFIRM, TOKEN_TYPE_PASSWORD_RESET } = require( "../../const" );
const q = {
...
/** class: Session
    ...
    class extends Super {
             ...
             * summary: Signin user.
             * description: >-
             *   If `credentials` parameter is specified - will try to sign in user using
             *   username and password, otherwise will try to sign in user, using currently
             *   used api token.
             *
             *   **Returns**:
             *
             ...
             *   -   `403`: Not authorized.
             *
             *   -   Any other status means bad request, internal or connection error. Refer to
             *   the response `reason` property for more information.
             * permissions:
             *   - '*'
             * params:
             *   - name: credentials
             ...
             *     summary: >-
             *       Array of permissions. Only user, who has this permissions can sign in. If
             *       `null` permissions will not check.
             *     schema:
             *       anyOf:
             *         - type: 'null'
             *         - type: array
            ...
            async API_signin ( auth, credentials, signinPermissions ) {
                ...
                if ( credentials ) {
                    ...
                    // not authenticated
                    if ( !auth.isAuthenticated ) return [[401, "Not authenticated"]];

                    // check allowed permissions
                    if ( signinPermissions && !auth.hasPermissions( signinPermissions ) ) return [[403, "You are not authorized to access this area."]];
            ...
            async API_signup ( auth, fields ) {
                ...
                var res = await this.#api.createUser( username, password, this.#api.newUserEnabled, this.#api.newUserPermissions, fields );

                if ( res.ok ) {
                    return [[200, "You were registered."]];
                }
                else {
                    return res;
                }
            ...
            async API_send_confirmation_email ( auth, userId ) {
                var token = await this.#api.createUserActionToken( userId, TOKEN_TYPE_EMAIL_CONFIRM );
            ...
            async API_send_password_reset_email ( auth, userId ) {
                var token = await this.#api.createUserActionToken( userId, TOKEN_TYPE_PASSWORD_RESET );
            ...
            _sendConfirmationEmail ( email, token ) {
                var url = `${this.#api.getAppSettings().app_url}#/confirm-email/${token}`,
                    text = `
...
`;
            ...
            async _sendPasswordResetEmail ( email, token ) {
                var url = `${this.#api.getAppSettings().app_url}#/reset-password/${token}`,
                    text = `
