...
export default class API extends mixins( LocalMixin, AuthCacheMixin, ConnectionsMixin ) {
    ...
    validatePassword ( value ) {
        const error = validate.isInvalidPassword( value );

        if ( error ) return result( [400, error] );

        return result( 200 );
    }

    validateUsername ( value ) {
        const error = validate.isInvalidUsername( value );
    ...
    validateTelegramUsername ( value ) {
        const error = validate.isInvalidTelegramUsername( value );
    ...
    validateEmail ( value ) {
        const error = validate.isInvalidEmail( value );
