...
const Api = mixin( ( Super ) =>
    class extends Super {
                ...
                for ( const file of files ) {
                    ...
                    this.#methods["/" + version + "/" + name + "/" + "test"] = {
                        ...
                        "permissions": {
                            ...
                            // TODO set to null if no permissions required
                            "admin": true,
                            "users": false,
            ...
            getWebsocketConfig () {
                return {
                    "open": this.onConnectionOpen.bind( this ),
                    "close": this.onConnectionClose.bind( this ),
                    "message": this.onConnectionMessage.bind( this ),
                };
            }

            async onConnectionOpen ( ws, req ) {
            ...
            onConnectionClose ( ws, status, reason ) {

                // reason = Buffer.from( reason );
            }

            async onConnectionMessage ( ws, msg, isBinary ) {
                ...
                if ( msg.type === "auth" ) {
                    ...
                    if ( auth.isAuthenticated ) {
                        ...
                        for ( const permission in auth.permissions ) {
                            ws.subscribe( "client/permissions/" + permission );
                        }
                    }
                    else {
                        ws.subscribe( "client/all" );
