...
import { ROOT_USER_NAME, ROOT_USER_ID } from "#lib/const";
...
export default class Api extends mixins( Local, Events ) {
    ...
    async #loadMethods ( index ) {
        ...
        for ( const methodId in schema ) {
            ...
            const objectId = url.pathToFileURL( path + "/" + schema[methodId].apiClass + ".js" ).href;
        ...
        if ( !res.isAborted ) {
            res.cork( () => {
                ...
                if ( isJSON ) {
                    ...
                    res.writeHeader( "Content-Type", "application/msgpack" ).end( MSGPACK.encode( apiRes ) );
                }
            } );
        }
    }
