...
const Api = mixin( ( Super ) =>
    class extends Super {
            ...
            constructor ( app, backend, options = {} ) {
                ...
                this.defaultGravatarUrl = `https://s.gravatar.com/avatar/${crypto.createHash( "MD5" ).update( this.defaultGravatarEmail ).digest( "hex" )}?d=${this.defaultGravatarImage}`;
            ...
            async loadMethods ( path, constructorOptions = {} ) {
                const doc = new Doc( path ),
                    ...
                    files = await fs.readTree( path ),
                    objects = {};

                for ( const file of files ) {
                    const Class = require( path + "/" + file );

                    objects["/" + file] = new Class( this.#app, this, constructorOptions );
                }

                for ( const methodId in schema ) {
                    schema[methodId].object = objects[schema[methodId].moduleName + ".js"];
                }
