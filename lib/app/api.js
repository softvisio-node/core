...
const Api = mixin( ( Super ) =>
    class extends Super {
            ...
            async _onConnectionOpen ( ws, req ) {
                ws.subscribe( "user/all" );
                ws.subscribe( "user/not-authenticated" );
            ...
            async _onConnectionMessage ( ws, msg, isBinary ) {
                ...
                if ( msg.type === "auth" ) {
                    ...
                    if ( auth.isAuthenticated ) {
                        ...
                        if ( auth.isRoot() ) ws.subscribe( "user/root" );
                        ws.subscribe( "user/" + auth.userId );

                        for ( const permission in auth.permissions ) {
                            ws.subscribe( "user/permission/" + permission );
                        }
                    }
                    else {
                        ws.subscribe( "user/all" );
                ...
                // event
                    ...
                    this.#app.emitDirect( "user/" + msg.name, ws.auth, ...msg.args );
                }
