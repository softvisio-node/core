...
export default class AppApi extends Events {
    ...
    async authenticate ( token ) {
        ...
        // parse token
        token = Token.new( this, token );

        // token is corrupted
        if ( !token.id ) return new Auth( this, token );

        var cachedToken;

        if ( token.isUserToken ) {
            cachedToken = await this.cache.getUserTokenById( token.id );
        }
        else if ( token.isUserSessionToken ) {
            cachedToken = await this.cache.getUserSessionTokenById( token.id );
        }
        else {
        ...
        // token is invalid
        if ( !cachedToken ) return new Auth( this );

        // token is invalid
        if ( !( await cachedToken.verify( token ) ) ) return new Auth( this );

        const user = await this.cache.getUserById( cachedToken.userId );

        return new Auth( this, cachedToken, user );
