...
const Api = mixin( Super =>
    class extends Super {
            ...
            validatePermissionName ( name, allowInternals ) {
                if ( allowInternals && ( name === "!" || name === "@" || name === "*" ) ) return result( 200 );

                if ( !this.app.permissions[name] ) return result( [400, "Permission name is unknown"] );

                if ( /[^a-z-]/.test( name ) ) return result( [400, "Permission name is invalid"] );
