...
const Api = mixin( Super =>
    class extends Super {
            ...
            async init ( options = {} ) {
                ...
                // init methods
                process.stdout.write( "Loading API methods ... " );
                if ( options.methods ) res = await this._loadMethods( options.methods );
                console.log( res + "" );
                if ( !res.ok ) return res;
            ...
            async _loadMethods ( path ) {
                ...
                for ( const methodId in schema ) {
                    ...
                    for ( const permission of schema[methodId].permissions ) {
                        const res = this.validateApiPermissionName( permission );

                        if ( !res.ok ) return result( [500, `Permission "${permission}" for API method "${schema[methodId].name}" in "${schema[methodId].module.namespacePath}" is not valid`] );
                    }
