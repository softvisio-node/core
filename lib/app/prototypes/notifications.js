...
export default class extends mixins( Read, Base ) {
    async API_getNotificationsSettings ( ctx ) {
        ...
        const data = {
            ...
            "telegramBotUsername": await this.app.notifications.getTelegramBotUsername(),
            "notifications": [],
        };
        ...
        for ( const type in profile.notifications ) {
            ...
            data.notifications.push( {
                "user_id": profile.userId,
                type,
                "name": this.app.settings.notifications[type].name,
                "description": this.app.settings.notifications[type].description,
    ...
    async API_read ( ctx, options = {} ) {
        ...
        const totalQuery = sql`SELECT count(*)::int4 AS total, count( nullif( user_internal_notification.read, TRUE ) )::int4 AS total_undone_unread FROM internal_notification, user_internal_notification`.WHERE( where );
