...
export default class extends mixins( Read, Base ) {
    async API_read ( ctx, options = {} ) {
        var where = this.dbh.where( sql`user_internal_notification.user_id = ${ctx.userId} AND notification.id = user_internal_notification.notification_id AND ( notification.expires IS NULL OR notification.expires > CURRENT_TIMESTAMP )` );
        ...
        const totalQuery = sql`SELECT count(*)::int4 AS total, count( nullif( user_internal_notification.read, TRUE ) )::int4 AS total_undone_unread FROM notification, user_internal_notification`.WHERE( where );

        const mainQuery = sql`SELECT notification.*, user_internal_notification.done, user_internal_notification.read FROM notification, user_internal_notification`.WHERE( where );
