...
const QUERIES = {
    ...
    "getStats": sql`
SELECT
    count( nullif( done, TRUE ) ) AS totalUndone,
    count( coalesce( nullif( done, FALSE ), nullif( read, TRUE ) ) ) AS totalUndoneUnread
FROM
    user_notification
...
export default class extends mixins( Read, Base ) {
    async API_read ( ctx, options = {} ) {
        ...
        const totalQuery = sql`SELECT count(*) AS total, count( nullif( user_notification.read, TRUE ) ) AS totalUndoneUnread FROM notification, user_notification`.WHERE( where );
