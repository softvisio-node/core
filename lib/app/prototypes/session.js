...
export default class extends Base {
    ...
    async API_signin ( ctx, { email, password, oauth } ) {
        var auth;

        if ( oauth ) {
            auth = await this.api.authenticateUserOauth( oauth );
        }
        else {
    ...
    async API_authorize ( ctx, { password, oauth } = {} ) {
    ...
    async API_signup ( ctx, email, fields ) {
        ...
        if ( res.ok ) {
            if ( this.app.config.newUserEnabled ) {
                return this.API_signin( ctx, { "email": res.data.email, "password": res.data.password } );
            }
            else {
    ...
    async #getAppSettings ( ctx ) {
        if ( !this.#settings ) {
            this.#settings = {
                ...
                "passwordsStrength": this.app.config.passwordsStrength,

                "signup_enabled": this.app.config.signupEnabled,
                "push_notifications_supported": this.app.notifications.pushEnabled,

                "oauth_google_client_id": this.app.env.api?.oauth?.google?.clientId,
                "oauth_facebook_client_id": this.app.env.api?.oauth?.facebook?.clientId,
                "oauth_github_client_id": this.app.env.api?.oauth?.github?.clientId,
            };
        }
