...
export default class extends Base {
    ...
    async API_signin ( ctx, credentials ) {
        ...
        await this._sendNewSigninNotification( ctx, session.data );
    ...
    async API_signout ( ctx, sessionId ) {
        if ( ctx.type !== constants.tokenTypeUserSession ) return result( 404 );

        return await this.api.deleteUserSession( sessionId || ctx.id, { "userId": ctx.userId } );
    }
    ...
    async API_signup ( ctx, fields ) {
        ...
        const { username, password, "device_id": deviceId } = fields;
        ...
        if ( res.ok ) {
            if ( this.app.config.newUserEnabled ) {
                return this.API_signin( ctx, { username, password, "device_id": deviceId } );
            }
            else {
    ...
    async API_sendPasswordRecoveryEmail ( ctx, userId ) {
...
`;
    ...
    async _sendNewSigninNotification ( ctx, { userAgent, remoteAddress } ) {
...
`
    ...
    async #getAppSettings ( ctx ) {
        ...
        settings.backend_git_id = await env.getGitId();
        ...
        settings.signin_google_enabled = false;
        settings.signin_facebook_enabled = false;
        settings.signin_github_enabled = false;
