...
export default class extends Base {
    ...
    async API_signup ( ctx, fields ) {
        ...
        const res = await this.api.createUser( email, password, this.app.config.newUserEnabled, null, fields );

        if ( res.ok ) {
            if ( this.app.config.newUserEnabled ) {
                return this.API_signin( ctx, { email, password } );
            }
            else {
    ...
    async API_sendConfirmationEmail ( ctx, email ) {
        var token = await this.api.createUserActionToken( email, constants.tokenTypeEmailConfirmation );
    ...
    async API_sendPasswordRecoveryEmail ( ctx, email ) {
        var token = await this.api.createUserActionToken( email, constants.tokenTypePasswordReset );
