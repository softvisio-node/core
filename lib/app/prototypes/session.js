...
export default class extends Base {
    ...
    async API_signin ( ctx, { email, password, firebaseUserAccessToken } ) {
        var auth;

        if ( firebaseUserAccessToken ) {
            auth = await this.api.authenticateUserFirebaseToken( email, firebaseUserAccessToken );
        }
        else {
