...
export default class extends Base {
    ...
    async API_signin ( ctx, { username, password, "device_guid": deviceGuid } ) {
        const auth = await this.api.authenticateUserCredentials( username, password );
    ...
    async API_signup ( ctx, fields ) {
        ...
        const { username, password, "device_guid": deviceGuid } = fields;

        delete fields.username;
        delete fields.password;
        delete fields.device_guid;
        ...
        if ( res.ok ) {
            if ( this.app.config.newUserEnabled ) {
                return this.API_signin( ctx, { username, password, "device_guid": deviceGuid } );
            }
            else {
    ...
    async API_createTrustedSession ( ctx, password ) {
        const auth = await this.api.authenticateUserCredentials( ctx.username, password );

        // not authenticated
        if ( !auth ) return result( 401 );

        return this.api.createTrustedSession( ctx.id, ctx.data.remoteAddress + "", ctx.data.userAgent );
    }
