...
export default class extends Base {
    ...
    async API_sendConfirmationEmail ( ctx ) {
        const token = await this.api.createUserActionToken( ctx.email, constants.tokenTypeEmailConfirmation );
    ...
    async API_confirmEmailByToken ( ctx, token ) {
        return this.api.activateUserActionToken( ctx.userId, token, constants.tokenTypeEmailConfirmation );
    }
    ...
    async API_sendPasswordRecoveryEmail ( ctx, email ) {
        ...
        return this._sendPasswordRecoveryEmail( email, token.data.token );
    }

    async API_setPasswordByToken ( ctx, token, password ) {
        return this.dbh.begin( async dbh => {
            const res = await this.api.activateUserActionToken( ctx.userId, token, constants.tokenTypePasswordReset, { dbh } );
            if ( !res.ok ) throw res;
