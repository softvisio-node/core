...
export default class extends Base {
    ...
    async API_signin ( ctx, { username, password, "device_id": deviceId } ) {
        const auth = await this.api.authenticate( username, password );

        // not authenticated
        if ( !auth.ok ) return result( 401 );

        // create user session
        const session = await this.api.createUserSession( auth.data.userId, {
            deviceId,
