...
export default class extends mixins( Read, Base ) {
    ...
    async API_read ( ctx, options = {} ) {
        ...
        // get all matched rows
            ...
            if ( options.where?.search ) {
                where.and( { "user.name": options.where.search }, "OR", { "user.email": options.where.search }, "OR", { "user.telegram_name": options.where.search } );
    ...
    async API_suggest ( ctx, options ) {
        ...
        if ( options?.where?.name ) {
            where = this.dbh.where( { "name": ["LIKE", options.where.name[1]] }, "OR", { "email": ["LIKE", options.where.name[1]] }, "OR", { "telegram_name": ["LIKE", options.where.name[1]] } );
        }
