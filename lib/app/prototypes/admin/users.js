...
export default class extends mixins( Read, Base ) {
    ...
    async API_read ( ctx, args = {} ) {
        ...
        // get all matched rows
            ...
            if ( args.where && args.where.search ) {
                where.and( { "user.name": args.where.search }, "OR", { "user.email": args.where.search }, "OR", { "user.telegramName": args.where.search } );
    ...
    async API_suggest ( ctx, args ) {
        ...
        if ( args && args.where && args.where.name ) {
            where = this.dbh.where( { "name": ["LIKE", args.where.name[1]] }, "OR", { "email": ["LIKE", args.where.name[1]] }, "OR", { "telegramName": ["LIKE", args.where.name[1]] } );
        }
