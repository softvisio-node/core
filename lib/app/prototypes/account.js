...
`;
    ...
    async API_setEmailByToken ( ctx, token ) {
        const res = await this.dbh.begin( async dbh => {
            var res = await this.api.activateUserActionToken( ctx.userId, token, constants.tokenTypeEmailChange, { dbh } );
            if ( !res.ok ) throw res;

            res = await this.api.setUserEmail( ctx.userId, res.data.email );
            if ( !res.ok ) throw res;
