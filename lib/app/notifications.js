...
export default class Notifications {
    ...
    constructor ( app ) {
        ...
        this.#notifications = this.#app.const?.notifications || {};
        ...
        this.dbh.on( "event/api/user-email/update", data => {
            const profile = this.#userIdCache.get( data.user_id );

            if ( profile ) profile.email = data.email;
        } );

        this.dbh.on( "event/api/user-telegram-username/update", data => this.#invalidateCache( data.user_id ) );
        this.dbh.on( "event/api/user-enabled/update", data => this.#invalidateCache( data.user_id ) );
        this.dbh.on( "event/api/user/delete", data => this.#invalidateCache( data.user_id ) );
        this.dbh.on( "event/api/user-notification-type/update", data => this.#invalidateCache( data.user_id ) );
    }
    ...
    async #onTelegramMessage ( data ) {
        if ( data.message ) {
            ...
            // not authorized
            if ( !profile ) {
                await this.#telegram.sendMessage( chatId, `You are not authorized to use this bot. Please set your telegram username at the project notification settings panel and type "/start" again.` );
            }
            else {
