...
export default class Notifications {
    ...
    // XXX set cache invalidation listeners, this.#invalidateCache( userId )
    // XXX init push
    constructor ( app ) {
        this.#app = app;
        ...
        this.dbh.on( "event/api/user-telegram-chat/update", data => this.#invalidateCache( data.user_id ) );
    }
    ...
    async #getUserNotificationsProfileByTelegramUsername ( telegramUsername ) {
        var profile = this.#telegramUsernameCache.get( telegramUsername );
    ...
    #buildProfile ( data ) {
        ...
        for ( const type in this.#notifications ) {
            ...
            profile.notifications[type] = {
                "intenal": defaultChannels.internal == null ? null : channels.internal ?? defaultChannels.internal ?? false,
    ...
    // XXX remove chat if send failure
