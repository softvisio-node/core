...
WHERE
    ...
    -- AND "notification"."max_age"
ORDER BY
    "date" DESC
`,
    "insert": sql`INSERT INTO "notification" ("subject", "body", "max_age") VALUES (?, ?, ?) RETURNING "id"`.prepare(),
};
...
export default class Notifications {
    ...
    // XXX cluster /api event
    async sendUI ( users, subject, body, options = {} ) {
        const res = await this.dbh.begin( async dbh => {
            var res;

            res = await dbh.selectRow( QUERIES.insert, [subject, body, options.maxAge || DEFAULT_MAX_AGE] );
