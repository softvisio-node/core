...
SELECT
    ...
    "notification"."body",
    "notificationUser"."read" AS "read"
FROM
    "notification",
    "notificationUser"
WHERE
    "notification"."id" = "notificationUser"."notificationId"
    AND "notificationUser"."userId" = ?
    AND "notificationUser"."deleted" = FALSE
...
export default class Notifications {
    ...
    async sendUi ( users, subject, body, options = {} ) {
        const res = await this.dbh.begin( async dbh => {
            ...
            res = await dbh.do( sql`INSERT INTO "notificationUser"`.VALUES( users.map( userId => {
                return {
                    notificationId,
                    userId,
                };
    ...
    async markRead ( userId, notifications ) {
        const res = await this.dbh.do( sql`UPDATE "notificationUser" SET "read" = TRUE WHERE "userId" = ${userId} AND "notification_id"`.IN( notifications ) );

        return res;
    }

    async markUnread ( userId, notifications ) {
        const res = await this.dbh.do( sql`UPDATE "notificationUser" SET "read" = FALSE WHERE "userId" = ${userId} AND "notificationId"`.IN( notifications ) );
    ...
    async delete ( userId, notifications ) {
        ...
        const res = await this.dbh.do( sql`UPDATE "notificationUser" SET "deleted" = TRUE WHERE "userId" = ${userId} AND "notificationId"`.IN( notifications ) );
