...
export default class Token {
    ...
    async getHash () {
        if ( !this.#hash ) {
            ...
            this.#hash = await hash( TOKEN_HASH, this.#random );
        }
    ...
    async verify ( token ) {
        ...
        if ( this.#verifiedToken ) return this.#verifiedToken === token.token;

        if ( this.#hash.equals( await token.getHash() ) ) {
            this.#verifiedToken = token.token;
