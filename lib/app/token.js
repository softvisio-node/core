...
const { AUTH_USER, AUTH_TOKEN, AUTH_SESSION } = require( "../const" );
...
class Token {
    ...
    static generateToken ( id ) {
        const type = Number( BigInt( id ) >> 55n );
        ...
        if ( type === AUTH_TOKEN ) length = 16;
        else if ( type === AUTH_SESSION ) length = 24;
        else length = 16;

        buf = Buffer.concat( [buf, crypto.randomBytes( length )] );

        return new this( type, id, this.getHash( buf ), toBase58( buf ) );
    }
