...
export default class ApiSchema {
    ...
    // public
    loadSchema () {
        const indexValidator = Ajv.new().addSchema( readConfig( "#resources/schemas/api.schema.yaml", { "resolve": import.meta.url } ) );
        ...
        try {
            ...
            for ( const file of modules ) {
                ...
                if ( /^v\d+\.yaml$/.test( file ) ) {
                    const index = readConfig( this.#url + "/" + file );

                    if ( !indexValidator.validate( "index", index ) ) {
                        console.log( `API index "${file}" is not valid, inspect errors below:` );

                        console.log( indexValidator.errors );

                        process.exit( 2 );
                    }

                    if ( index.emits ) index.emits.forEach( name => this.#addEvent( name ) );
                }
                ...
                // module
                    ...
                    if ( module.namespace && module.emits ) module.emits.forEach( name => this.#addEvent( name ) );
    ...
    findMethod ( id ) {
        ...
        while ( 1 ) {
            ...
            params.unshift( id.substring( idx + 1 ) );
    ...
    #addEvent ( name ) {
        if ( name === "*" ) {
            ...
            if ( !isKebabCase( name, { "sep": "/" } ) ) {
                throw result( [500, `Event name "${name}" must be in the kebab-case`] );
            }
