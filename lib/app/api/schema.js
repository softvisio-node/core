...
export default class APISchema {
    ...
    async #getParamDescription ( method, param ) {
        ...
        if ( param.schema.apiRead ) {
            desc += await Ajv.keywords.apiReadKeyword.getDescription( param.schema.apiRead, method.meta );
        }

        // upload file
        else if ( method.meta?.type === "upload" && param.schema?.instanceof === "File" ) {
            desc = "<File>" + ( param.description ? " " + param.description.trim() : "" ) + ` Maximim file size: \`${method.meta.uploadMaxSize}\` bytes.`;
