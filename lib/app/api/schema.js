        ...
        for ( const module of Object.values( this.#modules ) ) {
            ...
            for ( const method of Object.values( module.methods ) ) {
                ...
                for ( const param of method.spec.params ) {
                    ...
                    param.markdownSchema = " ".repeat( 4 ) + param.templateSchema.replace( /^/gm, " ".repeat( 4 ) ).trim();
                }
            }

            fileTree.add( { "path": module.namespace.substr( 1 ) + ".md", "data": await ejs.renderFile( moduleTmpl, { module } ) } );
        }

        // generate main index
        if ( !fileTree.isEmpty ) {
            fileTree.add( { "path": "README.md", "data": await ejs.renderFile( indexTmpl, { "modules": index } ) } );
        }
