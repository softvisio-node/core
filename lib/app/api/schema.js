...
export default class APISchema {
    ...
    // private
    #getParamDescription ( methodId, param ) {
        var desc = "";

        // XXX API read method
        if ( param.schema.apiRead ) return desc;
        ...
        if ( !desc.match( /^<([\w.]+)(\[\])?\\>/ ) ) {
            ...
            if ( !types ) throw `Method "${methodId}" parameter "${param.name}" has no type defined`;

            if ( !Array.isArray( types ) ) types = [types];

            console.log( methodId, types );
