...
export default class APISchema {
    ...
    validateMethodParams ( methodSpec, args ) {
        if ( methodSpec.noParamsValidation || methodSpec.validate( args ) ) return OK;

        // log validation errors
        if ( env.isDevelopment ) console.log( `Params validation errors for method "${methodSpec.methodId}":`, methodSpec.validate.errors );
