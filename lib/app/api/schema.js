...
export default class APISchema {
    ...
    async generate ( options = {} ) {
        ...
        const httpURL = new URL( options.url.endsWith( "/" ) ? options.url : options.url + "/" );
        ...
        for ( const version in versions ) {
            fileTree.add( {
                ...
                "data": await ejs.renderFile( apiSchemaTmpl, {
                    ...
                    httpURL,
                    wsURL,
                    "resolveParamType": this.#resolveParamType.bind( this ),
                } ),
            } );
        }
    ...
    #resolveParamType ( param ) {
        ...
        return type.map( type => "<" + ( MARKDOWN_TYPE[type] || type ) + "\\>" ).join( " | " );
    }
}
