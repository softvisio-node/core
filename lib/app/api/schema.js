...
export default class APISchema {
    ...
    async generate () {
        ...
        for ( const module of Object.values( this.#modules ) ) {
            ...
            fileTree.add( { "path": "v" + module.version + "/" + module.name.replaceAll( "/", "-" ) + ".md", "data": await ejs.renderFile( moduleTmpl, { module } ) } );
        }

        // generate main index
        if ( !fileTree.isEmpty ) {
            for ( const version in index ) {
                fileTree.add( { "path": "v" + version + "/README.md", "data": await ejs.renderFile( readmeTmpl, { version, "modules": index[version] } ) } );

                fileTree.add( { "path": "v" + version + "/_sidebar.md", "data": await ejs.renderFile( sidebarTmpl, { version, "modules": index[version] } ) } );
            }
        }
