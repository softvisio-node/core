    ...
    async #call ( methodId, args, isVoidCall ) {
        ...
        if ( method.permission && !this.isRoot && !api.acl.staticPermissions.has( method.permission ) ) {
            ...
            if ( !aclResolvers ) {
                ...
                // no acl ids were provided in params
                if ( method.aclResolvers.size ) return result( -32811 );

                if ( !( await api.acl.checkAclPermissions( this.#user.id, method ) ) ) {
                    return result( -32811 );
                }
            }
            ...
            // dynamic acl
                ...
                for ( const [aclObjectId, aclResolver] of Object.entries( aclResolvers ) ) {
                    if ( !( await api.acl.checkAclPermissions( this.#user.id, method, { aclObjectId, aclResolver } ) ) ) {
                        return result( -32811 );
                    }
                }
            }
