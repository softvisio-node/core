...
export default class Context {
    ...
    async #call ( methodId, args, isVoidCall ) {
        ...
        var res, method, callStartDate, statsIncremented, stats, statsId;
        ...
        try {
            ...
            if ( api.isApi && !isPrivateCall ) {
                ...
                statsId = ( this.#token.id ? this.#token.type + "/" + this.#token.id : this.#remoteAddress ) + "/" + method.id;

                // too many calls
                if ( stats[statsId] >= method.maxActiveCallsPerSession ) {
                    throw result( -32802 );
                }
        ...
        if ( statsIncremented ) {
            stats[statsId]--;
            if ( !stats[statsId] ) delete stats[statsId];
