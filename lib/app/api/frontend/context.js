    ...
    async #call ( methodId, args, isVoidCall ) {
        ...
        try {
            ...
            // telegram bot user
                ...
                const telegramBotUser = await this.#telegramBotUser.bot.users.getById( this.#telegramBotUser.id );

                if ( !telegramBotUser ) throw result( -32814 );

                this.#telegramBotUser = telegramBotUser;

                // telegram bot user is banned
                if ( this.#telegramBotUser.isBanned ) throw result( -32813 );

                const user = await this.#telegramBotUser.getApiUser();

                if ( user === false ) throw result( -32814 );
            ...
            if ( api.isApi ) {
                ...
                if ( method.permission ) {
                    if ( method.permission === "telegramBotUsers" ) {
...
// |==========+==========+==========================================================================|
// | ERROR    |      1:8 | no-unused-vars, 'Token' is defined but never used.                       |
// +----------+----------+--------------------------------------------------------------------------+
//
