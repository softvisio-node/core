...
export default class Context {
    ...
    updateLastActivity () {
        ...
        this.#api.stats.updateTokenLastActivity( this.#token );
    }
    ...
    async update () {
        ...
        if ( token !== this.#token ) {
            ...
            if ( !token ) {
                ...
                // close token related connections
                this.#api.frontend.closeTokenConnections( this.#token.type, this.#token.id, result( 4401 ) );
        ...
        if ( this.#token.userId ) {
            ...
            if ( user !== this.#user ) {
                ...
                if ( !user ) {
                    ...
                    // close user related connections
                    this.#api.frontend.closeUserConnections( this.#user.id, result( 4401 ) );
    ...
    async #call ( methodId, args, isVoid ) {
        ...
        if ( this.#token.id ) {
            ...
            // api backend is down
            if ( !api.isConnected ) return result( 503 );

            // user was deleted
            if ( !( await this.update() ) ) return result( -32811 );
        }

        // context is disabled
        if ( !this.isEnabled ) return result( -32811 );
        ...
        if ( method.private && this.#isPrivate == null ) return result( -32805 );
