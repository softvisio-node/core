...
export default class Context {
    ...
    async #call ( methodId, args, isVoidCall ) {
        ...
        try {
            ...
            // check call limits
            if ( callDescriptor.log ) {
                const res = await frontend.checkApiCallLimits( callDescriptor, this, method, isPrivateCall );

                // too many requests
                if ( !res.ok ) throw result( -32802 );
            }
            ...
            callDescriptor.startDate = new Date();
        ...
        callDescriptor.endDate = new Date();

        // finish call
        await frontend.endApiCall( callDescriptor, res );
