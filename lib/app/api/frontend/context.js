...
export default class Context {
    ...
    constructor ( api, { token, user, isDeleted, connection, signal, hostname, userAgent, remoteAddress, method, isVoidCall, isPrivateCall } = {} ) {
        this.#api = api;
    ...
    async #call ( methodId, args, isVoidCall ) {
        ...
        try {
            ...
            if ( method.permission && !this.isRoot && !api.acl.staticPermissions.has( method.permission ) ) {
                ...
                const permissions = await this.app.acl.checkAclPermission( this.#user.id, aclResolvers );

                // XXX
                if ( !permissions.hash( method.permission ) ) throw result( -32811 );
            }
