...
export default class Context {
    ...
    async #call ( methodId, args, isVoidCall ) {
        ...
        var res, method, callStartDate, statsIncremented, stats, statsUserId;
        ...
        try {
            ...
            method = frontend.schema.methods[methodId];

            // method not found
            if ( !method ) throw result( -32809 );

            // check call limits
            if ( api.isApi && !this.#isPrivateCall ) {
                stats = frontend.stats;
                statsUserId = this.#user.id || 0;
                ...
                // too many calls
                if ( stats.userActiveCalls[statsUserId] >= totalMaxActiveCalls ) throw result( -32802 );

                stats.methodActiveCalls[method.id] ||= {};

                // too many calls
                if ( stats.methodActiveCalls[method.id][statsUserId] >= methodMaxActiveCalls ) throw result( -32802 );
                ...
                stats.userActiveCalls[statsUserId] ??= 0;
                stats.userActiveCalls[statsUserId]++;

                stats.methodActiveCalls[method.id][statsUserId] ??= 0;
                stats.methodActiveCalls[method.id][statsUserId]++;
            }
        ...
        if ( statsIncremented ) {
            ...
            if ( statsUserId ) {
                if ( !stats.userActiveCalls[statsUserId] ) delete stats.userActiveCalls[statsUserId];
                if ( !stats.methodActiveCalls[method.id][statsUserId] ) delete stats.methodActiveCalls[method.id][statsUserId];
            }

            // log api call, if call was stated
            if ( callStartDate ) {
                await frontend.logApiCall( method.id, callStartDate, new Date(), res );
            }
        }
