...
export default class WebSocketConnection extends ServerWebSocketConnection {
    ...
    constructor ( { server, ws, options, incomingEvents, outgoingEvents, publishRemoteIncomingEvent, sessionStateEvents } ) {
        ...
        if ( this.api.isApi && this.ctx.user.id ) {
            ...
            // user disabled
            this.#sessionStateEvents.on( `user/disable/${this.ctx.user.id}`, () => {
                console.log( "---- user disabled" );
            } );

            // api token disabled
            this.#sessionStateEvents.on( `api-token/disable/${this.ctx.token.id}`, () => {
                console.log( "---- user disabled" );
            } );

            // api token delete
            this.#sessionStateEvents.on( `api-token/delete/${this.ctx.token.id}`, () => {
                console.log( "---- user disabled" );
            } );

            // session token delete
            this.#sessionStateEvents.on( `session-token/delete/${this.ctx.token.id}`, () => {
                console.log( "---- user disabled" );
            } );
        }
    }
