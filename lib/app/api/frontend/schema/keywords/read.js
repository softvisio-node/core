...
function generateMacro ( type, operators, _const, _enum, aclObjectType ) {
    ...
    if ( equalOperator ) {
        macros.push( generateType( type, _const, _enum, aclObjectType ) );
    }

    if ( scalarOperators.length ) {
        macros.push( { "type": "array", "items": [scalarOperators.length === 1 ? { "const": scalarOperators[0] } : { "enum": scalarOperators }, generateType( type, _const, _enum, aclObjectType )] } );
    }

    if ( arrayOperators.length ) {
        macros.push( { "type": "array", "items": [arrayOperators.length === 1 ? { "const": arrayOperators[0] } : { "enum": arrayOperators }, { "type": "array", "items": generateType( type, _const, _enum, aclObjectType ), "minItems": 1 }] } );
    }
...
function generateType ( type, _const, _enum, aclObjectType ) {
    const macro = {};
    ...
    if ( aclObjectType ) macro["aclObjectType"] = aclObjectType;
    ...
    "macro": ( schema, parentSchema, it ) => {
        ...
        for ( const fieldName in schema.fields ) {
            ...
            if ( fieldName === "id" ) {
                ...
                if ( field["aclObjectType"] ) macro.properties.id["aclObjectType"] = field["aclObjectType"];
            }
            ...
            if ( field.operator ) {
                ...
                macro.properties.where.properties[fieldName] = generateMacro( field.type, field.operator, field.const, field.enum, field["aclObjectType"] );
            }
        }
