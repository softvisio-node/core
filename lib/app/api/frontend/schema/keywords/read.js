...
function generateMacro ( type, operators, _const, _enum, aclResolver ) {
    ...
    if ( scalarOperators.length ) {
        macros.push( {
            "type": "array",
            "items": [scalarOperators.length === 1 ? { "const": scalarOperators[0] } : { "enum": scalarOperators }, generateType( type, _const, _enum, aclResolver )],
        } );
    }
    ...
    if ( arrayOperators.length ) {
        macros.push( {
            "type": "array",
            "items": [arrayOperators.length === 1 ? { "const": arrayOperators[0] } : { "enum": arrayOperators }, { "type": "array", "items": generateType( type, _const, _enum, aclResolver ), "minItems": 1 }],
        } );
    }
                    ...
                    { "type": "string" },
                    {
                        ...
                        "items": {
                            "anyOf": [
                                ...
                                { "type": "array", "items": [{ "type": "string", "format": "snake-case" }, { "enum": ["asc", "desc"] }] },
                            ],
                        },
                        "minItems": 1,
                    },
    ...
    "macro": ( schema, parentSchema, it ) => {
        ...
        for ( const fieldName in schema.fields ) {
            ...
            if ( fieldName === "id" ) {
                ...
                if ( field.format ) macro.properties.id.format = field.format;
