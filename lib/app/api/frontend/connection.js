...
export default class Connection extends ServerConnection {
    ...
    async onMessage ( msg, isBinary ) {
        ...
        if ( msg.method ) {
            ...
            // check health
            else if ( msg.method === "/check-health" ) {
                if ( msg.id ) {
                    const res = await this.app.healthcheck.checkHealth();
            ...
            // rpc
                ...
                // regular call
                if ( msg.id ) {
                    this.#abortControllers[msg.id] = new AbortController();

                    const signal = this.#abortControllers[msg.id].signal;
