    ...
    async logApiCall ( methodId, start, end, res ) {
        ...
        if ( !row ) {
            this.#apiCalls.set( key, {
                ...
                duration,
                "calls": 1,
                "exceptions": res.exception ? 1 : 0,
            } );
        }
        else {
            row.duration += duration;
            row.calls++;
            if ( res.exception ) row.exceptions++;
        }
    }
...
INSERT INTO api_health_call
`.VALUES( values, { "index": "firstRow" } ).sql`
ON CONFLICT ( method_id, date ) DO UPDATE SET
    duration = api_health_call.duration + EXCLUDED.duration,
    calls = api_health_call.calls + EXCLUDED.calls,
    exceptions = api_health_call.exceptions + EXCLUDED.exceptions
` );
...
UPDATE user_token SET
    last_activity = CASE
        WHEN t.last_activity::timestamptz > user_token.last_activity THEN t.last_activity::timestamptz
        ELSE user_token.last_activity END
...
UPDATE user_session SET
    last_activity = CASE
        WHEN t.last_activity::timestamptz > user_session.last_activity THEN t.last_activity::timestamptz
        ELSE user_session.last_activity END,
    expires = CASE
        WHEN t.expires::timestamptz > user_session.expires THEN t.expires::timestamptz
        ELSE user_session.expires END
FROM`.VALUES_AS( "t", values, { "index": "firstRow" } ).sql`
