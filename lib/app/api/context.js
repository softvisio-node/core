export default class Context {
    ...
    async #call ( methodId, args, isVoid ) {
        var [res, method, stat, aclObjects] = this.#startRequest( methodId, args );
        ...
        // check acl roles
        if ( aclObjects ) {
            for ( const [objectId, objectIdType] of Object.entries( aclObjects ) ) {
                if ( !( await this.api.checkAclPermissions( objectId, this.userId, method, objectIdType ) ) ) {
    ...
    #startRequest ( methodId, args ) {
        ...
        const aclObjects = global[Symbol.for( "checkAcl" )];
        global[Symbol.for( "checkAcl" )] = null;
        ...
        return [null, method, { stat, started, userId }, aclObjects];
    }
