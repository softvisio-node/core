export default class Context {
    ...
    async #call ( methodId, args, isVoid ) {
        var [res, method, stat, checkObjectPermisisonsIds] = this.#startRequest( methodId, args );

        // error
        if ( res ) return res;

        // check objects permissions
        if ( checkObjectPermisisonsIds ) {
            for ( const [objectId, objectResolverType] of Object.entries( checkObjectPermisisonsIds ) ) {
                if ( !( await this.api.checkObjectPermissions( objectId, objectResolverType, this.userId, methodId ) ) ) {
                    if ( stat ) this.#finishRequest( method, res, stat );

                    return this.objectPermissionsError();
                }
            }
        }
    ...
    #startRequest ( methodId, args ) {
        ...
        global[Symbol.for( "checkObjectPermisisonsIds" )] = null;
        const res = method.validateParams( args );
        if ( !res.ok ) return [res];

        const checkObjectPermisisonsIds = global[Symbol.for( "checkObjectPermisisonsIds" )];
        ...
        return [null, method, { stat, started, userId }, checkObjectPermisisonsIds];
    }
