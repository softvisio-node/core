export default class Context {
    ...
    async #call ( methodId, args, isVoid ) {
        var [res, method, stat, checkAcl] = this.#startRequest( methodId, args );
        ...
        // check objects roles
        if ( checkAcl ) {
            for ( const [objectId, objectResolverType] of Object.entries( checkAcl ) ) {
                if ( !( await this.api.checkAcl( objectId, objectResolverType, this.userId, method ) ) ) {
    ...
    #startRequest ( methodId, args ) {
        ...
        const checkAcl = global[Symbol.for( "checkAcl" )];
        ...
        return [null, method, { stat, started, userId }, checkAcl];
    }
