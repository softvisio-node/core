...
export default class Connection extends _Connection {
    ...
    constructor ( api, ws, _in, out, publishRemoteIncomingEvent, call ) {
        super( ws );
    ...
    #eventsListener ( name, args, cache, publisherId ) {
        ...
        if ( this.isBinary ) {
            this.send( ( cache.binary ||= msgpack.encode( cache.msg ) ), true );
        }
        else {
            this.send( ( cache.text ||= JSON.stringify( cache.msg ) ), false );
        }
    }
}
