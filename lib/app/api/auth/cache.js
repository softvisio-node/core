...
export default class AuthCache {
    ...
    invalidateUser ( userId ) {
        ...
        if ( tokens ) {
            for ( const tokenCacheId of tokens ) {
                this.#deleteAuthentication( tokenCacheId );
            }
        }
    ...
    #onAuthenticationDelete ( tokenCacheId, auth ) {
        this.#deleteUserAuthentication( auth );
    }

    #deleteAuthentication ( tokenCacheId ) {
    ...
    #deleteUserAuthentication ( auth ) {
        const userId = auth.userId,
            userTokens = this.#userAuthentications[userId];

        if ( !userTokens ) return;

        userTokens.delete( auth.token.cacheId );
