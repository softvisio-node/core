...
const LAST_ACTIVITY_DROP_INTERVAL = 1000 * 60; // 1 minute
...
export default class AuthCache {
    ...
    constructor ( api, size ) {
        ...
        setInterval( this.#dropLastUsedStats.bind( this ), LAST_ACTIVITY_DROP_INTERVAL );
    }
    ...
    updateAuthLastActivity ( auth ) {
        ...
        if ( auth.token.isUserToken ) {
            this.#userTokenLastActivity.set( auth.token.id, `UPDATE user_token SET last_activity = '${new Date().toISOString()}' WHERE id = ${auth.token.id};` );
        }
        else if ( auth.token.isUserSessionToken ) {
            this.#userSessionLastActivity.set( auth.token.id, `UPDATE user_session SET last_activity = '${new Date().toISOString()}' WHERE id = ${auth.token.id};` );
        }
    }
