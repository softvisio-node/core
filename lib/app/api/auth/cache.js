...
export default class AuthCache {
    ...
    updateAuthLastActivity ( auth ) {
        ...
        if ( auth.token.isUserToken ) {
            this.#userTokenLastActivity.set( auth.token.id, `UPDATE user_api_key SET last_activity = '${lastActivity.toISOString()}' WHERE id = ${auth.token.id};` );
        }
        else if ( auth.token.isUserSessionToken ) {
            const expires = new Date( Date.now() + this.#api.app.config.sessionMaxAge );

            this.#userSessionLastActivity.set( auth.token.id, `UPDATE user_session_key SET last_activity = '${lastActivity.toISOString()}', expires = '${expires.toISOString()}' WHERE id = ${auth.token.id};` );
        }
    }
