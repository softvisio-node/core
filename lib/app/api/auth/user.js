export default class {
    #api;
    #id;
    #email;
    #enabled;
    #locale;
    #emailConfirmed;
    #avatar;
    #telegramUsername;
    #telegramConnected;

    // XXX remove
    #roles;

    // XXX roles
    constructor ( api, data ) {
        this.#api = api;

        if ( data ) {
            this.#id = data.id;

            this.update( data );

            this.#roles = new Set();
        }
        else {
            this.#roles = new Set();
        }
    }

    // properties
    get id () {
        return this.#id;
    }

    get email () {
        return this.#email;
    }

    get enabled () {
        return this.#enabled;
    }

    get locale () {
        return this.#locale;
    }

    get emailConfirmed () {
        return this.#emailConfirmed;
    }

    get avatar () {
        return this.#avatar;
    }

    get telegramUsername () {
        return this.#telegramUsername;
    }

    get telegramConnected () {
        return this.#telegramConnected;
    }

    // XXX remove
    get roles () {
        return this.#roles;
    }

    get isRoot () {
        return this.#id && this.#api.validate.userIsRoot( this.#id );
    }

    // public
    hasRoles ( roles ) {
        if ( this.isRoot ) return true;

        // method has no roles
        if ( !roles ) return false;

        if ( !Array.isArray( roles ) ) roles = [roles];

        // nothing to check
        if ( !roles.length ) return false;

        for ( const role of roles ) {

            // any
            if ( role === "*" ) return true;

            // guest (not authenticated)
            else if ( role === "guest" ) {
                if ( !this.#id ) return true;
            }

            // user (any authenticated)
            else if ( role === "user" ) {
                if ( this.#id ) return true;
            }

            // root
            else if ( role === "root" ) {
                if ( this.isRoot ) return true;
            }

            // compare
            else {
                if ( this.#roles.has( role ) ) return true;
            }
        }

        return false;
    }

    update ( data ) {
        if ( data.email ) this.#email = data.email;
        if ( data.enabled != null ) this.#enabled = data.enabled;

        this.setLocale( data.locale );

        if ( data.email_confirmed != null ) this.#emailConfirmed = data.email_confirmed;

        if ( data.gravatar != null ) this.#avatar = `https://s.gravatar.com/avatar/${data.gravatar}?d=${this.#api.config.defaultGravatarEncoded}`;

        if ( data.telegram_username != null ) this.#telegramUsername = data.telegram_username;
        if ( data.telegram_connected != null ) this.#telegramConnected = data.telegram_connected;
    }

    setLocale ( locale ) {
        if ( !locale ) {
            this.#locale = this.#api.config.defaultLocale;
        }
        else if ( !this.config.locales.includes( locale ) ) {
            this.#locale = this.#api.config.defaultLocale;
        }
        else {
            this.#locale = locale;
        }
    }

    // XXX roles
    toJSON () {
        if ( this.#id ) {
            return {
                "user_id": this.#id,
                "email": this.#email,
                "locale": this.#locale,
                "avatar": this.#avatar,
                "roles": this.#roles.size ? [...this.#roles] : null,
            };
        }
        else {
            return null;
        }
    }
}
