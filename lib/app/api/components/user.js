...
export default class extends Component {
    ...
    async createUser ( email, fields = {}, { dbh, root, roles, parentUserId } = {} ) {
        ...
        const user = await dbh.begin( async dbh => {
            ...
            if ( !root && roles && roles.length ) {
                const res = await this.api.acl.addAclUser( constants.defaultAclId, userId, { roles, parentUserId, dbh } );
    ...
    async setUserPassword ( userId, password, { dbh } = {} ) {
        ...
        const user = await this.api.cache.getUserById( userId, { dbh } );
        ...
        const res = await dbh.do( QUERIES.upsertHash, [user.id, passwordHash] );
    ...
    async setUserEnabled ( userId, enabled, { dbh } = {} ) {
        ...
        const res = await dbh.do( QUERIES.setUserEnabled, [enabled, user.id, !enabled] );
...
`
    ...
    async #authenticateUser ( email, { password } = {} ) {
        const user = await this.api.cache.getUserByEmail( email.toLowerCase() );
