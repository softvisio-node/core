...
const QUERIES = {
    ...
    "setTelegramUsername": sql`UPDATE "user" SET telegram_username = ?, telegram_user_id = ( SELECT id FROM telegram_user WHERE name = ? ) WHERE id = ? AND NOT EXISTS ( SELECT FROM "user" WHERE telegram_username = ? ) RETURNING telegram_user_id`.prepare(),
...
export default class extends Component {
    ...
    // public
    async authenticateUserPassword ( email, password ) {
    ...
    async authenticateUserOauth ( oauthProvider, oauthCode, oauthRedirectUri ) {
        const res = await this.#oauth( oauthProvider, oauthCode, oauthRedirectUri );

        if ( !res.ok ) return;

        return this.#authenticateUser( res.data.email );
    }
    ...
    async createUser ( email, fields = {}, { dbh, root } = {} ) {
        ...
        // validate email
        const emailIsValid = this.api.validate.validateEmail( email );
        ...
        // validate password
            ...
            const res = this.api.validate.validatePassword( password );
        ...
        // validate locale
        if ( fields.locale ) {
            if ( !this.api.config.locales.includes( fields.locale ) ) {
        ...
        if ( fields.telegram_username != null ) {
            ...
            const res = this.api.validate.validateTelegramUsername( fields.telegram_username );
            if ( !res.ok ) return res;
        }
        ...
        const passwordHash = await this.api.argon2.createHash( password );
        ...
        const user = await dbh.begin( async dbh => {
            ...
            res = await dbh.do( QUERIES.upsertHash, [userId, passwordHash] );
    ...
    async setUserEmail ( userId, email, { dbh } = {} ) {
        ...
        // validate user name
        var res = this.api.validate.validateEmail( email );
        if ( !res.ok ) return res;
    ...
    async setUserPassword ( userId, password, { dbh } = {} ) {
        ...
        var user = await this.getUser( userId, { dbh } );
        ...
        const res = await dbh.do( QUERIES.upsertHash, [user.data.id, passwordHash] );
    ...
    async setUserEnabled ( userId, enabled, { dbh } = {} ) {
        ...
        if ( this.api.validate.userIsRoot( userId ) ) return result( [400, "Unable to modify root user"] );
    ...
    async setTelegramUsername ( userId, telegramUsername, { dbh } = {} ) {
        if ( telegramUsername ) {
            telegramUsername = telegramUsername.toLowerCase();

            const isValid = this.api.validate.validateTelegramUsername( telegramUsername );
        ...
        const res = await dbh.selectRow( QUERIES.setTelegramUsername, [telegramUsername, telegramUsername, userId, telegramUsername] );

        if ( !res.meta.rows ) return result( [500, `Unable to change telegram username`] );

        if ( res.ok ) {
            this.api.notifications.sendNotification(
                "security",
                userId,
                "Telegram username changed",
                `Your telegram username was changed.

New telegram username: ${telegramUsername || "-"}.
`
            );
        }
    ...
    async setUserLocale ( userId, locale, { dbh } = {} ) {
        if ( !this.api.config.locales.includes( locale ) ) return result( [400, `Locale is not valid`] );
    ...
    async deleteUser ( userId, { dbh } = {} ) {
        ...
        if ( this.api.validate.userIsRoot( userId ) ) return result( [400, "Unable to delete root user"] );
    ...
    async getUser ( userId, { dbh } = {} ) {
        ...
        if ( !res.ok ) {
            ...
            res.data.roles = this.api.userRoles.buildUserRoles( userId, res.data.roles );
    ...
    async #authenticateUser ( email, { password } = {} ) {
        const user = await this.dbh.selectRow( QUERIES.authUser, [email.toLowerCase()] );

        // user not found or disabled
        if ( !user.data ) return;

        if ( password ) {
            if ( !this.api.argon2.verifyHash( user.data.hash, password ) ) return;
        }

        return new Auth( this.api, null, {
            "id": user.data.id,
