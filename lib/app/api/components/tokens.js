...
const SQL = {
    "storeHash": sql`INSERT INTO api_token_hash ( api_token_id, fingerprint, hash ) VALUES ( ?, ?, ? )`.prepare(),
...
export default class extends Component {
    ...
    async createToken ( userId, name, enabled, options = {} ) {
        ...
        var res = await dbh.begin( async dbh => {
            ...
            // generate token
            const token = Token.generate( this.app, constants.tokenTypeUserToken, id, { "length": this.api.config.userTokenLength } );
    ...
    // protected
    async _init () {
        this.#cache = new Cache( this.api, this.api.config.tokensCacheMaxSize );
