...
WITH deleted AS (
    DELETE FROM api_internal_notification WHERE expires <= CURRENT_TIMESTAMP RETURNING done
)
SELECT
...
` );
        ...
        if ( res.ok ) {
            ...
            // this.app.publish( "/api/notifications/update/", users, res.data );
        }
