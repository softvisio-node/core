...
import Fcm from "#lib/api/google/fcm";
...
export default class extends Component {
    ...
    async registerPushNotificationsToken ( token, userId ) {
        ...
        res = await this.#fcm.subscribeToTopic( `${this.#pushNotificationsPrefix}all`, token );
        if ( !res.ok ) return res;

        if ( userId ) {
            res = await this.#fcm.subscribeToTopic( `${this.#pushNotificationsPrefix}users`, token );
            if ( !res.ok ) return res;

            res = await this.#fcm.subscribeToTopic( `${this.#pushNotificationsPrefix}user.${userId}`, token );
            if ( !res.ok ) return res;
        }
        else {
            res = await this.#fcm.subscribeToTopic( `${this.#pushNotificationsPrefix}guests`, token );
            if ( !res.ok ) return res;
        }
    ...
    async _init () {
        ...
        if ( this.api.config.notifications.firebase?.serviceAccount ) {
            ...
            this.#fcm = new Fcm( this.api.config.notifications.firebase.serviceAccount );
        }
        else {
