...
export default class extends Component {
    ...
    get telegramlNotificationsEnabled () {
    ...
    async registerPushNotificationsToken ( token, userId ) {
        ...
        if ( userId ) {
            ...
            counter.inc();
            this.#googleCloudMessagingApi.subscribeToTopic( `${this.#pushNotificationsPrefix}users.${userId}`, token ).thrn( then );
        }
        else {
    ...
    async getUserNotificationsProfile ( userId ) {
        const profile = {
            ...
            "telegramlNotificationsEnabled": this.telegramlNotificationsEnabled,
    ...
    async _init () {
        ...
        if ( this.api.config.notifications.telegramBot.apiKey ) {
            this.#channels.telegram = this.api.config.notifications.telegramlNotificationsEnabled;
        }
        else {
