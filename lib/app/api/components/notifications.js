...
import GoogleCloudMessaging from "#lib/api/google/cloud/messaging";
...
export default class extends Component {
    ...
    async registerPushNotificationsToken ( token, userId ) {
        ...
        res = await this.#googleCloudMessaging.subscribeToTopic( `${this.#pushNotificationsPrefix}all`, token );
        if ( !res.ok ) return res;

        if ( userId ) {
            res = await this.#googleCloudMessaging.subscribeToTopic( `${this.#pushNotificationsPrefix}users`, token );
            if ( !res.ok ) return res;

            res = await this.#googleCloudMessaging.subscribeToTopic( `${this.#pushNotificationsPrefix}user.${userId}`, token );
            if ( !res.ok ) return res;
        }
        else {
            res = await this.#googleCloudMessaging.subscribeToTopic( `${this.#pushNotificationsPrefix}guests`, token );
            if ( !res.ok ) return res;
        }
    ...
    async sendInternalNotification ( users, subject, body, options = {} ) {
        ...
        const res = await this.dbh.begin( async dbh => {
            ...
            res = await dbh.selectRow( SQL.insertInternalNotification, [subject, body, options.meta, expires] );
    ...
    async sendPushNotification ( topic, subject, body, options = {} ) {
        ...
        return this.#googleCloudMessaging.send( message );
    }
    ...
    async _init () {
        ...
        if ( this.api.config.notifications.telegramBotKey ) {
            ...
            this.#telegramBot = this.app.telegram.createBot( "core-support-bot", this.api.config.notifications.telegramBotKey );
        }
        else {
        ...
        if ( this.api.config.notifications.firebase?.serviceAccount ) {
            ...
            this.#googleCloudMessaging = new GoogleCloudMessaging( this.api.config.notifications.firebase.serviceAccount );
        }
        else {
    ...
    _run () {
        ...
        setInterval( () => this.dbh.do( SQL.clearInternalNotifications ), 1000 * 60 * 60 * 24 );
    ...
    async #onTelegramUpdate ( update ) {
        ...
        if ( update.type === "my_chat_member" ) {
            ...
            // bot restarted
                ...
                this.#telegramBot.sendMessage( telegramUserId, `You are subscribed to the notifications. This bot doesn't support any additional commands.` );
            }
        }
        else {
            ...
            if ( !user ) {
                await this.#telegramBot.sendMessage( telegramUserId, `You are not authorized to use this bot. Please set your telegram username at the project notification settings and type "/start" again.` );
            }
            else {
                this.#telegramBot.sendMessage( telegramUserId, `This is notifications bot. It doesn't support any additional commands.` );
            }
        }
    }
