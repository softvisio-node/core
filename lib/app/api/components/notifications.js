...
export default class extends Component {
    ...
    async registerPushNotificationsToken ( token, userId ) {
        ...
        res = await this.#firebase.messaging.subscribeToTopic( `${this.#firebasePrefix}devices.all`, token );
        if ( !res.ok ) return res;

        if ( userId ) {
            res = await this.#firebase.messaging.subscribeToTopic( `${this.#firebasePrefix}users.all`, token );
            if ( !res.ok ) return res;

            res = await this.#firebase.messaging.subscribeToTopic( `${this.#firebasePrefix}users.${userId}`, token );
            if ( !res.ok ) return res;
        }
        else {
            res = await this.#firebase.messaging.subscribeToTopic( `${this.#firebasePrefix}guests.all`, token );
            if ( !res.ok ) return res;
        }
    ...
    async sendNotification ( type, users, subject, body, options = {} ) {
        ...
        for ( const userId of users ) {
            ...
            // push
            if ( this.pushEnabled && defaultChannels.push != null ) {
    ...
    async sendPushNotification ( topic, subject, body, options = {} ) {
        ...
        const message = {
            ...options,
            "topic": this.#firebasePrefix + topic,
        };
    ...
    async _init () {
        ...
        if ( this.api.config.notifications.firebase?.serviceAccount ) {
            ...
            this.#firebasePrefix = this.api.config.notifications.firebase.prefix ? this.api.config.notifications.firebase.prefix + "." : "";
