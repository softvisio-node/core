...
export default class extends Component {
    ...
    async init () {
        ...
        this.#categories = this.api.config.notifications.categories || {};
    ...
    async sendNotification ( type, users, subject, body, options = {} ) {
        if ( !this.#categories[type] ) throw `Notification type "${type}" is invalid`;
    ...
    #buildProfile ( data ) {
        ...
        for ( const type in this.#categories ) {
            const defaultChannels = this.#categories[type].channels,
                channels = idx[type];
