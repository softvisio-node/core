...
export default class extends Component {
    ...
    async createUser ( { email, password, enabled, locale, root, roles, parentUserId, dbh } = {} ) {
        return this.app.users.createUser( {
            email,
            password,
            enabled,
            locale,
            root,
            dbh,
            "beforeCommitCallback": async ( user, dbh ) => {
    ...
    async createUserOauth ( oauthProvider, oauthCode, oauthRedirectUri, { password, enabled, locale } = {}, options ) {
        const res = await this.#oauth( oauthProvider, oauthCode, oauthRedirectUri );
