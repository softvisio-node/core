...
` );
    ...
    // protected
    async _init () {
        const res = this.#configureAclType();
        if ( !res.ok ) return res;
    ...
    #configureAclType () {
        ...
        // XXX
        {
            for ( const [aclType, roles] of Object.entries( this.app.acl.types ) ) {
    ...
    async #getAclUser ( aclId, userId, { dbh } = {} ) {
        ...
        if ( !res.ok ) {
            ...
            for ( const role of user.roles ) {
                for ( const permission of this.#rolePermissions[user.aclTyoe][role] ) {
