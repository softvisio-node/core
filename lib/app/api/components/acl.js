...
` );
    ...
    async addAclUserScopes ( aclId, userId, scopes, parentUserId, { dbh } = {} ) {
        ...
        if ( user === false ) {
            ...
            return this.addAclUser( aclId, userId, parentUserId, { scopes, dbh } );
        }
        ...
        for ( const scope of scopes ) {
            if ( !parentUserIsRoot && !parentUser.scopes.has( scope ) ) return result( [400, `ACL scopes are invalid`] );
        ...
        const res = await dbh.do( sql`INSERT INTO acl_user_scope`.VALUES( addScopes.map( scope => {
            return {
                "acl_id": aclId,
                "user_id": userId,
                "acl_scope_id": sql`SELECT acl_scope.id FROM acl_scope, acl_type WHERE acl_scope.acl_type_id = acl_type.id AND acl_type.type = ${aclType} AND acl_scope.scope = ${scope}`,
...
` );
    ...
    async getAclUserScopes ( aclId, userId, parentUserId ) {
        const userIsRoot = this.api.validate.userIsRoot( userId ),
            parentUserIsRoot = this.api.validate.userIsRoot( parentUserId );

        var user, parentUser;

        if ( !userIsRoot && !parentUserIsRoot ) {
            parentUser = await this.#getAclUser( aclId, parentUserId );

            if ( !parentUser ) return result( [400, `Unable to get ACL scopes`] );
        }
