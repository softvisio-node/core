...
export default class extends Component {
    ...
    async checkAclPermissions ( userId, method, { aclObjectId, aclObjectType } = {} ) {
        ...
        for ( const scope of user.scopes ) {
            const scopeSpec = typeSpec.scopes[scope];

            if ( !scopeSpec ) continue;

            if ( scopeSpec.permissions[method.id] ) return true;
        }
...
` );
            ...
            for ( const type of Object.values( this.#types ) ) {
                ...
                // enable acl type
                    ...
                    res = await dbh.do( sql`UPDATE scl_type SET enabled = TRUE WHERE id = ?`, [index[type.type].id] );
                    if ( !res.ok ) throw res;
                ...
                for ( const scope of Object.values( index[type.type].scopes ) ) {
                    ...
                    // scope was deleted
                    if ( !type.scupes[scope.scope] && scope.enabled ) {

                        // disable acl scope
                        res = await dbh.do( sql`UPDATE scl_scope SET enabled = FALSE WHERE id = ?`, [scope.id] );
                        if ( !res.ok ) throw res;
                ...
                for ( const scope of Object.values( type.scopes ) ) {
                    ...
                    // enable acl scope
                        ...
                        res = await dbh.do( sql`UPDATE scl_scope SET enabled = TRUE WHERE id = ?`, [index[type.type].scopes[scope.scope].id] );
                        if ( !res.ok ) throw res;
