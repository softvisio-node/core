...
` );
            ...
            for ( const type of Object.values( index ) ) {
                ...
                if ( !this.#types[type.type] && type.enabled ) {
                    ...
                    // disable acl scopes
                    res = await dbh.do( sql`UPDATE acl_scope SET enabled = FALSE WHERE acl_type_id = ?`, [type.id] );
                    if ( !res.ok ) throw res;

                    // remove scl scope permissions
                    res = await dbh.do( sql`DELETE FROM acl_permission WHERE acl_scope_id`.IN( Object.values( type.scopes ).map( scope => scope.id ) ) );
                    if ( !res.ok ) throw res;
                }
            }
            ...
            for ( const type of Object.values( this.#types ) ) {
                ...
                // enable acl type
                if ( !index[type.type].enabled ) {
                    res = await dbh.dp( sql`UPDATE scl_type SET enabled = TRUE WHERE id = ?`, [index[type.type].id] );
                    if ( !res.ok ) throw res;
                }
