...
export default class extends Component {
    ...
    // XXX rename te addAclUser???
    async setAclUserScopes ( aclId, userId, { dbh, editorUserId, enabled, scopes = [] } = {} ) {
        if ( this.api.validate.userIsRoot( userId ) ) return result( [400, `Unable to set scopes`] );
        ...
        if ( editorUserId && !this.api.validate.userIsRoot( editorUserId ) ) {
            ...
            if ( !parentUser || !parentUser.enabled ) return result( [400, `Unable to set user scopes`] );
        }
    ...
    async setAclUserScopeEnabled ( aclId, userId, scope, enabled, editorUserId, { dbh } = {} ) {
        dbh ||= this.dbh;

        // XXX root user - do not allow

        console.log( "---", aclId, userId, scope, enabled, editorUserId );
