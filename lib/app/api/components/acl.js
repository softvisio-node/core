...
SELECT
    ...
    acl_user.created,
    acl_user.enabled,
    acl_user_scopes( acl_user.acl_id, acl_user.user_id ),
...
export default class extends Component {
    ...
    // XXX
    async readAclUsers ( aclId, { fields, orderBy, offset, limit, defaultLimit, maxLimit, maxResults } = {} ) {
        const where = this.dbh.where( `
acl_user.acl_id = ${aclId}
AND acl_user.user_id = "user".id

` );

        if ( fields.enabled ) {
            where.and( { "acl_user.enabled": fields.enabled } );
        }

        if ( fields.email ) {
            where.and( { '"user".email': fields.email } );
        }
...
SELECT
    ...
    acl_user.created,
    acl_user.enabled,
    acl_user_scopes( acl_user.acl_id, acl_user.user_id ) AS scoppes
FROM
    "user",
    acl_user
`.WHERE( where );

        const res = await this.dbh.read( query, {
            orderBy,
            offset,
            limit,
            defaultLimit,
    ...
    async addAclUser ( aclId, userId, { enabled = true, scopes, parentUserId, dbh } = {} ) {
        ...
        res = await dbh.begin( async dbh => {
            ...
            res = await dbh.do( QUERIES.addAclUser, [aclId, userId, enabled] );
    ...
    async deleteAclUser ( aclId, userId, { parentUserId, dbh } = {} ) {
        ...
        res = await dbh.do( QUERIES.deleteAclUser, [aclId, userId] );
    ...
    async setAclUserEnabled ( aclId, userId, enabled, { parentUserId, dbh } = {} ) {
        ...
        res = await dbh.do( QUERIES.setAclUserEnabled, [enabled, aclId, userId] );
...
` );
    ...
    async getAclScopes ( aclId ) {
        const aclType = await this.#getAclType( aclId );

        if ( !aclType ) return result( [400, `ACL id is invalid`] );

        var data = this.#aclTypeScopesCache[aclType];
        ...
        if ( !data ) {
            ...
            this.#aclTypeScopesCache[aclType] = data;
        }
    ...
    async #getAclUser ( aclId, userId, { dbh } = {} ) {
        ...
        if ( !res.ok ) {
            ...
            user = {
                ...
                "scopes": new Set( res.data.acl_user_scopes ),
