...
INSERT INTO acl_user_scope (
    ...
    ?,
    ?,
    ( SELECT acl_scope.id FROM acl_scope, acl_type WHERE acl_scope.acl_type_id = acl_type.id AND acl_type.type = ? AND acl_scope.scope = ? )
) ON CONFLICT ( acl_id, user_id, acl_scope_id ) DO NOTHING
`.prepare(),
...
WHERE
    acl_id = ?
    AND user_id = ?
    AND acl_scope_id = ( SELECT acl_scope.id FROM acl_scope, acl_type WHERE acl_scope.acl_type_id = acl_type.id AND acl_type.type = ? AND acl_scope.scope = ? )
`.prepare(),
};
...
export default class extends Component {
    ...
    async addAclUser ( aclId, userId, parentUserId, { enabled = true, scopes, dbh } = {} ) {
        ...
        const res = await dbh.begin( async dbh => {
            ...
            res = await this.#addAclUser( aclId, userId, { enabled, dbh } );
    ...
    async deleteAclUser ( aclId, userId, { dbh } = {} ) {
        dbh ||= this.dbh;
    ...
    async setAclUserEnabled ( aclId, userId, enabled, { dbh } = {} ) {
        dbh ||= this.dbh;
    ...
    async setAclUserScopes ( aclId, userId, scopes, parentUserId, { dbh } = {} ) {
        ...
        res = await dbh.selectRow( QUERIES.getAclUser, [aclId, userId] );

        if ( !res.ok ) return res;

        if ( !res.data ) return result( [400, `ACL user not found`] );

        const aclType = res.data.type,
            userScopes = new Set( res.data.scopes );

        const parentUserIsRoot = this.api.validate.userIsRoot( parentUserId );

        if ( !parentUserIsRoot ) {
            var parentUser = await this.#getAclUser( aclId, parentUserId, { dbh } );

            if ( !parentUser ) return result( [400, `Unable to get parnet user`] );
        }
        ...
        for ( const scope of scopes ) {
            if ( !this.#types[aclType].scopes[scope] || !parentUser.scopes.has( scope ) ) return result( [400, `ACL scopes are invalid`] );

            if ( !userScopes.has( scope ) ) addScopes.push( scope );
        }
...
` );
    ...
    async deleteAclUserScopes ( aclId, userId, scopes, parentUserId, { dbh } = {} ) {
        ...
        for ( const scope of scopes ) {
            ...
            if ( userScopes.has( scope ) ) deleteScopes.push( scope );
        }
...
` );
    ...
    // XXX
    async suggestAclUsers () {}

    async getAclUserScopes ( aclId, userId, editorUserId ) {
        const userIsRoot = this.api.validate.userIsRoot( userId ),
            editorUserIsRoot = this.api.validate.userIsRoot( editorUserId );

        var user, editorUser;

        if ( !userIsRoot && !editorUserIsRoot ) {
            editorUser = await this.#getAclUser( aclId, editorUserId );

            if ( !editorUser ) return result( [400, `Unable to get ACL scopes`] );
        }
        ...
        const aclType = user?.type || editorUser?.type || ( await this.#getAclType( aclId ) );
        ...
        for ( const scope of Object.values( this.#types[aclType].scopes ) ) {
            ...
            else if ( userId === editorUserId ) {
                readonly = true;
            }
            else if ( !editorUserIsRoot ) {
                readonly = !editorUser.scopes.has( scope.scope );
            }
            else {
                readonly = false;
            }
...
` );
    ...
    async #addAclUser ( aclId, userId, { enabled = true, dbh } = {} ) {
        dbh ||= this.dbh;

        return dbh.do( sql`INSERT INTO acl_user ( acl_id, user_id, enabled ) VALUES ( ?, ?, ? ) ON CONFLICT ( acl_id, user_id ) DO NOTHING`, [aclId, userId, enabled] );
    }
}
