...
const QUERIES = {
    "storeHash": sql`INSERT INTO user_session_hash ( user_session_id, seed, hash ) VALUES ( ?, ?, ? )`.prepare(),
...
export default class extends Component {
    ...
    async createUserSession ( userId, { hostname, userAgent, remoteAddress, dbh } = {} ) {
        ...
        var res = await dbh.begin( async dbh => {
            ...
            // generate token
            const token = Token.generate( this.api, constants.tokenTypeUserSession, id );

            // insert hash
            res = await dbh.do( QUERIES.storeHash, [token.id, token.seed, await token.getHash()] );
            if ( !res.ok || !res.meta.rows ) throw result( 500 );
