...
export default class extends Component {
    ...
    async getUserSessionTokenById ( tokenId, { dbh } = {} ) {
        ...
        if ( token ) {
            ...
            this.#userSessionTokensCache.set( token.id, res.data );
        }
    ...
    async _init () {
        ...
        this.#userTokensCache = new CacheLru( { "maxSize": this.api.config.usersCacheMaxSize } ); // .on( "delete", this.#onDeleteUserFromCache.bind( this ) );

        // XXX
        this.#userSessionTokensCache = new CacheLru( { "maxSize": this.api.config.usersCacheMaxSize } ); // .on( "delete", this.#onDeleteUserFromCache.bind( this ) );
