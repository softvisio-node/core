...
BEGIN
    ...
    PERFORM pg_notify( 'api/user-notifications-profile/update', json_build_object(
        'id', _user_id::text,
...
CREATE FUNCTION user_internal_notification_after_delete_trigger() RETURNS TRIGGER AS $$
BEGIN

    IF NOT EXISTS ( SELECT FROM user_internal_notification WHERE api_internal_notification_id = OLD.api_internal_notification_id ) THEN
...
CREATE TRIGGER user_internal_notification_after_delete AFTER DELETE ON user_internal_notification FOR EACH ROW EXECUTE FUNCTION user_internal_notification_after_delete_trigger();
