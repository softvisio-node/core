...
CREATE MATERIALIZED VIEW api_call_log_latest AS (
    ...
    SELECT
        series.method_id,
        series.date,
        load.total_accepted::int4,
        load.total_declined::int4,
        requests.avg_runtime::float4,
        ( ( requests.total_exceptions::numeric / nullif( requests.total_requests, 0 ) ) * 100 )::float4 AS errors_percent
    FROM
        series
...
BEGIN
    IF NOT EXISTS ( SELECT FROM api_call_log_updated WHERE type = 'latest' AND last_updated IS NOT NULL AND last_updated > CURRENT_TIMESTAMP - INTERVAL '10 seconds' ) AND pg_try_advisory_xact_lock( get_lock_id( 'api/call-log/latest/generate' ) ) THEN
...
BEGIN
    IF NOT EXISTS ( SELECT FROM api_call_log_updated WHERE type = 'historic' AND last_updated IS NOT NULL AND last_updated > CURRENT_TIMESTAMP - INTERVAL '10 seconds' ) AND pg_try_advisory_xact_lock( get_lock_id( 'api/call-log/historic/generate' ) ) THEN
