...
CREATE OR REPLACE FUNCTION acl_user_scopes ( _acl_id int8, _user_id int8 ) RETURNS json STABLE AS $$
BEGIN
    ...
    IF _user_id = ${constants.rootUserId} THEN
        RETURN (
            ...
            WHERE
                acl_scope.enabled
                AND acl_scope.acl_type_id = acl_type.id
    ...
    ELSE
        RETURN (
            ...
            FROM
                ...
                acl_scope
            WHERE
                acl_user_scope.acl_id = _acl_id
                AND acl_user_scope.user_id = _user_id
...
BEGIN
    ...
    RETURN (
        ...
        FROM
            ...
            acl_user
        WHERE
            acl_permission.acl_scope_id = acl_scope.id AND acl_scope.enabled
            AND acl_type.id = acl_scope.acl_type_id AND acl_type.enabled
            AND acl_user_scope.acl_scope_id = acl_scope.id
            AND acl_user.acl_id = acl_user_scope.acl_id AND acl_user.user_id = acl_user_scope.user_id
