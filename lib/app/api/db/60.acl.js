...
CREATE FUNCTION acl_permissions ( _acl_id int8, _user_id int8, _acl_object_type text ) RETURNS json STABLE AS $$
BEGIN

    RETURN acl_permissions( _acl_id, _user_id, ( SELECT id FROM acl_object WHERE type = _acl_object_type ) );

END;
$$ LANGUAGE plpgsql;

CREATE FUNCTION acl_permissions ( _acl_id int8, _user_id int8, _acl_object_type_id int8 ) RETURNS json STABLE AS $$
DECLARE
    row record;
    res jsonb = '{}'::jsonb;
BEGIN
