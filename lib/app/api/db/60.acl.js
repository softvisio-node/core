...
CREATE TABLE acl_scope_permission (
...
BEGIN
    ...
    IF _user_id = ${constants.rootUserId} THEN
        RETURN ( SELECT
            json_agg( DISTINCT acl_scope_permission.permission )
        FROM
            acl,
            acl_type,
            LEFT JOIN acl_scope ON ( acl_type.id = acl_scope.acl_type_id )
            LEFT JOIN acl_scope_permission ON ( acl_scope.id = acl_scope_permission.acl_scope_id )
        WHERE
            acl.id = _acl_id
            AND acl.acl_type_id = acl_type.id
            AND acl_type.enabled
