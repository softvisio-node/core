...
export default class {
    ...
    async #onHttpRequest ( req, methodId ) {
        ...
        try {
            ...
            if ( req.method === "post" ) {
                ...
                if ( req.headers.contentLength ) {
                    body = await req.buffer( { "maxLength": this.api.config.frontend.maxApiRequestLength } ).catch( e => {
        ...
        req.end( {
            ...
            "compress": this.api.config.compress,
        } );
    }
            ...
            for ( const name in fields ) {
                ...
                if ( !Array.isArray( field ) ) {
                    ...
                    // json field
                        ...
                        args[name] = this.#decodeArguments( field.headers.contentType, field.value );
                    }
                }
                else {
                    args[name] = [];
                    ...
                    for ( const field of fields[name] ) {
                        ...
                        // json field
                            ...
                            args[name].push( this.#decodeArguments( field.headers.contentType, field.value ) );
                        }
                    }
                }
            }
    ...
    #decodeArguments ( contentType, args ) {
        ...
        // invalid content type
        if ( contentType && !contentType.type.startsWith( "application/json" ) ) result( -32803 );
