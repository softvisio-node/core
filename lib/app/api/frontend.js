...
export default class {
    ...
    closeUserConnections ( userId, res ) {
        const connections = this.#userConnections[userId];

        if ( !connections ) return;

        for ( const connection of connections ) connection.disconnect( res );
    }

    closeTokenConnections ( tokenType, tokenId, res ) {
        var connections;

        if ( tokenType === constants.tokenTypeUserToken ) {
            connections = this.#userTokenConnections[tokenId];
        }
        else if ( tokenType === constants.tokenTypeUserSession ) {
            connections = this.#userSessionConnections[tokenId];
        }
    ...
    async #onWebSocketUpgrade ( req ) {
        ...
        // backend is down
        if ( !ctx ) return req.end( { "status": 503 } );
    ...
    #onWebSocketConnect ( connection ) {
        ...
        this.#userConnections[ctx.user.id] ??= new Set();
        this.#userConnections[ctx.user.id].add( connection );

        if ( ctx.token.isUserToken ) {
            this.#userTokenConnections[ctx.token.id] ??= new Set();
            this.#userTokenConnections[ctx.token.id].add( connection );
        }
        else if ( ctx.token.isUserSessionToken ) {
            this.#userSessionConnections[ctx.token.id] ??= new Set();
            this.#userSessionConnections[ctx.token.id].add( connection );
        }
    }

    #onWebsocketDisconnect ( connection, res ) {
        const ctx = connection.ctx;

        if ( !ctx.isAuthenticated ) return;

        this.#userConnections[ctx.user.id].delete( connection );
        if ( !this.#userConnections[ctx.user.id].size ) delete this.#userConnections[ctx.user.id];

        if ( ctx.token.isUserToken ) {
            this.#userTokenConnections[ctx.token.id].delete( connection );
            if ( !this.#userTokenConnections[ctx.token.id].size ) delete this.#userTokenConnections[ctx.token.id];
        }
        else if ( ctx.token.isUserSessionToken ) {
            this.#userSessionConnections[ctx.token.id].delete( connection );
            if ( !this.#userSessionConnections[ctx.token.id].size ) delete this.#userSessionConnections[ctx.token.id];
        }
    }
    ...
    async #onHttpRequest ( req, methodId ) {
        ...
        try {
            ...
            // healthcheck
                ...
                throw await this.api.healthCheck.check();
            }
