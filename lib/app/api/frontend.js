...
export default class extends Component {
    ...
    publish ( name, ...args ) {
        ...
        if ( name.endsWith( "/" ) ) {
            ...
            for ( const target of targets ) {
                this.#outgoingEvents.publish( name + target, args, cache, publisherId );
            }
        }
        else {
    ...
    async #authenticate ( token, { hostname, userAgent, remoteAddress } = {} ) {
        ...
        if ( token.isUserToken ) {
            internalToken = this.api.cache.getCachedTokenById( token.id ) ?? ( await this.api.cache.getTokenById( token.id ) );
        }
        else if ( token.isUserSessionToken ) {
            internalToken = this.api.cache.getCachedSessionById( token.id ) ?? ( await this.api.cache.getSessionById( token.id ) );
        }
