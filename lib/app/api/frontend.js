...
export default class extends Component {
    ...
    publish ( name, ...args ) {
        ...
        if ( name.startsWith( "/" ) ) {
            ...
            var prefix = this.api.isApo ? "to-api/" : "to-rpc",
        ...
        if ( name.endsWith( "/" ) ) {
            ...
            for ( const target of targets ) {
                this.#outgoingEvents.emit( name + target, args, cache, publisherId );

                cluster?.publish( prefix + name + target, ...args );
            }
        }
        else {
            this.#outgoingEvents.emit( name, args, cache, publisherId );

            cluster?.publish( prefix + name, ...args );
        }
    ...
    async _init ( getSchema ) {
        ...
        // link to the cluster
        if ( this.app.cluster ) {
            const prefix = this.api.isApo ? "to-api/" : "to-rpc";
