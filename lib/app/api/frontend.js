...
export default class extends Component {
    ...
    async checkApiCallLimits ( callDescriptor, ctx, method, isPrivateCall ) {
        ...
        const stats = this.#stats,
            statsId = ( ctx.token ? ctx.token.type + "/" + ctx.token.id : ctx.remoteAddress ) + "/" + method.id;
    ...
    async _init ( getSchema ) {
        ...
        if ( this.api.isApi ) {
            ...
            // user disable
            this.dbh.on( "users/user/enabled/update", data => {
                if ( !data.enabled ) this.#sessionStateEvents.emit( `user/disable/${ data.id }` );
            } );

            // user email update
            this.dbh.on( "users/user/email/update", data => {
                this.#sessionStateEvents.emit( `user/email-update/${ data.id }` );
            } );

            // api token disabled
            this.dbh.on( "api/token/enabled/update", data => {
                if ( !data.enabled ) this.#sessionStateEvents.emit( `api-token/disable/${ data.id }` );
            } );

            // api token delete
            this.dbh.on( "api/token/delete", data => {
                this.#sessionStateEvents.emit( `api-token/delete/${ data.id }` );
            } );

            // session delete
            this.dbh.on( "api/session/delete", data => {
                this.#sessionStateEvents.emit( `api-session/delete/${ data.id }` );
            } );
        }
    ...
    #createWebSocketConnection ( publishRemoteIncomingEvent, server, ws, options ) {
        const connection = new WebSocketConnection( {
            ...
            "sessionStateEvents": this.#sessionStateEvents,
        } );
    ...
    async #authenticate ( token, { signal, hostname, userAgent, remoteAddress } = {} ) {
        ...
        if ( token.startsWith( "telegram%3A" ) ) {
            try {
                token = JSON.parse( decodeURIComponent( token ).replace( /^telegram:/, "" ) );
            }
            catch ( e ) {
                return;
            }

            const bot = this.app.telegram?.bots.getBot( token.telegram_bot_id );
            if ( !bot ) return;
