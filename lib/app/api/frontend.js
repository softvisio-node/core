...
export default class {
    ...
    async #onWebSocketOpen ( connection ) {
        const ctx = await this.#authenticate( connection.data.token, {
            connection,
            "hostname": connection.data.hostname,
            "userAgent": connection.data.userAgent,
            "remoteAddress": connection.data.remoteAddress,
        } );

        // backend is down
        if ( !ctx ) return connection.end( 4503 );

        // XXX
        if ( this.authCache ) {

            // register authenticated connection
            if ( ctx.isAuthenticated ) this.authCache.registerConnection( connection );

            // update auth last activity timestamp
            this.authCache.updateAuthLastActivity( ctx );
        }
