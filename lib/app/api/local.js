...
import ObjectUsersMixin from "./local/object-users.js";
...
export default Super =>
    class extends mixins( UserPermissions, User, UserActionToken, ObjectUsersMixin, UserSession, UserToken, ApiCallLog, HealthCheck, ApiSchema, Super ) {
        #dbh;
        ...
        async authenticate ( token ) {
            if ( token.type === constants.tokenTypeUserCredentials ) {
                return this._authenticateUserCredentials( token );
            }
            else if ( token.type === constants.tokenTypeUserToken ) {
        ...
        async _init ( options = {} ) {
            ...
            // setup events
            this.#dbh.on( "connect", () => this.app.publish( "api/connect" ) );
            this.#dbh.on( "disconnect", () => this.app.publish( "api/disconnect" ) );
            ...
            res = await this.createUser( constants.rootUsername, password, true, null, {
                "email": process.env.APP_ROOT_EMAIL,
            ...
            if ( !res.ok ) {
                ...
                // root user already exists
                    ...
                    res = await this.setUserPassword( constants.rootUsername, password );
                    console.log( res + ( res.ok ? ", password" + ( password == null ? `: ${res.data.password}` : " taken from environment" ) : "" ) );
                    process.exit( 0 );
                }
            }
