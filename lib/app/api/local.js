...
module.exports = class extends mix( User, UserActionToken, UserObjectPermissions, UserSession, UserToken, AppSettings ) {
    ...
    async init () {
        ...
        let password, resetPasword, showPassword;
        ...
        console.log( res + ( res.ok ? ", password: " + ( showPassword ? res.data.password : 'from ".config.yaml" file' ) : "" ) );
        if ( !res.ok ) {
            ...
            // user already exists
                ...
                process.stdout.write( 'Setting root password from ".config.yaml" file ... ' );
                res = await this.setUserPassword( ROOT_USER_NAME, password );
                console.log( res + ( res.ok && showPassword ? ", password: " + res.data.password : "" ) );
                if ( !res.ok ) return res;
            }
        }
