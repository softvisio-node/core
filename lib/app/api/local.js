...
module.exports = class extends mix( User, UserActionToken, UserObjectPermissions, UserSession, UserToken, AppSettings ) {
    ...
    async init () {
        ...
        // create root user
        let password;
        if ( fs.existsSync( ".root-password" ) ) password = fs.readFileSync( ".root-password", "utf8" ).trim();
        process.stdout.write( "Creating root user ... " );
        res = await this.createUser( ROOT_USER_NAME, password, true );
        console.log( res + ( res.ok ? ", password: " + res.data.password : "" ) );
        if ( !res.ok ) {
            ...
            // user already exists
                ...
                process.stdout.write( "Settings root password from file ... " );
