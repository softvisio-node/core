...
const { uuidToBuffer, toBase58 } = require( "../../util" );
...
module.exports = class extends mix( User, UserActionToken, UserObjectPermissions, UserSession, UserToken, AppSettings ) {
    ...
    _generateToken ( type ) {
        ...
        if ( type === TOKEN_TYPE_TOKEN ) {
            id = uuidv4();
            tokenBuf = Buffer.concat( [crypto.randomBytes( 16 ), uuidToBuffer( id )] );
        ...
        // session token
            ...
            id = uuidv4();
            tokenBuf = Buffer.concat( [crypto.randomBytes( 32 ), uuidToBuffer( id )] );
