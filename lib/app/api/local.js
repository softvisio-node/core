...
const ObjectPermissions = require( "./local/object-permissions" );
...
module.exports = class extends mix( UserPermissions, User, UserActionToken, ObjectPermissions, UserSession, UserToken, AppSettings, ApiCallLog, EventEmitter ) {
    #dbh;
    ...
    _generateToken ( type, id ) {
        ...
        var hash = this._getHash( buf );

        var token = toBase58( buf );

        return { type, id, token, hash };
    }
};
