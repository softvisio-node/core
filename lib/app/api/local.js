...
export default Super =>
    class extends mixins( UserPermissions, User, UserActionToken, ObjectUsersMixin, UserSession, UserToken, ApiCallLog, HealthCheck, ApiSchema, Super ) {
        ...
        async _new ( options = {} ) {
            ...
            if ( this.#dbh.isPgsql ) {
                res = await this.#dbh.migrateSchema( new URL( "local/db", import.meta.url ) );
            }
            else {
            ...
            // setup events
            this.#dbh.on( "connect", () => this.emit( "ready" ) );
            this.#dbh.on( "disconnect", () => this.emit( "disconnect" ) );

            // migrate db schema
            if ( options.dbSchema ) {
                process.stdout.write( "Migrating applications database schema ... " );
                res = await this.#dbh.migrateSchema( options.dbSchema );
                console.log( res + "" );
                if ( !res.ok ) return res;
            }
            ...
            res = await this.createUser( CONST.ROOT_USER_NAME, password, true, null, {
