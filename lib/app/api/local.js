...
const { ROOT_USER_NAME, AUTH_USER, AUTH_TOKEN, AUTH_SESSION } = require( "../../const" );
...
module.exports = Super =>
    class extends mixins( UserPermissions, User, UserActionToken, ObjectMixin, UserSession, UserToken, AppSettings, ApiCallLog, Super ) {
        #dbh;
        ...
        constructor ( dbh ) {
            ...
            this.#dbh.on( "unsubscribe", () => {
                this._authCache.invalidateAll();
                this._objectsPermissionsCache.invalidateAll();
            } );

            this.#dbh.on( "event/api/auth-cache/invalidate/user", userId => this._authCache.invalidateUser( userId ) );
            this.#dbh.on( "event/api/auth-cache/invalidate/user-token", tokenId => this._authCache.invalidateUserToken( tokenId ) );

            this.#dbh.on( "event/api/object-permissions-cache/invalidate", cacheId => this._objectPermissionsCache.invalidate( cacheId ) );

            this.#dbh.on( "event/api/settings/update", async () => {
                await this._loadAppSettings();

                this.app.publish( ":local/api/settings/update", this.appSettings );
            } );
        }
