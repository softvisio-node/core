    ...
    class Connection extends ( Super || Object ) {
        ...
        _onWebsocketOpen ( ws ) {
            ...
            if ( auth.isAuthenticated ) {
                ws.subscribe( "user" ); // authenticated
                if ( auth.isRoot ) ws.subscribe( "root" );
                ws.subscribe( auth.userId );

                for ( const permission in auth.permissions ) {
                    if ( auth.permissions[permission] ) ws.subscribe( permission );
                }
            }
            else {
                ws.subscribe( "guest" ); // not authenticated
            }
        }

        _onRemoteEvent ( auth, name, args ) {
            this.app.publish( "api/event/" + name, auth, ...args );
        }
        ...
        async #onWebsocketMessage ( ws, msg, isBinary ) {
            ...
            if ( msg.method ) {
                ...
                // event
                    ...
                    this._onRemoteEvent( ws.auth, name, msg.params );
                }
                ...
                // rpc
                    ...
                    if ( method?.upload ) {
                        if ( msg.id ) this.#wsSend( ws, isBinary, result( -32900 ).toRPC( msg.id ) );
                    }
        ...
        async #onHttpRequest ( req, methodId ) {
            ...
            try {
                ...
                if ( methodId === "/event" ) {
                    ...
                    this._onRemoteEvent( auth, params[0], params[1] );
