...
const LOCAL_EVENTS = new Set( ["connect", "disconnect", "subscribe", "unsubscribe", "remote", "client/connect", "client/disconnect"] );
...
class Client extends Events {
    ...
    #subscribe ( name ) {
        ...
        this.#api.app.hub.on( "api/out", name, this.#eventsListener );
    }
    ...
    #unsubscribe ( name ) {
        ...
        this.#api.app.hub.off( "api/out", name, this.#eventsListener );
    }
...
export default Super =>
    class Connection extends ( Super || Object ) {
        ...
        async #onWebSocketMessage ( ws, msg, isBinary ) {
            ...
            if ( msg.method ) {
                ...
                // subscribe
                    ...
                    const names = msg.params.shift();

                    ws.client.subscribe( names );
                }
                ...
                // publish
                    ...
                    this.#onRemoteEvent( name, ws.auth, msg.params );
                }
        ...
        async #onHttpRequest ( req, methodId ) {
            ...
            try {
                ...
                if ( methodId === "/publish" ) {
                    ...
                    this.#onRemoteEvent( params[0], auth, params[1] );
        ...
        // XXX filter local events, filter using allowed events list, check permissions???
        // XXX event should not start with "/"
        #onRemoteEvent ( name, auth, args ) {
            if ( LOCAL_EVENTS.has( name ) ) return;

            this.emit( "remote", name, auth, args );
        }
    };
