        ...
        async #upload ( req, methodId, token ) {
            ...
            try {
                ...
                formDataDecoder.on( "field", ( formDataDecoder, name, value, options ) => {
                    ...
                    // unexpected field
                    if ( name !== "params" ) return formDataDecoder.destroy( result( -32907 ) );

                    try {
                        [isBinary, params] = this.#decodeParams( value, options["content-type"] );
                    }
                    catch ( e ) {
                        formDataDecoder.destroy( e );
                    }
                } );

                formDataDecoder.on( "file", async ( formDataDecoder, name, stream, options ) => {
                    ...
                    if ( name === "file" ) {
                        ...
                        if ( method.meta.uploadContentType ) {
                            ...
                            if ( !types.has( options["content-type"] ) ) return formDataDecoder.destroy( result( -32906 ) );
                        }

                        const tmpFile = await stream.writeToTmpFile();

                        if ( !tmpFile ) return formDataDecoder.destroy( result( -32906 ) );
                    ...
                    // params
                        ...
                        if ( !buffer ) return formDataDecoder.destroy( result( -32907 ) );

                        // decode params
                        try {
                            [isBinary, params] = this.#decodeParams( buffer, options["content-type"] );
                        }
                        catch ( e ) {
                            formDataDecoder.destroy( e );
                        }
                    ...
                    // unexpected field
                        ...
                        return formDataDecoder.destroy( result( -32907 ) );
                    }
                } );
