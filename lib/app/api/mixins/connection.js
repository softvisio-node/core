...
export default Super =>
    class Connection extends ( Super || Object ) {
        ...
        async _onWebsocketEvent ( ws, msg ) {
            ...
            this.app.publish( "user/" + msg.name, ws.auth, ...msg.args );
        }
        ...
        async #onWebsocketMessage ( ws, msg, isBinary ) {
            ...
            // rpc
                ...
                if ( msg.method ) {
                    const id = msg.id,
                        methodSpec = this.schema.getMethod( msg.method );
                    ...
                    // void call
                        ...
                        this.#call( ws.auth, msg.method, msg.args, true );
                    }

                    // regular call
                    else {
                        const res = await this.#call( ws.auth, msg.method, msg.args );
        ...
        async #onHttpRequest ( req, method ) {
            ...
            var args, res, isBinary, isUpload;
            ...
            try {
                ...
                // get
                    ...
                    try {
                        ...
                        for ( const [name, value] of req.searchParams ) args[name] = value;
                    }
                    catch ( e ) {
                ...
                if ( isUpload ) {
                    ...
                    // check permissions
                    if ( !auth.hasPermissions( methodSpec.permissions ) ) throw result( [403, "Insufficient permissions"] );
                    ...
                    res = await this.#call( auth, method, args );
                }
            }
            catch ( e ) {
        ...
        async #upload ( auth, method, req ) {
            ...
            try {
                ...
                if ( data === undefined ) res = await this.#call( auth, method, [file] );
                else res = await this.#call( auth, method, [file, data] );
            }
            catch ( e ) {
                res = result.catchResult( e );
            }
        ...
        async #call ( auth, method, args, isVoid ) {
