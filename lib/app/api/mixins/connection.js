...
export default Super =>
    class Connection extends ( Super || Object ) {
        ...
        async _onWebsocketEvent ( ws, name, params ) {
            if ( params === undefined ) params = [];
            else if ( !Array.isArray( params ) ) return;

            // re-authenticate
            await ws.auth.authenticate();

            this.app.publish( "user/" + name, ws.auth, ...params );
        }
        ...
        async #onWebsocketMessage ( ws, msg, isBinary ) {
            ...
            if ( msg.method ) {
                ...
                // event
                    ...
                    this._onWebsocketEvent( ws, name, msg.params );
                }
                ...
                // rpc
                    ...
                    if ( method?.upload ) {
                        if ( id ) {
                            this.#wsSend( ws, isBinary, result( [-32600, `Uploads using websockets is not supported`] ).toRPC( id ) );
                        }
                    }
                    ...
                    // regular call
                        ...
                        this.#wsSend( ws, isBinary, res.toRPC( id ) );
                    }
                }
            }
        }
