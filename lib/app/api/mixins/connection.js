...
export default Super =>
    class Connection extends ( Super || Object ) {
        ...
        _onUserEvent ( auth, name, params ) {
            this.app.publish( "user/" + name, auth, ...params );
        }
        ...
        async #onWebsocketMessage ( ws, msg, isBinary ) {
            ...
            if ( msg.method ) {
                ...
                if ( msg.method === "/ping" ) {
                    ...
                    if ( msg.id ) this.#wsSend( ws, isBinary, { "jsonrpc": "2.0", "id": msg.id, "method": "/pong" } );
                }
                ...
                // healthcheck
                    ...
                    if ( msg.id ) this.#wsSend( ws, isBinary, res.toRPC( msg.id ) );
                }
                ...
                // event
                    ...
                    this._onUserEvent( ws.auth, name, msg.params );
                }
                ...
                // rpc
                    ...
                    if ( method?.upload ) {
                        if ( msg.id ) this.#wsSend( ws, isBinary, result( [-32600, `Upload using websockets is not supported`] ).toRPC( msg.id ) );
                    }
                    ...
                    // regular call
                        ...
                        this.#wsSend( ws, isBinary, res.toRPC( msg.id ) );
                    }
        ...
        // XXX
        // XXX if method is upload - but content type is not multipart - error
            ...
            try {
                ...
                if ( req.method === "get" ) {
                    try {
                        ...
                        // XXX make params undef if empty
                        for ( const [name, value] of req.searchParams ) params[name] = value;
                    }
                    catch ( e ) {
                        throw result( -32602 ); // invalid params
                    }
                }
                ...
                if ( methodId === "/event" ) {
                    ...
                    if ( !name ) throw result( 200 );

                    this._onUserEvent( auth, name, params );
                ...
                // rpc
                    ...
                    // void api call
                    if ( voidCall ) {
                        this.#call( auth, methodId, params );
        ...
        async #upload ( req, methodId, token ) {
            ...
            try {
                ...
                // method not found
                if ( !method ) throw result( -32601 );

                // not upload method
                if ( !method.upload ) throw result( -32600 );
                ...
                // XXX get and cache headers before await call
                // req.headers;
                ...
                if ( !file ) throw result( [-32602, `File is required`] );
                ...
                res = await this.#call( auth, methodId, [file, ...params] );
            }
            catch ( e ) {
                res = result.catch( e );
            }
