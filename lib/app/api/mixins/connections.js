...
const LOCAL_EVENTS = new Set( ["connect", "disconnect", "backend/connect", "backend/disconnect"] );
...
class Connection extends Events {
    ...
    disconnect ( res ) {
        ...
        if ( this.isConnected ) return this.#ws.end( res.status, res.statusText );
    ...
    #subscribe ( name ) {
        ...
        this.#api.app.on( ( this.#api.isRpc ? "rpc/out:" : "api/out:" ) + name, this.#eventsListener );
    }
    ...
    #unsubscribe ( name ) {
        ...
        this.#api.app.off( ( this.#api.isRpc ? "rpc/out:" : "api/out:" ) + name, this.#eventsListener );
    }

    #eventsListenerTemplate ( name, args, cache = {}, publisherId ) {
...
export default Super =>
    class Connections extends ( Super || Object ) {
        ...
        async #onWebSocketOpen ( ws, req ) {
            ws.connection = new Connection( this, ws );

            if ( this.isApi ) this.authCache.connect( ws.connection );

            this.app.publish( "api/connect", ws.connection );
        }

        #onWebSocketClose ( ws, status, statusText ) {
            ws.isClosed = true;
            ws.connection.disconnect( result( [status, Buffer.from( statusText ).toString()] ) );

            if ( this.isApi ) this.authCache.disconnect( ws.connection );

            this.app.publish( "api/disconnect", ws.connection );
        }
        ...
        async #onWebSocketMessage ( ws, msg, isBinary ) {
            ...
            if ( msg.method ) {
                ...
                // subscribe
                    ...
                    ws.connection.subscribe( ...msg.params );
                }

                // unsubscribe
                else if ( msg.method === "/unsubscribe" ) {
                    ws.connection.unsubscribe( ...msg.params );
                }
        ...
        #publishRemoteEvent ( name, auth, args ) {
            ...
            if ( this.isRpc ) this.app.publish( "rpc/in:" + name, ...args );
            else this.app.publish( "api/in:" + name, auth, ...args );
        }
    };
