...
class Connection extends Events {
    ...
    onSubscribe ( names ) {
        ...
        for ( const name of names ) {
            ...
            if ( this.#api.isRpc ) {
                this.#subscribe( listeners, name, name );
            }
            else {
                this.#subscribe( listeners, name, `${name}/*` );

                if ( auth.isAuthenticated ) {
                    this.#subscribe( listeners, name, `${name}/user` );
                    if ( auth.isRoot ) this.#subscribe( listeners, name, `${name}/root` );

                    this.#subscribe( listeners, name, `${name}/${auth.userId}` );

                    for ( const [permission, enabled] of Object.entries( auth.permissions ) ) {
                        if ( enabled ) this.#subscribe( listeners, name, `${name}/${permission}` );
                    }
                }
                else {
                    this.#subscribe( listeners, name, `${name}/guest` );
                }
            }
        }
    }
    ...
    // private
    #subscribe ( listeners, name, listenName ) {
        const listener = this.#eventsListener.bind( this, name );

        listeners.push( [listenName, listener] );
...
export default Super =>
    class Connections extends ( Super || Object ) {
        #in = new EventsHub();
        #out = new EventsHub( { "maxListeners": Infinity } );
        ...
        async #onWebSocketOpen ( ws, req ) {
            ...
            // bad credentials, signouy
            if ( ws.badCredentials ) return ws.end( 4000 );
