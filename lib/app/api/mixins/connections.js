        ...
        for ( const name of names ) {
            ...
            if ( this.#api.isRpc ) {
                ...
                if ( auth.isAuthenticated ) {
                    this.#subscribe( `${name}/user` );
                    if ( auth.isRoot ) this.#subscribe( `${name}/root` );

                    this.#subscribe( `${name}/${auth.userId}` );

                    for ( const permission in auth.permissions ) {
                        if ( auth.permissions[permission] ) this.#subscribe( `${name}/${permission}` );
                    }
                }
                else {
                    this.#subscribe( `${name}/guest` );
                }
            }
        }
    }
