...
class Connection extends Events {
    ...
    #subscribe ( name ) {
        ...
        this.#out.on( name, this.#eventsListener );
    }
    ...
    #unsubscribe ( name ) {
        ...
        this.#out.off( name, this.#eventsListener );
    }
    ...
    class Connections extends ( Super || Object ) {
        ...
        publish ( name, ...args ) {
            if ( name.endsWith( "/" ) ) {
                ...
                for ( const target of targets ) {
                    ...
                    this.#out.publish( targetName, targetName, args, cache );
                }
            }
            else {
                this.#out.publish( name, name, args, {} );
            }
        }

        forwardEvent ( name, args, cache, publisherId ) {
            this.#out.publish( name, name, args, cache, publisherId );
        }

        forwardSubscriptions ( target, { on, off, listener } = {} ) {
            listener ||= ( name, ...args ) => this.#out.publish( name, name, args, {} );
        ...
        async #onWebSocketMessage ( ws, msg, isBinary ) {
            ...
            if ( msg.method ) {
                ...
                // publish
                    ...
                    this.#publishRemoteEvent( ws.auth, msg.params );
                }
        ...
        async #onHttpRequest ( req, methodId ) {
            ...
            try {
                ...
                // publish
                if ( methodId === "/publish" ) {
                    this.#publishRemoteEvent( auth, params );
