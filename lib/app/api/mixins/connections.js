...
export default Super =>
    class Connections extends ( Super || Object ) {
        ...
        getHttpServerConfig () {
            return {
                "ws": {
                    ...
                    "maxBackpressure": 0,
                    "upgrade": this.#onWebSocketUpgrade.bind( this ), // res, req, context
                    "open": this.#onWebSocketOpen.bind( this ),
                    "close": this.#onWebSocketClose.bind( this ),
                    "message": this.#onWebSocketMessage.bind( this ),

                    // "maxPayloadLength": 16 * 1024,
                    // "idleTimeout": 120, // seconds, 0 - disable
                    // "sendPingsAutomatically": true, // sends pings every idleTimeout - 2 secinds, disabled, if idleTimeout is 0
        ...
        async #onWebSocketOpen ( ws ) {

            // backend is down
            if ( !ws.auth ) return ws.end( 4503 );

            // token is invalid
            if ( !ws.auth.isValid ) return ws.end( 4401 );

            ws.connection = new Connection( this, ws, this.#out );

            // register authenticated connection
            if ( ws.auth.isAuthenticated ) this.authCache.registerConnection( ws.connection );
