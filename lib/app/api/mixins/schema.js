...
export default Super =>
    class Schema extends ( Super || Object ) {
        ...
        async _init ( options ) {
            ...
            res = await this.#loadMethods( options.methods );
            console.log( res + "" );
            if ( !res.ok ) return res;
        ...
        async #loadMethods ( url ) {
            ...
            for ( const methodId in schema ) {
                ...
                const res = this._validateSchemaPermissions( schema[methodId].permissions );
        ...
        _validateSchemaPermissions ( permissions ) {
            return this.validateApiPermissions( permissions );
        }
    };
