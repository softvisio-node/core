...
const QUERIES = {
    "getObjectUserRole": sql`SELECT role FROM object_user WERE user_id = ? AND object_id = ?`.prepare(),

    "upsertObjectUser": sql`
INSERT INTO object_user ( user_id, object_id, role ) VALUES ( ?, ?, ? )
ON CONFLICT ( user_id, object_id ) DO UPDATE SET role = ?
`.prepare(),

    "deleteObjectUser": sql`DELETE FROM object_user WHERE user_id = ? AND object_id = ?`.prepare(),
...
WHERE object_user.user_id = "user".id AND object.object_id = ?
`.prepare(),
};
...
export default Super =>
    class extends ( Super || Object ) {
        ...
        async getObjectUserRole ( objectId, userId ) {
            ...
            var role = this.#cache.get( cacheKey );

            if ( role ) return role;

            const res = await this.dbh.selectRow( QUERIES.getObjectUserRole, [userId, objectId] );
        ...
        async setObjectUserRole ( objectId, userId, role, options ) {
            const objectType = this.#getObjectType( objectId );
            ...
            return ( options?.dbh || this.dbh ).do( QUERIES.upsertObjectUser, [userId, objectId, role, role] );
        }

        async deleteObjectUser ( objectId, userId, options ) {
            return ( options?.dbh || this.dbh ).do( QUERIES.deleteObjectUser, [userId, objectId] );
        }
        ...
        async isObjectUserCanEditRoles ( objectId, userId ) {
            ...
            return this.#objects[objectType].edit.has( role );
        }
        ...
        async getObjectUsers ( objectId ) {
            ...
            if ( res.data ) {
                ...
                res.data = res.data
                    .filter( row => roles[row.role] )
                    .map( row => {
                        row.role_name = roles[row.role].name;
                        row.role_description = roles[row.role].decription;
        ...
        async #init ( _objects ) {
            ...
            for ( const type in _objects ) {
                ...
                for ( const roleId in object.roles ) {
                    ...
                    if ( role.edit ) objects[objectId].edit.add( roleId );
                ...
                if ( !objects[objectId].edit.size ) return result( [500, `Object type "${type}" has no roles with edit permission`] );
            }
