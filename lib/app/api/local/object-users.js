...
export default Super =>
    class extends ( Super || Object ) {
        ...
        async checkObjectPermissions ( objectId, objectResolverType, userId, methodId ) {
            ...
            const role = await this.dbh.select( sql`SELECT role FROM object_user WHERE object_id = ? AND object_user_id = ?`, [objectId, userId] );

            if ( !role.data?.role ) return;

            return this.#objects[res.data.type]?.roles?.[role.data.role]?.permissions?.[methodId];
        }

        // XXX remove from context
