...
export default Super =>
    class extends ( Super || Object ) {
        ...
        // public
        async checkAclPermissions ( objectId, userId, method, aclType ) {
            if ( this.userIsRoot( userId ) ) return true;

            // resolve object id
            objectId = await this.#resolveObjectId( objectId, aclType );
        ...
        async getAclPermissions ( userId, aclId, aclIdType ) {
            ...
            if ( !this.#cache ) {
                ...
                for ( const aclIdType in this.#types ) {
                    if ( this.#types[aclIdType].roles ) continue;

                    for ( const resolvedAclIdType in this.#types ) {
                        if ( !this.#types[resolvedAclIdType].roles ) continue;

                        for ( const role in this.#types[resolvedAclIdType].roles ) {
                            const id = `${aclIdType}/${resolvedAclIdType}/${role}`;

                            const permissions = {};

                            for ( const method of Object.values( this.schema.methods ) ) {
                                if ( !method.aclTypes ) continue;

                                if ( ( method.aclTypes.has( "acl" ) && this.#types[resolvedAclIdType].roles ) || method.aclTypes.has( aclIdType ) ) {
                                    if ( this.#matchPermissions( method, this.#types[resolvedAclIdType].roles[role].permissions ) ) {
                ...
                await this.dbh.do( sql`INSERT INTO acl_type_permissions`.VALUES( Object.values( this.#cache ) ).sql`ON CONFLICT ( acl_type_id, acl_type_role_id ) DO UPDATE SET permissions = EXCLUDED.permissions` );
            }

            // resolve acl id
            aclId = await this.#resolveObjectId( aclId, aclIdType );
            if ( !aclId ) return {};

            const resolvedAclIdType = await this.#getAclObjectType( aclId );
            if ( !resolvedAclIdType ) return {};

            // root user
            if ( this.userIsRoot( userId ) ) {
                return Object.assign( {}, ...Object.keys( this.#types[resolvedAclIdType].roles ).map( role => this.#cache[`${aclIdType}/${resolvedAclIdType}/${role}`] ) );
            }
            ...
            // non-root user
                ...
                return Object.assign( {}, ...[...user.roles].map( role => this.#cache[`${aclIdType}/${resolvedAclIdType}/${role}`].permissions ) );
            }
        }
        ...
        async #init ( acl ) {
            ...
            for ( const [type, spec] of Object.entries( acl ) ) {
                ...
                if ( this.#types[type] ) throw `ACL type "${type}" is alredy registered`;

                this.#types[type] = {
                    "resolver": spec.resolver ? sql( spec.resolver ).prepare() : null,
                    "roles": spec.roles,
                };
            }
        ...
        async #resolveObjectId ( aclId, aclIdType ) {
            const resolver = this.#types[aclIdType].resolver;

            if ( !resolver ) return aclId;

            const cacheId = `${aclIdType}/${aclId}`;

            var resolvedAclId = this.#objectResolverCache.get( cacheId );

            if ( resolvedAclId ) return resolvedAclId;

            const mutex = this.#mutexSet.get( `resolve/${cacheId}` );
            if ( !mutex.tryDown() ) return mutex.signal.wait();

            const res = await this.dbh.selectRow( resolver, [aclId] );

            resolvedAclId = res.data?.id;

            if ( resolvedAclId ) this.#objectResolverCache.set( cacheId, resolvedAclId );

            mutex.signal.broadcast( resolvedAclId );
            mutex.up();

            return resolvedAclId;
        }
