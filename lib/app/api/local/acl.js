    ...
    class extends ( Super || Object ) {
        ...
        async getAclPermissions ( userId, aclId, aclIdType ) {
            ...
            if ( this.userIsRoot( userId ) ) {
                return Object.assign( {}, ...Object.keys( this.#types[resolvedAclIdType].roles ).map( role => this.#objectPermissions[`${aclIdType}/${resolvedAclIdType}/${role}`] ) );
            }
            ...
            // non-root user
                ...
                return Object.assign( {}, ...[...user.roles].map( role => this.#objectPermissions[`${aclIdType}/${resolvedAclIdType}/${role}`].permissions ) );
            }
        }
        ...
        async #init ( acl ) {
            ...
            // XXX
            {
                    ...
                    for ( const aclType in this.#types ) {
                        for ( const role in this.#types[aclType].roles ) {
                            ...
                            for ( const method of Object.values( this.schema.methods ) ) {
                                ...
                                if ( method.aclObjectTypes.has( aclObjectType ) || ( this.#objectTypes[aclObjectType].isAclObject && method.aclObjectTypes.has( "acl" ) ) ) {
                ...
                console.log( this.#objectPermissions );
                process.exit();

                await this.dbh.do( sql`INSERT INTO acl_type_permissions`.VALUES( Object.values( this.#objectPermissions ) ).sql`ON CONFLICT ( acl_type_id, acl_type_role_id ) DO UPDATE SET permissions = EXCLUDED.permissions` );
            }
