            ...
            const res = await this.dbh.begin( async dbh => {
                ...
                // update object permissions
                {
                    if ( objectPermissions.length ) {
                        res = await this.dbh.do( sql`INSERT INTO acl_object_permissions`.VALUES( objectPermissions ).sql`ON CONFLICT ( acl_object_id, acl_role_id ) DO UPDATE SET permissions = EXCLUDED.permissions` );
                        if ( !res.ok ) throw res;
                    }
                }
            } );
