...
SELECT
    ...
    get_acl_permissions( acl_user.acl_id, acl_user.user_id, 'acl' ) AS acl_permissions,
    CASE
...
export default Super =>
    class extends ( Super || Object ) {
        ...
        async getAcl ( aclId, userId ) {
            return this.dbh.selectRow( sql`SELECT get_acl_permissions( ?, ?, 'acl' ) AS acl_permissions`, [aclId, userId] );
        }
        ...
        async getAclUsers ( aclId, { parentUserId } ) {
            ...
            const res = await this.dbh.select( QUERIES.getUsers, [
                ...
                //
                "?d=" + this.app.config.defaultGravatarImage,
                this.app.config.defaultGravatarUrl,
                aclId,
            ] );
            ...
            if ( res.data ) {
                ...
                for ( const row of res.data ) {
                    row.readonly = parentUserId ? row.id === parentUserId : false;
        ...
        // protected
        async _init ( options ) {
            this.#aclIdCache = new CacheLru( { "maxSize": this.app.config.objectUserCacheMaxSize } );
            this.#aclUserUserCache = new CacheLru( { "maxSize": this.app.config.objectUserCacheMaxSize } );
            this.#aclObjectIdCache = new CacheLru( { "maxSize": this.app.config.objectUserCacheMaxSize } );
        ...
        async #init ( acl ) {
                ...
                for ( const [aclType, spec] of Object.entries( acl.types ) ) {
                    ...
                    if ( spec.roles ) {
                        for ( const [role, roleSpec] of Object.entries( spec.roles ) ) {
                            ...
                            PERMISSON: for ( const permission in roleSpec.permissions ) {
                                for ( const method of Object.values( this.schema.methods ) ) {
                                    if ( this.#matchPermissionsList( method, [permission] ) ) {
                                        continue PERMISSON;
                                    }
                                }

                                return result( [500, `ACL type "${aclType}", role "${role}", permission mask "${permission}" does not match any API method`] );
                            }
            ...
            // calculate object root permissions
            {
                for ( const aclObjectType in this.#objectTypes ) {
                    const permissions = {};
            ...
            const res = await this.dbh.begin( async dbh => {
                ...
                // update object root permissions
                {
                    if ( objectRootPermissions.length ) {
                        res = await dbh.do( sql`INSERT INTO acl_object_root_permissions`.VALUES( objectRootPermissions ).sql`ON CONFLICT ( acl_object_id ) DO UPDATE SET permissions = EXCLUDED.permissions` );
                        if ( !res.ok ) throw res;
                    }
                }
            } );
        ...
        async #resolveAclId ( aclObjectId, aclObjectType ) {
            if ( aclObjectType === ACL_OBJECT_TYPE ) return aclObjectId;
