...
export default Super =>
    class extends ( Super || Object ) {
        ...
        async checkAclPermissions ( objectId, userId, method, aclType ) {
            ...
            // resolve object id
            objectId = await this.#resolveObjectId( objectId, aclType, method );
        ...
        async getAclPermissions ( userId, objectId, aclType, method ) {

            // pre-calc permissions
            if ( !this.#cache ) {
                this.#cache = {};

                for ( const aclType in this.#resolvers ) {
                    for ( const type in this.#types ) {
                        for ( const role in this.#types[type].roles ) {
                            const id = `${aclType}/${type}/${role}`;
                            ...
                            for ( const method of Object.values( this.schema.methods ) ) {
                                ...
                                if ( method.aclTypes.has( "acl" ) || method.aclTypes.has( aclType ) ) {
            ...
            const p = Object.assign( {}, ...Object.keys( this.#types[type].roles ).map( role => this.#cache[`${aclType}/${type}/${role}`] ) );
        ...
        async #resolveObjectId ( id, type, method ) {
            if ( type === "acl" ) return id;

            const resolver = this.#resolvers[type];

            if ( !resolver ) throw `ACL type "${type}" is not registered`;
