...
const QUERIES = {
    "getAclObjectType": sql`SELECT acl_type.type FROM acl_type, acl_object WHERE acl_object.acl_type_id = acl_type.id AND acl_object.id = ?`.prepare(),
...
export default Super =>
    class extends ( Super || Object ) {
        ...
        async setAclUserRoles ( objectId, userId, { dbh, parentUserId, enabled, roles = [] } = {} ) {
            ...
            const res = await dbh.do( QUERIES.upsertObjectUser, [objectId, userId, enabled, _roles.size ? [..._roles] : null] );
        ...
        async setAclUserRoleEnabled ( objectId, userId, role, enabled, { dbh, parentUserId } = {} ) {
            ...
            const res = await dbh.do( QUERIES.setAclUserRoles, [roles.size ? [...roles] : null, objectId, userId] );
            ...
            if ( types.size ) {
                ...
                if ( types.size ) {
                    ...
                    const res = await this.dbh.begin( async dbh => {
                        ...
                        // set transactio level lock
                        var res = await dbh.selectRow( sql`SELECT pg_advisory_xact_lock(${constants.locks.migration})` );
                        if ( !res.ok ) throw res;

                        res = await dbh.do( sql`INSERT INTO acl_type`.VALUES( [...types].map( type => ( { type } ) ) ).sql`ON CONFLICT ( type ) DO NOTHING` );
                        if ( !res.ok ) throw res;
