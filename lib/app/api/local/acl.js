...
export default Super =>
    class extends ( Super || Object ) {
        ...
        // public
        async checkAclPermissions ( userId, aclObjectId, aclObjectType, method ) {
            if ( this.userIsRoot( userId ) ) return true;

            // resolve object id
            const aclId = await this.#resolveAckId( aclObjectId, aclObjectType );
            if ( !aclId ) return false;

            const aclType = await this.#getAclObjectType( aclId );
        ...
        async getAclPermissions ( userId, aclId, aclIdType ) {
            ...
            // resolve acl id
            aclId = await this.#resolveAckId( aclId, aclIdType );
            if ( !aclId ) return {};
        ...
        async #resolveAckId ( aclObjectId, aclObjectType ) {
            const resolver = this.#types[aclObjectType].resolver;
