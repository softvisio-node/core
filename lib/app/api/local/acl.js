...
const QUERIES = {
    "getObjectType": sql`SELECT acl_type.type FROM acl_type, acl_registry WHERE acl_registry.acl_type_id = acl_type.id AND acl_registry.id = ?`.prepare(),

    "getObjectUser": sql`SELECT enabled, roles FROM acl WHERE object_id = ? AND user_id = ?`.prepare(),
...
`.prepare(),
    ...
    "deleteObjectUser": sql`DELETE FROM acl WHERE object_id = ? AND user_id = ?`.prepare(),

    "setObjectUserEnabled": sql`UPDATE acl SET enabled = ? WHERE object_id = ? AND user_id = ?`.prepare(),

    "setObjectUserRoles": sql`UPDATE acl SET roles = ? WHERE object_id = ? AND user_id = ?`.prepare(),
...
export default Super =>
    class extends ( Super || Object ) {
        ...
        async setObjectUserRoleEnabled ( objectId, userId, role, enabled, { dbh, parentUserId } = {} ) {
            dbh ||= this.dbh;
            ...
            const res = await dbh.do( QUERIES.setObjectUserRoles, [[...roles], objectId, userId] );
