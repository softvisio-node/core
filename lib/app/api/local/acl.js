...
export default Super =>
    class extends ( Super || Object ) {
        ...
        // XXX root user
        // XXX do not add acl to all objects
        async getAclPermissions ( userId, aclId, aclIdType ) {
            ...
            if ( !this.#cache ) {
                ...
                for ( const aclIdType in this.#resolvers ) {
                    for ( const type in this.#types ) {
                        for ( const role in this.#types[type].roles ) {
                            const id = `${aclIdType}/${type}/${role}`;
                            ...
                            for ( const method of Object.values( this.schema.methods ) ) {
                                ...
                                if ( method.aclTypes.has( "acl" ) || method.aclTypes.has( aclIdType ) ) {
                                    if ( this.#matchPermissions( method, this.#types[type].roles[role].permissions ) ) {
            ...
            const type = await this.#getAclObjectType( aclId );
            if ( !type ) return {};

            const user = await this.#getAclUser( aclId, userId );
            if ( !user || !user.enabled ) return {};

            const p = Object.assign( {}, ...Object.keys( this.#types[type].roles ).map( role => this.#cache[`${aclIdType}/${type}/${role}`] ) );
        ...
        async #init ( acl ) {
            ...
            for ( const [type, spec] of Object.entries( acl ) ) {
                ...
                // types
                for ( const [type, resolver] of Object.entries( spec.types ) ) {
                    if ( this.#resolvers[type] ) throw `ACL type "${type}" already exists`;

                    if ( resolver === true ) {
                        this.#resolvers[type] = true;
                    }
                    else {
                        this.#resolvers[type] = sql( resolver ).prepare();
                    }
                }
            }
        ...
        async #resolveObjectId ( aclId, aclIdType ) {
            if ( aclIdType === "acl" ) return aclId;

            const resolver = this.#resolvers[aclIdType];

            if ( !resolver ) throw `ACL type "${aclIdType}" is not registered`;
