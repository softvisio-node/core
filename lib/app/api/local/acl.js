...
WHERE
    acl_id = ?
    AND user_id = ?
    AND acl_type_id = ( SELECT acl_role.id FROM acl_role, acl_type WHERE acl_role.acl_type_id = acl_type.id AND acl_type.type = ? AND acl_role.role = ? )
`.prepare(),
};
                ...
                if ( roles.size ) {
                    ...
                    for ( const role of roles ) {
                        values.push( {
                            "acl_id": aclId,
                            "user_id": userId,
                            "acl_type_role_id": sql`SELECT acl_role.id FROM acl_role, acl_type WHERE acl_role.acl_type_id = acl_type.id AND acl_type.type = ${aclType} AND acl_role.role = ${role}`,
                        } );
                    }
