...
const { TOKEN_TYPE_EMAIL, TOKEN_TYPE_PASSWORD_RECOVERY } = require( "../../../const" );
...
module.exports = mixin( ( Super ) =>
    class extends Super {
            ...
            async confirmUserActionTokenEmail ( token ) {
                token = await this.verifyUserActionToken( token, TOKEN_TYPE_EMAIL );

                if ( !token.isOk() ) return token;

                return await this.#dbh.begin( async ( dbh ) => {
                    var res = await this.removeUserActionToken( TOKEN_TYPE_EMAIL, token.data.email, dbh );
            ...
            async setUserActionTokenPassword ( token, password ) {
                token = await this.verifyUserActionToken( token, TOKEN_TYPE_PASSWORD_RECOVERY );
                ...
                return await this.#dbh.begin( async ( dbh ) => {
                    ...
                    res = await this.removeUserActionToken( TOKEN_TYPE_PASSWORD_RECOVERY, token.data.email, dbh );

                    if ( !res.isOk() ) throw res;

                    res = await dbh.do( q.setEmailConfirmaed, token.data.userId );
