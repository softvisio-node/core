...
const DEFAULT_MAX_AGE = 1000 * 60 * 60 * 24; // 1 day
...
const QUERIES = {
    ...
    "verify": sql`SELECT user_action_token.*, user_action_token_hash.hash FROM user_action_token, user_action_token_hash WHERE user_action_token.id = user_action_token_hash.user_action_token_id AND user_action_token.id = ? AMD user_action_token.expires > CURRENT_TIMESTAMP`.prepare(),
    ...
    class extends ( Super || Object ) {
        async createUserActionToken ( userId, tokenType, options = {} ) {
            ...
            var res = await dbh.begin( async dbh => {
                const expires = new Date( Date.now() + DEFAULT_MAX_AGE );
        ...
        async confirmUserActionTokenEmail ( token, options = {} ) {
            ...
            return await dbh.begin( async dbh => {
                ...
                var res = await this.removeUserActionTokens( constants.tokenTypeEmailConfirmation, token.data.email, { dbh } );
        ...
        async setUserActionTokenPassword ( token, password, options = {} ) {
            ...
            return await dbh.begin( async dbh => {
                ...
                res = await this.removeUserActionTokens( constants.tokenTypePasswordReset, token.data.email, { dbh } );

                if ( !res.ok ) throw res;

                // remove all email confirmation tokens
                res = await this.removeUserActionTokens( constants.tokenTypeEmailConfirmation, token.data.email, { dbh } );
