...
const QUERIES = {
    "get": sql`SELECT "id", "email", "emailConfirmed" FROM "user" WHERE "enabled" = TRUE AND ("name" = ? OR "email" = ?)`.prepare(),
    "insertHash": sql`INSERT INTO "userActionTokenHash" ("userActionTokenId", "hash") VALUES (?, ?)`.prepare(),
    "insertToken": sql`INSERT INTO "userActionToken" ("userId", "type", "email") VALUES (?, ?, ?) RETURNING "id"`.prepare(),
    "verify": sql`SELECT "userActionToken".*, "userActionTokenHash"."hash" FROM "userActionToken", "userActionTokenHash" WHERE "userActionToken"."id" = "userActionTokenHash"."userActionTokenId" AND "userActionToken"."id" = ?`.prepare(),
    "remove": sql`DELETE FROM "userActionToken" WHERE "type" = ? AND email = ?`.prepare(),
    "setEmailConfirmed": sql`UPDATE "user" SET "emailConfirmed" = TRUE WHERE "id" = ?`.prepare(),
};
...
export default Super =>
    class extends ( Super || Object ) {
        async createUserActionToken ( userId, tokenType, options = {} ) {
            ...
            if ( tokenType === CONST.AUTH_EMAIL_CONFIRM && user.data.emailConfirmed ) return result( 400, "User email already confirmed" );

            var res = await dbh.begin( async dbh => {

                // insert token
                let res = await dbh.selectRow( QUERIES.insertToken, [user.data.id, tokenType, user.data.email] );
