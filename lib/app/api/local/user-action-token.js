    ...
    class extends Super {
        async createUserActionToken ( userId, tokenType, options = {} ) {
            ...
            var res = await dbh.begin( async dbh => {
                ...
                // get token id
                const id = await dbh.selectRow( QUERIES.generateId, [tokenType] );

                // dbh error
                if ( !id.ok ) throw id;

                // generate token
                const token = this._generateToken( tokenType, id.data.id );

                // insert token
                res = await dbh.do( QUERIES.insertToken, [token.id, user.data.id, token.type, user.data.email] );
