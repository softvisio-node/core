...
`.prepare(),
    ...
    "getSession": sql`SELECT last_updated, remote_address, user_agent FROM user_session WHERE id = ?`.prepare(),
};
...
export default Super =>
    class extends ( Super || Object ) {
        ...
        async checkSessionAuthorization ( sessionId, remoteAddress, userAgent ) {
            ...
            if ( Date.parse( session.data.last_updated ) + this.app.config.authorizedSessionMaxAge < Date.now() ) return result( -32812 );
