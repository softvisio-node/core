...
`.prepare(),
    ...
    "deleteDeviceSessions": sql`DELETE FROM user_session WHERE user_id = ? AND device_guid = ?`.prepare(),
...
`.prepare(),
    ...
    "createTrustedSession": sql`INSERT INTO user_trusted_session ( user_session_id, expires, user_agent, remote_address ) VALUES ( ?, ?, ?, ? ) ON CONFLICT ( user_session_id ) DO UPDATE SET expires = EXCLUDED.expires, user_agent = EXCLUDED.user_agent, remote_address = EXCLUDED.remote_address`.prepare(),

    "getTrustedSession": sql`SELECT remote_address, user_agent FROM user_trusted_session WHERE user_session_id = ? AND expires >= CURRENT_TIMESTAMP`.prepare(),
};
...
export default Super =>
    class extends ( Super || Object ) {
        ...
        async createUserSession ( userId, { deviceGuid, userAgent, remoteAddress, dbh } = {} ) {
            ...
            var res = await dbh.begin( async dbh => {
                ...
                if ( deviceGuid ) {
                    ...
                    res = await dbh.do( QUERIES.deleteDeviceSessions, [userId, deviceGuid] );
        ...
        async createTrustedSession ( sessionId, remoteAddress, userAgent ) {
            const expires = new Date( Date.now() + this.app.config.trustedSessionMaxAge );

            return this.dbh.do( QUERIES.createTrustedSession, [
        ...
        async checkTrustedSession ( sessionId, remoteAddress, userAgent ) {
            const session = await this.dbh.selectRow( QUERIES.getTrustedSession, [sessionId] );
