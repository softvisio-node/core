...
`.prepare(),
    ...
    "getTrustedSession": sql`SELECT last_updated, remote_address, user_agent FROM user_session WHERE id = ?`.prepare(),
};
...
export default Super =>
    class extends ( Super || Object ) {
        ...
        async checkTrustedSession ( sessionId, remoteAddress, userAgent ) {
            ...
            // trusted session is expired
            if ( Date.parse( session.data.last_updated ) + this.app.config.trustedSessionMaxAge < Date.now() ) return result( -32812 );

            // trusted session signature is invalid
