...
SELECT
    ...
    CASE
        WHEN user_session.id = ? THEN TRUE
        ELSE FALSE
...
`.prepare(),
    ...
    "createTrustedSession": sql`INSERT INTO user_trusted_session ( id, expires, user_agent, remote_address ) VALUES ( ?, ?, ?, ? ) ON CONFLICT ( id ) DO UPDATE SET expires = EXCLUDED.expires, user_agent = EXCLUDED.user_agent, remote_address = EXCLUDED.remote_address`.prepare(),

    "getTrustedSession": sql`SELECT remote_address, user_agent FROM user_trusted_session WHERE id = ? AND expires >= CURRENT_TIMESTAMP`.prepare(),
};
...
export default Super =>
    class extends ( Super || Object ) {
        ...
        // XXX return trusted session is required status
        async checkTrustedSession ( sessionId, remoteAddress, userAgent ) {
            ...
            if ( session.data?.remote_address !== remoteAddress + "" || session.data?.user_agent !== userAgent + "" ) return result( 400 );
