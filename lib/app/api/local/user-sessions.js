...
`.prepare(),
    ...
    "deleteDeviceSessions": sql`DELETE FROM user_session WHERE user_device_id = ?`.prepare(),
        ...
        async createUserSession ( userId, { deviceGuid, userAgent, remoteAddress, dbh } = {} ) {
            ...
            var res = await dbh.begin( async dbh => {
                ...
                if ( deviceGuid ) {
                    ...
                    // upsert device
                    let res = await dbh.selectRow( QUERIES.upsertDevice, [
                    ...
                    res = await dbh.do( QUERIES.deleteDeviceSessions, [deviceId] );
                    if ( !res.ok ) throw res;
                }
