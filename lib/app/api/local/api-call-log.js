...
module.exports = mixin( Super =>
    class extends Super {
            ...
            async init ( options = {} ) {
                if ( CLEANUP_INTERVAL ) {
                    setInterval( async () => {
                        ...
                        await this.dbh.do( `DELETE FROM "api_call_log" WHERE "started" < CURRENT_TIMESTAMP - ? * INTERVAL '1 DAY'`, [CLEANUP_INTERVAL] );
                    }, 1000 * 60 * 60 * 24 );
                }

                if ( CACHE_DROP_INTERVAL ) setInterval( () => this._dropApiCallLogCache(), CACHE_DROP_INTERVAL );
            ...
            logApiCallLoad ( data ) {
                ...
                if ( !CACHE_DROP_INTERVAL ) this._dropApiCallLogCache();
            }
