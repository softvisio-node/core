...
export default Super =>
    class extends ( Super || Object ) {
        ...
        async _new ( options = {} ) {
            ...
            if ( this.app.settings.permissions ) {
                process.stdout.write( "Loading user permissions ... " );
                res = await this.#init( this.app.settings.permissions );
                console.log( res + "" );
        ...
        async #init ( permissions ) {
            ...
            for ( const id in permissions ) {
                ...
                if ( !isSnakeCase( id ) ) return result( [400, `Permission name "${id}" must be in the snake_case`] );
