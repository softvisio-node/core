    ...
    class extends ( Super || Object ) {
        ...
        async #init ( objects ) {
            ...
            for ( const objectType in objects ) {
                ...
                if ( !isSnakeCase( objectType ) ) return result( [400, `Object type name "${objectType}" must be in the snake_case`] );
                ...
                for ( const permission in object.permissions ) {
                    if ( !isSnakeCase( permission ) ) return result( [400, `Permission name "${permission}" for object type "${objectType}" must be in the snake_case`] );
