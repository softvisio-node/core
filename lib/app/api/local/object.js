...
const QUERIES = {
    "get": sql`SELECT "enabled", "permissions" FROM "objectPermissions" WHERE "objectId" = ? AND "userId" = ?`.prepare(),
    "set": sql`INSERT INTO "objectPermissions" ("objectId", "userId", "permissions") VALUES (?, ?, ?) ON CONFLICT ("objectId", "userId") DO UPDATE SET "permissions" = ?`.prepare(),
    "remove": sql`DELETE FROM "objectPermissions" WHERE "objectId" = ? AND "userId" = ?`.prepare(),
    "setEnabled": sql`UPDATE "objectPermissions" SET "enabled" = ? WHERE "objectId" = ? AND "userId" = ? AND "enabled" = ?`.prepare(),
    "getUsers": sql`
        SELECT
            "objectPermissions"."objectId" AS "objectId",
            "objectPermissions"."userId" AS "userId",
            "objectPermissions"."enabled" AS "enabled",
            "objectPermissions"."permissions" AS "permissions",
        ...
        WHERE
            "objectId" = ?
            AND "objectPermissions"."userId" = "user"."id"
    `.prepare(),
};
    ...
    class extends ( Super || Object ) {
        ...
        async _init ( options ) {
            ...
            if ( options.objects ) {
                ...
                res = await this.#init( options.objects );
                console.log( res + "" );
        ...
        async #init ( objects ) {
            ...
            for ( const objectType in objects ) {
                ...
                if ( !isValidCamelCase( objectType ) ) return result( [400, `Object type name "${objectType}" is invalid`] );
                ...
                for ( const permission in object.permissions ) {
                    if ( !isValidCamelCase( permission ) ) return result( [400, `Permission name "${permission}" for object type "${objectType}" is invalid`] );
            ...
            // setup dbh events
            this.dbh.on( "event/api/invalidate-object-permissions", cacheId => this.#cache.invalidate( cacheId ) );
