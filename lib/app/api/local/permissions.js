                ...
                // non-root user
                    ...
                    if ( userPermissions ) {
                        for ( const permission in userPermissions ) {
                            ...
                            if ( permission in permissions ) permissions[permission] = userPermissions[permission];
                        }
                    }
                }

                // apply token permissions
                if ( tokenPermissions ) {
                    for ( const permission in tokenPermissions ) {

                        // override only permissions, which are exists and enabled
                        if ( permissions[permission] ) permissions[permission] = tokenPermissions[permission];
                    }
                }
