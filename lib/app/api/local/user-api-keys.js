...
const QUERIES = {
    ...
    "getUserToken": sql`SELECT * FROM user_api_key WHERE id = ?`.prepare(),
    "getUserTokenWithRoles": sql`SELECT user_api_key.*, "user".roles AS user_roles FROM user_api_key, "user" WHERE user_api_key.id = ? AND user_api_key.user_id = "user".id`.prepare(),
};
...
export default Super =>
    class extends ( Super || Object ) {
        ...
        async deleteUserToken ( tokenId, options = {} ) {
            const dbh = options.dbh || this.dbh;
