...
BEGIN
    ...
    FOR row IN SELECT * FROM new_table GROUP BY new_table.acl_id LOOP
...
BEGIN
    ...
    IF NOT EXISTS ( SELECT FROM acl WHERE id = old_table.id ) THEN
        RETURN NULL;
    END IF;

    FOR row IN SELECT * FROM old_table GROUP BY old_table.acl_id LOOP
