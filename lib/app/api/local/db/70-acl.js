...
BEGIN
    RETURN ( SELECT json_agg( acl.user_id ) FROM acl WHERE acl_user.acl_id = _acl_id AND acl.enabled );
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    ...
    RETURN get_acl_permissions( ( SELECT id FROM acl_object WHERE type = _acl_object_type ) );

END;
$$ LANGUAGE plpgsql;

CREATE or replace FUNCTION get_acl_permissions ( _acl_id int8, _user_id int8, _acl_object_type_id int8 ) RETURNS json STABLE AS $$
DECLARE
    row record;
    res jsonb = '{}'::jsonb;
BEGIN
...
CREATE FUNCTION user_acl_role_after_insert_trigger() RETURNS TRIGGER AS $$
DECLARE
    row record;
BEGIN
