...
BEGIN
    ...
    IF NEW.enabled = FALSE THEN
        DELETE FROM user_session_key WHERE user_id = NEW.id;
    END IF;
...
CREATE FUNCTION user_password_hash_hash_after_update_trigger() RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER user_password_hash_hash_after_update AFTER UPDATE OF hash ON user_password_hash FOR EACH ROW EXECUTE FUNCTION user_password_hash_hash_after_update_trigger();
...
BEGIN
    PERFORM pg_notify( 'api/user-roles/update', json_build_object( 'user_id', NEW.id::text )::text );
