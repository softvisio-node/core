...
BEGIN
    IF NOT EXISTS ( SELECT FROM api_call_log_updated WHERE type = 'latest' AND last_updated IS NOT NULL AND last_updated > CURRENT_TIMESTAMP - INTERVAL '10 seconds' ) AND pg_try_advisory_xact_lock( get_advisory_lock_id( 'api/call-log/latest/generate' ) ) THEN
...
BEGIN
    IF NOT EXISTS ( SELECT FROM api_call_log_updated WHERE type = 'historic' AND last_updated IS NOT NULL AND last_updated > CURRENT_TIMESTAMP - INTERVAL '10 seconds' ) AND pg_try_advisory_xact_lock( get_advisory_lock_id( 'api/call-log/historic/generate' ) ) THEN
