...
CREATE TABLE user_api_key_hash (
    user_api_key_id int8 PRIMARY KEY REFERENCES user_token ( id ) ON DELETE CASCADE,
    hash text NOT NULL
);

CREATE FUNCTION user_token_invalidate_trigger() RETURNS TRIGGER AS $$
BEGIN
    PERFORM pg_notify( 'api/invalidate-user-key', json_build_object( 'token_type', ${constants.tokenTypeUserToken}, 'token_id', OLD.id::text )::text );
