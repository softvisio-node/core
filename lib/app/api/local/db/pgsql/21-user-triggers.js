...
BEGIN
    ...
    DELETE FROM "userSession" WHERE "userId" = OLD."id";
...
BEGIN
    PERFORM pg_notify('api/invalidate-user', OLD."userId"::text);

    RETURN NULL;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER "userHash_hash_after_update_trigger" AFTER UPDATE OF "hash" ON "userHash" FOR EACH ROW EXECUTE PROCEDURE user_invalidate_hash_trigger();
