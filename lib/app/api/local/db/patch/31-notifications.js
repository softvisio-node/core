...
CREATE FUNCTION user_notification_type_trigger() RETURNS TRIGGER AS $$
BEGIN

    -- deleter
    IF NEW.user_id IS NULL THEN
        PERFORM pg_notify( 'api/user-notification-type/update', json_build_object( 'user_id', OLD.id::text, 'type', OLD.type )::text );

    -- updated
    ELSE
        PERFORM pg_notify( 'api/user-notification-type/update', json_build_object( 'user_id', NEW.id::text, 'type', NEW.type )::text );
    END IF;
...
CREATE TRIGGER user_notification_type_after_update AFTER INSERT OR UPDATE OR DELETE ON user_notification_type FOR EACH ROW EXECUTE PROCEDURE user_notification_type_trigger();
