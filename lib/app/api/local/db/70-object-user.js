...
CREATE FUNCTION get_object_users ( _object_id int8 ) RETURNS json AS $$
BEGIN

    RETURN ( SELECT json_agg( object_user.user_id ) FROM object_user WHERE object_user.object_id = _object_id AND object_user.enabled );
END;
$$ LANGUAGE plpgsql;
