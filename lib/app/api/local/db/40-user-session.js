...
CREATE TABLE user_session_key_hash (
    user_session_key_id int8 PRIMARY KEY REFERENCES user_session ( id ) ON DELETE CASCADE,
    hash text NOT NULL
);

CREATE FUNCTION user_session_invalidate_trigger() RETURNS TRIGGER AS $$
BEGIN
    PERFORM pg_notify( 'api/invalidate-user-key', json_build_object( 'token_type', ${constants.tokenTypeUserSession}, 'token_id', OLD.id::text )::text );
