...
const { AUTH_TOKEN } = require( "../../../const" );
    ...
    class extends Super {
            ...
            async createUserToken ( userId, name, enabled, permissions = {}, options = {} ) {
                ...
                res = await dbh.begin( async dbh => {
                    ...
                    // insert token
                    if ( dbh.isSqlite ) {
                        const res = await dbh.do( QUERIES.insertTokenSqlite, [userId, name, enabled, sql.json( permissions )] );
                        ...
                        const res = await dbh.selectRow( QUERIES.insertTokenPgsql, [userId, name, enabled, sql.json( permissions )] );
                    ...
                    // generate token
                    const token = Token.generateToken( AUTH_TOKEN, id );
