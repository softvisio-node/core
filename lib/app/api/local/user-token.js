    ...
    class extends Super {
            ...
            async _authenticateUserToken ( privateToken ) {
                ...
                return {
                    ...
                    "permissions": this._mergeUserPermissions( res.data.id, res.data.user_permissions, res.data.token_permissions ),
                };
            }
            ...
            async createUserToken ( userId, name, enabled, permissions = {}, options = {} ) {
                ...
                res = await dbh.begin( async dbh => {
                    ...
                    return result( 200, {
                        ...
                        "permissions": this._mergeUserPermissions( userId, user.data.permissions, permissions ),
                    } );
                } );
            ...
            async getUserToken ( tokenId, options = {} ) {
                ...
                ( token.data.permissions = this._mergeUserPermissions( token.data.user_id, token.data.user_permissions, token.data.permissions ) ), delete token.data.user_permissions;
            ...
            async getUserTokens ( userId, options ) {
                ...
                if ( tokens.data ) {
                    for ( const token of tokens.data ) {
                        ( token.permissions = this._mergeUserPermissions( userId, token.user_permissions, token.permissions ) ), delete token.user_permissions;
                    }
                }
            ...
            async getUserTokenPermissions ( tokenId, options = {} ) {
                ...
                const basePermissions = this._mergeUserPermissions( token.data.user_id, token.data.user_permissions );

                const permissions = [];

                for ( const id in basePermissions ) {

                    // skip disabled permissions
                    if ( !basePermissions[id] ) continue;

                    permissions.push( {
                        id,
                        "name": userPermissions[id].name,
                        "description": userPermissions[id].description,
                        "enabled": token.data.permissions[id] ?? basePermissions[id],
                    } );
                }
            ...
            async updateUserTokenPermissions ( tokenId, permissions, options = {} ) {
                ...
                // check, that token belongs to user
                if ( options.userId && options.userId !== token.data.user_id ) return result( [404, `Token not found`] );

                const userPermissions = this._mergeUserPermissions( token.data.user_id, token.data.user_permissions );
                ...
                // validate permissions
                for ( const permission in permissions ) {
                    if ( !userPermissions[permission] ) return result( [400, `Invalid permissions`] );
                }
