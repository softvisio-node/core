...
module.exports = mixin( Super =>
    class extends Super {
            ...
            async getUserTokenPermissions ( tokenId, options = {} ) {
                ...
                const userPermissionsIdx = this._userPermissions,
                    userPermissions = this._buildUserPermissions( token.data.user_id, token.data.user_permissions ),
                    permissions = [];

                for ( const id in userPermissions ) {

                    // skip disabled permissions
                    if ( !userPermissions[id] ) continue;

                    permissions.push( {
                        id,
                        "name": userPermissionsIdx[id].name,
                        "description": userPermissionsIdx[id].description,
