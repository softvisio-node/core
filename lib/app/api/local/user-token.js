...
const QUERIES = {
    ...
    "insertToken": sql`INSERT INTO "user_token" ("user_id", "name", "enabled", "permissions" ) VALUES (?, ?, ?, ?) RETURNING "id"`.prepare(),
...
export default Super =>
    class extends ( Super || Object ) {
        ...
        async getUserToken ( tokenId, options = {} ) {
            ...
            token.data.permissions = this._buildUserPermissions( token.data.user_id, token.data.user_permissions, token.data.permissions );
        ...
        async getUserTokens ( userId, options, ctx ) {
            ...
            if ( tokens.data ) {
                for ( const token of tokens.data ) {
                    token.permissions = this._buildUserPermissions( userId, token.user_permissions, token.permissions );
