    ...
    class extends Super {
            ...
            async createUserToken ( userId, name, enabled, permissions = {}, options = {} ) {
                ...
                var res = await dbh.begin( async dbh => {
                    ...
                    // insert token
                    var res = await dbh.do( q.storeToken, [token.id, userId, name, enabled, sql.JSON( permissions )] );
            ...
            async getUserToken ( tokenId, options = {} ) {
                ...
                var token = await dbh.selectRow( q.getUserTokenWithPermissons, [tokenId] );

                if ( !token.ok ) return token;

                if ( !token.data ) return result( 404 );

                token.data.permissions = this._mergeUserTokenPermissions( token.data.user_permissions, token.data.permissions );

                delete token.data.user_permissions;
            ...
            async getUserTokens ( userId, options ) {
                if ( !this.#read ) {
                    this.#read = new ( Read() )( null, this, { "dbh": this.dbh } );
                }
            ...
            async setUserTokenEnabled ( tokenId, enabled, options = {} ) {
                ...
                const res = await dbh.do( q.setEnabled, [enabled, tokenId] );
            ...
            async setUserTokenPermissions ( tokenId, permissions, options = {} ) {
                ...
                var res = await dbh.do( q.update, [sql.JSON( permissions ), tokenId] );
            ...
            async updateUserTokenPermissions ( tokenId, permissions, options = {} ) {
                ...
                var token = await dbh.selectRow( q.getUserToken, [tokenId] );
            ...
            _mergeUserTokenPermissions ( userPermissions, tokenPermissions ) {
                ...
                for ( const permission in userPermissions ) {
                    if ( userPermissions[permission] ) {
                        permissions[permission] = tokenPermissions[permission] == null ? true : tokenPermissions[permission];
                    }
                }
