...
const QUERIES = {
    ...
    "getObjectUserRoles": sql`SELECT roles FROM object_user WHERE object_id = ? AND user_id = ?`.prepare(),

    "upsertObjectUser": sql`
INSERT INTO object_user ( object_id, user_id , roles ) VALUES ( ?, ?, ? )
...
export default Super =>
    class extends ( Super || Object ) {
        ...
        async setObjectUserRoles ( objectId, userId, roles = [], { dbh, parentUserId } = {} ) {
            ...
            if ( parentUserId && !this.userIsRoot( parentUserId ) ) {
                parentUserRoles = await this.getObjectUserRoles( objectId, parentUserId, { dbh } );

                if ( !parentUserRoles ) return result( [400, `Unable to set user roles`] );
            }
            ...
            for ( const role of roles ) {
                if ( !this.#types[objectId] ) return result( [400, `Role is invalid`] );

                if ( parentUserRoles && !parentUserRoles[role] ) return result( [400, `Unable to set roles`] );

                _roles[role] = true;
            }

            const res = await dbh.do( QUERIES.upsertObjectUser, [objectId, userId, _roles] );
        ...
        async addObjectUserRole ( objectId, userId, role, { dbh, parentUserId } = {} ) {
            ...
            if ( parentUserId && !this.userIsRoot( parentUserId ) ) {
                const parentUserRoles = await this.getObjectUserRoles( objectId, parentUserId, { dbh } );

                if ( !parentUserRoles ) return result( [400, `Unable to set role`] );

                if ( !parentUserRoles[role] ) return result( [400, `Unable to set role`] );
            }
            ...
            // update cache
            const cacheId = objectId + "/" + userId;
            if ( this.#objectUserRoleCache.has( cacheId ) ) this.#objectUserRoleCache.get( cacheId )[role] = true;
        ...
        async deleteObjectUserRole ( objectId, userId, role, { dbh, parentUserId } = {} ) {
            ...
            // update cache
            const cacheId = objectId + "/" + userId;
            if ( this.#objectUserRoleCache.has( cacheId ) ) delete this.#objectUserRoleCache.get( cacheId )[role];
        ...
        async getObjectRoles ( objectId, { parentUserId } = {} ) {
            ...
            // check parent user roles
            if ( parentUserId && !this.userIsRoot( parentUserId ) ) {
                parentUserRoles = await this.getObjectUserRoles( objectId, parentUserId );

                if ( !parentUserRoles ) return result( [400, `Unable to get role`] );
            }
            ...
            for ( const roleId of Object.keys( this.#types[objectType] ).sort() ) {
                ...
                roles.push( {
                    ...
                    "editable": parentUserRoles ? !!parentUserRoles[role.id] : true,
                } );
            }
            ...
            if ( res.data ) {
                ...
                for ( const row of res.data ) {
                    ...
                    for ( const role of roles ) {
                        userRoles.puah( {
                            ...role,
                            "enabled": !!row.roles[role.id],
                            "editable": parentUserId && parentUserId === row.id ? false : role.editable,
                        } );
                    }
        ...
        async #init ( objects ) {
            ...
            this.dbh.on( "api/object-user/update", data => {
                ...
                if ( this.#objectUserRoleCache.has( cacheId ) ) this.#objectUserRoleCache.set( cacheId, data.roles );
            } );
