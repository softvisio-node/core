...
export default Super =>
    class extends ( Super || Object ) {
        ...
        async getObjectUser ( objectId, userId, { dbh } = {} ) {
            ...
            if ( res.data ) this.#objectUserCache.set( cacheId, res.data );
        ...
        async setObjectUserRoles ( objectId, userId, roles = [], { dbh, parentUserId } = {} ) {
            ...
            for ( const role of roles ) {
                ...
                if ( parentUser && !parentUser.roles[role] ) return result( [400, `Unable to set roles`] );
        ...
        async addObjectUserRole ( objectId, userId, role, { dbh, parentUserId } = {} ) {
            ...
            if ( parentUserId && !this.userIsRoot( parentUserId ) ) {
                ...
                if ( !parentUser.roles[role] ) return result( [400, `Unable to set role`] );
            }

            // role is already added
            if ( user.roles[role] ) return result( 200 );

            user.roles[role] = true;

            const res = await dbh.do( QUERIES.upsertObjectUser, [objectId, userId, user.roles] );

            if ( !res.ok ) return res;

            // update cache
            const cacheId = objectId + "/" + userId;
            if ( this.#objectUserCache.has( cacheId ) ) this.#objectUserCache.get( cacheId ).roles[role] = true;
        ...
        async deleteObjectUserRole ( objectId, userId, role, { dbh, parentUserId } = {} ) {
            ...
            // update cache
            const cacheId = objectId + "/" + userId;
            if ( this.#objectUserCache.has( cacheId ) ) delete this.#objectUserCache.get( cacheId ).roles[role];
        ...
        async #init ( objects ) {
            ...
            this.dbh.on( "api/object-user/update", data => {
                ...
                if ( this.#objectUserCache.has( cacheId ) ) this.#objectUserCache.set( cacheId, data.roles );
            } );
