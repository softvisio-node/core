...
export default Super =>
    class extends ( Super || Object ) {
        ...
        async #init ( objects ) {
            ...
            // resolvers
            if ( types.size ) {
                const res = await this.dbh.select( sql`SELECT * FROM object_type` );
                if ( !res.ok ) return res;
                ...
                if ( types.size ) {
                    const res = await this.dbh.do( sql`INSERT INTO object_type`.VALUES( [...types].map( type => ( { type } ) ) ).sql`ON CONFLICT ( type ) DO NOTHING` );
                    if ( !res.ok ) return res;
                }
            }
