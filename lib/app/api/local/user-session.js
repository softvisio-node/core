...
const QUERIES = {
    "auth": sql`
        ...
        WHERE
            ...
            AND "user_hash"."hash" = ?
    `.prepare(),
    "getId": sql`SELECT gen_user_id(?) AS "id"`.prepare(),
    ...
    class extends Super {
        ...
        async createUserSession ( userId, options = {} ) {
            ...
            var res = await dbh.begin( async dbh => {
                ...
                // get token id
                const id = await dbh.selectRow( QUERIES.getId, [TOKEN_TYPE_SESSION] );

                // dbh error
                if ( !id.ok ) throw id;

                // insert token
                var res = await dbh.do( QUERIES.storeToken, [id.data.id, userId] );
