...
const QUERIES = {
    "auth": sql`
        ...
        WHERE
            ...
            AND "user_hash"."hash" = ?
    `.prepare(),
    "generateId": sql`SELECT gen_user_id(?) AS "id"`.prepare(),
...
module.exports = mixin( Super =>
    class extends Super {
        async _authenticateUserSession ( privateToken ) {
            var user = await this.dbh.selectRow( QUERIES.auth, [privateToken.id, privateToken.hash] );
        ...
        async createUserSession ( userId, options = {} ) {
            ...
            var res = await dbh.begin( async dbh => {
                ...
                // get token id
                const id = await dbh.selectRow( QUERIES.generateId, [TOKEN_TYPE_SESSION] );

                // dbh error
                if ( !id.ok ) throw id;

                // generate token
                const token = this._generateToken( TOKEN_TYPE_SESSION, id.data.id );

                // insert token
                var res = await dbh.do( QUERIES.storeToken, [token.id, userId] );
