...
const { AUTH_SESSION } = require( "../../../const" );
...
const QUERIES = {
    "auth": sql`
        ...
        WHERE
            ...
            AND "user_session"."id" = "user_session_hash"."id"
            AND "user"."enabled" = TRUE
            AND "user_session"."id" = ?
            AND "user_session_hash"."hash" = ?
    `.prepare(),
    "storeHash": sql`INSERT INTO "user_session_hash" ("id", "hash") VALUES (?, ?)`.prepare(),
    ...
    class extends Super {
        ...
        async createUserSession ( userId, options = {} ) {
            ...
            var res = await dbh.begin( async dbh => {
                ...
                // generate token
                const token = Token.generateToken( AUTH_SESSION, id );
