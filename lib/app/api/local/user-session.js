...
const QUERIES = {
    ...
    "insertToken": sql`INSERT INTO user_session ( user_id, expires, user_agent, remote_address ) VALUES ( ?, ?, ?, ? ) RETURNING id`.prepare(),
    "delete": sql`DELETE FROM user_session WHERE id = ?`.prepare(),
...
export default Super =>
    class extends ( Super || Object ) {
        ...
        async createUserSession ( userId, { userAgent, remoteAddress, dbh } = {} ) {
            dbh ||= this.dbh;
            ...
            var res = await dbh.begin( async dbh => {
                ...
                // insert token
                let res = await dbh.selectRow( QUERIES.insertToken, [userId, expires, userAgent, remoteAddress] );
