...
const QUERIES = {
    ...
    "authUser": sql`SELECT "user"."id", "user"."permissions", "user"."gravatar", "user_hash"."hash" FROM "user" LEFT JOIN "user_hash" ON "user"."id" = "user_hash"."user_id" WHERE "user"."name" = ? AND "user"."enabled" = TRUE`.prepare(),
...
export default Super =>
    class extends ( Super || Object ) {
        ...
        async _authenticateUserCredentials ( token ) {
        ...
        async createUser ( username, password, enabled, permissions, fields, options = {} ) {
            ...
            // validate user name
            if ( this.app.config.usernameIsEmail ) {
            ...
            const user = await dbh.begin( async dbh => {
                ...
                let res = await dbh.selectRow( sql`INSERT INTO "user"`.VALUES( fields ).sql`RETURNING "id"` );

                if ( !res.ok ) {

                    // user already exists or email is not unique
                    if ( res.meta.code === dbh.errors.UNIQUE_VIOLATION ) return result( [409, "User already exists"] );
