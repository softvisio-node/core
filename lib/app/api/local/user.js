...
export default Super =>
    class extends ( Super || Object ) {
        ...
        async authenticateUserOauth ( oauth ) {
            const res = await this.#oauth( oauth );
        ...
        async createUser ( email, fields = {}, { dbh, root } = {} ) {
            ...
            if ( typeof email === "object" ) {
                const res = await this.#oauth( email );
        ...
        async #oauth ( { provider, code, redirect } = {} ) {
            var res;

            if ( provider === "google" ) {
                res = await this.#oauthGoogle( code, redirect );
            }
            else if ( provider === "github" ) {
                res = await this.#oauthGitHub( code, redirect );
            }
            else {
        ...
        async #oauthGoogle ( code, redirect ) {
            try {
        ...
        async #oauthGitHub ( code, redirect ) {
            try {
                var res = await fetch( "https://github.com/login/oauth/access_token", {
                    ...
                    "body": new URLSearchParams( {
                        "client_id": this.app.env.api.oauth.github.clientId,
                        "client_secret": this.app.env.api.oauth.github.clientSecret,
                        "redirect_uri": redirect,
                        "code": code,
                    } ),
                } );
