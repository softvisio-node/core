...
const QUERIES = {
    ...
    "upsertHash": sql`INSERT INTO "auth_hash" ("id", "hash") VALUES (?, ?) ON CONFLICT ("id") DO UPDATE SET "hash" = ?`.prepare(),
    "authUser": sql`SELECT "user"."id", "user"."permissions", "auth_hash"."hash" FROM "user" LEFT JOIN "auth_hash" ON "user"."id" = "auth_hash"."id" WHERE "user"."name" = ? AND "user"."enabled" = TRUE`.prepare(),
