...
export default Super =>
    class extends ( Super || Object ) {
        ...
        async authenticateUserOauth ( provider, code ) {
            if ( provider === "google" ) {
                const res = await fetch( "https://oauth2.googleapis.com/token", {
                    ...
                    "body": new URLSearchParams( {
                        "client_id": "717442500180-d2ksm56b66jq0fbuvauktn0h66d24mh5.apps.googleusercontent.com",
                        "client_secret": "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",
                ...
                const res1 = await fetch( "https://www.googleapis.com/oauth2/v1/userinfo", {
                    "headers": {
                ...
                const providerUser = await res1.json().catch( e => null );
                if ( !providerUser.email ) return;

                const user = await this.dbh.selectRow( QUERIES.authUser, [providerUser.email] );
                if ( !user.data ) return;
