...
export default Super =>
    class extends ( Super || Object ) {
        ...
        async authenticateUserFirebaseToken ( email, firebaseUserAccessToken ) {
            const user = await this.dbh.selectRow( QUERIES.authUser, [email] );

            // user not found or disabled
            if ( !user.data ) return;

            const valid = await this.app.notifications.firebase.auth.verifyIdToken( firebaseUserAccessToken );
