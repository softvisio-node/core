...
const QUERIES = {
    ...
    "authUser": sql`SELECT "user".id, "user".locale, "user".roles, "user".gravatar, user_password_hash.hash FROM "user" LEFT JOIN user_password_hash ON "user".id = user_password_hash.user_id WHERE "user".name = ? AND "user".enabled = TRUE`.prepare(),
...
export default Super =>
    class extends ( Super || Object ) {
        ...
        async setUsername ( userId, username, password, { dbh } = {} ) {
            dbh ||= this.dbh;
        ...
        // XXX validate locale in app allowed locales
