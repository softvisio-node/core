...
const QUERIES = {
    "getIdSqlite": sql`SELECT "id" FROM "user" WHERE "rowid" = ?`.prepare(),
    "getUserById": sql`SELECT "id", "name", "enabled", "permissions" FROM "user" WHERE "id" = ?`.prepare(),
    "getUserByName": sql`SELECT "id", "name", "enabled", "permissions" FROM "user" WHERE "name" = ?`.prepare(),
    "removeUser": sql`DELETE FROM "user" WHERE "id" = ?`.prepare(),
    "setUserEnabled": sql`UPDATE "user" SET "enabled" = ? WHERE "id" = ? AND "enabled" = ?`.prepare(),
    "upsertHash": sql`INSERT INTO "user_hash" ("user_id", "hash") VALUES (?, ?) ON CONFLICT ("user_id") DO UPDATE SET "hash" = ?`.prepare(),
    "authUser": sql`SELECT "user"."id", "user"."permissions", "user_hash"."hash" FROM "user" LEFT JOIN "user_hash" ON "user"."id" = "user_hash"."user_id" WHERE "user"."name" = ? AND "user"."enabled" = TRUE`.prepare(),
...
module.exports = Super =>
    class extends ( Super || Object ) {
        ...
        async _authenticateUserPassword ( token ) {
            var user = await this.dbh.selectRow( QUERIES.authUser, [token.id] );

            // user not found or disabled
            if ( !user.data ) return;

            const valid = await this.#verifyPasswordHash( user.data.hash, token.hash );
        ...
        async _getUser ( userId, options = {} ) {
            ...
            if ( !res.ok ) {
                ...
                res.data.permissions = this._buildUserPermissions( userId, res.data.permissions );
        ...
        async createUser ( username, password, enabled, permissions, fields, options = {} ) {
            ...
            const res = this.validateUserPermissions( Object.keys( permissions ) );
            ...
            const hash = await this.#generatePasswordHash( username, password );
            ...
            const user = await dbh.begin( async dbh => {
                ...
                if ( dbh.isSqlite ) {
                    ...
                    if ( res.rows ) {
                        userId = ( await dbh.selectRow( QUERIES.getIdSqlite, [dbh.lastInsertRowId] ) ).data.id;
                    }
                }
                ...
                // insert user hash
                res = await dbh.do( QUERIES.upsertHash, [userId, hash.data, hash.data] );
        ...
        async setUserName ( userId, username, password, options = {} ) {
            ...
            const res = await dbh.begin( async dbh => {
                ...
                // update user hash
                res = await dbh.do( QUERIES.upsertHash, [user.data.id, hash.data, hash.data] );

                // unable to insert user hash
                if ( !res.ok ) throw res;

                return result( 200, { username, password } );
            } );

            if ( res.ok ) this.app.publish( "#api/api/auth-cache/invalidate/user", user.data.id );
        ...
        async setUserPassword ( userId, password, options = {} ) {
            ...
            // generate password hash
            const hash = await this.#generatePasswordHash( user.data.name, password );

            if ( !hash.ok ) return hash;

            const res = await dbh.do( QUERIES.upsertHash, [user.data.id, hash.data, hash.data] );
            ...
            if ( !res.ok ) {
                ...
                this.app.publish( "#api/api/auth-cache/invalidate/user", user.data.name );
        ...
        async setUserEnabled ( userId, enabled, options = {} ) {
            ...
            if ( !res.ok ) {
                ...
                this.app.publish( "#api/api/auth-cache/invalidate/user", user.data.id );
        ...
        async removeUser ( userId, options = {} ) {
            ...
            const res = await dbh.do( QUERIES.removeUser, [userId] );

            if ( !res.ok ) {
                return res;
            }
            else if ( res.rows ) {
                this.app.publish( "#api/api/auth-cache/invalidate/user", userId );
        ...
        async getUserPermissions ( userId, superPermissions, options ) {
            ...
            for ( const permission in superPermissions ) {
                permissions.push( {
                    "id": permission,
                    "enabled": !!user.data.permissions[permission],
                } );
            }
        ...
        async setUserPermissions ( userId, permissions, options = {} ) {
            ...
            // validate permissions
            var res = this.validateUserPermissions( Object.keys( permissions ) );

            // permissions are invalid
            if ( !res.ok ) return res;

            res = await dbh.do( QUERIES.updatePermissions, [permissions, userId] );
        ...
        async updateUserPermissions ( userId, permissions, options = {} ) {
            ...
            res = await dbh.do( QUERIES.updatePermissions, [permissions, user.data.id] );
        ...
        async #generatePasswordHash ( username, password ) {
            return this.app.threads.call( "argon2/hash", Token.getHash( password + username ) );
        }

        async #verifyPasswordHash ( hash, passwordHash ) {
