...
const QUERIES = {
    ...
    "upsertHash": sql`INSERT INTO "userHash" ("userId", "hash") VALUES (?, ?) ON CONFLICT ("userId") DO UPDATE SET "hash" = ?`.prepare(),
    "authUser": sql`SELECT "user"."id", "user"."permissions", "user"."gravatar", "userHash"."hash" FROM "user" LEFT JOIN "userHash" ON "user"."id" = "userHash"."userId" WHERE "user"."name" = ? AND "user"."enabled" = TRUE`.prepare(),
    "updatePermissions": sql`UPDATE "user" SET "permissions" = ? WHERE "id" = ?`.prepare(),
};

export default Super =>
    class extends ( Super || Object ) {
        #verifyPasswordHashCache = new CacheLru( {
            "maxSize": 10000,
        } );
        ...
        async createUser ( username, password, enabled, permissions, fields, options = {} ) {
            ...
            if ( fields.telegramName != null ) {
                fields.telegramName = fields.telegramName.toLowerCase();

                const res = this.validateTelegramUsername( fields.telegramName );
            ...
            const user = await dbh.begin( async dbh => {
                ...
                if ( this.userIsRoot( username ) ) userId = fields.id = CONST.ROOT_USER_ID;

                let res = await dbh.selectRow( sql`INSERT INTO "user"`.VALUES( fields ).sql`ON CONFLICT DO NOTHING RETURNING "id"` );

                if ( !res.ok ) throw res;

                if ( !res.meta.rows ) return result( [409, "User already exists"] );
