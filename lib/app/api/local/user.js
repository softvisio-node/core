...
const QUERIES = {
    ...
    "authUser": sql`SELECT "user".id, "user".roles, "user".gravatar, user_password_hash.hash FROM "user" LEFT JOIN user_password_hash ON "user".id = user_password_hash.user_id WHERE "user".name = ? AND "user".enabled = TRUE`.prepare(),
