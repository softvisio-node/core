    ...
    class extends ( Super || Object ) {
        ...
        async createUser ( email, fields = {}, { dbh, root } = {} ) {
            ...
            const user = await dbh.begin( async dbh => {
                ...
                let res = await dbh.selectRow( sql`INSERT INTO "user"`.VALUES( fields ).sql`ON CONFLICT ( email ) DO NOTHING RETURNING "id"` );
        ...
        async setUserEmail ( userId, email, { dbh } = {} ) {
            ...
            // change user email
            res = await dbh.do( QUERIES.setUserEmail, [email, userId] );
