...
function generateMacro ( type, format, operators, _const, _enum, aclResolver ) {
    ...
    if ( scalarOperators.length ) {
        macros.push( {
            "type": "array",
            "items": [scalarOperators.length === 1 ? { "const": scalarOperators[0] } : { "enum": scalarOperators }, generateType( type, format, _const, _enum, aclResolver )],
        } );
    }
    ...
    if ( arrayOperators.length ) {
        macros.push( {
            "type": "array",
            "items": [arrayOperators.length === 1 ? { "const": arrayOperators[0] } : { "enum": arrayOperators }, { "type": "array", "items": generateType( type, format, _const, _enum, aclResolver ), "minItems": 1 }],
        } );
    }
...
const keyword = {
    ...
    "metaSchema": {
        ...
        "properties": {
            ...
            "order_by": {
                "type": "object",
                "propertyName": { "format": "snake-case" },
                "additionslProperties": { "enum": ["asc", "desc"] },
                "minProperties": 1,
            },
