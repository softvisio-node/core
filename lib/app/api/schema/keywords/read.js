...
function generateMacro ( type, operators, _const, _enum, checkAcl ) {
    ...
    if ( equalOperator ) {
        macros.push( generateType( type, _const, _enum, checkAcl ) );
    }

    if ( scalarOperators.length ) {
        macros.push( { "type": "array", "items": [scalarOperators.length === 1 ? { "const": scalarOperators[0] } : { "enum": scalarOperators }, generateType( type, _const, _enum, checkAcl )] } );
    }

    if ( arrayOperators.length ) {
        macros.push( { "type": "array", "items": [arrayOperators.length === 1 ? { "const": arrayOperators[0] } : { "enum": arrayOperators }, { "type": "array", "items": generateType( type, _const, _enum, checkAcl ), "minItems": 1 }] } );
    }
...
function generateType ( type, _const, _enum, checkAcl ) {
    const macro = {};
    ...
    if ( checkAcl ) macro["check-acl"] = checkAcl;
    ...
    "macro": ( schema, parentSchema, it ) => {
        ...
        for ( const fieldName in schema.fields ) {
            ...
            if ( fieldName === "id" ) {
                ...
                if ( field["check-acl"] ) macro.properties.id["check-acl"] = field["check-acl"];
            }
            ...
            if ( field.operator ) {
                ...
                macro.properties.where.properties[fieldName] = generateMacro( field.type, field.operator, field.const, field.enum, field["check-acl"] );
            }
        }
