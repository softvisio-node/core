...
function generateMacro ( type, operators, _const, _enum ) {
    ...
    if ( equalOperator ) {
        macros.push( generateType( type, _const, _enum ) );
    }

    if ( scalarOperators.length ) {
        macros.push( { "type": "array", "items": [scalarOperators.length === 1 ? { "const": scalarOperators[0] } : { "enum": scalarOperators }, generateType( type, _const, _enum )] } );
    }

    if ( arrayOperators.length ) {
        macros.push( { "type": "array", "items": [arrayOperators.length === 1 ? { "const": arrayOperators[0] } : { "enum": arrayOperators }, { "type": "array", "items": generateType( type, _const, _enum ), "minItems": 1 }] } );
    }
...
function generateType ( type, _const, _enum ) {
    if ( _const !== undefined ) return { "const": _const };
    ...
    "macro": ( schema, parentSchema, it ) => {
        ...
        for ( const fieldName in schema.fields ) {
            ...
            if ( field.operator ) {
                ...
                macro.properties.where.properties[fieldName] = generateMacro( field.type, field.operator, field.const, field.enum );
            }
        }
