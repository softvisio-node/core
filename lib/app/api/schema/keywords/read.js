...
function generateMacro ( type, operators, _enum ) {
    ...
    if ( equal ) {
        macros.push( generateType( type, _enum ) );
    }

    if ( scalar.length ) {
        macros.push( { "type": "array", "items": [{ "enum": scalar }, generateType( type, _enum )] } );
    }

    if ( array.length ) {
        macros.push( { "type": "array", "items": [{ "enum": array }, { "type": "array", "items": generateType( type, _enum ), "minItems": 1 }] } );
    }
...
function generateType ( type, _enum ) {
