...
function generateMacro ( type, operators, _const, _enum, checkObjectPermissions ) {
    ...
    for ( const operator of operators ) {
        ...
        // group operators
        if ( operator === "=" ) equalOperator = true;

        if ( ARRAY_OPERATORS.has( operator ) ) arrayOperators.push( operator );
        else scalarOperators.push( operator );
    }

    const macros = [];

    if ( equalOperator ) {
        macros.push( generateType( type, _const, _enum, checkObjectPermissions ) );
    }

    if ( scalarOperators.length ) {
        macros.push( { "type": "array", "items": [scalarOperators.length === 1 ? { "const": scalarOperators[0] } : { "enum": scalarOperators }, generateType( type, _const, _enum, checkObjectPermissions )] } );
    }

    if ( arrayOperators.length ) {
        macros.push( { "type": "array", "items": [arrayOperators.length === 1 ? { "const": arrayOperators[0] } : { "enum": arrayOperators }, { "type": "array", "items": generateType( type, _const, _enum, checkObjectPermissions ), "minItems": 1 }] } );
    }
...
function generateType ( type, _const, _enum, checkObjectPermissions ) {
    const macro = {};

    if ( _const !== undefined ) macro.const = _const;
    else if ( _enum ) macro.enum = _enum;
    else macro.type = type;

    if ( checkObjectPermissions ) macro["check-object-permissions"] = checkObjectPermissions;
                ...
                "patternProperties": {
                    ...
                    "^[a-z]+(?:_[a-z\\d]+)*$": {
                        ...
                        "properties": {
                            ...
                            "check-object-permissions": { "type": ["boolean", "string"] },
                        },
    ...
    "macro": ( schema, parentSchema, it ) => {
        ...
        for ( const fieldName in schema.fields ) {
            ...
            if ( fieldName === "id" ) {
                ...
                if ( field["check-object-permissions"] ) macro.properties.id["check-object-permissions"] = field["check-object-permissions"];
            }
            ...
            if ( field.operator ) {
                ...
                macro.properties.where.properties[fieldName] = generateMacro( field.type, field.operator, field.const, field.enum, field["check-object-permissions"] );
            }
        }
