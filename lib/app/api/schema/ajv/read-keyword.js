...
const keyword = {
    ...
    "macro": ( schema, parentSchema, it ) => {
        ...
        for ( const fieldName in schema.fields ) {
            ...
            if ( field.operator ) {
                const spec = [],
                    types = Array.isArray( field.type ) ? field.type : [field.type],
                    operators = Array.isArray( field.operator ) ? field.operator : [field.operator];

                for ( const type of types ) {
                    for ( const operator of operators ) {
                        spec.push( READER_SCHEMAS[type][operator] );
                    }
                }
