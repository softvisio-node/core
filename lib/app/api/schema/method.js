...
export default class Method {
    ...
    get uploadMaxSize () {
        if ( this.#uploadMaxSize == null ) {
            if ( this.isUpload ) {
                this.#uploadMaxSize = this.#spec.params?.[0]?.schema?.file.uploadMaxSize;
            }
            else {
    ...
    async loadApi ( api ) {
        if ( this.#spec.validateParams !== false ) {
            ...
            try {
                this.#validate = Ajv.new().addKeyword( apiReadKeyword.keyword ).addKeyword( fileKeyword.keyword ).compile( paramsSchema );
            }
            catch ( e ) {
