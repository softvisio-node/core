...
export default class Method {
    ...
    // read
    get readOffsetLimit () {
        this.#readOffsetLimit ??= {
            "maxResults": this.#spec.params?.[0]?.schema?.read?.max_results,
            "defaultLimit": this.#spec.params?.[0]?.schema?.read?.limit?.default,
            "maxLimit": this.#spec.params?.[0]?.schema?.read?.limit?.max,
        };

        return this.#readOffsetLimit;
    }
