...
export default class Method {
    ...
    async loadApi ( api ) {
        if ( this.#spec.validateParams !== false ) {
            ...
            try {
                ...
                objectIdResolvers = global[Symbol.for( "checkObjectPermisisonsIds" )];
                global[Symbol.for( "checkObjectPermisisonsIds" )] = null;
            }
            catch ( e ) {
            ...
            // check object is resolvers
            if ( objectIdResolvers ) {
                for ( const id in objectIdResolvers ) {
                    if ( !this.module.objectIdResolvers[id] ) {
                        throw result( [500, `Object id resolver "${id}" is not registered in "${this.module.name}/${this.#name}"`] );
                    }
                }
            }
        }
    ...
    validateParams ( params ) {
        if ( !this.#validator || this.#validator( params ) ) return result( 200 );
