...
const SNAKE_CASE_RE = /^_?[a-z][a-z0-9_]+$/;
...
export default class Method {
    ...
    constructor ( module, name, spec ) {
        ...
        if ( !KEBAB_CASE_RE.test( this.#name ) ) throw result( [500, `API Method name "${this.#name}" in module "${this.module.name}" must be in the kebab_case`] );
        ...
        if ( spec.params ) {
            for ( const param of spec.params ) if ( !SNAKE_CASE_RE.test( param.name ) ) throw result( [500, `Parameter name "${param.name}" for module "${this.module.name}" method "${this.name}" must be in the snake_case`] );
        }
        ...
        if ( this.namespace ) {
            ...
            this.#apiName = "API_" + this.#name.replaceAll( /-(.)/g, ( match, letter ) => letter.toUpperCase() );

            console.log( this.#id, this.#apiName );
        }
    }
