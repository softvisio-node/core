...
export default class Method {
    ...
    constructor ( module, name, spec ) {
        ...
        if ( !KEBAB_CASE_RE.test( this.#name ) ) throw result( [500, `API Method "${this.module.name}/${this.#name}" must be in the kebab-case`] );
        ...
        if ( spec.params ) {
            for ( const param of spec.params ) if ( !SNAKE_CASE_RE.test( param.name ) ) throw result( [500, `API method "${this.module.name}/${this.#name}", parameter "${param.name}" must be in the snake_case`] );
        }
