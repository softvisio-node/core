...
export default class Method {
    ...
    constructor ( module, name, spec ) {
        ...
        if ( !SNAKE_CASE_RE.test( this.#name ) ) throw result( [500, `API Method name "${this.#name}" in module "${this.module.name}" must be in the camelCase`] );
        ...
        if ( spec.params ) {
            for ( const param of spec.params ) if ( !SNAKE_CASE_RE.test( param.name ) ) throw result( [500, `Parameter name "${param.name}" for module "${this.module.name}" method "${this.name}" must be in the camelCase`] );
        }
