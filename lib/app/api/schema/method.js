...
export default class Method {
    ...
    constructor ( module, name, spec ) {
        ...
        if ( spec.params ) {
            for ( const param of spec.params ) if ( !PARAM_NAME_RE.test( param.name ) ) throw result( [500, `Parameter name for module "${this.module.name}" method "${this.name}" must be in the camelCase`] );
        }
