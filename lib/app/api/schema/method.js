...
export default class Method {
    ...
    constructor ( module, name, spec ) {
        ...
        if ( spec.params ) {
            for ( const param of spec.params ) if ( !SNAKE_CASE_RE.test( param.name ) ) throw result( [500, `Method "${this.module.name}/${this.#name}", parameter "${param.name}" must be in the snake_case`] );
        }
