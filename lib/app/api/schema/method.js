...
const PARAM_NAME_RE = /^[a-z][a-zA-Z\d]+$/;
...
export default class Method {
    ...
    constructor ( module, name, spec ) {
        ...
        if ( !PARAM_NAME_RE.test( this.#name ) ) throw result( [500, `API Method name "${this.#name}" in module "${this.module.name}" must be in the camelCase`] );
        ...
        if ( spec.params ) {
            for ( const param of spec.params ) if ( !PARAM_NAME_RE.test( param.name ) ) throw result( [500, `Parameter name "${param.name}" for module "${this.module.name}" method "${this.name}" must be in the camelCase`] );
        }
