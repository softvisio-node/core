...
const SPEC_VALIDATOR = ajv().addSchema( fs.config.read( "#resources/schemas/api.meta.schema.yaml", { "resolve": import.meta.url } ) );
...
export default class Module {
    ...
    constructor ( schema, url ) {
        ...
        // validate spec
        if ( !SPEC_VALIDATOR.validate( "module", this.#spec ) ) throw `Schema error in module "${this.#url}":\nspec:\n${JSON.stringify( this.#spec, null, 4 )}\nerrors:\n${JSON.stringify( SPEC_VALIDATOR.errors, null, 4 )}`;
