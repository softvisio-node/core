...
const SPEC_VALIDATOR = Ajv.new().addSchema( config.read( "#resources/schemas/api.meta.schema.yaml", { "resolve": import.meta.url } ) );
...
export default class Module {
    ...
    constructor ( schema, url ) {
        ...
        this.#spec = config.read( url + SPEC_EXTENSION );
        ...
        // validate emits
        this.#spec.emits = new Set( this.#spec.emits );
        for ( const name of this.#spec.emits ) {
            if ( !isKebabCase( name, { "sep": "/" } ) ) throw result( [500, `Event name "${name}" in module "${this.#url}" must be in the kebab-case`] );
        }
        this.#spec.emits = [...this.#spec.emits];
