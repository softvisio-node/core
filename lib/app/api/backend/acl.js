...
export default Super =>
    class extends ( Super || Object ) {
        ...
        async setAclUserRoles ( aclId, userId, { dbh, editorUserId, enabled, roles = [] } = {} ) {
            ...
            if ( editorUserId && !this.userIsRoot( editorUserId ) ) {
        ...
        async getAclUsers ( aclId, { editorUserId } = {} ) {
            ...
            const res = await this.dbh.select( QUERIES.getUsers, [
                ...
                //
                editorUserId || -1,
                "?d=" + this.config.defaultGravatarEncoded,
                aclId,
            ] );
        ...
        // protected
        async _init () {
            this.#aclIdCache = new CacheLru( { "maxSize": this.config.aclCacheMaxSize } );
            this.#aclUserUserCache = new CacheLru( { "maxSize": this.config.aclCacheMaxSize } );
            this.#aclObjectIdCache = new CacheLru( { "maxSize": this.config.aclCacheMaxSize } );

            var res;

            if ( this.config.acl ) {
                process.stdout.write( "Loading ACL ... " );
                res = await this.#init( this.config.acl );
                console.log( res + "" );

                if ( !res.ok ) return res;
            }

            res = super._init ? await super._init() : result( 200 );
