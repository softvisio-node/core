...
export default class extends Component {
    ...
    async init () {
        ...
        if ( this.api.config.roles ) {
            process.stdout.write( "Loading user roles ... " );
            res = await this.#init( this.api.config.roles );
            console.log( res + "" );
    ...
    async getUserRoles ( userId, parentRoles, options ) {
        var user = await this.backend.user.getUser( userId, options );
        ...
        this.addRolesMetadata( roles );

        return result( 200, roles );
    }

    validateApiRoles ( roles ) {
        if ( !Array.isArray( roles ) ) roles = [roles];
    ...
    async #init ( roles ) {
        ...
        for ( const method of Object.values( this.schema.methods ) ) {
            const res = this.validateApiRoles( method.roles );
