...
export default class extends Component {
    ...
    async init () {
        ...
        this.#notifications = this.api.config.notifications.notifications || {};

        for ( const type in this.#notifications ) {
            for ( const channel in this.#notifications[type].channels ) {
                if ( !CHANNELS.has( channel ) ) {
                    return result( [500, `Notification channel "${channel}" for type "${type}" is invalid`] );
                }

                if ( typeof this.#notifications[type][channel] !== "boolean" && this.#notifications[type][channel] !== null ) {
                    return result( [500, `Notification channel "${channel}" value for type "${type}" must be boolean or null`] );
                }
            }
        }
