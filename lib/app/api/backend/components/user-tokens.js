...
import Read from "#lib/app/components/api/mixins/read";
...
export default class extends Component {
    ...
    async createUserToken ( userId, name, enabled, roles = {}, options = {} ) {
        ...
        // resolve user
        var user = await this.backend.user.getUser( userId, { dbh } );
        ...
        res = await dbh.begin( async dbh => {
            ...
            return result( 200, {
                ...
                "roles": this.backend.userRoles.buildUserRoles( userId, user.data.roles, roles ),
            } );
        } );
    ...
    async getUserToken ( tokenId, { dbh } = {} ) {
        ...
        token.data.roles = this.backend.userRoles.buildUserRoles( token.data.user_id, token.data.user_roles, token.data.roles );
    ...
    async getUserTokens ( userId, options, ctx ) {
        ...
        if ( tokens.data ) {
            for ( const token of tokens.data ) {
                token.roles = this.backend.userRoles.buildUserRoles( userId, token.user_roles, token.roles );
    ...
    async getUserTokenRoles ( tokenId, { dbh } = {} ) {
        ...
        const userRoles = this.backend.userRoles.buildUserRoles( token.data.user_id, token.data.user_roles ),
            roles = [];
        ...
        this.backend.userRoles.addRolesMetadata( roles );
    ...
    async _authenticateUserToken ( token ) {
        const res = await this.dbh.selectRow( QUERIES.auth, [token.id, token.hash] );
        ...
        return {
            ...
            "roles": this.backend.userRoles.buildUserRoles( res.data.id, res.data.user_roles, res.data.token_roles ),
