...
module.exports = class AuthDescriptor {
    ...
    async call ( methodId, ...args ) {
        ...
        // validate arguments
        if ( can.data[1].validate && !can.data[1].validate( args ) ) return result( [400, "Method arguments are invalid, refer to the api documentation"], can.data[1].validate.errors );
    ...
    async callVoid ( methodId, ...args ) {
        ...
        // validate arguments
        if ( can.data[1].validate && !can.data[1].validate( args ) ) return;

        can.data[1].object[can.data[1].codeName]( can.data[0], ...args );
    }
