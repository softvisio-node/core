...
module.exports = class Auth {
    ...
    hasPermissions ( ...permissions ) {
        ...
        if ( Array.isArray( permissions[0] ) ) permissions = permissions[0];
    ...
    async call ( methodId, ...args ) {
        ...
        // validate arguments
        if ( !can.data[1].skipParamsValidation && !can.data[1].validate( args ) ) return result( [400, "Method arguments are invalid, refer to the api documentation"] );

        // call method
        try {
            return parseResult( await can.data[1].object[can.data[1]._name]( can.data[0], ...args ) );
        }
        catch ( e ) {
            console.log( e );
    ...
    async callVoid ( methodId, ...args ) {
        ...
        // validate arguments
        if ( !can.data[1].skipParamsValidation && !can.data[1].validate( args ) ) return;

        try {
            can.data[1].object[can.data[1]._name]( can.data[0], ...args );
        }
        catch ( e ) {
            console.log( e );
        }
    ...
    async _authorizeMethod ( methodId ) {
        ...
        if ( !this.hasPermissions( method.permissions ) ) result( [403, "Insufficient permissions"] );
