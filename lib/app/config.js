...
export function validateAppConfig ( appLocation, config, env ) {
    ...
    if ( config.apiEnabled ) {
        ...
        if ( !configSchema.validate( "objects", config ) ) return result( [400, `Application config is not valid`], configSchema.errors );
    }
    ...
    // root user
    if ( !envSchema.validate( "root", env.env ) ) return result( [400, `Application environment is not valid`], envSchema.errors );

    // smtp settings
    if ( !envSchema.validate( "smtp", env.env ) ) return result( [400, `Application environment is not valid`], envSchema.errors );

    // app settings
    if ( !envSchema.validate( "app-settings", env.env ) ) return result( [400, `Application environment is not valid`], envSchema.errors );

    // api database
    if ( config.apiEnabled && !envSchema.validate( "database", env.env ) ) return result( [400, `Application environment is not valid`], envSchema.errors );
