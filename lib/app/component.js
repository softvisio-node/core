...
export default class Component {
    ...
    constructor ( components, spec ) {
        this.#components = components;
        this.#name = spec.name;
        this.#location = spec.location;
        this.#config = mergeObjects( {}, spec.appConfig, spec.runtimeConfig );
    }
    ...
    async configure () {
        ...
        if ( fs.existsSync( envSchemaPath ) ) {
            ...
            if ( !validate( process.env ) ) {
                return result( [400, `Component "${this.name}" environment is not valid: ` + validate.errors] );
            }
        }
        ...
        if ( fs.existsSync( configSchemaPath ) ) {
            ...
            if ( !validate( this.#config ) ) {
                return result( [400, `Component "${this.name}" config is not valid: ` + validate.errors] );
            }
        }
