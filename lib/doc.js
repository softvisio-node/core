...
module.exports = class {
    ...
    async _scanNamespace () {
        ...
        console.log( JSON.stringify( this.#classes, null, 4 ) );
    ...
    _processFunction ( spec ) {
        ...
        if ( this.#modules[spec._module.id].members[spec._id] ) throw `Function "${spec._name}" for module "${spec._module.id}" is already defined.`;
        ...
        if ( spec.extends ) {
            ...
            for ( const superClass of spec.extends ) {
                ...
                for ( const memberId in this.#classes[superClassId]._members ) {
                    ...
                    // TODO check, that members has similar types: property or method

                    // inhertit member spec
                    spec._members[memberId] = {
                        ...( spec._members[memberId] || {} ),
                        ...this.#classes[superClassId]._members[memberId],
                    };
                    ...
                    if ( spec._members[memberId]._isMethod && spec.permissions ) spec._members[memberId].permissions = spec.permissions;
                }
            }
    ...
    _processProperty ( spec, classId ) {
        ...
        if ( this.#classes[classId]._members[spec._id] ) {

            // property is inherited, merge specs
            if ( this.#classes[classId]._members[spec._id]._inheritedFrom ) {
                const mergedSpec = {
                    ...this.#classes[classId]._members[spec._id],
                    ...spec,
                };

                delete mergedSpec._inheritedFrom;

                spec = mergedSpec;
            }
            else {
                throw `Symbol "${spec._id}" for class "${classId}" is already defined`;
            }
        }
    ...
    _processMethod ( spec, classId ) {
        ...
        if ( spec._name.substr( 0, 4 ) === "API_" ) {
            ...
            if ( !spec.permissions && this.#classes[classId].permissions ) spec.permissions = this.#classes[classId].permissions;
        }

        // method is already defined
        if ( this.#classes[classId]._members[spec._id] ) {

            // method is inherited, merge specs
            if ( this.#classes[classId]._members[spec._id]._inheritedFrom ) {
                const mergedSpec = {
                    ...this.#classes[classId]._members[spec._id],
                    ...spec,
                ...
                // do not inherit "async" property
                if ( !spec.async ) delete mergedSpec.async;
