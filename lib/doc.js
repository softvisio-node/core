...
module.exports = class {
    ...
    async getApiSchema ( apiPath ) {
        ...
        for ( const classId in this.#classes ) {
            ...
            for ( const member in this.#classes[classId]._members ) {
                if ( !this.#classes[classId]._members[member].isApiMethod ) continue;
        ...
        if ( spec.extends ) {
            ...
            for ( const superClass of spec.extends ) {
                ...
                for ( const memberId in this.#classes[superClassId]._members ) {
                    ...
                    if ( spec._members[memberId].isApiMethod && spec.permissions ) spec._members[memberId].permissions = spec.permissions;
                }
            }
    ...
    _processProperty ( spec, classId ) {
        ...
        if ( this.#classes[classId]._members[spec._id] && !this.#classes[classId]._members[spec._id].inheritedFrom ) throw `Property "${spec._id}" for class "${classId}" is already defined.`;
    ...
    _processMethod ( spec, classId ) {
        ...
        if ( this.#classes[classId]._members[spec._id] && !this.#classes[classId]._members[spec._id].inheritedFrom ) throw `Method "${spec._id}" for class "${classId}" is already defined.`;
