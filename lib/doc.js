...
const ajv = new Ajv().addSchema( fs.config.read( require.resolve( "@softvisio/core/resources/schemas/doc.meta.schema.yaml" ) ) );
const SCHEMA_INDETNT = 4;
...
module.exports = class {
    ...
    async generate () {
        ...
        const fileTree = fs.fileTree(),
            ...
            // indexTmpl = require.resolve( __dirname + "/../resources/tmpl/doc/index.md.ejs" ),
            classTmpl = require.resolve( __dirname + "/../resources/tmpl/doc/api-class.md.ejs" );
        ...
        for ( const classId in this.#classes ) {
            ...
            // api class
            if ( spec.module.namespacePath.match( /api\/v\d+\/.+/ ) ) {
                spec.isApiClass = true;

                spec.apiNamespace = spec.module.namespacePath.substring( 3, spec.module.namespacePath.length - path.extname( spec.module.namespacePath ).length ).replace( /_/g, "-" );
            }
            ...
            for ( const memberId in spec.members ) {
                ...
                if ( member.isMethod ) {
                    ...
                    // bitbucket: All spaces will be replaced with hyphens
                    // all special characters (including dots) will be removed
                    // the ID will also be all lower-case
                    member.markdownId = "markdown-header-" + member.markdownId;
                    ...
                    if ( member.params ) {
                        ...
                        for ( const param of member.params ) {
                            param.templateSchema = YAML.safeDump( param.schema, { "indent": SCHEMA_INDETNT, "flowLevel": 2, "noArrayIndent": false } );

                            // indent with 8 spaces
                            param.templateSchema = ( "```\n" + param.templateSchema.replace( /^/gm, "        " ) ).trim() + "\n```\n";
                        }
                    }
            ...
            fileTree.add( spec.module.namespacePath + ".md", await ejs.renderFile( classTmpl, { "spec": spec } ) );
        }
    ...
    _loadModule ( modulePath, from ) {
        ...
        // add .js extension
        if ( path.extname( modulePath ) !== ".js" ) modulePath = modulePath + ".js";
