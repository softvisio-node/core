...
const ajv = util.ajv().addSchema( fs.config.read( __dirname + "/../resources/schemas/doc.meta.schema.yaml" ) );
...
module.exports = class {
        ...
        for ( const classId in this.#classes ) {
            ...
            for ( const member in this.#classes[classId].members ) {
                ...
                if ( !methods[methodId].noParamsValidation ) {
                    ...
                    try {
                        methods[methodId].validate = util.ajv().compile( paramsSchema );
                    }
                    catch ( e ) {
    ...
    async generate () {
        ...
        // generate main index
        if ( !util.isEmptyObject( fileTree.files ) ) {
            fileTree.add( "index.md", await ejs.renderFile( indexTmpl, { apiClasses, "hasApi": !util.isEmptyObject( apiClasses ) } ) );
        }
