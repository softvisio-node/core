...
const API_HELPERS_SCHEMA = fs.config.read( __dirname + "/../resources/schemas/api-helpers.schema.yaml" );
        ...
        for ( const classId in this.#classes ) {
            ...
            for ( const member in this.#classes[classId].members ) {
                ...
                if ( !methods[methodId].noParamsValidation ) {
                    ...
                    try {
                        methods[methodId].validate = _ajv().addSchema( API_HELPERS_SCHEMA ).compile( paramsSchema );
                    }
                    catch ( e ) {
