...
module.exports = class {
    ...
    _loadModule ( modulePath, from ) {
        ...
        var symbolName, absModulePath, moduleId, moduleName, namespaceId;
        ...
        if ( modulePath.charAt( 0 ) === "." ) {
            ...
            // remove ".js"
            moduleId = absModulePath.substr( 0, absModulePath.length - 3 );

            moduleName = path.relative( this.#namespace, moduleId );

            if ( moduleName.charAt( 0 ) !== "." ) namespaceId = moduleName;
        }

        // global node module path
        else {
            absModulePath = require.resolve( modulePath, { "paths": [from] } );
        ...
        if ( !this.#loadedModules[moduleId] ) {
            ...
            this.#loadedModules[moduleId] = {
                "id": moduleId, // absolute path without .js
                "path": absModulePath, // absolute module path.js
                "name": moduleName,
                "namespaceId": namespaceId,
            };

            // parse module documentation
            this._parseModule( this.#loadedModules[moduleId] );
        }
    ...
    _processFunction ( spec ) {
        ...
        // register module
        this.#modules[spec._memberOf].members[spec._id] = spec;
    }

    _processClass ( spec ) {
        spec._name = spec.class;
        spec._id = spec._moduleId + "#" + spec._name;
        spec._members = {};
        ...
        if ( spec.extends ) {
            ...
            //     const superClassId = this._loadModule( superClass, spec._moduleId );
            //     if ( !this.#classes[superClassId] ) throw `Unable to resolve dependency "${superClass}" for class "${spec._name}" in module "${spec._moduleId}"`;
            //     inheritedSpec = {
            //         ...inheritedSpec,
            //         ...this.#classes[superClassId],
            //     };
            ...
            //     ...inheritedSpec,
            //     ...spec,
            //     "extends": Object.keys( resolvedExtends ),
            // };
            // inherit members of super classes
            //     for (const superClass of spec.extends) {
            //         for (const symbol in this.#symbols) {
            //             if (this.#symbols[symbol].memberOf === superClass) {
            //                 const memberSpec = {
            //                     ...this.#symbols[symbol],
            //                 };
            //                 memberSpec.inheritedFrom = superClass;
            //                 // inherit property
            //                 if (this.#symbols[symbol].isProperty) {
            //                     const propertySpec = { ...this.#symbols[symbol] };
            //                     propertySpec.inheritedFrom;
            //                 }
            //                 // inherit method
            //                 else if (this.#symbols[symbol].isMethod) {
            //                 }
            //             }
            //         }
            //     }
