...
export default class {
    ...
    async generate () {
        ...
        for ( const classId in this.#classes ) {
            ...
            if ( spec.isApiClass ) {
                fileTree.add( { "path": spec.module.namespacePath + ".md", "buffer": await ejs.renderFile( classTmpl, { spec } ) } );
            }
        }

        // generate main index
        if ( !fileTree.isEmpty ) {
            fileTree.add( { "path": "index.md", "buffer": await ejs.renderFile( indexTmpl, { apiClasses, "hasApi": !Object.isEmpty( apiClasses ) } ) } );
        }
