...
module.exports = class {
    ...
    _processFunction ( spec ) {
        ...
        if ( this.#modules[spec.module.id].members[spec._id] ) throw `Function "${spec._name}" for module "${spec.module.id}" is already defined.`;

        // validate spec
        if ( !ajv.validate( "function", spec ) ) throw `Schema error in module "${spec.module.id}":\nspec:\n${JSON.stringify( spec, null, 4 )}\nerrors:\n${JSON.stringify( ajv.errors, null, 4 )}`;

        // register function
        this.#modules[spec.module.id].members[spec._id] = spec;
    }

    _processClass ( spec ) {
        spec._name = spec.class;
        spec._id = spec.module.id + "#" + spec._name;
        spec.members = {};

        // class is already defined
        if ( this.#classes[spec._id] ) throw `Class "${spec._name}" is already defined in module "${spec.module.id}".`;
        ...
        if ( spec.extends ) {
            ...
            for ( const superClass of spec.extends ) {
                ...
                if ( !this.#classes[superClassId] ) throw `Unable to resolve super class "${superClass}" for class "${spec._name}" in module "${spec.module.id}".`;
    ...
    _processProperty ( spec, classId ) {
        ...
        if ( !classId ) throw `Property "${spec._name}" is not belongs to any class in module "${spec.module.id}".`;

        spec._id = spec._name;
        if ( spec.static ) spec._id += "-static";

        // property is already defined
        if ( this.#classes[classId].members[spec._id] && !this.#classes[classId].members[spec._id].inheritedFrom ) throw `Property "${spec._id}" for class "${classId}" is already defined.`;
        ...
        // register property
        this.#classes[classId].members[spec._id] = spec;
    }
    ...
    _processMethod ( spec, classId ) {
        ...
        if ( !classId ) throw `Method "${spec._name}" is not belongs to any class in module "${spec.module.id}".`;

        spec._id = spec._name;
        if ( spec.static ) spec._id += "-static";

        // method is already defined
        if ( this.#classes[classId].members[spec._id] && !this.#classes[classId].members[spec._id].inheritedFrom ) throw `Method "${spec._id}" for class "${classId}" is already defined.`;
