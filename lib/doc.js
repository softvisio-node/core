...
import _ajv from "#lib/ajv";
import fs from "#lib/fs";
import path from "path";
import YAML from "js-yaml";
import url from "url";

const ajv = _ajv().addSchema( fs.config.read( "#resources/schemas/doc.meta.schema.yaml", { "resolve": import.meta.url } ) );
...
export default class {
    ...
    async getApiSchema ( apiPath ) {
        ...
        for ( const classId in this.#classes ) {
            ...
            for ( const member in this.#classes[classId].members ) {
                ...
                const methodId = ( apiClass + "/" + member.substr( 4 ) ).replaceAll( "_", "-" );
                ...
                if ( methods[methodId].validateParams !== false ) {
                    const paramsSchema = this.#buildParamsSchema( this.#classes[classId].members[member] );

                    // compile method params schema validator
                    try {
                        methods[methodId].validate = _ajv().addApiKeywords().compile( paramsSchema );
                    }
                    catch ( e ) {
    ...
    async generate () {
        ...
        const fileTree = new fs.FileTree(),
            { "defult": ejs } = await import( "ejs" ),
            indexTmpl = fs.resolve( "#resources/tmpl/doc/index.md.ejs", import.meta.url ),
            classTmpl = fs.resolve( "#resources/tmpl/doc/api-class.md.ejs", import.meta.url );
        ...
        for ( const classId in this.#classes ) {
            ...
            if ( ( match = spec.module.namespacePath.match( /api(\/(v\d+)\/.+)/ ) ) ) {
                ...
                spec.apiNamespace = match[1].replaceAll( "_", "-" );
            ...
            for ( const memberId in spec.members ) {
                ...
                if ( member.isMethod ) {
                    ...
                    if ( member.isApiMethod ) {
                        member.apiName = member.name.substr( 4 ).replaceAll( "_", "-" );
            ...
            if ( spec.isApiClass ) {
                fileTree.add( { "path": spec.module.namespacePath + ".md", "data": await ejs.renderFile( classTmpl, { spec } ) } );
            }
        }

        // generate main index
        if ( !fileTree.isEmpty ) {
            fileTree.add( { "path": "index.md", "data": await ejs.renderFile( indexTmpl, { apiClasses, "hasApi": !Object.isEmpty( apiClasses ) } ) } );
        }
    ...
    async #scanNamespace () {
        ...
        for ( const modulePath of modules ) {
            ...
            this.#loadModule( "./" + modulePath, this.#namespace + "/file.js" );
        }
    }

    #loadModule ( modulePath, from ) {
        ...
        if ( !this.#loadedModules[id] ) {
            ...
            if ( namespacePath != null ) namespacePath = namespacePath.replaceAll( "\\", "/" );
            ...
            this.#parseModule( this.#loadedModules[id] );
        }
    ...
    #parseModule ( module ) {
        const content = fs.readFileSync( url.fileURLToPath( module.id ), "utf8" );
        ...
        if ( docBlocks ) {
            for ( let docBlock of docBlocks ) {
                ...
                if ( Object.hasOwnProperty.call( spec, "function" ) ) {
                    this.#processFunction( spec );
                }

                // class
                else if ( Object.hasOwnProperty.call( spec, "class" ) ) {
                    this.#processClass( spec );
                ...
                // property
                    ...
                    this.#processProperty( spec, currentClassId );
                }

                // method
                else if ( Object.hasOwnProperty.call( spec, "method" ) ) {
                    this.#processMethod( spec, currentClassId );
                }

                // module
                else {
                    this.#processModule( spec );
                }
            }
        }
    }
    ...
    #processClass ( spec ) {
        ...
        if ( spec.extends ) {
            ...
            for ( const superClass of spec.extends ) {
                const superClassId = this.#loadModule( superClass, spec.module.id );
