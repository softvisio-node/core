...
module.exports = class {
            ...
            for ( const superClass of spec.extends ) {
                ...
                for ( const memberId in this.#classes[superClassId].members ) {
                    ...
                    spec._members[memberId] = {
                        ...( spec._members[memberId] || {} ),
                        ...this.#classes[superClassId].members[memberId],
                    };

                    // set inheritedFrom property
                    spec._members[memberId].inheritedFrom = superClassId;
    ...
    _processProperty ( spec, classId ) {
        ...
        // check, that symbol with this id is not registered
        if ( this.#classes[classId]._members[spec._id] ) throw `Symbol "${spec._id}" for class "${classId}" is already defined`;
