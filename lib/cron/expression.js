...
export default class CronExpression {
    ...
    constructor ( expression, { timezone, ignoreSeconds } = {} ) {
        ...
        if ( fields.length < 5 || fields.length > 6 ) {
            throw Error( `Cron expression is not valid: ${ expression }` );
        }
    ...
    #parseField ( fieldValue, field ) {
        ...
        this.#fields[ field.name ] = {
            restricted,
            "values": usedValues && usedValues < field.max ? values : null,
    ...
    #throwError ( fieldValue, field ) {
        throw Error( `Cron expression field ${ field.name } is not valid: ${ fieldValue }` );
    }
    ...
    #getNextDate ( date ) {
        while ( true ) {
            ...
            if ( this.#fields.months.values && !this.#fields.months.values[ date.month ] ) {
                date = date
                    .add( {
            ...
            if ( this.#fields.days.values && !this.#fields.days.values[ date.day ] ) {
                addDays = this.#getAddValue( date.day, FIELDS_INDEX.days );
            }

            // current day of week is not allowed
            if ( this.#fields.daysOfWeek.values && !this.#fields.daysOfWeek.values[ date.dayOfWeek === 7 ? 0 : date.dayOfWeek ] ) {
            ...
            if ( this.#fields.hours.values && !this.#fields.hours.values[ date.hour ] ) {
                date = date
                    .add( {
            ...
            if ( this.#fields.minutes.values && !this.#fields.minutes.values[ date.minute ] ) {
                date = date
                    .add( {
            ...
            if ( this.#fields.seconds?.values && !this.#fields.seconds.values[ date.second ] ) {
                date = date.add( {
    ...
    #getAddValue ( currentValue, field ) {
        ...
        throw Error( `Cron unable to find next value` );
    }
}
