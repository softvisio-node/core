...
export default class PasswordHash {
    ...
    constructor ( { preset, version, memoryCost, timeCost, parallelism, cost, blockSize, maxMem, iterations, saltLength, hashLength } = {} ) {
        ...
        // scrypt
            ...
            this.#options.maxMem = maxMem;
            if ( !this.#options.maxMem ) {

                // 128 * p * r + 128 * ( 2 + N ) * r
                this.#options.maxMem = 128 * this.#options.parallelism * this.#options.blockSize + 128 * ( 2 + this.#options.cost ) * this.#options.blockSize;
            }
        }
    ...
    // public
    async createHash ( password, { phc = true, salt, hashLength, keyId, data } = {} ) {
    ...
    async verifyHash ( digest, password, { update, phc = true, keyId } = {} ) {
        try {
            ...
            if ( update && requireUpdate ) {
                const res = await this.createHash( password, { phc } );
                if ( !res.ok ) return res;
    ...
    async #createScryptHash ( password, { id, phc, salt, hashLength, cost, blockSize, parallelism, maxMem } ) {
