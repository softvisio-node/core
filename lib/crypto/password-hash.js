...
export default class PasswordHash {
    ...
    async #createArgon2Hash ( password, { id, phc, salt, hashLength, version, memoryCost, timeCost, parallelism, secret, data } ) {
        secret ||= Buffer.allocUnsafe( 0 );
        if ( !( secret instanceof Buffer ) ) throw "Secret value is not valid";

        data ||= Buffer.allocUnsafe( 0 );
        if ( !( data instanceof Buffer ) || data.length > 32 ) throw "Data value is not valid";

        if ( !CREATE_ARGON2_HASH ) {
            ( { "default": CREATE_ARGON2_HASH } = await import( "@softvisio/argon2" ) );
        }

        const hash = await CREATE_ARGON2_HASH( password, {
            id,
            version,
            memoryCost,
            timeCost,
