...
export default class AvlTree {
    ...
    append ( key, value ) {
        return this.#insert( key, value, this.#allowDuplicates );
    }
    ...
    load ( keys = [], values = [], presort ) {
        if ( this.#size !== 0 ) throw Error( `Tree is not empty` );

        const size = keys.length;

        if ( presort ) this.#sort( keys, values, 0, size - 1, this.#comparator );

        this.#root = this.#loadRecursive( null, keys, values, 0, size );
    ...
    // private
    #insert ( key, value, allowDuplicates ) {
        if ( !this.#root ) {
    ...
    #loadRecursive ( parent, keys, values, start, end ) {
        ...
        if ( size > 0 ) {
            ...
            node.left = this.#loadRecursive( node, keys, values, start, middle );
            node.right = this.#loadRecursive( node, keys, values, middle + 1, end );
