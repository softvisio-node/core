    ...
    async _transform ( chunk, encoding, callback ) {
        try {
            if ( !this.#decipher ) {
                ...
                if ( this.#openssl ) {
                    const saltLength = this.#openssl.saltLength || OPENSSL_DEFAULT_SALT_LENGTH,
                        offset = 8 + saltLength;
                    ...
                    // openssl header is complete
                        ...
                        const salt = this.#buffer.subarray( 8, offset );

                        const res = await deriveOpensslKey( this.#key, { ...this.#openssl, salt } );

                        if ( res.ok ) {
                            this.#decipher = crypto.createDecipheriv( "aes-256-cbc", res.data.key, res.data.iv );

                            callback( null, this.#decipher.update( this.#buffer.subarray( offset ) ) );
                        }
                        else {
                            callback( res );
                        }
                    ...
                    const { "value": [ algorithm, keyId, iv, authTag ] = [], offset } = msgpack.decodeStream( this.#buffer ) || {};
                    ...
                    // header is complete
                        ...
                        if ( typeof key === "function" ) {
                            this.#key = await this.#key( keyId );
                        }

                        if ( this.#key.key ) this.#key = this.#key.key;

                        this.#decipher = crypto.createDecipheriv( algorithm, this.#key, iv );
                ...
                callback( null, this.#decipher.update( chunk, encoding ) );
            }
        }
        catch ( e ) {
            callback( e );
...
export async function encrypt ( key, data, { inputEncoding, outputEncoding, openssl } = {} ) {
    ...
    if ( data instanceof stream.Readable ) {
        ...
        streams.push( new EncryptStream( cipher, header ) );
...
export async function decrypt ( key, data, { inputEncoding, outputEncoding, openssl } = {} ) {
    ...
    if ( data instanceof stream.Readable ) {
        ...
        streams.push( new DecryptStream( key, {
            openssl,
        } ) );
