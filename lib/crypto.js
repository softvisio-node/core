...
import { Decrypt, Encrypt, Hash, Hmac, Sign } from "#lib/stream/crypto";
...
export function hash ( algorithm, data, { inputEncoding, outputEncoding } = {} ) {
    if ( data instanceof stream.Readable ) {
        return createPipeline( data, new Hash( algorithm, { outputEncoding } ), { inputEncoding } );
    }
    else {
...
export function hmac ( algorithm, key, data, { inputEncoding, outputEncoding } = {} ) {
    if ( data instanceof stream.Readable ) {
        return createPipeline( data, new Hmac( algorithm, key, { outputEncoding } ), { inputEncoding } );
    }
    else {
        return crypto.createHmac( algorithm, key ).update( data, inputEncoding ).digest( outputEncoding );
    }
}
...
export function sign ( algorithm, data, privateKey, { inputEncoding, outputEncoding } = {} ) {
    if ( data instanceof stream.Readable ) {
        return createPipeline( data, new Sign( algorithm, privateKey, { outputEncoding } ), { inputEncoding } );
    }
    else {
        return crypto.createSign( algorithm ).update( data, inputEncoding ).sign( privateKey, outputEncoding );
    }
}
