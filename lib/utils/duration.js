...
export default class Duration {
    ...
    static parse ( value, unit = "millisecond" ) {
        ...
        // number
            ...
            const millisecond = Math.abs( value ) * UNIT_MILLISECONDS[unit],
        ...
        // date
            ...
            if ( !isNaN( number ) ) {
                const millisecond = Math.abs( number ) * UNIT_MILLISECONDS[unit],
                    nanosecond = ( millisecond - Math.floor( millisecond ) ) * 1_000_000;
    ...
    getFormatRelativeDateParams () {
        if ( !this.#formatRelativeDateParams ) {
            ...
            for ( const unit of Object.keys( ["year", "month", "week", "day", "hour", "minute", "second"] ) ) {
                if ( !this.#units[unit] ) continue;

                this.#formatRelativeDateParams = [this.#negative ? 0 - this.#units[unit] : this.#units[unit], unit];

                break;
            }

            if ( !this.#formatRelativeDateParams ) this.#formatRelativeDateParams = [0, "second"];
        }

        console.log( this.#formatRelativeDateParams );
