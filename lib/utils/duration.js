...
export default class Duration {
    ...
    #nurmalize () {
        if ( !this.#normalizedUnits ) {
            ...
            // nanoseconds
            if ( units.nanosecond >= 1000 ) {
                units.microsecond += Math.floor( units.nanosecond / 1000 );
                units.nanosecond = units.nanosecond % 1000;
            }

            // microseconds
            if ( units.microsecond >= 1000 ) {
                units.millisecond += Math.floor( units.microsecond / 1_000 );
                units.microsecond = units.microsecond % 1000;
            }
