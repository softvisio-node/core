...
export function isKebabCase ( string, { sep } = {} ) {
    ...
    for ( string of strings ) if ( !/^[a-z]+(?:-[a-z\d]+)*$/.test( string ) ) return false;
...
export function isSnakeCase ( string, { sep, allowProtected } = {} ) {
    ...
    for ( string of strings ) {
        ...
        if ( !/^[a-z]+(?:_[a-z\d]+)*$/.test( string ) ) return false;
    }

    return true;
}

export function isConstantCase ( string, { sep, allowProtected } = {} ) {
    var strings;
