import idnEmail from "ajv-formats/formats/idn-email.js";
...
export function isInvalidEmail ( value ) {
    if ( !idnEmail( value ) ) return `Email address is invalid`;

    // if ( !/^[a-z\d][a-z\d._-]*[a-z\d]@(?:[a-z\d][a-z\d-]*.)+[a-z\d][a-z\d-]*[a-z\d]$/i.test( value ) ) return `Email address is invalid`;
}
