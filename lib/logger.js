...
export default class Logger extends Console {
    ...
    constructor ( { stdout, stderr, colorMode } = {} ) {
        ...
        if ( colorMode === "auto" ) {
            console.log( this._stdout?.isTTY );
            console.log( this._stdout.getColorDepth );

            this.#stdoutColorMode = this._stdout?.isTTY && ( typeof this._stdout.getColorDepth === "function" ? this._stdout.getColorDepth() > 2 : true );

            this.#stderrColorMode = this._stderr?.isTTY && ( typeof this._stderr.getColorDepth === "function" ? this._stderr.getColorDepth() > 2 : true );
        }
        else {
