...
export default class CachePersistent {
    ...
    has ( key ) {
        ...
        if ( listEntry ) {
            if ( listEntry.value.isExpired ) return false;
            else return true;
        }

        const cacheEntry = this.#dbh.selectRow( query.get, [key] );
    ...
    get ( key ) {
        ...
        if ( listEntry ) {
            if ( listEntry.value.isExpired ) return;
            else return this.#cacheLru.get( key );
        }
