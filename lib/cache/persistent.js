...
const query = {
    "schema": sql`
CRETE TABLE IF NOT EXISTS cache (
    id text PRIMARY KEY NOT NULL,
    value bytea NOT NULL,
    expires int8
);
`,
    "get": sql`SELECT * FROM cache WHERE id = ?`.prepare(),
    "set": sql`INSERT INTO cache ( id, value, expires ) VALUES ( ?, ?, ? )  WHERE id = ?`.prepare(),
...
export default class CachePersistent {
    ...
    has ( key ) {
        ...
        if ( entry.data.expires ) {
            maxAge = entry.data.expires - Date.now();
    ...
    set ( key, value, maxAge ) {
        ...
        if ( !maxAge || maxAge > 0 ) {
            ...
            this.#cache.set( key, value, maxAge );
            this.#dbh.do( query.set, [key, msgpack.encode( value ), expires, key] );
        }
        else {
            this.delete( key );
        }
