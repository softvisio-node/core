...
export default class Cron extends Events {
    ...
    constructor ( source, { timezone, maxTicks, runMissed } = {} ) {
        ...
        if ( this.#isRealDate ) {
            if ( this.#timezone ) {
                this.#source = this.#source.toZonedDateTime( this.#timezone );
            }
            else {
                this.#source = this.#source.toZonedDateTime( Temporal.Now.timeZoneId() );
            }
        }
    }
    ...
    getNextDates ( { num = 1, from } = {} ) {
        ...
        if ( this.#timezone ) {
            date = date.toZonedDateTime( this.#timezone );
        }
        else {
        ...
        if ( this.#isRealDate ) {
            if ( date.epochMilliseconds >= this.#source.epocMilliseconds ) {
                dates.push( new Date( this.#source.epocMilliseconds ) );
            }
        }
        else {
            for ( ; num > 0; num-- ) {
