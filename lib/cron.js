...
export default class Cron extends Events {
    ...
    constructor ( source, zone ) {
        ...
        if ( typeof zone === "number" ) {
            ...
            const dt = DateTime.fromObject( {}, { "zone": zone } );
    ...
    get nextDate () {
        return this.nextDates( 1 )[0];
    }

    get timeout () {
        return Math.max( -1, this.nextDate - DateTime.local() );
    }
    ...
    toJSON () {
        return TIME_UNITS.map( timeName => this.#wcOrAll( timeName ) );
    }

    nextDates ( i = 1 ) {
        var date = this.#isRealDate ? this.#source : DateTime.local();
        ...
        if ( typeof this.#utcOffset !== "undefined" ) {
            ...
            if ( date.invalid ) {
                throw Error( "You specified an invalid UTC offset." );
            }
        }

        if ( this.#isRealDate ) {
            if ( DateTime.local() > date ) {
                throw Error( "Date in past. Will never be fired." );
            }
    ...
    #getNextDateFrom ( start, zone ) {
        ...
        while ( true ) {
            ...
            if ( !( date.day in this.dayOfMonth ) && Object.keys( this.dayOfMonth ).length !== 31 && !( this.#getWeekDay( date ) in this.dayOfWeek && Object.keys( this.dayOfWeek ).length !== 7 ) ) {
                date = date.plus( { "days": 1 } );
                date = date.set( { "hour": 0, "minute": 0, "second": 0 } );
                continue;
            }

            if ( !( this.#getWeekDay( date ) in this.dayOfWeek ) && Object.keys( this.dayOfWeek ).length !== 7 && !( date.day in this.dayOfMonth && Object.keys( this.dayOfMonth ).length !== 31 ) ) {
                date = date.plus( { "days": 1 } );
                date = date.set( { "hour": 0, "minute": 0, "second": 0 } );
                continue;
            }
