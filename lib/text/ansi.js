...
class ANSI {
    ...
    addStyles ( styles ) {
        for ( const name in styles ) this.addStyle( name, styles[name] );
    }
    ...
    addColors ( colors ) {
        for ( const name in colors ) this.addColor( name, colors[name] );
    }
...
ansi.addStyles( {
    "hl": ansi.brightWhite,
    "dim": ansi.gray,
    "ok": ansi.brightWhite.onColor( 0x6400 ),
    "warn": ansi.color( 0x0 ).onColor( 0xcccc00 ),
    "error": ansi.brightWhite.onRed,
