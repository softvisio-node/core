...
export default class Table extends Events {
    ...
    #begin () {
        ...
        // calculate columns content length
        if ( this.#rows ) {
            for ( const row of this.#rows ) {
                ...
                for ( const column of this.#columns ) {
                    ...
                    const contentLength = ansi.remove( value ).length;

                    column.contentLength ??= contentLength;

                    if ( contentLength > column.contentLength ) {
                        column.contentLength = contentLength;
                    }
                }
            }
        ...
        if ( !width ) {
            width = this.#stream?.columns || DEFAULT_WIDTH;
        }
