...
export default class IpAddress {
    ...
    constructor ( address ) {
        ...
        // ipv4 number
            ...
            // check range
            if ( address < 0 || address > MAX_IPV4_VALUE ) throw Error( PARSING_ERROR_MESSAGE );
        ...
        // ipv6 number
            ...
            // check range
            if ( address < 0n || address > MAX_IPV6_VALUE ) throw Error( PARSING_ERROR_MESSAGE );
        ...
        if ( !this.#string ) {
            if ( this.#isIpV4 ) {
                ...
                if ( this.#value === 0n ) {
                    ...
                    for ( let n = 0; n < 8; n++ ) {
                        fullHextets[n] = str.substring( n * 4, n * 4 + 4 );
    ...
    getPreviousAddr () {
        ...
        if ( !this.#previousAddr ) {
            if ( this.#isIpV4 ) this.#previousAddr = new this.constructor( this.#value - 1 );
    ...
    getNextAddr () {
        ...
        if ( !this.#nextAddr ) {
            if ( this.#isIpV4 ) this.#nextAddr = new this.constructor( this.#value + 1 );
