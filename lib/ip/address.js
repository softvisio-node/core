...
try {
    var { "default": geoipCity } = await import( "@softvisio/geoip-city" );
}
catch {}

try {
    var { "default": geoipAsn } = await import( "@softvisio/geoip-asn" );
}
catch {}
...
export default class IpAddress {
    ...
    // static
    static new ( ipAddress ) {
        if ( ipAddress instanceof IpAddress ) return ipAddress;

        return new this( ipAddress );
    }
    ...
    compare ( ipAddress ) {
        ...
        if ( this.#value < ipAddress.value ) {
            return -1;
        }
        else if ( this.#value === ipAddress.value ) {
            return 0;
        }
        else {
            return 1;
    ...
    eq ( ipAddress ) {
        return this.#value === IpAddress.new( ipAddress ).value;
    }
