...
export default class IPRange {
    ...
    contains ( range ) {
        ...
        else if ( range instanceof IPAddr ) min = max = range.toValue();
        else {
            if ( typeof range === "string" ) range = new IPRange( range );

            if ( range instanceof IPRange ) {
                min = range.firstAddr.toValue();
                max = range.lastAddr.toValue();
            }
        }

        if ( typeof min === "number" && this.#isV6 ) return false;

        if ( this.firstAddr.toValue() > min ) return false;

        if ( this.lastAddr.toValue() < max ) return false;
    ...
    inside ( range ) {
        ...
        if ( this.firstAddr.toValue() < range.firstAddr.toValue() ) return false;

        if ( this.lastAddr.toValue() > range.lastAddr.toValue() ) return false;
