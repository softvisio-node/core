...
export default class IpRange {
    ...
    constructor ( range, prefix ) {
        ...
        // range string
            ...
            this.#firstAddress = new IpAddress( range.substring( 0, idx ) );
            this.#lastAddress = new IpAddress( range.substring( idx + 1 ) );

            if ( this.#firstAddress.isIpV4 && !this.#lastAddress.isIpV4 ) throw Error( PARSING_ERROR_MESSAGE );
        ...
        // cidr or value or ip address
            ...
            if ( typeof range === "string" ) {
                ...
                // cidr string
                if ( idx > 0 ) {
                    prefix ??= +range.substring( idx + 1 );
                    range = new IpAddress( range.substring( 0, idx ) );
                }
    ...
    getRandomAddress () {
        ...
        if ( this.isIpV4 ) {
            ...
            return new IpAddress( min + Math.floor( Math.random() * max ) );
        }
        else {
            return new IpAddress( getRandomBigInt( min, max ) );
        }
    }
    ...
    combine ( range ) {
        ...
        if ( this.firstAddress.value >= range.firstAddress.value && this.firstAddress.value <= range.lastAddress.value ) {
            return new this.constructor( [range.firstAddress, this.lastAddress.value > range.lastAddress.value ? this.lastAddress : range.lastAddress] );
        }

        if ( range.firstAddress.value >= this.firstAddress.value && range.firstAddress.value <= this.lastAddress.value ) {
            return new this.constructor( [this.firstAddress, this.lastAddress.value > range.lastAddress.value ? this.lastAddress : range.lastAddress] );
        }
    }
}
