...
export default class IpRange {
    ...
    constructor ( range, prefix ) {
        ...
        // range string
            ...
            this.#firstAddr = new IpAddress( range.substring( 0, idx ) );
            this.#lastAddr = new IpAddress( range.substring( idx + 1 ) );

            if ( this.#firstAddr.isIpV4 && !this.#lastAddr.isIpV4 ) throw Error( PARSING_ERROR_MESSAGE );
            ...
            if ( range instanceof IpAddress ) {
                ...
                if ( range.isIpV4 ) {
                    ...
                    if ( prefix === 32 ) {
                        ...
                        this.#firstAddr = new IpAddress( ( range.value & V4_MASK0[prefix] ) >>> 0 );

                        V4_MASK1[prefix] ??= Number( "0b" + "0".repeat( prefix ) + "1".repeat( 32 - prefix ) );
                        this.#lastAddr = new IpAddress( ( range.value | V4_MASK1[prefix] ) >>> 0 );
                    }
                }
                ...
                // v6
                    ...
                    if ( prefix === 128 ) {
                        ...
                        this.#firstAddr = new IpAddress( range.value & V6_MASK0[prefix] );

                        V6_MASK1[prefix] ??= BigInt( "0b" + "0".repeat( prefix ) + "1".repeat( 128 - prefix ) );
                        this.#lastAddr = new IpAddress( range.value | V6_MASK1[prefix] );
                    }
                }
    ...
    // properties
    get isIpV4 () {
        return this.#firstAddr.isIpV4;
    }

    get isIpV6 () {
        return this.#firstAddr.isIpV6;
    }
    ...
    get prefix () {
        if ( this.#prefix === undefined ) {
            const isIpV4 = this.#firstAddr.isIpV4;
    ...
    contains ( range ) {
        ...
        else if ( range instanceof IpAddress ) min = max = range.value;
        else {
        ...
        if ( typeof min === "number" && this.isIpV6 ) return false;
    ...
    inside ( range ) {
        ...
        if ( this.isIpV4 && range.isIpV6 ) return false;
