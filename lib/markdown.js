...
export default class Markdown {
    ...
    toAnsi ( { styles = {} } = {} ) {
        ...
        return this.toMarkdown( {
            "handlers": {
                "inlineCode": node => {
                    return styles.inlineCode( this.#prepareString( node ) );
                },
                ...
                "heading": ( node, parent, context ) => {
                    ...
                    return styles.heading( value
                        ? prefix + " " + value
                        : prefix );
                },

                "emphasis": ( node, parent, context ) => {
                    return styles.emphasis( this.#prepareString( node ) );
                },

                "strong": ( node, parent, context ) => {
                    return styles.strong( this.#prepareString( node ) );
                },

                "thematicBreak": ( node, parent, context ) => {
                    return styles.thematicBreak( "—".repeat( width ) );
                },
            },
        } );
    }
