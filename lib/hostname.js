...
const PUBLIC_SUFFIXES = await externalResources.add( "softvisio-node/core/resources/public-suffixes" ).on( "update", loadResources ).check();

const TLD = await externalResources.add( "softvisio-node/core/resources/tld" ).on( "update", loadResources ).check();
