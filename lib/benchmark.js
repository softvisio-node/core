...
class Benchmark {
    ...
    constructor ( tests, { iterations, time, threads } = {} ) {
        this.#tests = tests;
    ...
    async run ( options, parent ) {
        console.log( `Iterations: ${ this.iterations }, time: ${ this.time }, threads: ${ this.threads }` );
        ...
        for ( const name in meta ) {
            ...
            if ( this.time ) {
                meta[ name ].maxDuration = BigInt( this.time ) * 1000000n;
            }
        }
...
export default async function benchmark ( tests, { iterations, time, threads } = {} ) {
    const bench = new Benchmark( tests, { iterations, time, threads } );
