...
import ReadLine from "./stream/readline.js";
...
async function readLineSearch ( stream, eol, maxSize, buf ) {
    return new Promise( resolve => {
        var streamSearch = new StreamSearch( eol ),
            buffers = [],
            onReadable,
            onInfo,
            totalSize = 0,
        ...
        onReadable = () => {
            ...
            buf = stream.read( this.readableLength > readSize ? readSize : null );
        ...
        onInfo = ( isMatch, buf, start, end ) => {
            ...
            if ( isMatch ) {
                ...
                if ( buf ) {
                    if ( end - start > 0 ) buffers.push( buf.slice( start, end ) );

                    // unshift rest of the data
                    if ( buf.length > end + eol.length ) stream.unshift( buf.slice( end + eol.length ) );
            ...
            // not matched
                ...
                // unshift rest of the data
                if ( found ) {
                    stream.unshift( buf.slice( start, end ) );
