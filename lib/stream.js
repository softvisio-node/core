...
stream.Readable.prototype.readChunk = async function ( chunkLength, { encoding, lastChunk } = {} ) {
    ...
    if ( this.destroyed ) {
        if ( lastChunk && this.readableLength ) return this.read();
        else return;
    }
    ...
    return new Promise( ( resolve, reject ) => {
        ...
        const end = () => {
            ...
            if ( !lastChunk && buffer.length < chunkLength ) {
        ...
        readable = () => {
            ...
            const buf = this.read( this.readableLength > readSize ? readSize : this.readableLength );
...
async function readLineSearch ( stream, eol, maxLength ) {
    return new Promise( ( resolve, reject ) => {
        ...
        readable = () => {
            ...
            // max length reached
            else if ( totalLength >= maxLength + eol.length ) end();
        };
