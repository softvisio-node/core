...
stream.file = {
    "PassThrough": fileStream( stream.PassThrough ),

    "Transform": fileStream( stream.Transform ),

    "Duplex": fileStream( stream.Duplex ),

    "Readable": class extends fileStream( stream.Readable ) {
        static from ( buffer, { name, type, size, ...options } = {} ) {
            const readable = super.from( buffer, options );

            readable.setName( name ).setType( type ).setSize( size );

            return readable;
        }
    },

    "Writable": fileStream( stream.Writable ),
};
