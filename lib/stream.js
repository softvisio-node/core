...
stream.Readable.prototype.tmpFile = async function ( options = {} ) {
    ...
    if ( this.destroyed ) {
        if ( this.readableLength ) {
            if ( options.maxLength && this.readableLength > options.maxLength ) throw `Length limit exceeded`;
...
stream.Readable.prototype.readChunk = async function ( chunkLength ) {

    // chunk is already buffered
    if ( this.readableLength >= chunkLength ) return this.read( chunkLength );

    // stream is destroyed, no more data can be read
    if ( this.destroyed ) return Buffer.from( "" );
    ...
    return new Promise( ( resolve, reject ) => {
        ...
        const end = () => {
            ...
            // no data
            if ( !buffers.length ) return resolve( Buffer.from( "" ) );
            ...
            // data length > required chunk length
                ...
                this.unshift( buffer.slice( chunkLength ) );

                resolve( buffer.slice( 0, chunkLength ) );
            }
            else {
                resolve( buffer );
            }
...
stream.Readable.prototype.readHttpHeaders = async function ( { maxLength } = {} ) {
    maxLength ||= DEFAULT_HTTP_HEADERS_MAX_LENGTH;
