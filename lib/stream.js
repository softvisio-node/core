...
const DEFAULT_HEADERS_MAX_LENGTH = 1024 * 64;

Stream.Readable.prototype.writeToBuffer = async function ( options = {} ) {
    const buffers = [];
    ...
    this.on( "data", data => {
        ...
        if ( options.maxSize && size > options.maxSize ) {
            this.destroy( "Size limit exceeded" );
        }
        else {
            buffers.push( data );
        }
...
Stream.Readable.prototype.writeToTmpFile = async function ( options = {} ) {
    const tmpFile = new fs.TmpFile( options ),
    ...
    return new Promise( resolve => {
        if ( options.maxSize ) {
            ...
            this.on( "data", data => {
                ...
                if ( size > options.maxSize ) this.destroy( "Size limit exceeded" );
            } );
        }
...
Stream.Readable.prototype.readHttpHeaders = async function ( options = {} ) {
    const maxLength = options.maxLength || DEFAULT_HEADERS_MAX_LENGTH;

    const buf = await this.readLine( { "eol": HEADERS_EOL, maxLength, "encoding": options.encoding ?? "binary" } );
