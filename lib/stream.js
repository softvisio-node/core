...
stream.Readable.prototype.readLine = async function ( { eol, maxLength, encoding, lastEolRequired } = {} ) {
    ...
    return new Promise( ( resolve, reject ) => {
        ...
        readable = () => {
            ...
            if ( !buffer ) {
                buffer = this.read( this.readableLength );

                idx = buffer.indexOf( eol );
            }
            else {
                const start = buffer.length - eol.length;

                buffer = Buffer.concat( [buffer, this.read( this.readableLength )] );

                idx = buffer.indexOf( eol, start );
            }
