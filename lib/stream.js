...
const DEFAULT_HTTP_HEADERS_MAX_LENGTH = 16384;

stream.Readable.prototype.blackhole = async function () {

    // strem is destroyed
    if ( this.destroyed ) {
        if ( this.readableLength ) this.read( null );
...
// lastChunk - return last chunk, even if it length less than chunkLength
stream.Readable.prototype.readChunk = async function ( chunkLength, { encoding } = {} ) {
    ...
    return new Promise( ( resolve, reject ) => {
        ...
        readable = () => {
            ...
            const buffer = this.read( this.readableLength > readSize ? readSize : null );
...
stream.Readable.prototype.readLine = async function ( { eol, maxLength, encoding, lastEolRequired } = {} ) {
    ...
    eol = eol == null ? DEFAULT_EOL : Buffer.isBuffer( eol ) ? eol : Buffer.from( eol, encoding );
    maxLength ||= Infinity;
    ...
    return new Promise( ( resolve, reject ) => {
        ...
        error = e => {
            ...
            // found
                ...
                resolve( encoding ? line.toString( encoding ) : line );
            }
        };
        ...
        readable = () => {
            ...
            if ( !buffer ) {
                ...
                buffer = this.read( size > this.readableLength ? this.readableLength : size );
            }
            else {
                ...
                buffer = Buffer.concat( [buffer, this.read( size > this.readableLength ? this.readableLength : size )] );
            }
            ...
            // found
                ...
                if ( buffer.length > idx + eol.length ) this.unshift( buffer.slice( idx + eol.length ) );
