...
export default class Acme {
    ...
    // static
    static canGetCertificate ( domain ) {
        try {
    ...
    async getCertificate ( { domains, createChallenge, deleteChallenge } = {} ) {
        ...
        for ( const domain of domains ) {
            if ( !this.canGetCertificate( domain ) ) return result( [400, `Domains are not valid`] );
        }
    ...
    canGetCertificate ( domain ) {
        return this.constructor.canGetCertificate( domain );
    }
