...
import { createCsr } from "#lib/crypto/certificates";
...
export default class Acme {
        ...
        for ( const domain of domains ) {
            try {
                ...
                if ( domain.startsWith( "*." ) ) {
                    ...
                    if ( !hostname.isDomain || !hostname.isValid || hostname.isTld || !hostname.tldIsValid ) {
                        return false;
                    }
                }
    ...
    async getCertificate ( { domains, checkDomain, createChallenge, deleteChallenge } = {} ) {
        ...
        if ( checkDomain ) {
            ...
            for ( const record of Object.values( index ) ) {
                ...
                if ( canGetCertificate instanceof Promise ) {
                    ...
                    canGetCertificate.then( canGetCertificate => {
        ...
        for ( const authorization of authorizations ) {
            ...
            if ( !authorizationDone ) {
                ...
                return result( [ 500, `ACME failed authorize domain: "${ ( authorization.wildcard
                    ? "*."
                    : "" ) + record.domain }"` ] );
            }
        }
        ...
        // create csr
        const { csr, privateKey } = await createCsr( domains );
    ...
    async #verifyChallenge ( { type, domain, dnsTxtRecordName, httpLocation, content, dnsTtl } ) {
        var attempt,
            interval = 3; // seconds
            ...
            // dns
                ...
                const res = await new Promise( resolve => {
                    resolver.resolveTxt( dnsTxtRecordName, ( error, records ) => {
                        if ( error ) return resolve( result.catch( error, { "log": false } ) );
