...
export default class Acme {
        ...
        for ( const authorization of authorizations ) {
            ...
            for ( const challenge of authorization.challenges ) {
                ...
                try {
                    ...
                    console.log( "--- CREATED", type, record.domain, res + "" );
                    ...
                    console.log( "--- VER", type, record.domain, res + "" );
                    ...
                    console.log( "--- COMPLETE", type, record.domain, res + "" );
    ...
    async createAccount () {
        ...
        if ( !this.#accountUrl ) {
            ...
            if ( this.#mutex.tryLock() ) {
                if ( !this.#accountKey ) {
                    this.#accountKey = ( await certificates.createEcdsaKeyPair() ).privateKey;
                }
    ...
    async #finalizeOrder ( order, csr ) {
        csr = csr.replace( "-----BEGIN CERTIFICATE REQUEST-----", "" ).replace( "-----END CERTIFICATE REQUEST-----", "" ).replaceAll( "\r\n", "" );

        csr = Buffer.from( csr, "base64" ).toString( "base64url" );
