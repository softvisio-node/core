        ...
        for ( const authorization of authorizations ) {
            ...
            for ( const challenge of authorization.challenges ) {
                const type = challenge.type,
                    ...
                    wildcardDomain = domain.startsWith( "*." ),
                    dnsTxtRecordName = `_acme-challenge.${wildcardDomain ? domain.substring( 2 ) : domain}`,
                    httpLocation = `/.well-known/acme-challenge/${challenge.token}`,
                    token = challenge.token,
                    content = this.#getChallengeKeyAuthorization( challenge );

                if ( wildcardDomain && type !== "dns-01" ) continue;
            ...
            if ( !authorizationDone ) {
                ...
                return result( [500, `Failed to perform authorization`] );
            }
        }
