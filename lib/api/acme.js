...
export default class Acme {
    ...
    async #verifyChallenge ( { type, domain, dnsTxtRecordName, httpLocation, content } ) {
        if ( type === "dns-01" ) {
            const rootDomain = new Hostname( domain ).rootDomain.ascii,
                recordName = `${dnsTxtRecordName}.${rootDomain}`;
            ...
            while ( true ) {
                ...
                try {
                    const record = dns.promises.resolveTxt( recordName );
