...
export default class Acme {
    ...
    constructor ( { provider, test, accountKey } = {} ) {
    ...
    async createAccount ( email ) {
        if ( this.#accountUrl ) {
            ...
            const res = await this.#createAccount( email );
    ...
    async waitForValidStatus ( item ) {
        ...
        const verifyFn = async abort => {
            ...
            else if ( res.data.status === "ready" || res.data.status === "valid" ) {
                return res.data;
            }

            throw new Error( `Unexpected item status: ${res.data.status}` );
        };
