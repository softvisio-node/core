...
export default class Acme {
            ...
            for ( const challenge of authorization.challenges ) {
                try {
                    ...
                    res = await createChallenge( {
                        ...
                        "dnsTxtRecordNamename": `_acme-challenge.${authorization.identifier.value}`,
    ...
    async #verifyChallenge ( authorization, challenge ) {
        if ( !authorization.url || !challenge.url ) {
        ...
        const verifyFn = async () => {
            await verify[challenge.type]( authorization, challenge, keyAuthorization );
        };
