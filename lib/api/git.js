...
export default class Git {
    ...
    async getStatus ( { pushStatus = true, releases = true } = {} ) {
        ...
        counter.value++;
        this.getCurrentVersion().then( res => {
            if ( res.ok ) {
                status.currentVersion = res.data.version;
                status.currentVersionDistance = res.data.versionDistance;
            }
            else {
                hasError = true;
            }
    ...
    async getCurrentVersion ( { release } = {} ) {
        var res,
            data = {
                "version": new Semver(),
                "versionDistance": 0,
        ...
        res = await this.exec( [ "describe", "--tags", "--always", "--match", filter ] );
    ...
    async getPushStatus () {
        const res = await this.exec( [ "branch", "-v", "--no-color" ] );
