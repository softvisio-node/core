...
export default class Git {
    ...
    async getCurrentRelease ( { commit, release } = {} ) {
        if ( Semver.isValid( commit ) ) {
            commit = Semver.new( commit );
        }

        if ( commit instanceof Semver ) {
            if ( commit.isNull ) {
                commit = null;
            }
            else {
    ...
    // XXX return Commit objects
    async getLog ( { startCommit, endCommit } = {} ) {

        // prepare start commit
        if ( Semver.isValid( startCommit ) ) {
            startCommit = Semver.new( startCommit );
        }

        if ( startCommit instanceof Semver ) {
        ...
        // prepare end commit
        if ( Semver.isValid( endCommit ) ) {
            endCommit = Semver.new( endCommit );
        }

        if ( endCommit instanceof Semver ) {
            if ( endCommit.isNull ) {
                endCommit = null;
            }
            else {
        ...
        // prepare git command
        const args = [ "log", "--pretty=format:%H%n%h%n%cI%n%cN%n%(decorate:prefix=,suffix=,pointer=->,tag=%x00tag:separator=%x00%x00)%n%P%n%B%x00%x00%x00" ];

        if ( startCommit ) {
            args.push( `${ startCommit }..${ endCommit }` );
        }
        else {
            args.push( `${ endCommit }` );
        }
