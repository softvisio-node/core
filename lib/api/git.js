    ...
    async getRelease ( { commit, stable } = {} ) {
        ...
        if ( commit.isRelease ) {
            ...
            if ( commit.releaseVersion.isStableRelease ) {
                ...
                data.previousRelease = releases.getPreviousVersion( commit.releaseVersion, { stable } );
            }

            // commit is pre-release
            else {
                if ( stable ) {
                    data.currentRelease = releases.getNextVersion( commit.releaseVersion, { "stable": true } );
                    data.previousRelease = releases.getPreviousVersion( commit.releaseVersion, { "stable": true } );
                }
                else {
                    data.currentRelease = commit.releaseVersion;
        ...
        // commit is not release
            ...
            if ( match && SemanticVersion.isValid( match.groups.version ) ) {
                ...
                if ( stable && !data.previousRelease.isStableRelease ) {
                    data.previousRelease = releases.getPreviousVersion( data.previousRelease, { stable } );
                }
            }

            data.currentRelease = releases.getNextVersion( data.previousRelease, { stable } );
        }
