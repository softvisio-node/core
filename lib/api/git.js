...
export default class Git {
    ...
    async getStatus ( { pushStatus = true, releases = true } = {} ) {
        ...
        this.getRelease().then( res => {
            if ( res.ok ) {
                ...
                status.currentVersionDistance = res.data.unreleasedCommits;
            }
            else {
                error = res;
            }
    ...
    async getRelease ( { commit, stable } = {} ) {
        ...
        if ( commit.isRelease ) {
            if ( stable ) {
                ...
                // commit release is stable
                if ( commit.releaseVersion.isStableRelease ) {
                    data.version = commit.releaseVersion;
                }

                // commit release is pre-release
                else {
                    res = await this.getReleses();
                    if ( !res.ok ) return res;
                    const releases = res.data;

                    data.version = releases.getNextVersion( commit.releaseVersion, { "stable": true } );

                    // no stable release found
                    if ( !data.version ) {
                        const previousStableRelease = releases.getPreviousVersion( commit.releaseVersion, { "stable": true } );

                        res = await this.getCommitsDistance( [ previousStableRelease, "HEAD" ] );
                        if ( !res.ok ) return res;
        ...
        // commit is not release commit
        else {

            // get previous release
            res = await this.exec( [ "describe", "--tags", "--always", "--candidates=1000", "--match=v[0-9]*.[0-9]*.[0-9]*", commit.hash ] );
            ...
            const description = res.data.trim(),
                match = description.match( /^(?<version>v\d.+?)-(?<distance>\d+)-(?<hash>g[\da-f]+)$/ );

            var previousRelease, nextRelease;

            // previous release found
            if ( match && SemanticVersion.isValid( match.groups.version ) ) {
                previousRelease = SemanticVersion.new( match.groups.version );
            }

            // previous release found
            // NOTE: covered by commit identification
            // else if ( description.startsWith( "v" ) && SemanticVersion.isValid( description ) ) {
            //     previousRelease = SemanticVersion.new( description );
            // }
            ...
            if ( previousRelease ) {
                nextRelease = releases.getNextRelease( previousRelease, { stable } );
            }
            else {
                if ( stable ) {
                    nextRelease = releases.firstStableRelease;
                }
                else {
                    nextRelease = releases.firstRelease;
                }
            }

            // commit is released
            if ( nextRelease ) {
                data.version = nextRelease;
            }

            // commit is not released
            else {
                res = await this.getCommitsDistance( [ previousRelease, nextRelease ] );
                if ( !res.ok ) return res;
    ...
    async getReleases () {
        const res = await this.exec( [ "tag", "--list", "v[0-9]*.[0-9]*.[0-9]*" ] );
