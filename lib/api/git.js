...
export default class Git {
    ...
    async getStatus ( { pushStatus, releases } = {} ) {
        const status = {
    ...
    async getId ( { commit } = {} ) {
        ...
        if ( !res.data ) return result( [ 404, "Commit not found" ] );
    ...
    async getCurrentRelease ( { commit, stable, changes } = {} ) {
        ...
        if ( !commit.isBranchHead && !commit.isRelease ) {
        ...
        if ( !data.currentRelease ) {
            ...
            if ( res.data ) {
                ...
                data.unreleasedCommits = res.data.distance;
            }
        }
    ...
    async getRelease ( { commit, stable, changes } = {} ) {
        ...
        if ( res.data ) {
            data.previousRelease = releases.get( res.data.version );
        }
        ...
        // get changes
        if ( changes ) {
            res = await this.getChanges( [ data.previousRelease, commit.hash + offset ] );
            if ( !res.ok ) return res;
    ...
    async #findPreviousRelease ( commit, { stable } ) {
        ...
        const match = res.data.trim().match( /^(?<version>v\d+\.\d+\.\d+.*)-(?<distance>\d+)-(?<abbrev>[\da-f]+)$/ );
