...
export default class Git {
    ...
    async getCommits ( commitsRange ) {
        ...
        const tagId = uuid(),
            separator = uuid(),
            args = [ "log", `--pretty=format:%H%n%h%n%cI%n%cN%n%(decorate:prefix=,suffix=,pointer=->,tag=${ tagId },separator=${ separator })%n%P%n%B%x00`, commitsRange ];
        ...
        if ( res.data ) {
            for ( const commit of res.data.split( "\0" ) ) {
                ...
                // parse refs
                const refs = refsSegment.split( separator );
