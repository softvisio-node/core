...
export default class Git {
    ...
    async getRelease ( { commit, next, release } = {} ) {
        ...
        res = await this.exec( [ "describe", ...( next
            ? [ "--contains" ]
            : [] ), "--tags", "--always", "--candidates=1000", ...filter, commit ] );
        ...
        const match = description.match( /^(?<version>v\d.+?)-(?<distance>\d+)-(?<hash>g[\da-f]+)$/ );
        ...
        // release not found
            ...
            // get distance from commit to HEAD
            if ( next ) {
                res = await this.exec( [ "rev-list", "--count", `${ commit }..HEAD` ] );
            }
    ...
    async getHeadCommit () {
        return this.getCommit( "-1" );
    }

    async getCommit ( commit ) {
        const res = await this.getCommits( commit );
    ...
    async getCommits ( commitsRange ) {
        ...
        const rowsSeparator = uuid(),
            ...
            tagId = uuid(),
            args = [ "log", `--pretty=format:%H${ separator }%h${ separator }%cI${ separator }%cN${ separator }%(decorate:prefix=,suffix=,pointer=->,tag=${ tagId },separator=${ decorateSeparator })${ separator }%P${ separator }%B${ rowsSeparator }`, commitsRange ];
