...
export default class Git {
    ...
    async getStatus ( { pushStatus = true, releases = true } = {} ) {
        ...
        this.getRelease().then( res => {
            if ( res.ok ) {
                status.currentRelease = res.data.currentRelease || res.data.previousRelease;
    ...
    async getRelease ( { commit, stable } = {} ) {
        ...
        if ( commit.isRelease ) {
            ...
            if ( commit.releaseVersion.isStableRelease ) {
                data.currentRelease = commit.releaseVersion;
                data.previousRelease = releases.getPreviousRelease( commit.releaseVersion, { stable } );
            }

            // commit is pre-release
            else {
                if ( stable ) {
                    data.currentRelease = releases.getNextRelease( commit.releaseVersion, { "stable": true } );
                    data.previousRelease = releases.getPreviousRelease( commit.releaseVersion, { "stable": true } );
                }
                else {
                    data.currentRelease = commit.releaseVersion;
                    data.previousRelease = commit.releaseVersion;
                }
            }
        }
        ...
        // commit is not release
            ...
            if ( match && SemanticVersion.isValid( match.groups.version ) ) {
                data.previousRelease = SemanticVersion.new( match.groups.version );

                // expand previous release to the stable version
                if ( stable && !data.previousRelease.isStableRelease ) {
                    data.previousRelease = releases.getPreviousRelease( data.previousRelease, { stable } );
                }
            }

            data.currentRelease = releases.getNextRelease( data.previousRelease, { stable } );
        }

        if ( data.currentRelease ) {
            data.currentRelease = releases.get( data.currentRelease );
        }

        if ( data.previousRelease ) {
            data.previousRelease = releases.get( data.previousRelease );
        }
