...
export default Super =>
    class extends ( Super || Object ) {
        ...
        async buildImage ( context, { signal, credentials, options = {} } = {} ) {
            ...
            // add .dockerignore
            if ( !index.has( ".dockerignore" ) ) files.push( ".dockerignore" );

            // add Dockerfile
            if ( options.dockerFile ) {
                const dockerFile = path.posix.join( context, options.dockerFile );

                if ( !index.has( dockerFile ) && !fs.existsSync( dockerFile ) ) {
