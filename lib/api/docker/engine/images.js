    ...
    class extends ( Super || class {} ) {
        ...
        async pushImage ( image, { signal, credentials } ) {
            ...
            return this._doRequest( "post", `images/${ encodeURIComponent( image ) }/push`, {
                ...
                "registryAuth": this._getImageAuth( image, credentials ),
            } );
        }
        ...
        // XXX https://github.com/moby/moby/issues/48112
        async buildImage ( context, { signal, credentials, options = {} } = {} ) {
            ...
            // add .dockerignore
            if ( !index.has( ".dockerignore" ) && ( await pathExists( context + "/.dockerignore" ) ) ) {
                files.push( ".dockerignore" );
            }

            // add dockerfile
            if ( options.dockerFile ) {
                if ( !index.has( options.dockerFile ) && !( await pathExists( context + "/" + options.dockerFile ) ) ) {
                    files.push( options.dockerFile );
                }
            }
            else if ( !index.has( "dockerfile" ) && !index.has( "Dockerfile" ) ) {
                if ( await pathExists( context + "/dockerfile" ) ) {
                    files.push( "dockerfile" );
                }
                else if ( await pathExists( context + "/Dockerfile" ) ) {
                    files.push( "Dockerfile" );
                }
            }
            ...
            return this._doRequest( "post", "build", {
                ...
                "params": {
                    ...
                    "version": 2, // build using BuildKit
                },
                body,
