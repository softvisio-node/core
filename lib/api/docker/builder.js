...
export default class {
    ...
    constructor ( context, { composeFile, signal, credentials, commitId, autoTags, push, remove, interactive, force, build = {} } = {} ) {
    ...
    async #prepareContext () {
        ...
        // identify commit by tag
        res = await this.#git.getId( { "commit": this.#commitId } );
        ...
        if ( !this.#commitId ) {
            this.#commitId = this.#gitId.branch || this.#gitId.abbrev;
        }

        this.#tags = new Set( [ this.#commitId ] );

        // apply autobuild tags
        for ( const tag of this.#autoTags ) {
            if ( this.#gitId.branch === tag || this.#gitId.tags.includes( tag ) ) {
                this.#tags.add( tag );
            }
        }

        const clone = this.#gitId.isDirty || head.hash !== this.#gitId.hash;
        ...
        if ( clone ) {
            ...
            logger.write( `Checking out "${ this.#commitId }" ... ` );
            res = await this.#git.exec( [ "checkout", this.#gitId.hash ] );
            if ( !res.ok ) return result( res );
            logger.log( res + "\n" );
        }
