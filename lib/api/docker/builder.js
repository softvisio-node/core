...
export default class {
    ...
    constructor ( context, { signal, tag, autoTags, push, remove, interactive, force, args } = {} ) {
    ...
    async #buildImages () {
        ...
        if ( !this.#interactive ) dockerEngine = new DockerEngine();
            ...
            if ( this.#push ) {
                for ( const tag of image.tags ) {
                    if ( this.#interactive ) {
                        ...
                        let res = await dockerEngine.pushImage( tag, {
                            "username": "zdm@softvisio.net",
                            "password": "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",
                        } );
                        ...
                        res = await new Promise( resolve => {
                            stream.on( "data", data => {
                                if ( ( data, status ) ) {
                                    ...
                                    this.#logger.log( JSON.stringify( data.aux, null, 4 ) );
