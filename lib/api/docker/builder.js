...
const schema = new Ajv().addSchema( readConfig( "#resources/schemas/docker-compose.schema.yaml", { "resolve": import.meta.url } ) );
...
export default class {
    ...
    async #prepareContext () {
        ...
        if ( !schema.validate( "docker-compose", this.#composeFile ) ) return result( [ 400, `not valid, errors:\n${ schema.errors }` ] );
        logger.log( `OK` );
        ...
        for ( const service of Object.values( this.#composeFile.services || {} ) ) {
            ...
            if ( images[ imageName ] ) {
                if ( service.depends_on ) {
                    images[ imageName ].dependencies.push( ...( Array.isArray( service.depends_on ) ? service.depends_on : Object.keys( service.depends_on ) ) );
                }
            ...
            const image = {
                "image": imageName,
                "dependencies": service.depends_on ? ( Array.isArray( service.depends_on ) ? [ ...service.depends_on ] : Object.keys( service.depends_on ) ) : [],
                "build": { ...this.#build },
            };
