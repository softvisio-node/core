...
export default class {
    ...
    async #prepareContext () {
        ...
        // identify build commit
        res = await this.#git.getId( { "commitRef": this.#commitRef === "."
            ? "HEAD"
            : this.#commitRef } );
        ...
        if ( this.#commitRef === "." && this.#gitId.isDirty ) {
            this.#tags.add( "dirty" );
        ...
        // build by commit ref
            ...
            if ( this.#commitRef && this.#commitRef !== "." ) {
                if ( this.#gitId.branches.includes( this.#commitRef ) || this.#gitId.tags.includes( this.#commitRef ) ) {
                    this.#tags.add( this.#commitRef );
                }
                else if ( this.#gitId.hash.startsWith( this.#commitRef ) ) {
                    this.#tags.add( this.#gitId.abbrev );
                }
            }

            // add autobuild tags
            for ( const tag of this.#autoTags ) {
                if ( this.#gitId.branches.includes( tag ) || this.#gitId.tags.includes( tag ) ) {
                    this.#tags.add( tag );
                }
            }
            ...
            if ( this.#gitId.isDirty ) {
                clone = this.#gitId.abbrev;
            }
            else {
