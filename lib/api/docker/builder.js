...
export default class {
    ...
    async #prepareContext ( tag ) {
        ...
        // define tag to clone
        tag ||= this.#composeFile["x-build"].default_tag || status.branch;
        ...
        if ( clone ) {
            ...
            let res = await git.run( "clone", "--quiet", context, this.#context );
            if ( !res.ok ) return result( res );
            logger.log( res + "" );

            git = new Git( this.#context );
        ...
        if ( this.#composeFile["x-build"].auto_tags ) {
            ...
            for ( const tag of this.#composeFile["x-build"].auto_tags ) {
                if ( status.branch === tag || status.tags.includes( tag ) ) this.#tags.add( tag );
            }
        }
