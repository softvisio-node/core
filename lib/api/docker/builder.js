...
export default class {
    ...
    async run () {
        ...
        try {
            ...
            // prepare context
            res = await this.#prepareContext( logger, signal, rootPackage.root, process.cli.arguments.tag );
            ...
            if ( !tags.size ) throw `No tags to build found`;

            const composeFile = res.data.composeFile;

            // prepare images
            res = await this.#prepareImages( logger, signal, tags, composeFile );
            if ( !res.ok ) throw res;
        }
        catch ( e ) {
            res = result.catch( e, { "silent": true, "keepError": true } );

            if ( !res.ok ) logger.logError( res + "" );
        }
    ...
    async run1 () {
        ...
        // sort services
        const res = this.#sort( services );
        if ( !res.ok ) this._throwError( res.statusText, dockerRoot );
    ...
    // primary
    async #prepareContext ( logger, signal, context, tag ) {

        // read docker-compose.yaml
        if ( !fs.existsSync( context + "/docker-compose.yaml" ) ) return result( [404, `docker-compose.yaml not found`] );

        const composeFile = readConfig( context + "/docker-compose.yaml" );

        // validate docker-compose.yaml
        logger.write( `Validating docker-compose.yaml ... ` );
        if ( !validate( composeFile ) ) return result( [400, `not valid, errors:\n${validate.errors}`] );
        logger.log( `OK` );

        var git = new Git( context );

        var status = await git.getId();
        if ( !status.ok ) return status;
        if ( !status.data.hash ) return result( [500, `Unable to identify current changeset`] );
        status = status.data;

        // define tag to clone
        tag ||= composeFile["x-build"].default_tag || status.branch;

        const clone = status.isDirty || !( status.branch === tag || status.hash.startsWith( tag ) || status.tags.includes( tag ) );

        // clone git repo
        if ( clone ) {
            const tmpContext = new TmpDir();

            logger.write( `Cloning ... ` );
            let res = await git.run( "clone", "--quiet", context, tmpContext );
            if ( !res.ok ) return result( res, { "context": tmpContext } );
            logger.log( res + "" );
            ...
            res = await git.run( "checkout", tag );
            if ( !res.ok ) return result( res, { context } );
            logger.log( res + "\n" );

            status = await git.getId();
            if ( !status.ok ) return result( status, { context } );
            status = status.data;
        }
        ...
        // apply autobuild tags
        if ( composeFile["x-build"].auto_tags ) {
            if ( !Array.isArray( composeFile["x-build"].auto_tags ) ) {
                composeFile["x-build"].auto_tags = [composeFile["x-build"].auto_tags];
            }

            for ( const tag of composeFile["x-build"].auto_tags ) {
                if ( status.branch === tag || status.tags.includes( tag ) ) tags.add( tag );
            }
        }
    ...
    async #prepareImages ( logger, signal, tags, composeFile ) {
        var services = {};
        ...
        for ( const service of Object.values( composeFile.services ) ) {
            ...
            if ( services[image] ) {
                ...
                if ( service.depends_on ) {
                    for ( const name of service.depends_on ) {
                        const baseImage = composeFile.services[name]?._baseImage;
            ...
            services[image] = {
                image,
                "dependencies": ( service.depends_on || [] ).map( name => composeFile.services[name]?._baseImage ).filter( image => image ),
                "images": new Set(),
                "params": service.build,
            };
    ...
    #sort ( services ) {
        const sorted = [],
            colors = {},
            sort = function ( service ) {
                const color = colors[service.image] || "white";
                ...
                if ( color === "black" ) {
                    ...
                    colors[service.image] = "grey";

                    for ( const deps of service.dependencies ) {

                        // dependency not exists
                        if ( !services[deps] ) continue;

                        // same image
                        if ( services[deps].image === service.image ) continue;

                        const res = sort( services[deps] );
                        if ( !res.ok ) return res;
                    }

                    sorted.push( service );

                    colors[service.image] = "bkack";
        ...
        for ( const service of Object.values( services ) ) {
            const res = sort( service );
            if ( !res.ok ) return res;
        }

        return result( 200, sorted );
    }
}
