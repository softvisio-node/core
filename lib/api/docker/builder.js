...
export default class {
    ...
    async #prepareContext () {
        ...
        // identify git commit
        this.#gitId = await this.#git.getId( { "commit": this.#tag } );
        if ( !this.#gitId.ok ) return this.#gitId;
        this.#gitId = this.#gitId.data;

        if ( !this.#tag ) {
            this.#tag = this.#gitId.branch || this.#gitId.abbrev;
        }
        ...
        const clone = this.#gitId.isDirty || !( this.#gitId.hash.startsWith( this.#tag ) || this.#tag === this.#gitId.branch || this.#gitId.tags.includes( this.#tag ) );
        ...
        if ( clone ) {
            ...
            let res = await this.#git.exec( [ "clone", "--quiet", context, this.#context.path ] );
            if ( !res.ok ) return result( res );
            logger.log( res + "" );

            this.#git = new Git( this.#context.path );

            logger.write( `Checking out "${ this.#gitId.hash }" ... ` );
