...
export default class {
    ...
    // XXX
    constructor ( context, { signal, credentials, tag, autoTags, push, remove, interactive, force, build = {} } = {} ) {
    ...
    async #prepareContext () {
        ...
        this.#upstream = await this.#git.getUpstream();
    ...
    async #prepareImages () {
        ...
        for ( const service of Object.values( this.#composeFile.services || {} ) ) {
            ...
            if ( this.#upstream?.isGithub ) {
                image.build.labels["org.opencontainers.image.source"] = image.build.upstream.homeUrl;
