...
export default class AiApi {
    ...
    async getEmbedding ( model, text ) {
        return this.#providers[ this.#models[ model ].provider ].getEmbedding( model, text, { "simple": true } );
    }

    async getCompletion ( model, text ) {
        return this.#providers[ this.#models[ model ].provider ].getCompletion( model, text, { "simple": true } );
    }
}
