...
export default class Gpg {
    ...
    async unlockKey ( keyId, password, { cacheKeys } = {} ) {
        ...
        const keygrips = [ foundKey.grip ];
        for ( const subkey of foundKey.subkeys ) keygrips.push( subkey.grip );
    ...
    async encrypt ( data, recipient, { cwd, armor, output, sign, cacheKeys } = {} ) {
        ...
        if ( sign ) {
            ...
            for ( const user of sign ) {
                ...
                if ( user.password ) {
                    const res = await this.unlockKey( user.key, user.password );
                    if ( !res.ok ) return res;
                }
            }
        }
        ...
        // encrypt
        args.push( "--encrypt", "--recipient", recipient );

        try {
            var proc = childProcess.spawn( GPG_BINARY, args, {
                cwd,
                "stdio": [ "pipe", output
                    ? "ignore"
                    : "pipe", "ignore" ],
            ...
            stream.pipeline( this.#createStream( data ), proc.stdin, e => {} );
            ...
            if ( output ) {
                ...
                return new stream.Readable().destroy( e );
            }
        }
    }
