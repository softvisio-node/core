...
export default class Gpg {
    ...
    async presetPassphrase ( keys, { subkeys = true, cacheKeys } = {} ) {
        ...
        KEY_ID: for ( const [ keyId, password ] of Object.entries( keys ) ) {
    ...
    async encrypt ( data, recipients, { cwd, armor, compressLevel, output, sign, cacheKeys } = {} ) {
        const args = [];
        ...
        if ( sign ) {
            ...
            for ( const user of this.#createUsersList( sign ) ) {
            ...
            if ( keys ) {
                const res = await this.presetPassphrase( keys, { cacheKeys } );
        ...
        for ( const user of this.#createUsersList( recipients ) ) {
            args.push( "--recipient", user.key );
        }
    ...
    async sign ( data, users, { cwd, armor, compressLevel, output, encrypt, cacheKeys } = {} ) {
        const args = [];
        ...
        for ( const user of this.#createUsersList( users ) ) {
            args.push( "--local-user", user.key );
        ...
        if ( encrypt ) {
            ...
            for ( const user of this.#createUsersList( encrypt ) ) {
                args.push( "--recipient", user.key );
            }
        }
    ...
    async detachSign ( data, users, { cwd, armor, compressLevel, output, cacheKeys } = {} ) {
        const args = [];
    ...
    async clearSign ( data, users, { cwd, compressLevel, output, cacheKeys } = {} ) {
        const args = [];
    ...
    async decrypt ( data, { cwd, output, users, cacheKeys } = {} ) {
        const args = [ "--decrypt" ];
    ...
    // XXX
    async verify ( data, { cwd, users, signature, cacheKeys } = {} ) {
        const args = [ "--verify" ];
    ...
    async importKeys ( data, { cwd, restore } = {} ) {
        const args = [ "--import", "--batch", "--passphrase", "" ];
    ...
    async #spawnGpg ( args, { cwd, input, output } = {} ) {
        args.unshift( "--pinentry-mode=loopback" );
