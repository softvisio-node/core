...
export default class GitUpstream {
    ...
    linkifyMessage ( message, { repositorySlug } = {} ) {
        repositorySlug ||= this.repositorySlug;

        // linkify issues
        return message.replaceAll( ISSUE_RE, ( match, issueRepositorySlug, issueId ) => {
            var link;

            if ( !issueRepositorySlug || issueRepositorySlug === repositorySlug ) {
                link = `[#${ issueId }](${ this.getIssueUrl( issueId, repositorySlug ) })`;
            }
            else {
                link = `[${ issueRepositorySlug }#${ issueId }](${ this.getIssueUrl( issueId, issueRepositorySlug ) })`;
            }
