...
export default class GitReleases {
    ...
    // XXX
    canRelease ( version, previousRelease ) {
...
You need to merge with the "${ version.versionString }" first.`,
        ...
        if ( this.has( version.stableVersion ) ) {
            return result( [
                500,
                `Version "${ version.stableVersion }" is already released.
You need to merge first.`,
        ...
        if ( latestPreRelease && previousRelease.ne( latestPreRelease ) ) {
            return result( [
                500,
    ...
    getLastPreRelease ( version ) {
        ...
        if ( this.#lastPreRelease1[ version ] === undefined ) {
            ...
            for ( const _version of this.#versions ) {
                ...
                if ( _version.isPreRelease && _version.stableVersion.eq( version.stableVersion ) ) {
