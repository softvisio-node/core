...
class SmtpConnection {
    ...
    async data ( { to, from, replyTo, cc, bcc, subject, text, html, attachments } ) {
        ...
        if ( to ) buf += `To:${to.join( "," )}\r\n`;
        if ( cc && cc.length ) buf += `Cc:${cc.join( "," )}\r\n`;

        if ( subject ) buf += `Subject:${subject}\r\n`;

        buf += `MIME-Version:1.0\r\n`;

        buf += `Date:${new Date().toDateString()}\r\n`;

        const boundary1 = "--" + uuidV4();

        // boundary2 = "--" + uuidV4();

        // has body
        if ( text || html || attachments ) {
            buf += `Content-Type:multipart/mixed; boundary="${boundary1}"\r\n`;
        }
