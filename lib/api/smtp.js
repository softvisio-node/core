...
export default class Smtp {
    ...
    constructor ( url, { proxy, maxThreads } = {} ) {
        ...
        this.#threadsRunner = new ThreadsRunner( { "maxThreads": maxThreads || DEFAULT_MAX_THREADS } );
    }
    ...
    get isSecure () {
        this.#isSecure ??= this.#url.protocol === "smtps:";
    ...
    async testSmtp () {
        return this.#threadsRunner.runThread( this.#testSmtp.bind( this ) );
    }

    async sendEmail ( { to, from, replyTo, cc, bcc, subject, text, html, attachments } = {} ) {
        return this.#threadsRunner.runThread( this.#sendEmail.bind( this, {
            to,
            from,
            replyTo,
            cc,
