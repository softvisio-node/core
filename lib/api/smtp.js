...
class SmtpConnection {
    ...
    async auth () {
        ...
        const sasl = await Sasl.new( this.extensions.AUTH.split( /\s+/ ), this.#username, this.#password );

        if ( !sasl ) return result( [535, "No authentication method supported"] );
...
export default class Smtp {
    ...
    constructor ( url, options = {} ) {
        ...
        this.#semaphore = new Semaphore( { "maxThreads": options.maxThreads || DEFAULT_MAX_THREADS } );
    }
    ...
    // replyTo, to, cc, bcc, subject, body
    async sendEmail ( options ) {
        ...
        try {
            ...
            // from
            res = await connection.from( options.from );
            if ( !res.ok ) throw res;
            ...
            // data
            res = await connection.data( options, addresses );
            if ( !res.ok ) throw res;
