...
class SmtpConnection {
    ...
    async mailFrom ( sendAs ) {
        this.#write( `MAIL FROM:<${sendAs || this.#smtp.sendAs || this.#smtp.username}>` );
    ...
    async data ( subject, body, options = {} ) {
        this.#write( "DATA" );
        ...
        // headers
        if ( options.from ) buf += "From:" + options.from + "\r\n";
        if ( options.replyTo ) buf += "Reply-To:" + options.replyTo + "\r\n";
        if ( options.to ) buf += "To:" + options.to.join( "," ) + "\r\n";
        if ( options.cc && options.cc.length ) buf += "Cc:" + options.cc.join( "," ) + "\r\n";
...
export default class Smtp {
    ...
    constructor ( url, { proxy, maxThreads } = {} ) {
        ...
        this.#sendAs = this.#url.searchParams.get( "sendAs" );
    ...
    async sendEmail ( to, subject, body, { from, replyTo, cc, bcc } = {} ) {
        await this.#semaphore.startThread();
        ...
        TRANSACTION: {
            ...
            const options = {
                "from": from || this.#from,
                "replyTo": replyTo || this.#replyTo || from || this.#from,
                "to": [],
                "cc": [],
                "bcc": [],
            };
            ...
            // rcptTo
            res = await connection.rcptTo( options );
            if ( !res.ok ) break TRANSACTION;

            // data
            res = await connection.data( subject, body, options );
            if ( !res.ok ) break TRANSACTION;
