...
export default class TelegramClientApi extends TelegramClient {
    ...
    // public
    async start ( { accountId, phoneCode, password } = {} ) {
        if ( !accountId ) {
    ...
    async connect () {
        if ( this.isConnected ) return result( 200 );

        if ( !this.#connectMutex.tryLock() ) return this.#connectMutex.wait();

        const isConnected = await super.connect();

        const res = result( isConnected
            ? 200
            : 500 );

        this.#connectMutex.unlock( res );
    ...
    async disconnect () {
        ...
        if ( !this.#disconnectMutex.tryLock() ) return this.#connectMutex.wait();

        await super.disconnect();

        this.#disconnectMutex.unlock();
    }
}
