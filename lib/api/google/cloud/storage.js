...
export default class GoogleCloudStorage {
    ...
    async uploadFile ( path, file, { predefinedAcl, metadata } = {} ) {
        ...
        const streamMultipart = new MultipartStreamEncoder( "related" );

        streamMultipart.append( JSON.stringify( metadata ), { "type": "application/json" } );

        streamMultipart.append( file, { "type": file.type } );
    ...
    async getStream ( path, { start, length } = {} ) {
        const { bucket, name } = this.#parsePath( path );
        ...
        if ( start || length != null ) {
            ...
            headers.range = `bytes=${ start }-${ length == null
                ? ""
                : start + length }`;
        }
