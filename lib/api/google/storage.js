...
const API_URL = "https://storage.googleapis.com/storage/v1/",
    ...
    DOWNLOAD_URL = "https://www.googleapis.com/storage/v1/";
...
export default class FirebaseMessaging {
    ...
    async getFileStream ( path, { headers } = {} ) {
        const { bucket, name } = this.#parsePath( path );
        ...
        const res = await this.#request( DOWNLOAD_URL + `b/${encodeURIComponent( bucket )}/o/${encodeURIComponent( name )}`, {
            "returnResult": true,
            "request": {
                headers,
            },
    ...
    async #request ( url, { params, request = {}, returnResult } = {} ) {
        ...
        if ( !( request.headers instanceof Headers ) ) request.headers = new Headers( request.headers );
