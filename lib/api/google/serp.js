...
module.exports = class GoogleSerp extends MaxThreads() {
    ...
    async _threadGetGoogleSearchResults ( keyword, options = {} ) {
        ...
        if ( !res.ok ) return this._onFinishRequest( page, res );
        ...
        while ( 1 ) {
            ...
            try {
                ...
                // serp parsing error
                return this._onFinishRequest( page, result( [500, "Google serp parsing error"] ) );
            }

            // serp parser returned no results
            if ( !serp.length ) return this._onFinishRequest( page, result( [500, "Google serp parsing error"] ) );
        ...
        return this._onFinishRequest( page, result( 200, data ) );
    }
    ...
    async _threadGetGoogleMapsResults ( keyword, options = {} ) {
        ...
        res = await page.evaluate( async ( max, target ) => {
            if ( target ) target = target.toLowerCase();

            // detect empty results
            if ( document.querySelector( "div.section-bad-query-title" ) ) return 200;
            ...
            while ( 1 ) {
                ...
                const els = document.querySelectorAll( "div.section-place-result-container-summary" );

                // no results elements found, this is selectors error
                if ( !els ) return [[500, "No elements found, possible invalid selector"]];

                for ( const el of els ) {

                    // skip ads
                    if ( el.querySelector( "span.section-place-summary-ad-badge-container" ) && el.querySelector( "span.section-place-summary-ad-badge-container" ).textContent ) continue;

                    let name;

                    // find business name
                    if ( ( name = el.querySelector( `span[jstcache="166"]` ) ) ) name = name.textContent;

                    // unable to find business name
                    if ( !name ) return [[500, "Unable to parse businesssss name"]];

                    if ( target && target === name.toLowerCase() ) return [200, { "pos": ++pos, name }];
                ...
                // XXX check clickable
                const next = document.querySelector( "span.n7lv7yjyC35__button-next-icon" );
        ...
        return this._onFinishRequest( page, parseResult( res ) );
    }
    ...
    async _checkCaptcha ( page ) {
        ...
        while ( 1 ) {
            ...
            if ( !this.resolveCaptcha || !this.#captcha || ++attempt > this.#maxRetries ) return result( [500, "Unable to get serp, max retries reached"] );
