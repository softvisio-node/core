...
module.exports = class GoogleSerp extends MaxThreads() {
    ...
    async _threadGetGoogleMapsResults ( keyword, options = {} ) {
        ...
        // open google
        await page.goto( `https://www.${host}/` );

        // type query
        // XXX try
        var el = await page.waitForSelector( 'input[name="q"]' );

        await el.focus();
        await el.type( keyword + "\r", { "delay": 100 } );
        ...
        while ( 1 ) {
            ...
            const els = await page.$$( "div.section-place-result-container-summary" );

            // no results elements found, this is selectors error
            if ( !els ) return this._onFinish( page, result( [500, "No elements found, possible invalid selector"] ) );

            for ( const el of els ) {

                // skip ads
                if ( await this._getTextContent( el, "span.section-place-summary-ad-badge-container" ) ) continue;

                // find business name
                const name = await this._getTextContent( el, `span[jstcache="166"]` );

                // unable to find business name
                if ( !name ) return this._onFinish( page, result( [500, "Unable to parse businesssss name"] ) );
                ...
                if ( target && target === name.toLowerCase() ) {
                    if ( options.emulateUser ) {
                        ...
                        // XXX return page
                        return this._onFinish( page, result( 200, { "pos": ++pos, name } ) );
                    }
                    else {
                        return this._onFinish( page, result( 200, { "pos": ++pos, name } ) );
                    }
            ...
            const next = await page.$( "button#n7lv7yjyC35__section-pagination-button-next:not([disabled])" );
        ...
        if ( target ) return this._onFinish( page, result( 404 ) );
    ...
    async _setGooglePreciseLocation ( page, country, coordinates ) {
        ...
        res = await page.evaluate( () => {
            const el = document.querySelector( "#eqQYZc" );
