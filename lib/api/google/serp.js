...
module.exports = class GoogleSerp extends mix( MaxThreads, EventEmitter ) {
    #debug;
    #timeout;
    #disableImages;
    #proxy;
    ...
    constructor ( options = {} ) {
        ...
        this.resoveCaptcha = options.resolveCaptcha;
        this.disableImages = options.disableImages;
    ...
    async getGoogleSearchResults ( keyword, options ) {
        return this.runThread( this.#threadGetGoogleSearchResults.bind( this ), keyword, options );
    }
    ...
    async getGoogleMapsResults ( keyword, options ) {
        return this.runThread( this.#threadGetGoogleMapsResults.bind( this ), keyword, options );
    }

    async #threadGetGoogleSearchResults ( keyword, options = {} ) {
        ...
        while ( attempt++ < this.#maxRetries ) {
            ...
            const page = await this.#getPage( options.country, options.coordinates, options.device );

            try {
                res = await this.#_threadGetGoogleSearchResults( page, target, keyword, options );
            }
            catch ( e ) {
    ...
    async #_threadGetGoogleSearchResults ( page, target, keyword, options = {} ) {
        const googleDomain = this.#getGoogleDomain( options.country ),
            max = options.max || 100;
        ...
        PAGE: while ( 1 ) {
            ...
            for ( const el of els ) {
                ...
                const targetData = {
                    ...
                    "description": await this._getTextContent( el, "span.aCOpRe" ),
                ...
                // if ( !targetData.description ) return result( [500, "Desc selector not found"] );
                ...
                // onItem
                // XXX
                if ( options.onItem ) await options.onItem( page, await el.$( "div.yuRUbf > a" ), targetData );
                ...
                TARGET: if ( target ) {
                    ...
                    let targetHost;
                    try {
                        targetHost = new URL( targetData.url ).hostname;
                    }
                    catch ( e ) {
    ...
    async #threadGetGoogleMapsResults ( keyword, options = {} ) {
        var res,
            attempt = 0;
        ...
        while ( attempt++ < this.#maxRetries ) {
            ...
            try {
                res = await this.#_threadGetGoogleMapsResults( page, keyword, options );
            }
            catch ( e ) {
                res = result( [500, e.message || e] );
            }
    ...
    async #_threadGetGoogleMapsResults ( page, keyword, options = {} ) {
        const googleDomain = this.#getGoogleDomain( options.country ),
            max = options.max || 100,
        ...
        PAGE: while ( 1 ) {
            ...
            for ( const el of els ) {
                ...
                if ( options.onItem ) await options.onItem( page, urlEl, targetData );
    ...
    async #getBrowser ( country, device ) {
    ...
    async #getPage ( country, coordinates, device ) {
        const domain = this.#getGoogleDomain( country );

        const browser = await this.#getBrowser( domain.country.iso2, device );
        ...
        // disable images
        await this.#disablePageImages( page );
