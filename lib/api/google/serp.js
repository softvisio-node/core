...
module.exports = class GoogleSerp extends MaxThreads() {
    ...
    async getGoogleMapsPosition ( keyword, name, options ) {
        name = name.toLowerCase();

        const serp = await this._threadGetGoogleMapsResults( keyword, options );

        if ( !serp.ok ) return serp;

        for ( const item of serp.data ) {
            if ( name === item.name ) return result( 200, item );
        }
    ...
    async _threadGetGoogleMapsResults ( keyword, options = {} ) {
        ...
        res = await page.evaluate( max => {
            ...
            while ( 1 ) {
                ...
                // if (!els) els = document.querySelectorAll( "div.section-result-content" );
