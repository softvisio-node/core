...
module.exports = class GoogleSerp extends MaxThreads() {
    ...
    async _threadGetGoogleSearchResults ( keyword, options = {} ) {
        ...
        if ( options.country ) {
            host = this._getGoogleDomain( options.country ).domain;
        }
        else {
            host = this._getGoogleDomain().domain;
        }

        // get browser page instance
        const page = await this._getPage( options.country );
    ...
    async _threadGetGoogleMapsResults ( keyword, options = {} ) {
        ...
        // var url = `https://www.${host}/maps/search/` + encodeURIComponent( keyword );
        // if ( options.coordinates ) url += `/@${options.coordinates.latitude},${options.coordinates.longtitude},15z/`;
        // url += "?hl=en";

        // await page.goto( url );

        await page.goto( `https://www.${host}/?hl=en` );

        // type query
        let el = await page.waitForSelector( 'input[name="q"]' );
        if ( !el ) return onFinish( result( [500, "Google query selector not found"] ) );

        await el.focus();
        await el.type( keyword + "\r" );
        await page.waitForNavigation();

        // use precise location
        el = await page.waitForSelector( "#eqQYZc" );
        if ( !el ) return onFinish( result( [500, "Google precise location selector not found"] ) );
        await el.click();

        await require( "@softvisio/core/util" ).confirm( "Continue?", ["y"] );

        // goto gmap
        el = await page.waitForSelector( `g-more-link` );
        if ( !el ) onFinish( result( [500, "Google map selector not found"] ) );

        await el.hover();
        await el.click();
        await page.waitForNavigation();

        return onFinish( result( 200 ) );
    }
    ...
    async _getBrowser ( country ) {
        country = this._getGoogleDomain( country ).country.iso2;

        const profile = this._getProfile();

        var browser;

        if ( this.#persistent ) browser = this.#browsers[country];
        ...
        if ( this.#proxy ) {
            ...
            await proxy.startSession( { country } );
        }
    ...
    async _getPage ( country, coordinates ) {
        const browser = await this._getBrowser( country );
        ...
        const domain = this._getGoogleDomain( country ).domain;
        await page.browserContext().overridePermissions( `https://www.${domain}/`, ["geolocation"] );
