...
module.exports = class GoogleSerp extends MaxThreads() {
    ...
    async _threadGetGoogleSearchResults ( keyword, options = {} ) {
        ...
        while ( 1 ) {
            ...
            for ( const el of els ) {
                ...
                // NOTE not all sites has validdescription
    ...
    async _onFinish ( page, res ) {
        ...
        // clean stat events listener on session close
        browser.proxyServer.removeAllListeners( "stat" );

        // copy traffic counters to result
        res.bytesWritten = page.bytesWritten;
        res.bytesRead = page.bytesRead;
    ...
    async _getPage ( country, coordinates, device ) {
        ...
        page.bytesRead = 0;
        page.bytesWritten = 0;
        browser.proxyServer.on( "stat", data => {
            page.bytesRead += data.bytesRead;
            page.bytesWritten += data.bytesWritten;
        } );
