...
module.exports = class GoogleSerp extends MaxThreads() {
    ...
    async getGoogleSearch ( keyword, options ) {
        return this.runThread( "_threadGoogleSearch", keyword, options );
    }
    ...
    async getGoogleSearchPosition ( keyword, url, options ) {
        ...
        const serp = await this._threadGoogleSearch( keyword, options );
    ...
    // XXX
    async getGoogleMyBusiness ( keyword, options ) {
        return this.runThread( "_threadGoogleMyBusiness", keyword, options );
    }

    // XXX
    async getGoogleMyBusinessPosition ( keyword, options ) {
        return this.runThread( "_threadGoogleMyBusiness", keyword, options );
    }

    async _threadGoogleSearch ( keyword, options = {} ) {
        var host,
            max = options.max || 100;

        // get google domain by country
        if ( options.country ) {
            host = googleDomains.get( options.country || "WORLDWIDE" ).domain;
        }
        else {
            host = googleDomains.get( "WORLDWIDE" ).domain;
        }
    ...
    // XXX
    async _threadGoogleMyBusiness ( keyword, options = {} ) {}
    ...
    async _getBrowser ( country ) {
        country = googleDomains.get( country || "WORLDWIDE" ).country.iso2;
