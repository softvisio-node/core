...
module.exports = class GoogleSerp extends MaxThreads() {
    ...
    async _threadGetGoogleSearchResults ( keyword, options = {} ) {
        ...
        while ( attempt++ < this.#maxRetries ) {
            ...
            // finish session
            const stat = await this._onFinish( page );
    ...
    async __threadGetGoogleSearchResults ( page, keyword, target, options = {} ) {
    ...
    async _onFinish ( page ) {
        ...
        // return traffic stat
        return browser.proxyServer.endSession();
    }
    ...
    async _getBrowser ( country, device ) {
        ...
        if ( browser ) {
            ...
            if ( browser.proxyServer.proxy ) browser.proxyServer.proxy.country = country;
        }
        else {
            let proxy;
            ...
            browser = await puppeteer().launch( {
                ...
                "proxy": proxy,
                "timezone": this.#persistent ? false : true, // XXX due to bug in puppeteer we unable to update timezone if it was alredy emulated
                "stealth": true,
