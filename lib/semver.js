...
const PARSING_ERROR = "Semantic version is not valid",
...
export default class Semver {
            ...
            if ( match.groups.meta ) {
                ...
                if ( preRelease ) {
                    ...
                    for ( let n = 0; n < tags.length; n++ ) {
                        ...
                        if ( !tag ) throw new Error( PARSING_ERROR );

                        if ( /^\d+$/.test( tag ) ) {
                            if ( tag.startsWith( "0" ) ) throw new Error( PARSING_ERROR );
    ...
    eq ( version ) {
        return this.compare( Semver.new( version ) ) === 0;
    }

    gt ( version ) {
        return this.compare( Semver.new( version ) ) === 1;
    }
