...
export default class File {
    ...
    get hasContent () {
        return !!( this.#content || ( this.#path && fs.existsSync( this.#path ) ) );
    }

    get isStreamRangeSupported () {
        return this.#content == null || Buffer.isBuffer( this.#content );
    }
    ...
    stream ( { start, end } = {} ) {
        if ( this.#content == null ) {
            ...
            let buffer = Buffer.isBuffer( this.#content ) ? this.#content : Buffer.from( this.#content );

            if ( start || end ) buffer = buffer.slice( start, end );
