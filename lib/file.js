...
import { pathExists, pathExistsSync, sliceFileSync } from "#lib/fs";
...
export default class File extends Blob {
    ...
    stream ( { name, type, start, end, length } = {} ) {
        if ( this.#buffer ) {
            const range = new Range( { start, end, length } ).calculateRange( this.size ),
                buffer = this.#buffer.subarray( range.start, range.end );
        ...
        else if ( this.#path && pathExistsSync( String( this.#path ) ) ) {
            const range = new Range( { start, end, length } ).calculateReadStreamRange( this.size );
            ...
            if ( range.length === 0 ) {
                ...
                readableStream = fs.createReadStream( String( this.#path ), {
                    "start": range.start,
                    "end": range.end,
                } );
            }
    ...
    slice ( start, end, type ) {
        ...
        if ( this.#buffer ) {
            const range = new Range( { start, end } ).calculateRange( this.#buffer.length );

            buffer = this.#buffer.subarray( range.start, range.end );
        }
        else if ( this.#path && pathExistsSync( String( this.#path ) ) ) {
            buffer = sliceFileSync( String( this.#path ), { start, end } );
        }
        else {
            buffer = Buffer.from( "" );
        }
    ...
    async #getBuffer () {
        ...
        else if ( this.#path && ( await pathExists( this.#path + "" ) ) ) {
