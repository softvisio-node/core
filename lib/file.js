...
export default class File extends Blob {
    ...
    stream ( { name, type, start, end, length } = {} ) {
        if ( this.#buffer ) {
            const range = new Range( { start, end, length } ).calculate( this.size );
            ...
            return fs
                .createReadStream( String( this.#path ), {
                    "start": range.start,
                    "end": range.end,
                } )
                .setName( name === undefined
    ...
    slice ( start, end, type ) {
        ...
        if ( this.#buffer ) {
            const range = new Range( { start, end } ).calculate( this.#buffer.length );
