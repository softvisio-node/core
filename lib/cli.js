...
export default class CLI {
    ...
    constructor ( object, argv, commandsStack, commandTitle ) {
        ...
        // validate cli spec
        cliSpecValidator ??= new Ajv().compile( readConfig( "#resources/schemas/cli.schema.yaml", { "resolve": import.meta.url } ) );

        // cle spec error
        if ( !cliSpecValidator( this.#spec ) ) {
            console.log( `CLI spec is not valid, inspect errors below:\n${cliSpecValidator.errors}` );
    ...
    static async parse ( spec ) {
        ...
        return this.new( spec );
    }

    static findSpec ( object ) {
    ...
    static async new ( object, argv, commandsStack, commandTitle ) {
        const cli = new this( object, argv, commandsStack, commandTitle );
    ...
    // private
    #findSpec ( object ) {
        return this.constructor.findSpec( object );
    }
    ...
    async #processCommands () {
        ...
        // command found
            ...
            return CLI.new( module, argv, this.#commandsStack, possibleCommands.title );
        }
    }
    ...
    #processOptions () {
        ...
        process.cli = {};
        process.cli.path = this.#commandsStack.join( "/" );
        process.cli.command = this.#commandsStack.at( -1 );
