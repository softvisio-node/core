...
// TODO cloudflare cidr validator
function getRealIp ( req ) {
    if ( !this.realIp ) {

        // detect load-balancer mode
        const loadBalancer = process.env.APP_LOADBALANCER_ID && process.env.APP_LOADBALANCER_HOST;
        ...
        if ( loadBalancer ) {
            ...
            clientIp = Buffer.from( this.getProxiedRemoteAddressAsText() || this.getRemoteAddressAsText() ).toString();
        }

        this.realIp = clientIp;

        const cloudFlareClientIp = req.getHeader( "cf-connecting-ip" );

        if ( cloudFlareClientIp ) {

            //     // TODO compare clientIp with cloudFlare CIDRs
            //     if ( isCloudflareIp(this.realIp) ) {
            this.realIp = cloudFlareClientIp;
