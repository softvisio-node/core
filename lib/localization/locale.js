...
export default class Locale {
    ...
    i18n ( message, num = 0 ) {
        ...
        if ( translation ) {
            ...
            // plural
            if ( Array.isArray( translation.message ) ) {
                const idx = this.pluralFunction?.( num ) || 0;

                translatedMessage = translation.message[idx];
            }
        ...
        return ( translatedMessage || message ).replaceAll( "%d", num );
    }
    ...
    i18nNull ( message, num = 0 ) {
        ...
        if ( translation ) {
            ...
            if ( translatedMessage ) {
                return translatedMessage.replaceAll( "%d", num );
            }
        }
    }
...
`;
        ...
        for ( const [id, translation] of Object.entries( this.#translations ) ) {
            ...
            // plural
            if ( Array.isArray( translation.message ) ) {
                text += `msgid_plural ${this.#genPoString( id )}`;

                const nplurals = this.nplurals || 1;

                for ( let n = 0; n < nplurals; n++ ) {
                    text += `msgstr[${n}] ${this.#genPoString( translation.message[n] )}`;
                }
            }

            // single
            else {
                text += `msgstr ${this.#genPoString( translation.message )}`;
            }
        }
