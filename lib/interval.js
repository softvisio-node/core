...
export default class Interval {
        ...
        if ( UNITS[ unit ].months ) {
            ...
            if ( !value.decimal.isZero ) {
                if ( unit === "months" ) {
                    ...
                    this.#addUnit( value.decimal.multiply( UNITS[ unit ].months ).number, "months" );
                }
            }
        }
        else {
    ...
    #getNormalizedDaysUnits () {
        if ( !this.#normalizedDaysUnits ) {
            this.#normalizedDaysUnits = {};

            let nanoseconds = this.toNanoseconds().bigint;

            for ( const unit in this.#units ) {
                if ( UNITS[ unit ].nanoseconds > math.abs( nanoseconds ) ) {
                    this.#normalizedDaysUnits[ unit ] = 0;
                }
                else {
                    this.#normalizedDaysUnits[ unit ] = Number( nanoseconds / UNITS[ unit ].nanoseconds );
