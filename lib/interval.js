...
export default class Interval {
    ...
    toNanoseconds () {
        this.#toUnit.nanoseconds ??= this.#nanoseconds + this.#months * UNITS.months.nanoseconds;

        return this.#toUnit.nanoseconds;
    }

    toMicroseconds () {
        return this.toNanoseconds() / 1000n;
    }

    toMilliseconds () {
        return Number( this.toNanoseconds() / UNITS.milliseconds.nanoseconds );
    }

    toSeconds () {
        return Number( this.toNanoseconds() / UNITS.seconds.nanoseconds );
    }

    toMinutes () {
        return Number( this.toNanoseconds() / UNITS.minutes.nanoseconds );
    }

    toHours () {
        return Number( this.toNanoseconds() / UNITS.hours.nanoseconds );
    }

    toDays () {
        return Number( this.toNanoseconds() / UNITS.days.nanoseconds );
    }

    toWeeks () {
        return Number( this.toNanoseconds() / UNITS.weeks.nanoseconds );
    }

    toMonths () {
        return Number( this.toNanoseconds() / UNITS.months.nanoseconds );
    }

    toYears () {
        return Number( this.toNanoseconds() / UNITS.years.nanoseconds );
    }
    ...
    #parse ( duration, unit ) {
        ...
        // object
            ...
            for ( const unit of Object.values( UNITS ) ) {
                if ( unit.months ) {
                    params.months ??= 0n;
                    params.months += BigInt( duration[ unit.name ] || 0 ) * unit.months;
                }
                else {
