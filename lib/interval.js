...
export default class Interval {
    ...
    constructor ( duration, { negative, unit = "millisecond" } = {} ) {
    ...
    // static
    static new ( duration, { negative, unit } = {} ) {
        if ( duration instanceof this ) {
            if ( negative == null || negative === duration.isNegative ) return duration;
        }

        return new this( duration, { negative, unit } );
    }
            ...
            if ( !Number.isNaN( number ) ) {
                ...
                if ( Number.isInteger( number ) ) {
                    ...
                    const millisecond = Math.abs( number ) * UNIT_MILLISECONDS[ UNITS[ unit ] ],
                        nanosecond = Math.floor( Decimal( millisecond ).sub( Math.floor( millisecond ) ) * 1_000_000 );
