...
export default class Interval {
    ...
    constructor ( duration, { unit = "milliseconds", useWeeks } = {} ) {
        this.#useWeeks = Boolean( useWeeks );
    ...
    getFormatRelativeDateParams () {
        if ( !this.#formatRelativeDateParams ) {
            for ( const unit of Object.values( UNITS ) ) {
                if ( !this.#useWeeks && unit.name === "weeks" ) continue;
    ...
    #parseUnits ( { daysToMonths } = {} ) {
        ...
        if ( units.microseconds ) {
            if ( math.abs( units.microseconds ) >= 1000n ) {
                units.milliseconds = Number( units.microseconds / 1000n );
        ...
        if ( units.milliseconds ) {
            if ( Math.abs( units.milliseconds ) >= 1000 ) {
                units.seconds = Math.trunc( units.milliseconds / 1000 );
                units.milliseconds = units.milliseconds % 1000;
            }
        }

        // seconds -> minutes
        if ( units.seconds ) {
            if ( Math.abs( units.seconds ) >= 60 ) {
                units.minutes = Math.trunc( units.seconds / 60 );
                units.seconds = units.seconds % 60;
            }
        }

        // minutes -> hours
        if ( units.minutes ) {
            if ( Math.abs( units.minutes ) >= 60 ) {
                units.hours = Math.trunc( units.minutes / 60 );
                units.minutes = units.minutes % 60;
            }
        }

        // hours -> days
        if ( units.hours ) {
            if ( Math.abs( units.hours ) >= 24 ) {
                units.days = Math.trunc( units.hours / 24 );
                units.hours = units.hours % 24;
            }
        }
        ...
        if ( units.days ) {
            if ( daysToMonths ) {
                ...
                if ( Math.abs( units.days ) >= 30 ) {
                    months += Math.trunc( units.days / 30 );
                    units.days = units.days % 30;
                }
            }
            else if ( this.#useWeeks ) {

                // days -> weeks
                if ( Math.abs( units.days ) >= 7 ) {
                    units.weeks = Math.trunc( units.days / 7 );
                    units.days = units.days % 7;
                }
            }
        }
