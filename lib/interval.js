...
export default class Interval {
    ...
    toNanoseconds () {
        if ( this.#toUnit.nanoseconds == null ) {
            ...
            // days -> months
            this.#normalizeUnit( "days", "months", 30 );

            // months -> years
            this.#normalizeUnit( "months" );
    ...
    #normalizeUnit ( unit, target, threshold ) {
        if ( !target ) {
        ...
        if ( Math.abs( this.#units[ unit ] ) >= threshold ) {
            this.#units[ target ] += Math.trunc( this.#units[ unit ] / threshold );

            this.#units[ unit ] = this.#units[ unit ] % threshold;
        }
    }
}
