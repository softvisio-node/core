...
export default class Interval {
    ...
    #addUnit ( value, unit ) {
        ...
        if ( Number.isInteger( value ) ) {
            if ( UNITS[ unit ].months ) {
                this.#months += value * UNITS[ unit ].months;
            }
            else {
                this.#nanoseconds += BigInt( value ) * UNITS[ unit ].nanoseconds;
            }
        }
        else {
    ...
    #normalizeUnits () {
        for ( const name of Object.keys( this.#units ) ) {
            ...
            if ( UNITS[ name ].months ) {
                if ( name === "months" ) {
                    this.#units[ name ] = this.#months;
                }
                else {
                    this.#units[ name ] = Math.trunc( this.#months / UNITS[ name ].months );
                    this.#months = this.#months % UNITS[ name ].months;
                }
            }
            ...
            // nanoseconds
                ...
                if ( name === "nanoseconds" ) {
                    this.#units[ name ] = Number( this.#nanoseconds );
                }
                else {
                    this.#units[ name ] = Number( this.#nanoseconds / UNITS[ name ].nanoseconds );
                    this.#nanoseconds = this.#nanoseconds % UNITS[ name ].nanoseconds;
                }
            }
        }
    }
    ...
    // XXX
    #normalizeUnit ( unit, units, target, threshold ) {
        if ( !target ) {
            const normalize = UNITS[ unit ].normalize;

            if ( !normalize ) return;

            [ threshold, target ] = normalize;
        }

        units ||= this.#units;

        if ( Math.abs( units[ unit ] ) >= threshold ) {
            units[ target ] += Math.trunc( units[ unit ] / threshold );

            units[ unit ] = units[ unit ] % threshold;
        }
    }
    ...
    #getNormalizedDaysUnits () {
        if ( !this.#normalizedDaysUnits ) {
            ...
            // normalize days
            this.#normalizeUnit( "days", this.#normalizedDaysUnits, "years", 365 );

            this.#normalizeUnit( "days", this.#normalizedDaysUnits, "months", 30 );

            this.#normalizeUnit( "months", this.#normalizedDaysUnits );
        }
