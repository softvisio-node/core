        ...
        // object
            ...
            if ( !params ) {
                ...
                for ( let n = 1; n < match.length; n += 3 ) {
                    const name = ALIASES[ match[ n + 1 ] ];

                    if ( !name ) throw new Error( `Duration format is not valid` );

                    if ( UNITS[ name ].parseNanoseconds ) {
                        params.nanoseconds ??= 0;
                        params.nanoseconds += +match[ n ] * UNITS[ name ].milliseconds * 1_000_000;
                    }
                    else {
                        params[ name ] ??= 0;
                        params[ name ] += +match[ n ];
