...
const UNITS = {
        "years": {
            ...
            "narrow": [ "y", "y" ],
            "contains": [ 12, "months" ],
            "nanoseconds": Numeric( 1_000_000n * 1000n * 60n * 60n * 24n * 365n ), // 365 days
            "months": 12,
            "nginx": "y",
        ...
        "quarters": {
            ...
            "narrow": [ "q", "q" ],
            "contains": [ 3, "months" ],
            "nanoseconds": Numeric( 1_000_000n * 1000n * 60n * 60n * 24n * 365n ).divide( 4 ), // 1 / 4 year
            "months": 3,
            "nginx": null,
            "relativeDateParam": true,
        },
        "months": {
            ...
            "aliases": [ "M" ],
            "contains": [ 30, "days" ],
            "nanoseconds": Numeric( 1_000_000n * 1000n * 60n * 60n * 24n * 365n ).divide( 12 ), // 1 / 12 year
            "months": 1,
            "nginx": "M",
            "relativeDateParam": true,
        },
        "weeks": {
            ...
            "narrow": [ "w", "w" ],
            "contains": [ 7, "days" ],
            "nanoseconds": Numeric( 1_000_000n * 1000n * 60n * 60n * 24n * 7n ), // 7 days
            "nginx": "w",
            "relativeDateParam": true,
        },
        "days": {
            ...
            "narrow": [ "d", "d" ],
            "contains": [ 24, "hours" ],
            "nanoseconds": Numeric( 1_000_000n * 1000n * 60n * 60n * 24n ),
            "nginx": "d",
            "relativeDateParam": true,
        },
        "hours": {
            ...
            "narrow": [ "h", "h" ],
            "contains": [ 60, "minutes" ],
            "nanoseconds": Numeric( 1_000_000n * 1000n * 60n * 60n ),
            "nginx": "h",
            "relativeDateParam": true,
        },
        "minutes": {
            ...
            "narrow": [ "m", "m" ],
            "contains": [ 60, "seconds" ],
            "nanoseconds": Numeric( 1_000_000n * 1000n * 60n ),
            "nginx": "m",
            "relativeDateParam": true,
        },
        "seconds": {
            ...
            "nanoseconds": Numeric( 1_000_000n * 1000n ),
            "nginx": "s",
            "relativeDateParam": true,
        },
        "milliseconds": {
...
export default class Interval {
    ...
    toNginx () {
        if ( this.#string.nginx == null ) {
            ...
            this.#string.nginx = units.join( " " );
        }

        return this.#string.nginx;
    }

    toNanoseconds () {
        if ( this.#toUnit.nanoseconds == null ) {
            this.#toUnit.nanoseconds = Numeric( 0 );

            for ( const name in this.#units ) {
                this.#toUnit.nanoseconds = this.#toUnit.nanoseconds.add( UNITS[ name ].nanoseconds.multiply( this.#units[ name ] ) );
            }
        }
    ...
    toMicroseconds ( scale ) {
        if ( scale ) {
            return this.toNanoseconds().divide( UNITS.microseconds.nanoseconds ).trunc( scale ).number;
        }
        else {
            this.#toUnit.microseconds ??= this.toNanoseconds().divide( UNITS.microseconds.nanoseconds ).trunc().number;
    ...
    toMilliseconds ( scale ) {
        if ( scale ) {
            return this.toNanoseconds().divide( UNITS.milliseconds.nanoseconds ).trunc( scale ).number;
        }
        else {
            this.#toUnit.milliseconds ??= this.toNanoseconds().divide( UNITS.milliseconds.nanoseconds ).trunc().number;
    ...
    toSeconds ( scale ) {
        if ( scale ) {
            return this.toNanoseconds().divide( UNITS.seconds.nanoseconds ).trunc( scale ).number;
        }
        else {
            this.#toUnit.seconds ??= this.toNanoseconds().divide( UNITS.seconds.nanoseconds ).trunc().number;
    ...
    toMinutes ( scale ) {
        if ( scale ) {
            return this.toNanoseconds().divide( UNITS.minutes.nanoseconds ).trunc( scale ).number;
        }
        else {
            this.#toUnit.minutes ??= this.toNanoseconds().divide( UNITS.minutes.nanoseconds ).trunc().number;
    ...
    toHours ( scale ) {
        if ( scale ) {
            return this.toNanoseconds().divide( UNITS.hours.nanoseconds ).trunc( scale ).number;
        }
        else {
            this.#toUnit.hours ??= this.toNanoseconds().divide( UNITS.hours.nanoseconds ).trunc().number;
    ...
    toDays ( scale ) {
        if ( scale ) {
            return this.toNanoseconds().divide( UNITS.days.nanoseconds ).trunc( scale ).number;
        }
        else {
            this.#toUnit.days ??= this.toNanoseconds().divide( UNITS.days.nanoseconds ).trunc().number;
    ...
    toWeeks ( scale ) {
        if ( scale ) {
            return this.toNanoseconds().divide( UNITS.weeks.nanoseconds ).trunc( scale ).number;
        }
        else {
            this.#toUnit.weeks ??= this.toNanoseconds().divide( UNITS.weeks.nanoseconds ).trunc().number;
    ...
    toMonths ( scale ) {
        if ( scale ) {
            return this.toNanoseconds().divide( UNITS.months.nanoseconds ).trunc( scale ).number;
        }
        else {
            this.#toUnit.months ??= this.toNanoseconds().divide( UNITS.months.nanoseconds ).trunc().number;
    ...
    toQuarters ( scale ) {
        if ( scale ) {
            return this.toNanoseconds().divide( UNITS.quarters.nanoseconds ).trunc( scale ).number;
        }
        else {
            this.#toUnit.quarters ??= this.toNanoseconds().divide( UNITS.quarters.nanoseconds ).trunc().number;
    ...
    toYears ( scale ) {
        if ( scale ) {
            return this.toNanoseconds().divide( UNITS.years.nanoseconds ).trunc( scale ).number;
        }
        else {
            this.#toUnit.years ??= this.toNanoseconds().divide( UNITS.years.nanoseconds ).trunc().number;
    ...
    #addUnit ( value, unit ) {
        ...
        if ( Number.isInteger( value ) ) {
            if ( UNITS[ unit ].months ) {
                ...
                this.#units.nanoseconds += UNITS[ unit ].nanoseconds.bigint * BigInt( value );
            }
        }
        ...
        for ( const name of Object.keys( this.#units ) ) {
            ...
            // nanoseconds
                ...
                if ( name === "nanoseconds" ) {
                    ...
                    this.#units[ name ] = Number( this.#units.nanoseconds / UNITS[ name ].nanoseconds.bigint );
                    this.#units.nanoseconds = this.#units.nanoseconds % UNITS[ name ].nanoseconds.bigint;
                }
            }
        }
    }
    ...
    #getNormalizedDaysUnits () {
        if ( !this.#normalizedDaysUnits ) {
            ...
            // days -> months
            if ( Math.abs( this.#normalizedDaysUnits.days ) >= DAYS_IN_MONTH ) {
