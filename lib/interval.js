...
export default class Interval {
    ...
    constructor ( duration, { unit = "milliseconds" } = {} ) {
        const { months, nanoseconds } = this.#parse( duration, unit );

        // nanoseconds -> microseconds
        if ( nanoseconds ) {
            if ( math.abs( nanoseconds ) >= 1000n ) {
                this.#units.microseconds = nanoseconds / 1000n;
                this.#units.nanoseconds = Number( nanoseconds % 1000n );
            }
            else {
                this.#units.nanoseconds = Number( nanoseconds );
            }
        }
        else {
            this.#units.nanoseconds = 0;
        }

        // microseconds -> milliseconds
        if ( this.#units.microseconds ) {
            if ( math.abs( this.#units.microseconds ) >= 1000n ) {
                this.#units.milliseconds = this.#units.microseconds / 1000n;
                this.#units.microseconds = Number( this.#units.microseconds % 1000n );
            }
            else {
                this.#units.microseconds = Number( this.#units.microseconds );
            }
        }
        else {
            this.#units.microseconds = 0;
        }

        // milliseconds -> seconds
        if ( this.#units.milliseconds ) {
            if ( math.abs( this.#units.milliseconds ) >= 1000n ) {
                this.#units.seconds = this.#units.milliseconds / 1000n;
                this.#units.milliseconds = Number( this.#units.milliseconds % 1000n );
            }
            else {
                this.#units.milliseconds = Number( this.#units.milliseconds );
            }
        }
        else {
            this.#units.milliseconds = 0;
        }

        // seconds -> minutes
        if ( this.#units.seconds ) {
            if ( math.abs( this.#units.seconds ) >= 60n ) {
                this.#units.minutes = this.#units.seconds / 60n;
                this.#units.seconds = Number( this.#units.seconds % 60n );
            }
            else {
                this.#units.seconds = Number( this.#units.seconds );
            }
        }
        else {
            this.#units.seconds = 0;
        }

        // minutes -> hours
        if ( this.#units.minutes ) {
            if ( math.abs( this.#units.minutes ) >= 60n ) {
                this.#units.hours = this.#units.minutes / 60n;
                this.#units.minutes = Number( this.#units.minutes % 60n );
            }
            else {
                this.#units.minutes = Number( this.#units.minutes );
            }
        }
        else {
            this.#units.minutes = 0;
        }

        // hours -> days
        if ( this.#units.hours ) {
            if ( math.abs( this.#units.hours ) >= 24n ) {
                this.#units.days = this.#units.hours / 24n;
                this.#units.hours = Number( this.#units.hours % 24n );
            }
            else {
                this.#units.hours = Number( this.#units.hours );
            }
        }
        else {
            this.#units.hours = 0;
        ...
        // days -> weeks
        if ( this.#units.days ) {
            if ( math.abs( this.#units.days ) >= 7n ) {
                this.#units.weeks = Number( this.#units.days / 7n );
                this.#units.days = Number( this.#units.days % 7n );
            }
            else {
                this.#units.days = Number( this.#units.days );
                this.#units.weeks = 0;
            }
        }
        else {
        ...
        if ( months ) {
            if ( math.abs( months ) >= 12n ) {
                this.#units.months = Number( months % 12 );
                this.#units.years = Number( months / 12n );
            }
            else {
                this.#units.months = Number( months );
                this.#units.years = 0;
            }
        }
        else {
