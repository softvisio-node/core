...
export default class Events {
    ...
    link ( target, { on, forwarder } = {} ) {
        ...
        // already linked
        if ( this.#subscribed.has( target ) ) this.unlink( target );
            ...
            "watcher": ( name, subscribe ) => {
                ...
                if ( subscribe ) {
                    ...
                    for ( const remoteName of remoteNames ) {
                        spec.names[name].push( [remoteName, listener] );
                ...
                // unsubscribe
                    ...
                    for ( let n = 0; n < spec.names[name].length; n++ ) {
                        target.off( ...spec.names[name][n] );
                    }
        ...
        this.#subscribed.set( target, spec );
    ...
    unlinkAll () {
        for ( const target of this.#subscribed.keys() ) {
            this.#unlink( target );
        }
    ...
    #unlink ( target ) {
        const spec = this.#subscribed.get( target );

        if ( !spec ) return;

        this.#subscribed.delete( target );
    ...
    #callWatcheers ( name, subscribe ) {
        ...
        if ( this.#subscribed.size ) {
            for ( const target of this.#subscribed.values() ) {
                target.watcher( name, subscribe );
            }
        }
    }
}
