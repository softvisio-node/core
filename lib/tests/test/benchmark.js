...
class Benchmark extends Test {
    ...
    async #bench ( meta ) {
        ...
        // multiple threds
            ...
            const threadsPool = new ThreadsPool( { "maxThreads": meta.threads, "maxWaitingThreads": Infinity } ).pause();
            ...
            for ( let j = 1; j <= iterations; j++ ) {
                ...
                threadsPool.runThread( wrapper ).then( () => counter.dec() );
            }

            const t0 = process.hrtime.bigint();

            // run threads
            threadsPool.maxThreads.resume();
