...
class Benchmark extends Test {
    ...
    printStatus ( indent ) {
        ...
        if ( this.result.status === 201 ) console.log( indentSpace, ansi.dim( " ○ " ), this.type, this.name );
        else if ( this.result.ok ) console.log( indentSpace, ansi.ok( " √ " ), this.type, this.name );
        else if ( !this.result.ok ) console.log( indentSpace, ansi.error( " × " ), this.type, this.name );
    }
