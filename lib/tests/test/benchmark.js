...
class Benchmark extends Test {
    ...
    printStatus () {
        const indentSpace = " ".repeat( 6 );

        if ( this.result.status === 201 ) console.log( ansi.dim( indentSpace + " ○ " + this.label + " " + this.name ) );
        else if ( this.result.ok ) console.log( indentSpace, ansi.ok( " √ " ), this.label, this.name );
        else if ( !this.result.ok ) console.log( indentSpace, ansi.error( " × " ), this.label, this.name );
    }
