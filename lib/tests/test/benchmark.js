...
class Benchmark extends Test {
    ...
    printStatus ( options ) {
        ...
        if ( this.isSkipped ) console.log( `     ${ansi.dark( "○" )} ${LABEL} ${this.name} ${this.result.reason}` );
        else if ( this.result.ok ) console.log( `     ${ansi.brightGreen( "√" )} ${LABEL} ${this.name}` );
        else if ( !this.result.ok ) console.log( `     ${ansi.brightRed( "×" )} ${LABEL} ${this.name}` );
    }
