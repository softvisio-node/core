...
export default class Module extends Test {
    ...
    printStatus ( options ) {
        ...
        if ( this.isSkipped ) console.log( `${ansi.dark( " SKIP " )} ${LABEL} ${this.name}, ${this.result.reason}` );
        else if ( this.result.ok ) console.log( `${ansi.ok( " PASS " )} ${LABEL} ${this.name}` );
        else if ( !this.result.ok ) console.log( `${ansi.error( " FAIL " )} ${LABEL} ${this.name}` );
