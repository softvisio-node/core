...
export default class Module extends Test {
    ...
    async run ( options ) {
        ...
        if ( options.testPathPattern && this.name && !options.testPathPattern.test( this.name ) ) return this._skip( "filtered" );
    ...
    printStatus ( options ) {
        if ( !options.verbose && this.isSkipped ) return;

        if ( this.isSkipped ) console.log( `${ansi.dark( " SKIP " )} module: ${this.name}, ${this.result.reason}` );
        else if ( this.result.ok ) console.log( `${ansi.ok( " PASS " )} module: ${this.name}` );
        else if ( !this.result.ok ) console.log( `${ansi.error( " FAIL " )} module: ${this.name}` );

        if ( options.verbose && !this.isSkipped ) for ( const test of this.#tests ) test.printStatus( options );
    }
}
