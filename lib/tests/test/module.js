...
export default class Module extends Test {
    ...
    plan ( options ) {
        ...
        if ( options.testPathPattern && this.name && !options.testPathPattern.test( this.name ) ) return;
    ...
    printStatus ( options ) {
        if ( this.isSkipped ) {
            if ( options.showSkipped ) console.log( `${ansi.dark( " SKIP " )} ● ${ansi.hl( this.name )} ${this._formatReason( this.result.reason )}` );
        }
