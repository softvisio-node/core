...
class Test extends _Test {
    ...
    async run ( options, parent, skipNameFilter ) {
        ...
        else if ( !options.runTests ) return this._skip( "filtered" );
        else if ( !skipNameFilter && options.testNamePattern && !options.testNamePattern.test( this.name ) ) return this._skip( "filtered" );
    ...
    printStatus ( options ) {
        ...
        if ( this.isSkipped ) {
            if ( options.showSkipped ) console.log( `     ${ansi.dark( "○" )} ${LABEL} ${this.name} ${this._formatReason( this.result.reason )}` );
        }
        else if ( this.result.ok ) {
            if ( options.showPassed ) console.log( `     ${ansi.brightGreen( "√" )} ${LABEL} ${this.name} ${this._formatReason( this.durationText )}` );
        }
        else if ( !this.result.ok ) {
            console.log( `     ${ansi.brightRed( "×" )} ${LABEL} ${this.name} ${this._formatReason( this.durationText )}` );
        }
    }
