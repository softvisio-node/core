...
class Test extends _Test {
    ...
    printStatus ( options, parent ) {
        ...
        if ( this.isSkipped ) {
            if ( options.showSkipped ) console.log( `${indent}○ ${this.name} ${this._formatReason( this.result.reason )}` );
        }
        else if ( this.result.ok ) {
            if ( options.showPassed ) console.log( `${indent}${ansi.brightGreen( "√" )} ${this.name} ${this._formatReason( this.durationText )}` );
        }
        else if ( !this.result.ok ) {
            console.log( `${indent}${ansi.brightRed( "×" )} ${this.name} ${this._formatReason( this.durationText )}` );
        }
    }
