...
class Group extends Test {
    ...
    printStatus () {
        ...
        if ( this.result.status === 201 ) console.log( indentSpace, ansi.dim( " SKIP " ), ansi.dim( this.label ), this.name );
        else if ( this.result.ok ) console.log( indentSpace, ansi.ok( " PASS " ), ansi.dim( this.label ), this.name );
        else if ( !this.result.ok ) console.log( indentSpace, ansi.error( " FAIL " ), ansi.dim( this.label ), this.name );

        if ( this.result.status !== 201 ) for ( const test of this.#tests ) test.printStatus();
    }
}
