...
class Group extends Test {
    ...
    async run ( options ) {
        ...
        if ( options.testNamePattern && !options.testNamePattern.test( this.name ) ) return this._skip( "filtered" );
    ...
    printStatus ( options ) {
        if ( !options.verbose && ( this.isSkipped || this.result.ok ) ) return;
        ...
        if ( !this.isSkipped ) for ( const test of this.#tests ) test.printStatus( options );
    }
}
