...
class Group extends Test {
    ...
    printStatus ( indent ) {
        ...
        if ( this.result.status === 201 ) console.log( indentSpace, ansi.dim( " SKIP " ), this.type, this.name );
        else if ( this.result.ok ) console.log( indentSpace, ansi.ok( " PASS " ), this.type, this.name );
        else if ( !this.result.ok ) console.log( indentSpace, ansi.error( " FAIL " ), this.type, this.name );

        for ( const test of this.#tests ) test.printStatus( indent + 1 );
    }
}
