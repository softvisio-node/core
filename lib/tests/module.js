...
export default class Module extends Test {
    ...
    printStatus () {
        if ( this.result.status === 201 ) console.log( ansi.dim( " SKIP " ), this.type, this.name );
        else if ( this.result.ok ) console.log( ansi.ok( " PASS " ), this.type, this.name );
        else if ( !this.result.ok ) console.log( ansi.error( " FAIL " ), this.type, this.name );

        for ( const test of this.#tests ) test.printStatus( 1 );
    }
}
