...
class Expect {
    ...
    toBe ( value ) {
        ...
        const pass = this.#check( this.#received === value );
    ...
    #composeResultExpected ( pass, name, description, value ) {
        ...
        if ( !pass ) {
            message = `expect( received )${this.#not ? ".not" : ""}.${name}( expected )${description ? " // " + description : ""}
...
Received: ${value}`;
    ...
    #composeResultReceived ( pass, name, description ) {
        ...
        if ( !pass ) {
            message = `expect( received )${this.#not ? ".not" : ""}.${name}()${description ? " // " + description : ""}
