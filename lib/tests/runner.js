...
class Runner {
    ...
    #onTestSkip ( test ) {
        console.log( this.#getIndent(), text.ansi.dim( ` ○  skipped ${test.name}` ) );
    }

    #onTestEnd ( test ) {
        if ( test.isTest ) {
            if ( test.ok ) console.log( this.#getIndent(), text.ansi.ok( " √ " ), `${test.name} (${test.duration} ms)` );
            else {
                console.log( this.#getIndent(), text.ansi.error( " ● " ), text.ansi.hl( `${test.name} (${test.duration} ms)` ) );
