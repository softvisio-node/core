...
class Runner {
    ...
    async run ( options = {} ) {
        ...
        if ( options.runTests ) console.log( `Tests:      total: ${stat.stat.totalTests}, skipped: ${stat.stat.skippedTests}, ran: ${stat.stat.ranTests}, pass: ${ansi.ok( " " + stat.stat.passTests + " " )}, fail: ${ansi.error( " " + stat.stat.failTests + " " )}` );

        if ( options.runBenchmarks ) console.log( `Benchmarks: total: ${stat.stat.totalBenchmarks}, skipped: ${stat.stat.skippedBenchmarks}, ran: ${stat.stat.ranBenchmarks}, pass: ${ansi.ok( " " + stat.stat.passBenchmarks + " " )}, fail: ${ansi.error( " " + stat.stat.failBenchmarks + " " )}` );
