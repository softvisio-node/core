        ...
        if ( typeof id === "object" ) {
            for ( const [id, locale] of Object.entries( id ) ) {
                if ( !this.#domains.has( id ) ) {
                    this.#domains.set( id, new Locale( locale ) );
                }
                else {
...
export default class Locale extends Intl.Locale {
    ...
    i18nt ( msgId, pluralMsgId, pluralNumber ) {
        return new LocaleTranslation( this, msgId, pluralMsgId, pluralNumber );
    }
    ...
    formatCurrency ( value, format ) {
        const formatterId = ( format || "" ) + "/" + this.currency;
    ...
    formatRelativeTime ( milliseconds, format ) {
        if ( milliseconds instanceof Date ) {
            milliseconds = milliseconds - new Date();
        }

        const absSeconds = Math.abs( milliseconds / 1000 );

        for ( const threshold of relativeTimeUnits ) {
