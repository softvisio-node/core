...
export default class Locale extends Intl.Locale {
    ...
    get script () {
        return super.script || LANGUGAGE_SCRIPT[this.language];
    }
    ...
    formatName ( name, format ) {
        ...
        this.#displayNamesFormatters[format] ??= new Intl.DisplayNames( this, this.parseFormat( format, displayNameFormats ) );

        return this.#displayNamesFormatters[format].of( name );
    }

    formatDate ( value, format ) {
        format ||= "";

        return ( this.#dateTimeFormatters[format] ??= new Intl.DateTimeFormat( this, this.parseFormat( format, dateTimeFormats ) ) ).format( value );
    }

    formatNumber ( value, format ) {
        format ||= "";

        return ( this.#numberFormatters[format] ??= new Intl.NumberFormat( this, this.parseFormat( format, numberFormats ) ) ).format( value );
    }
    ...
    formatPercent ( value, format ) {
        ...
        if ( !formatter ) {
            ...
            formatter = this.#percentFormatters[format] = new Intl.NumberFormat( this, parsedFormat );
        }
    ...
    formatCurrency ( value, format ) {
        ...
        var formatter = this.#currencyFormatters[formatterId];
        ...
        if ( !formatter ) {
            ...
            formatter = this.#currencyFormatters[formatterId] = new Intl.NumberFormat( this, parsedFormat );
        }
    ...
    formatRelativeTime ( milliseconds, format ) {
        ...
        for ( const threshold of timeUnits ) {
            if ( absSeconds >= threshold[1] ) {
                return ( this.#relativeTimeFormatters[format] ??= new Intl.RelativeTimeFormat( this, this.parseFormat( format, relativeTimeFormats ) ) ).format( Math.round( milliseconds / ( threshold[1] || 1 ) / 1000 ), threshold[0] );
            }
        }
    }
    ...
    formatDigitalSize ( value, format ) {
        ...
        var formatter = this.#digitalSizeFormatters[format];
        ...
        if ( !formatter ) {
            ...
            if ( !parsedFormat.unit ) {
                ...
                formatter = new Intl.NumberFormat( this, parsedFormat );
            }
            else {
                formatter = this.#digitalSizeFormatters[format] = new Intl.NumberFormat( this, parsedFormat );
            }
        }
    ...
    // XXX millisecond, microsecond, nanosecond - are not translated
    formatDuration ( milliseconds, format ) {
        ...
        return Object.entries( duration )
            .map( ( [name, value] ) => {
                try {
                    return `${value} ${this.formatName( name, "type:dateTimeField" )}`;
                }
                catch ( e ) {
                    return `${value} ${name}`;
                }
        ...
        // return ( this.#durationFormatters[format] ??= new Intl.DurationFormat( this, this.parseFormat( format, durationFormats ) ) ).format( duration );
    }
    ...
    parseFormat ( format, cache ) {
        ...
        if ( cache ) parsedFormat = cache[format];
