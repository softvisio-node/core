...
export default class Locale extends Intl.Locale {
    ...
    formatDate ( value, format ) {
        return ( this.#dateTimeFormatters[format || ""] ??= new Intl.DateTimeFormat( this, this.#parseFormat( dateTimeFormats, format ) ) ).format( value );
    }

    formatNumber ( value, format ) {
        return ( this.#numberFormatters[format || ""] ??= new Intl.NumberFormat( this, this.#parseFormat( numberFormats, format ) ) ).format( value );
    }
    ...
    formatRelativeTime ( milliseconds, format ) {
        ...
        for ( const threshold of relativeTimeUnits ) {
            if ( absSeconds >= threshold[1] ) {
                return ( this.#relativeTimeFormatters[format || ""] ??= new Intl.RelativeTimeFormat( this, this.#parseFormat( relativeTimeFormats, format ) ) ).format( Math.round( milliseconds / ( threshold[1] || 1 ) / 1000 ), threshold[0] );
            }
        }
    }
