...
export default class Locale extends Intl.Locale {
    ...
    // XXX params
    formatRelativeTime ( date, format, fromDate ) {
        ...
        const milliseconds = date - fromDate;

        for ( const [unit, threshold] of relativeTimeUnits ) {
            if ( Math.abs( milliseconds ) > threshold || unit === "second" ) {
                return ( this.#relativeTimeFormatters[format || ""] ??= new Intl.RelativeTimeFormat( this, this.#parseFormat( relativeTimeFormats, format ) ) ).format( Math.round( milliseconds / threshold ), unit );
            }
        }
    }
