...
export default class Locale {
    ...
    get pluralExpression () {
        if ( this.#pluralExpression === undefined ) {
            this.#pluralExpression = languages[this.#language]?.expression || null;
        }
    ...
    get pluralFunction () {
        if ( this.#pluralFunction === undefined ) {
            this.#pluralFunction = languages[this.#language]?.function || null;
    ...
    add ( locale ) {
        ...
        if ( !this.locale && locale.locale ) this.#setLocale( locale.locale );

        if ( locale.messages ) this.#messages = { ...this.#messages, ...locale.messages };

        if ( !this.#pluralExpression && !languages[this.#language] && locale.pluralExpression ) {
    ...
    toJSON () {
        ...
        if ( this.locale ) json.locale = this.locale;
    ...
    #setLocale ( locale ) {
        [this.#language, this.#country] = locale.toLowerCase().split( "_" );
