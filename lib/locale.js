...
import { LOCALES, PLURAL_EXPRESSIONS } from "#lib/locale/constants";

const DEFAULT_PLURAL_NUMBER = 0;
...
export default class Locale extends Intl.Locale {
    ...
    l10n ( single, plural, pluralNumber ) {
        ...
        TRANSLATE: {
            if ( this.language === "en" ) break TRANSLATE;

            const translations = this.#messages?.[ single ];

            if ( !translations ) break TRANSLATE;
            ...
            if ( plural ) {
                ...
                if ( typeof pluralNumber !== "number" ) pluralNumber = DEFAULT_PLURAL_NUMBER;
            ...
            // single
                ...
                idx = this.#singleFormIndex ??= this.#pluralExpression( 1 );
            }

            translation = translations[ idx ];
        }

        if ( !translation ) return this.#translateEnglish( single, plural, pluralNumber );
    ...
    l10nt ( single, plural, pluralNumber ) {
        return new Translation( this, single, {
            plural,
            pluralNumber,
        } );
    }
    ...
    add ( locale ) {
        ...
        if ( locale.messages ) {
            ...
            for ( const [ id, translations ] of Object.entries( locale.messages ) ) {
                const messageTranslations = this.#messages[ id ];

                // message exists
                if ( messageTranslations ) {
                    let length = translations.length;

                    if ( messageTranslations.length > length ) length = messageTranslations.length;

                    for ( let n = 0; n < length; n++ ) {
                        if ( messageTranslations[ n ] !== translations[ n ] ) {
                            console.warn( `Translation conflict found, locale: ${ this.id }], message: "${ id }":`, messageTranslations, translations );
    ...
    // private
    #translateEnglish ( single, plural, pluralNumber ) {
        var translation;
