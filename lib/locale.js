...
import DigitalSize from "#lib/utils/digital-size";
...
export default class Locale extends Intl.Locale {
    ...
    formatRelativeDate ( duration, format ) {
        format ||= "";
        ...
        return ( relativeTimeFormatters[this.id][format] ??= new Intl.RelativeTimeFormat( this.id, this.parseFormat( format, relativeTimeFormats ) ) ).format( ...duration.getFormatRelativeDateParams() );
    }
    ...
    formatDuration ( duration, format ) {
        ...
        return Object.entries( duration.getFormatDurationParams() )
            .map( ( [unit, value] ) => {
    ...
    formatDuration1 ( duration, format ) {
        format ||= "";
        ...
        return ( durationFormatters[this.id][format] ??= new Intl.DurationFormat( this.id, this.parseFormat( format, durationFormats ) ) ).format( duration.getFormatDurationParams() );
    }
