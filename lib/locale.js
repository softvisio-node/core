...
// durationFormats = {}; // XXX uncomment, when Intl.DurationFormat will be available
...
export default class Locale extends Intl.Locale {
    ...
    formatName ( name, format ) {
        ...
        return ( displayNamesFormatters[this.id][format] ??= new Intl.DisplayNames( this, this.parseFormat( format, displayNameFormats ) ) ).of( name );
    }
    ...
    formatDate ( value, format ) {
        ...
        return ( dateTimeFormatters[this.id][format] ??= new Intl.DateTimeFormat( this, this.parseFormat( format, dateTimeFormats ) ) ).format( value );
    }
    ...
    formatRelativeTime ( milliseconds, format ) {
        ...
        return ( relativeTimeFormatters[this.id][format] ??= new Intl.RelativeTimeFormat( this, this.parseFormat( format, relativeTimeFormats ) ) ).format( Math.round( milliseconds / timeunit[1] ), timeunit[0] );
    }
    ...
    formatDuration ( milliseconds, format ) {
        ...
        return Object.entries( duration )
            .map( ( [name, value] ) => {
                try {
                    return `${value} ${this.formatName( name, "type:dateTimeField,style:short" )}`;
                }
                catch ( e ) {
                    return `${value} ${name}`;
                }
        ...
        // return ( durationFormatters[this.id][format] ??= new Intl.DurationFormat( this, this.parseFormat( format, durationFormats ) ) ).format( duration );
    }
