...
export default class Locale {
    ...
    i18n ( strings, ...args ) {
        const message = strings.join( "${n}" ),
            translation = this.translate( message, args );

        if ( translation ) return translation;

        if ( args.length ) {
            return message.replaceAll( "${n}", () => args.shift() );
        }
        else {
            return message;
        }
    ...
    translate ( message, args ) {
        ...
        if ( Array.isArray( translation ) ) {
            ...
            const idx = this.pluralFunction?.( args[0] ) || 0;

            translation = translation[idx];
        }

        if ( translation ) {
            if ( args.length ) translation = translation.replaceAll( "${n}", () => args.shift() );
...
`;
        ...
        for ( const [id, message] of Object.entries( this.#messages ) ) {
            ...
            if ( message.plural ) {
                ...
                const nplurals = this.nplurals || message.translation?.length || 1;

                for ( let n = 0; n < nplurals; n++ ) {
                    text += `msgstr[${n}] ${this.#writePoString( message.translation?.[n] )}`;
                }
            }

            // single
            else {
                text += `msgstr ${this.#writePoString( message.translation )}`;
            }
        }
