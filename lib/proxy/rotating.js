...
module.exports = class extends Upstream {
    ...
    async getProxy ( options ) {
        ...
        if ( !this.#rotate || cache.options.session ) {
            if ( !cache.proxy ) {
                cache.proxy = await this._getProxy( cache );
            }
        ...
        if ( rotate || !cache.autoRotateProxy ) {
            ...
            if ( this.#rotateRandom ) {
                proxy = await this._getRandomProxy( cache, true );
            }
            else {
                proxy = await this._getNextProxy( cache, true );
            }
    ...
    async getNextProxy ( options ) {
        ...
        return await this._getNextProxy( cache, false );
    }

    async getRandomProxy ( options ) {
        const cache = this._getCache( options );

        return await this._getRandomProxy( cache, false );
    }
};
