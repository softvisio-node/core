...
module.exports = class extends Upstream {
    ...
    async getProxy ( options = {} ) {
        ...
        if ( options.session ) return this._getUpstream( options );
        ...
        if ( this.#rotateTimeout || this.#rotateRequests ) {
            ...
            // rotate by timeout
            if ( this.#rotateTimeout && new Date() - cache.lastRotated >= this.#rotateTimeout ) return this.#rotateProxy( cache, this.#rotateRandom );

            // rotate by number of requests
            if ( this.#rotateRequests && cache.requests >= this.#rotateRequests ) return this.#rotateProxy( cache, this.#rotateRandom );
            ...
            // or try to cache and return proxy
                ...
                return this.#rotateProxy( cache, this.#rotateRandom );
            }
        }
    ...
    async getNextProxy ( options ) {
        ...
        return this.#rotateProxy( cache, false );
    }
    ...
    async getRandomProxy ( options ) {
        ...
        return this.#rotateProxy( cache, true );
    }
    ...
    async #rotateProxy ( cache, random ) {
        var proxy;
