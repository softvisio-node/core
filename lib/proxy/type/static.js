...
export default class ProxyClientStatic extends mixins( OptionsCountry, OptionsRemoteAddress, OptionsLocalAddress, ProxyClient ) {
    ...
    async getRemoteAddress () {
        ...
        this.#mutex.signal.broadcast( address );
    ...
    // connect
    // XXX
    async connect ( url, options = {}, updateHttpGetRequest ) {
        if ( typeof url === "string" ) url = new URL( url );
        ...
        // socks5 proxy
        else if ( this.isSocks ) {
            if ( updateHttpGetRequest ) updateHttpGetRequest( { "hostname": resolve ? hostname : null } );
        ...
        // http get
            ...
            if ( updateHttpGetRequest ) {
                updateHttpGetRequest( {
                    "http": true,
    ...
    async #connectHttp ( hostname, port ) {
        return new Promise( ( resolve, reject ) => {
            const socket = this.isSecure ? new tls.Socket() : new net.Socket();
    ...
    // XXX do not send credentials at first request
