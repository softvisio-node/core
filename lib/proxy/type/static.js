...
export default class ProxyStatic extends mixins( OptionsCountry, OptionsRemoteAddr, OptionsLocalAddr, ProxyClient ) {
    #isHttp = false;
    #isSocks = false;
    ...
    async connect ( url, options = {}, updateHttpRequest ) {
        ...
        var hostname = url.hostname,
            resolve = this.resolve === "local" && !net.isIP( hostname );

        const port = url.port || URL.getDefaultPort( url.protocol );
...
ProxyStatic.register( "http:", ProxyStatic );
ProxyStatic.register( "socks:", ProxyStatic );
ProxyStatic.register( "socks5:", ProxyStatic );
ProxyStatic.register( "http+socks:", ProxyStatic );
ProxyStatic.register( "http+socks5:", ProxyStatic );
ProxyStatic.register( "socks+http:", ProxyStatic );
ProxyStatic.register( "socks5+http:", ProxyStatic );
