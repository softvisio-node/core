...
module.exports = class ProxyLuminati extends mixins( CountryMixin, Proxy ) {
    ...
    #updateProxy () {
        ...
        if ( this.session ) {
            url = new URL( "http://" + this.#sessionHost + ":" + DEFAULT_PORT );
        }
        else {
            url = new URL( "http://" + DEFAULT_HOST + ":" + DEFAULT_PORT );
        }
        ...
        if ( this.session ) username += "-session-" + this.#sessionId;
    ...
    set session ( value ) {
        ...
        // not updated
        if ( value === ( this.#session || null ) ) return;

        if ( value ) {
            const [sessionId, sessionHost] = value.split( "@" );
