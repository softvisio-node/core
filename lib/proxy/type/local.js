...
const OptionsLocalSubnet = require( "../mixins/options/local-subnet" );
...
module.exports = class ProxyLocal extends mixins( OptionsSession, OptionsLocalSubnet, Pool, Rotating, Upstream, Proxy ) {
    get isHttp () {
        return true;
    }
    ...
    _buildProxy ( options = {} ) {
        const addr = this.localSubnet.firstAddr;
    ...
    _rotateNextProxy ( cache ) {
        if ( !cache.addr ) {
            cache.addr = this.localSubnet.firstAddr;
        }
        else {
            const addr = new IPAddr( cache.addr.ipNum.nextIPNumber() );

            if ( this.localSubnet.contains( addr ) ) {
                cache.addr = addr;
            }
            else {
                cache.addr = this.localSubnet.firstAddr;
    ...
    _rotateRandomProxy ( cache ) {
        const addr = this.localSubnet.getRandomAddr();
