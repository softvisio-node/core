...
export default class ProxyClientSoftvisio extends mixins( OptionsZone, OptionsCountry, OptionsState, OptionsCity, OptionsSession, OptionsRotating, Upstream ) {
    ...
    _buildProxy ( bucket ) {
        ...
        if ( proxy.username !== "" ) username.set( "username", encodeURIComponent( proxy.username ) );

        if ( options.zone ) username.set( "zone", encodeURIComponent( options.zone ) );
        if ( options.country ) username.set( "country", encodeURIComponent( options.country ) );
        if ( options.state ) username.set( "state", encodeURIComponent( options.state ) );
        if ( options.city ) username.set( "city", encodeURIComponent( options.city ) );
        if ( options.resolve != null ) username.set( "resolve", encodeURIComponent( options.resolve ) );
        if ( options.session ) username.set( "session", encodeURIComponent( options.session ) );

        if ( options.rotateRequests != null ) username.set( "rotateRequests", encodeURIComponent( options.rotateRequests ) );
        if ( options.rotateTimeout != null ) username.set( "rotateTimeout", encodeURIComponent( options.rotateTimeout ) );
        if ( options.rotateRandom != null ) username.set( "rotateRandom", encodeURIComponent( options.rotateRandom ) );
