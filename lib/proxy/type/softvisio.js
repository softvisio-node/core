...
module.exports = class ProxySoftvisio extends mixins( OptionsZone, OptionsCountry, OptionsState, OptionsCity, OptionsResolveRemote, OptionsSession, OptionsRotating, Upstream ) {
    ...
    _buildProxy ( bucket ) {
        ...
        var username = this._escapeValue( proxy.username );
        ...
        if ( options.session ) username += ",session_" + this._escapeValue( options.session );

        username += ",rotateRequests_" + options.rotateRequests;
        username += ",rotateTimeout_" + options.rotateTimeout;
        username += ",rotateRandom_" + options.rotateRandom;
