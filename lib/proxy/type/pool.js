...
module.exports = class ProxyPool extends mixins( RotatingMixin, CountryMixin, Proxy ) {
    ...
    _updated () {
        this.#isHttp = this.#index["http/"].size ? true : false;
        this.#isSocks = this.#index["socks/"].size ? true : false;
    ...
    async connect ( url ) {
        ...
        const proxy = this._needRotate && this._needRotate() ? await this.rotateNextProxy( { type } ) : await this.getProxy( { type } );
