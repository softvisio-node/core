...
export default class ProxyServer extends net.Server {
    ...
    async getPlaywrightProxy ( options = {} ) {
        ...
        return Proxy.new( "softvisio://", options ).getPlaywrightProxy();
    }
    ...
    async #httpConnection ( clientSocket ) {
        ...
        // read http headers
        headers = await clientSocket.readHttpHeaders();
                ...
                if ( authorization ) {
                    ...
                    try {
                        ...
                        if ( idx === -1 ) {
                            ...
                            connection.options = this.#parseUsername( credentials.substr( 0, idx ) );
    ...
    async #createDirectConnection ( url ) {
        return new Promise( ( resolve, reject ) => {
            ...
            socket.connect( url.port || url.getDefaultPort(), url.hostname );
        } );
    }
    ...
    #parseUsername ( username ) {
        ...
        data.username = decodeURIComponent( params.shift() );

        for ( let param of params ) {
            param = param.split( /_(.*)/ );

            if ( param[1] == null ) data[param[0]] = true;
            else data[param[0]] = decodeURIComponent( param[1] );
        }
