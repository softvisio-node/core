...
export default class ProxyServer extends net.Server {
    ...
    constructor ( { address, port, proxy, auth } = {} ) {
        super();
    ...
    async #onConnect ( clientSocket ) {
        var chunk = await clientSocket.readChunk( 6 ).catch( e => null );
        if ( !chunk ) return clientSocket.end();

        var remoteAddress;

        // proxy protocol
        if ( chunk.toString( "latin1" ) === "PROXY " ) {

            // remote address is not trusted
            if ( !subnets.get( "google-cloud-load-balancers" ).includes( remoteAddress ) && !subnets.get( "cloudflare" ).includes( remoteAddress ) ) return clientSocket.end();
            ...
            clientSocket.unshift( chunk.subarray( 1 ) );
            chunk = chunk.subarray( 0, 1 );
        }

        // socks5 connection
        if ( chunk[0] === 0x05 ) {
            this.#socks5Connection( clientSocket, remoteAddress );
        }
        ...
        // http connection
            ...
            this.#httpConnection( clientSocket, remoteAddress );
        }
    }
    ...
    async #httpConnection ( clientSocket, remoteAddress ) {
        const connection = {
            "type": "http",
            remoteAddress,
            "options": {},
        ...
        connection.method = method[0].toUpperCase();
