...
export default class ProxyServer extends net.Server {
    ...
    constructor ( { address, port, proxy, auth, proxyProtocolTrustedSubnets } = {} ) {
        super();
        ...
        if ( proxyProtocolTrustedSubnets == null ) {
            this.#proxyProtocolTrustedSubnets = [...DEFAULT_PROXY_PROTOCOL_TRUSTED_SUBNETS];
        }
        else if ( typeof proxyProtocolTrustedSubnets === "boolean" ) {
            this.#proxyProtocolTrustedSubnets = proxyProtocolTrustedSubnets;
        }
        else {
            this.#proxyProtocolTrustedSubnets = Array.isArray( proxyProtocolTrustedSubnets ) ? proxyProtocolTrustedSubnets : [proxyProtocolTrustedSubnets];
        }
    }
    ...
    async getPlaywrightProxy ( options = {} ) {
        options = { ...options, "hostname": this.#address, "port": this.#port };

        const proxy = ProxyClient.new( "softvisio://", options );
    ...
    async #onConnect ( clientSocket ) {
        ...
        while ( 1 ) {
            ...
            // socks5 connection
            if ( chunk[0] === 0x05 ) return this.#socks5Connection( clientSocket, remoteAddress );
            ...
            // http connection
                ...
                return this.#httpConnection( clientSocket, remoteAddress, method );
            }
        }
    }

    async #socks5Connection ( clientSocket, remoteAddress ) {
        const connection = {
            "type": "socks",
            "url": new URL( "socks5://" ),
            remoteAddress,
    ...
    async #httpConnection ( clientSocket, remoteAddress, method ) {
        const connection = {
            "type": "http",
            method,
            "url": null,
        ...
        // invalid http method, connect must be used
        else {
