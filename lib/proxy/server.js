...
module.exports = class ProxyServer extends net.Server {
    ...
    async _socks5Connection ( clientSocket ) {
        ...
        if ( chunk[3] === 0x01 ) {
            ...
            connection.hostname = new IPAddr( chunk.readUInt32BE() ).toString();
        }
        ...
        // domain name
            ...
            connection.hostname = chunk.toString();
        }
        ...
        connection.port = chunk.readUInt16BE();
        ...
        if ( proxy ) {
            ...
            // resolve host name
            if ( this.#resolve ) {
                connection.hostname = await dns.resolve4( connection.hostname );

                // unable to resolve host name
                if ( !connection.hostname ) return clientSocket.end();
            }

            serverSocket = await proxy.connect( connection ).catch( e => {} );
        }

        // direct connection
        else {
            serverSocket = await this._createDirectConnection( connection ).catch( e => {} );
        }
    ...
    async _httpConnection ( clientSocket ) {
        ...
        var password, method, headers, url;
        ...
        if ( proxy ) {
            ...
            // resolve host name
            if ( this.#resolve ) {
                connection.hostname = await dns.resolve4( url.hostname );
    ...
    async _createDirectConnection ( connection ) {
        return new Promise( ( resolve, reject ) => {
            ...
            socket.connect( connection.port, connection.hostname );
        } );
    }
};
