...
export default class ProxyServer extends net.Server {
    ...
    async #onConnect ( clientSocket ) {
        ...
        while ( 1 ) {
            ...
            if ( method === "PROXY" ) {
                ...
                if ( subnets.get( "google-cloud-load-balancers" ).includes( remoteAddress ) || subnets.get( "cloudflare" ).includes( remoteAddress ) ) {
                    try {
    ...
    #parseUsername ( username ) {
        ...
        for ( const [name, value] of new URLSearchParams( username ) ) data[name] = decodeURIComponent( value );
