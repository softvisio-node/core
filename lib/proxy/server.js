...
module.exports = class ProxyServer extends net.Server {
    ...
    async _socks5Connection ( clientSocket ) {
        const remoteAddress = new IPAddr( clientSocket.remoteAddress );
        ...
        if ( authMethods[2] ) {
            ...
            if ( this.#auth ) {
                ...
                // authorize
                auth = await this.#auth( { username, password, remoteAddress, "type": "socks" } );
        ...
        // no auth
            ...
            if ( this.#auth ) {
                ...
                // authorize
                const auth = await this.#auth( { username, password, remoteAddress, "type": "socks" } );
    ...
    async _httpConnection ( clientSocket ) {
        ...
        while ( 1 ) {
            ...
            if ( this.#auth ) {
                ...
                // authorize
                auth = await this.#auth( { username, password, remoteAddress, "type": "http" } );
