...
export default class ProxyServer extends net.Server {
    ...
    async #createServerSocket ( proxy, connection ) {
        if ( this.#checkConnection ) {
            const resolvedIpAddress = await resolve4( connection.url.hostname );

            if ( !resolvedIpAddress ) return;

            const port = connection.url.port || net.getDefaultPort( connection.url.protocol );

            if ( !this.#checkConnection( resolvedIpAddress, port ) ) return;

            connection.options.resolvedIpAddress = resolvedIpAddress;
        }
