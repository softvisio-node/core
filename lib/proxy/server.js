...
class ProxyServer extends net.Server {
    #host = "127.0.0.1";
    #port;
    #proxy;
    #auth;
    ...
    async listen ( port, host ) {
        ...
        return super.listen( this.#port, this.#host );
    }
    ...
    async _socks5Connection ( clientSocket ) {
        const remoteAddress = this.#auth || this.#stat ? new IPAddr( clientSocket.remoteAddress ) : null;
        let userId;
        ...
        if ( authMethods[2] ) {
            ...
            if ( this.#auth ) {
                ...
                // authorize
                userId = await this.#auth( remoteAddress, username, password );
