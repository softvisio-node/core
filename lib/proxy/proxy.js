...
class Proxy {
    ...
    async connect ( url ) {
        ...
        if ( url.protocol === "http:" ) {
            if ( this.isSocks ) return this._createSocks5Tunnel( url.hostname, url.port || this._getDefaultPort( url.protocol ) );

            if ( this.isHttp ) return this._createHttpTunnel( url.hostname, url.port || this._getDefaultPort( url.protocol ) );
        }
        else if ( url.protocol === "https:" ) {
            if ( this.isSocks ) return this._createSocks5Tunnel( url.hostname, url.port || this._getDefaultPort( url.protocol ) );

            if ( this.isHttp ) return this._createHttpsTunnel( url.hostname, url.port || this._getDefaultPort( url.protocol ) );
        }
        else {
            if ( this.isSocks ) return this._createSocks5Tunnel( url.hostname, url.port || this._getDefaultPort( url.protocol ) );
        }
    ...
    // TODO
    async _createHttpTunnel ( host, port ) {
    ...
    // TODO
    async _createHttpsTunnel ( host, port ) {
        throw "not implemented";
    }

    // TODO
    async _createSocks4Tunnel ( host, port ) {
        throw "not implemented";
    }

    // TODO
    async _createSocks4aTunnel ( host, port ) {
        throw "not implemented";
    }
