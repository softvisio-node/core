...
class Proxy {
    ...
    get resolve () {
        return Object.hasOwnProperty.call( this.#options, "resolve" );
    }
    ...
    async getAddrInfo ( ip ) {
        if ( !ip ) ip = await this.getRemoteAddr();

        if ( !ip ) return;

        return maxmind.city.get( ip );
    }
    ...
    async connect ( url ) {
        ...
        if ( type === "http" ) {
            ...
            if ( this.resolve ) {
                ...
                if ( !ip ) return "Unable to resolve host name";
