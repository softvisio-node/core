...
export default class ProxyClientStatic extends mixins( OptionsCountry, OptionsRemoteAddress, OptionsLocalAddress, ProxyClient ) {
    ...
    _init ( url, options = {} ) {
        ...
        const types = new Set( this.protocol.substring( 0, this.protocol.length - 1 ).split( "+" ) );

        if ( types.has( "http" ) ) this.#isHttp = true;
        if ( types.has( "connect" ) ) this.#isConnect = true;
        if ( types.has( "socks" ) ) this.#isSocks = true;
