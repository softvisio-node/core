...
export default class ProxyClientStatic extends mixins( OptionsCountry, OptionsRemoteAddress, OptionsLocalAddress, ProxyClient ) {
    ...
    async connect ( url, { hostnameAddress } = {} ) {
        ...
        // http: protocol
            ...
            // http
                ...
                return this.#connectHttp( hostname, port, url );
            }
        }
    ...
    // private
    async #connectHttp ( hostname, port, url ) {
        return new Promise( ( resolve, reject ) => {
            ...
            if ( this.isTls ) {
                ...
                socket.once( "secureConnect", () => {
                    ...
                    resolve( this.#patchHttpSocket( socket, url.hostname, port ) );
                } );
            }
            else {
                socket = net.connect( {
    ...
    #patchHttpSocket ( socket, hostname, port ) {
        ...
        socket._write = async function ( data, encoding, callback ) {
            ...
            header[ 1 ] = "http://" + hostname + ( port === 80 ? "" : ":" + port ) + header[ 1 ];
