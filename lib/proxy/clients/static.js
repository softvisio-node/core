...
export default class ProxyClientStatic extends mixins( OptionsCountry, OptionsRemoteAddress, OptionsLocalAddress, ProxyClient ) {
    ...
    getPlaywrightProxy () {
        ...
        if ( this.isHttp ) {
            return {
                "server": ( this.isTls ? "https://" : "http://" ) + this.hostname + ":" + this.port,
        ...
        while ( true ) {
            this.#connect(
                ...
                async socket => {
                    ...
                    if ( basicAuth ) req.push( `Proxy-Authorization: Basic ${ basicAuth }\r\n` );
                    ...
                    const [ status, statusText ] = headers[ 0 ].split( " " ).slice( 1 );
                    ...
                    if ( status === "200" ) {
                        ...
                        if ( basicAuth || !this.basicAuth ) {
                            ...
                            // XXX
                            return this.#connectHttps( { hostname, port, connectTimeout, checkCertificate }, this.basicAuth );
                        }
                    }
                    else {
                        throw statusText;
