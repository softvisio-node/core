...
export default class ProxyClientStatic extends mixins( OptionsCountry, OptionsRemoteAddress, OptionsLocalAddress, ProxyClient ) {
    ...
    async connect ( url, { resolvedIpAddress } = {} ) {
        ...
        return new Promise( ( resolve, reject ) => {
            ...
            if ( this.isTls ) {
                ...
                socket.once( "secureConnect", () => {
                    ...
                    resolve( this.#patchHttpSocket( socket ) );
                } );
            }
            else {
                socket = net.connect( {
    ...
    #patchHttpSocket ( socket ) {
        ...
        socket._write = async function ( data, encoding, callback ) {
            if ( headersDone ) return write( data, encoding, callback );
            ...
            for ( let n = 1; n < headers.length; n++ ) {
                ...
                if ( id === "host" ) {
                    if ( patched[ id ] ) {
                        ...
                        host = headers[ n ].substring( idx + 1 ).trim();

                        headers[ n ] = name + ": " + hostname;
                    }
                }
                    ...
                    if ( patched[ id ] ) {
                        ...
                        if ( basicAuth ) {
                            headers[ n ] = "Proxy-Authorization: Basic " + basicAuth;
                        }
                        else {
                            headers[ n ] = null;
                        }
            ...
            if ( basicAuth && !patched[ "proxy-authorization" ] ) {
                headers.push( "Proxy-Authorization: Basic " + basicAuth );
            }

            const header = headers[ 0 ].split( " " );
            header[ 1 ] = "http://" + host + header[ 1 ];
