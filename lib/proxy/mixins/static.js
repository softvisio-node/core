    ...
    class extends ( Super || Object ) {
        ...
        async connect ( url, options, updateHttpRequest ) {
            ...
            if ( this.localAddr && this.localAddr.toString() === this.hostname ) {
                return this.#connectLocal( url.hostname, url.port || url.getDefaultPort() );
            }
            else if ( requireSocks || !this.isHttp ) {
                if ( updateHttpRequest ) updateHttpRequest( false );

                return this.#connectSocks5( url.hostname, url.port || url.getDefaultPort() );
            }
            else {

                // http
                if ( url.protocol === "http:" ) {
                    if ( updateHttpRequest ) updateHttpRequest( true, this.basicAuth );

                    return this.#connectHttp( url.hostname, url.port || url.getDefaultPort() );
                }
                ...
                // https
                    ...
                    return this.#connectHttps( url.hostname, url.port || url.getDefaultPort() );
                }
            }
        }
