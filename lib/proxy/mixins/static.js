...
module.exports = Super =>
    class extends ( Super || Object ) {
        ...
        async getRemoteAddr () {
            ...
            if ( !this.#mutex.tryDown() ) return await this.#mutex.wait();
        ...
        async connect ( url ) {
            ...
            if ( this.localAddr && this.localAddr.toString() === this.hostname ) {
                return this.#connectLocal( url.hostname, url.port || url.defaultPort );
            }
            else if ( requireSocks || !this.isHttp ) {
                if ( url.updateHttpRequest ) url.updateHttpRequest( false );

                return this.#connectSocks5( url.hostname, url.port || url.defaultPort );
            }
            else {

                // http
                if ( url.protocol === "http:" ) {
                    if ( url.updateHttpRequest ) url.updateHttpRequest( true, this.basicAuth );

                    return this.#connectHttp( url.hostname, url.port || url.defaultPort );
                }
                ...
                // https
                    ...
                    return this.#connectHttps( url.hostname, url.port || url.defaultPort );
                }
            }
        }
