...
export class StreamJoin3 extends stream.Transform {
    #stream;
    #transformCallback;
    #readableListener = this.#onReadable.bind( this );
    #errorListener = this.#onError.bind( this );
...
export class StreamJoin2 extends stream.Duplex {
    #queue = []; // new Queue();
    #stream;
    #callback;
    #readableListener = this.#onReadable.bind( this );
    ...
    _write ( chunk, encoding, callback ) {
        ...
        if ( chunk instanceof stream.Readable || Buffer.isBuffer( chunk ) ) {
    ...
    #read () {
        if ( this.#stream ) {
            ...
            else if ( chunk instanceof stream.Readable ) {
    ...
    #destroy () {
        ...
        for ( const [ chunk ] of queue ) {
            if ( chunk instanceof stream.Readable ) chunk.destroy();
        }
