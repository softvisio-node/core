import stream from "#lib/stream";
import StreamSearch from "#lib/stream/search";

const DEFAULT_EOL = Buffer.from( "\n" );

export default class StreamJson extends stream.Duplex {
    #eol;
    #streamSearch;
    #streams = new Set();
    #currentStream;
    #processingStreams = 0;
    #errorListener = () => {};

    constructor ( eol ) {
        super( { "readableObjectMode": true, "allowHalfOpen": true } );

        this.#eol = eol ? ( Buffer.isBuffer( eol ) ? eol : Buffer.from( eol ) ) : DEFAULT_EOL;

        this.#streamSearch = new StreamSearch( this.#eol );
        this.#streamSearch.on( "info", this.#onInfo.bind( this ) );
    }

    // properties
    get eol () {
        return this.#eol;
    }

    // protected
    // init stream, error can be returned
    _construct ( callback ) {
        callback();
    }

    _write ( chunk, encoding, callback ) {
        if ( Buffer.isBuffer( chunk ) ) chunk = chunk.toString();

        this.#streamSearch.push( chunk );

        callback();
    }

    _read ( size ) {}

    // protected
    async _processStream ( stream ) {
        return stream;
    }

    // XXX
    // called when writeable ended, delays finish event in order to make destroy calls possible
    _final ( callback ) {
        this.#endCurrentStream();

        callback();
    }

    // XXX
    // delays "close" event until all sub-streams will be winished
    _destroy ( error, callback ) {
        if ( error ) {
            this.#streams.forEach( stream => stream.destroy( error ) );
        }

        callback( error );
    }

    // private
    #onInfo ( isMatched, data, start, end ) {
        if ( !this.#currentStream ) this.#createStream();

        // match
        if ( isMatched ) {

            // push data
            if ( data ) {
                this.#currentStream.push( data.slice( start, end ) );
            }

            this.#endCurrentStream();
        }

        // not match
        else {
            this.#currentStream.push( data.slice( start, end ) );
        }
    }

    // XXX remove error listener on push
    async #createStream () {
        const _stream = new stream.Readable( { read () {} } );

        _stream.once( "close", this.#onStreamClose.bind( this, stream ) );
        _stream.once( "error", this.#errorListener );

        this.#streams.add( _stream );

        this.#currentStream = _stream;

        this.#processingStreams++;

        // process stream
        const data = await this._processStream( _stream );

        if ( data ) this.push( data );

        this.#processingStreams--;

        // end readable if writable stream ended and no processing streams
        if ( !this.#processingStreams && this.writableEnded ) this.push( null );
    }

    #endCurrentStream () {
        if ( !this.#currentStream ) return;

        // send eof
        this.#currentStream.push( null );

        this.#currentStream = null;
    }

    #onStreamClose ( stream ) {
        this.#streams.delete( stream );
    }
}
