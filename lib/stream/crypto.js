...
// private
async function opensslInit ( { buffer, key, algorithm, preset } = {} ) {
    preset ||= OPENSSL_DEFAULT_PRESET;

    const passwordHash = ( PASSWORD_HASH_CACHE[ preset ] ||= new PasswordHash( {
        preset,
    } ) );
...
async function deriveOpensslKey ( key, { passwordHash, salt, cipher } = {} ) {
    ...
    // derive key
    const res = await passwordHash.createHash( key, {
        "phc": false,
        salt,
