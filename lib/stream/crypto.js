    ...
    async _transform ( chunk, encoding, callback ) {
        ...
        if ( this.#init ) {
            try {
                ...
                this.#cipherOptions = getCipherInfo( res.algorithm );

                if ( !this.#cipherOptions?.isSupported ) {
                    throw new Error( "Cipher is not supported" );
                }

                this.#cipher = crypto.createCipheriv( this.#cipherOptions.algorithm, res.key, res.iv, {
                ...
                if ( this.#cipherOptions.authTagLength ) {
                    ...
                    buffer.writeUInt8( this.#cipherOptions.authTagLength );
    ...
    async _transform ( chunk, encoding, callback ) {
        ...
        if ( this.#init ) {
            try {
                ...
                this.#decipher = crypto.createDecipheriv( this.#cipherOptions.algorithm, res.key, res.iv, {
