    ...
    async _transform ( chunk, encoding, callback ) {
        ...
        if ( this.#init ) {
            try {
                ...
                this.#useAuthTag = AUTH_TAG_CIPHERS.has( this.#algorithm.toLowerCase() );
...
// private
async function opensslInit ( { buffer, key, settings, algorithm, digest, iterations, saltLength } = {} ) {
    settings = OPENSSL[ settings ] || OPENSSL.modern;
    ...
    if ( buffer ) {
        ...
        const salt = buffer.subarray( 8, 8 + saltLength ),
            res = await deriveOpensslKey( key, { settings, digest, iterations, salt } );
    ...
    // create header
        ...
        const res = await deriveOpensslKey( key, { settings, digest, iterations, saltLength } );
        if ( !res.ok ) throw res;
...
async function deriveOpensslKey ( key, { settings, digest, iterations, salt, saltLength } = {} ) {
    if ( typeof key === "string" ) key = Buffer.from( key );

    digest ||= settings.digest;

    iterations ||= settings.digestIterations?.[ digest ] || settings.iterations;

    salt ||= await new Promise( ( resolve, reject ) => {
        crypto.randomBytes( saltLength || settings.saltLength, ( e, buffer ) => {
            if ( e ) {
                reject( e );
            }
            else {
