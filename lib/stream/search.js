...
export default class StreamSearch extends Events {
    ...
    _sbmh_feed ( data ) {
        ...
        if ( pos < 0 ) {
            ...
            while ( pos < 0 && pos <= len - eolLength ) {
                const ch = this._sbmh_lookup_char( data, pos + eolLength - 1 );

                if ( ch === last_eol_char && this._sbmh_memcmp( data, pos, eolLength - 1 ) ) {
                    this._lookbehind_size = 0;
                    ++this.matches;
    ...
    _sbmh_memcmp = function ( data, pos, len ) {
        var i = 0;

        while ( i < len ) {
            if ( this._sbmh_lookup_char( data, pos + i ) === this._eol[i] ) ++i;
            else return false;
        }
