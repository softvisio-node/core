...
export default class StreamMultipart extends StreamCombined {
    ...
    // public
    append ( content, { type, size, name, filename, headers, transform, addContentLength } = {} ) {
        var length = size || 0;
        ...
        if ( headers ) {
            for ( const name in headers ) {
                ...
                else if ( id === "content-length" || id === "content-disposition" ) continue;
        ...
        // add Content-Length
        if ( length && addContentLength ) header += `Content-Length: ${length}\r\n`;
