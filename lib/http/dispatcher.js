...
export default class Dispatcher extends Agent {
    ...
    static setHeaders ( url, headers, { compress, cookies, browser } = {} ) {
        ...
        if ( browser ) {
            if ( browser === true ) {
                browser = RESOURCES[ Dispatcher.defaultBrowser ];
            }
            else {
    ...
    dispatch ( options, handlers ) {
        if ( !options._headersProcessed ) {
            ...
            this.constructor.setHeaders( options.origin + options.path, headers, this );
