...
export default class Dispatcher extends Agent {
    ...
    async #connect ( options, callback ) {
        ...
        if ( this.#proxy ) {
            try {
                httpSocket = await this.#proxy.connect( {
                    ...options,
                ...
                if ( options.protocol === "http:" ) {
                    if ( this.#connectOptions.keepAlive == null || this.#connectOptions.keepAlive ) {
                        const keepAliveInitialDelay = this.#connectOptions.keepAliveInitialDelay === undefined
                            ? 60e3
                            : this.#connectOptions.keepAliveInitialDelay;
