...
export default class Server extends Events {
    ...
    // public
    async listen ( { hostname = "127.0.0.1", port = 80 } = {} ) {
        if ( !port ) port = await getRandomFreePort();

        return new Promise( resolve => {
            this.#uws.listen( hostname, port, socket => {
                if ( socket ) {
                    this.emit( "listening", { hostname, port } );

                    resolve( result( 200, { hostname, port } ) );
                }
                else {
                    resolve( result( 500, { hostname, port } ) );
                }
            } );
        } );
    }
