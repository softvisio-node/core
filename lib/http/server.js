...
export default class HttpServer {
    ...
    constructor ( { ssl, compress, webSocketCompress, setRealIpFrom, realIpHeader, ...options } = {} ) {
        this.#uws = ssl
            ? uws.SSLApp( options )
            : uws.App( options );
    ...
    get isStarted () {
        return this.#startManager.isStarted;
    }
    ...
    async stop ( { graceful = true } = {} ) {
        return this.#startManager.stop( {
            graceful,
        } );
    }

    // addServerName ( hostname, options = {} ) {
    //     return this.#uws.addServerName( hostname, options );
    // }

    // domain ( domain ) {
    //     return this.#uws.domain( domain );
    // }
    ...
    #createRequest ( res, req, socketContext ) {
        ...
        if ( !this.#startManager.isStarted || this.#startManager.isStopping ) {
            req.close( -32_816 );
