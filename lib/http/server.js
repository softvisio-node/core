...
export default class Server extends Events {
    ...
    ws ( location, { upgrade, open, ping, pong, drain, maxPayloadLength, idleTimeout, sendPingsAutomatically, compression, minCompressionLength, maxBackpressure, connection } = {} ) {
        ...
        let _compression;
        compression.forEach( value => {
            if ( !COMPRESSION.has( value ) ) throw Error`Invalid compression constant ${value}`;

            if ( _compression == null ) _compression = uws[value];
            else _compression |= uws[value];
        } );

        compression = _compression;
        minCompressionLength ??= DEFAULT_MIN_COMPRESSION_LENGTH;
        ...
        // open
        options.open = ws => {
            const _connection = connection ? connection( ws, { minCompressionLength } ) : new Connection( ws, { minCompressionLength } );
