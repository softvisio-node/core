...
export default class HttpServer {
    ...
    constructor ( {
        ...
        this.#uws = certificatePath
            ? uws.SSLApp( {
                ...
                "ca_file_name": caPath,
                "dh_params_file_name": dhParamPath || certificates.dhParamPath,
                "passphrase": password,
    ...
    get isStarted () {
        return this.#activityManager.isStarted;
    }
    ...
    addServerName ( hostname, { certificatePath, privateKeyPath, caPath, dhParamPath, password, tlsCiphers, tlsPreferLowMemoryUsage } = {} ) {
    ...
    #createRequest ( res, req, socketContext ) {
        ...
        if ( !this.#activityManager.isStarted || this.#activityManager.isStopping ) {
            req.close( -32_816 );

            return;
        }
        else {
            this.#activityManager.startRequest();
    ...
    #finishRequest () {
        this.#activityManager.finishRequest();
    }

    async #start ( { address, port, exclusive, chmod } ) {
