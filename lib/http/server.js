...
export default class Server extends Events {
    ...
    ws ( location, options = {} ) {
        ...
        // 0 = no compression, 1 = shared compressor (recommended), 2 = dedicated compressor
        if ( !options.compresion ) options.compression = 0;
        else if ( options.compresion === true ) options.compression = 1; // shared compression
        ...
        if ( options.open ) {
            ...
            options.open = ws => {
                const connection = options.connection ? options.connection( ws ) : new Connection( ws );
