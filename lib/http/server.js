...
export default class Server extends Events {
    ...
    ws ( location, { onUpgrade, ping, pong, drain, maxPayloadLength, idleTimeout, sendPingsAutomatically, compress, compression, maxBackpressure, createConnection } = {} ) {
        ...
        options.close = ( ws, status, statusText ) => {
            ws.connection.onDisconnect( result( [status, Buffer.from( statusText ).toString()] ) );
        };
