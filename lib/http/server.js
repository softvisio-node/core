...
export default class Server extends Events {
    ...
    folder ( location, folder, options = {} ) {
        ...
        this.get( `${location}/*`, req => {
            ...
            if ( filepath === "/" ) {
                req.end( new File( { "path": folder + "/index.html" } ), options );
            }
            else {
                req.end( new File( { "path": folder + filepath } ), options );
            }
        } );
    ...
    webpack ( location, folder, { compress = true } = {} ) {
        location = this.#prepareLocation( location );
        ...
        this.get( `${location}/*`, req => {
            ...
            if ( filepath === "/" ) {
                req.end( new File( { "path": folder + "/index.html" } ), optionsRevalidate );
            }

            // files from the root dir - must revalidate
            else if ( filepath.indexOf( "/", 1 ) === -1 ) {
                req.end( new File( { "path": folder + filepath } ), optionsRevalidate );
            }

            // files from the sub-directories - cache forever
            else {
                req.end( new File( { "path": folder + filepath } ), optionsCacheForever );
            }
        } );
