...
export default class Server extends Events {
    ...
    ws ( location, { upgrade, open, ping, pong, drain, maxPayloadLength, idleTimeout, sendPingsAutomatically, compress, compression, maxBackpressure, connection } = {} ) {
        ...
        // open
        options.open = ws => {
            ws.connection = connection ? connection( this, ws, { compress } ) : new Connection( this, ws, { compress } );
        ...
        options.close = ( ws, status, statusText ) => {
            ws.connection.onClose( result( [status, Buffer.from( statusText ).toString()] ) );
        };
    ...
    file ( location, file, options = {} ) {
        ...
        this.get( location, req => {
            if ( file instanceof File ) {
                req.end( file, options );
            }
            else {
                req.end( new File( { "path": file } ), options );
            }
        } );
    ...
    directory ( location, directory, options = {} ) {
        ...
        this.get( `${location}/*`, req => {
            ...
            if ( filepath === "/" ) {
                req.end( new File( { "path": indexPath } ), options );
            }
            else {
                req.end( new File( { "path": path.join( directory, filepath ) } ), options );
            }
        } );
    ...
    webpack ( location, directory, { compress } = {} ) {
        ...
        this.get( `${location}/*`, req => {
            ...
            if ( filepath === "/" ) {
                req.end( new File( { "path": indexPath } ), optionsRevalidate );
            }

            // files from the root dir - must revalidate
            else if ( filepath.indexOf( "/", 1 ) === -1 ) {
                req.end( new File( { "path": path.join( directory, filepath ) } ), optionsRevalidate );
            }

            // files from the sub-directories - cache forever
            else {
                req.end( new File( { "path": path.join( directory, filepath ) } ), optionsCacheForever );
            }
        } );
    ...
    api ( location, { httpCallback, websocketsConfig } = {} ) {
        ...
        this.get( `${location}/oauth.html`, req => {
            req.end( new File( { "path": oauthHtmlPath } ), {
                "headers": {
                    "cache-control": "public, max-age=1",
                },
            } );
