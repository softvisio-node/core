            ...
            for ( const _domain in this.#cookies ) {
                ...
                if ( domain != null ) {
                    ...
                    if ( domain.startsWith( "." ) ) {
                        if ( !( "." + _domain ).endsWith( domain ) ) continue;
                    }
        ...
        // filter path
            ...
            for ( const _path in this.#cookies[ domain ] ) {
                ...
                if ( name == null && !session && !expired ) {
                    delete this.#cookies[ domain ][ _path ];
                ...
                // filter cookies
                    ...
                    for ( const _name in this.#cookies[ domain ][ _path ] ) {
                        const cookie = this.#cookies[ domain ][ _path ][ _name ];

                        if ( name === _name || ( session && cookie.isSession ) || ( expired && cookie.isExpired ) ) {
                            delete this.#cookies[ domain ][ _path ][ _name ];

                            cookieDeleted = true;
                        }
                    }

                    if ( cookieDeleted && objectIsEmpty( this.#cookies[ domain ][ _path ] ) ) {
                        delete this.#cookies[ domain ][ _path ];
            ...
            if ( pathDeleted && objectIsEmpty( this.#cookies[ domain ] ) ) {
                delete this.#cookies[ domain ];
            }
        }
    }
