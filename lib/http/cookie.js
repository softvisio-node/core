    ...
    // private
        ...
        if ( this.#encodedName == null ) {
            if ( this.#name == null ) {
                ...
                // forbidden chars: \x00-\x1F, \x7F-\xFF, \s, \t, ( ) < > @ , ; : \ " / [ ] ? = { }
                this.#encodedName = this.#name.replaceAll( /([\s\x00-\x1F"%(),/:;<=>?@[\\\]{}\x7F-\xFF])/g, match => "%" + match.charCodeAt( 0 ).toString( 16 ).padStart( 2, "0" ).toUpperCase() );
            }
        }
        ...
        if ( this.#encodedValue == null ) {
            if ( this.#value == null ) {
                ...
                // forbidden chars: \x00-\x1F, \x7F-\xFF, \s, \t, " , ; \
                this.#encodedValue = this.#value.replaceAll( /([\s\x00-\x1F"%,;\\\x7F-\xFF])/g, match => "%" + match.charCodeAt( 0 ).toString( 16 ).padStart( 2, "0" ).toUpperCase() );
            }
        }
