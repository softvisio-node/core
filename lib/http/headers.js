...
export default class Headers {
    ...
    get range () {
        ERROR: if ( this.#parsed["range"] === undefined ) {
            ...
            for ( const rangeMatch of match[2].matchAll( /(-?\d+)(?:-(\d+))?/g ) ) {
                const start = rangeMatch[1] ? +rangeMatch[1] : null,
                    end = rangeMatch[2] ? +rangeMatch[2] : null;

                // end < start
                if ( end != null ) {

                    // bytes=-1-2 || bytes=3-1
                    if ( start < 0 || start > end ) break ERROR;
                }
    ...
    *values () {
    ...
    createContentRange ( range, size, { unit = "bytes" } = {} ) {
