        ...
        NEXT_COOKIE: for ( const header of setCookieHeader ) {
            ...
            for ( const match of header.matchAll( /([^ ;=]+)\s*(?:=\s*([^;]+))?;?/gi ) ) {
                ...
                if ( name === "expires" ) {
                    ...
                    // invalid date
                    if ( Number.isNaN( value.getTime() ) ) continue NEXT_COOKIE;
                    ...
                    if ( !Number.isNaN( value ) ) cookie.maxAge = value;
                }
                else if ( name === "domain" ) {
                    cookie.domain = value;
                }
        ...
        ERROR: if ( this.#parsed[ "cookie" ] === undefined ) {
            ...
            for ( const match of header.matchAll( /([^ ;=]+)\s*=\s*([^;]*);?/gi ) ) {
                const cookie = new Cookie( {
                    "encodedName": match[ 1 ],
                    "encodedValue": match[ 2 ]?.trim(),
                } );

                this.#parsed[ "cookie" ][ cookie.name ] = cookie;
            }
        }
