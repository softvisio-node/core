...
const SINGLE_VALUE = new Set( [ "age", "authorization", "content-length", "content-type", "etag", "expires", "from", "host", "if-modified-since", "if-unmodified-since", "last-modified", "location", "max-forwards", "proxy-authorization", "referer", "retry-after", "server", "user-agent" ] );
...
const ORIGINAL_NAMES = {
    ...
    "server": null,
    "set-cookie": null,
    "tansfer-encoding": null,
    "user-agent": null,
    "www-authenticate": "WWW-Authenticate",
};

// prepare original names for known headers
for ( const name in ORIGINAL_NAMES ) {
    if ( !ORIGINAL_NAMES[ name ] ) {
        const originalName = name.charAt( 0 ).toUpperCase() + name.substring( 1 );

        ORIGINAL_NAMES[ name ] = originalName.replaceAll( /-([a-z])/g, ( originalName, letter ) => "-" + letter.toUpperCase() );
    }
}
