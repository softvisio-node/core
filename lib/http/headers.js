...
export default class Headers {
    ...
    get wwwAuthenticate () {
        ERROR: if ( this.#parsed["www-authenticate"] === undefined ) {
            ...
            for ( const match of header.matchAll( /([a-z-]+)\s*=\s*(?:"([^"]+)"|([^,]+))\s*,?/gi ) ) {
    ...
    get contentDisposition () {
        ERROR: if ( this.#parsed["content-disposition"] === undefined ) {
            ...
            const header = ( this.#headers["content-disposition"] || "" ).trim();
            ...
            this.#parsed["content-disposition"].type = header.substring( 0, idx ).toLowerCase();

            for ( const match of header.matchAll( /([a-z-]+)=(?:"([^"]+)"|([^;]+))(?:;\s*)?/gi ) ) {
                this.#parsed["content-disposition"][match[1].toLowerCase()] = match[2] ?? match[3];
            }
        }
