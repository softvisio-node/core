...
export default class Request extends Events {
    ...
    get path () {
        this.#path ??= path.posix.normalize( "/" + decodeURI( this.#req.getUrl() ).replaceAll( "\\", "/" ) );
    ...
    async end ( body, close ) {
        ...
        var status, headers, lastModified, compress, zlibOptions;

        if ( objectIsPlain( body ) ) ( { status, headers, body, lastModified, compress, zlibOptions } = body );
        else if ( objectIsPlain( close ) ) ( { status, headers, lastModified, compress, zlibOptions, close } = close );
        ...
        const skipBody = this.method === "head";
                ...
                if ( contentLength == null ) {
                    ...
                    if ( range ) {
                        ...
                        // valid range
                            ...
                            if ( !skipBody ) body = body.stream( { "start": range.start, "end": range.end } );
                        }
                    }
                    else {
                        contentLength = body.size;
                        if ( !skipBody ) body = body.stream();
                    }
                }
            }
            ...
            // blob
                ...
                if ( range ) {
                    ...
                    // valid range
                        ...
                        status = 206; // Partial Content
                        headers.set( "content-range", range.contentRange );
                        if ( !skipBody ) body = body.slice( range.start, range.end );
                    }
                }
                else {
                    contentLength = body.size;
                    if ( !skipBody ) body = await body.arrayBuffer();
                }
            }
                ...
                if ( range ) {
                    ...
                    // valid range
                        ...
                        if ( Buffer.isBuffer( body ) ) {
                            if ( !skipBody ) body = body.subarray( range.start, range.end );
                        }
                        else {
                            if ( !skipBody ) body = body.slice( range.start, range.end );
                        }
        ...
        // if ( contentLength && skipBody ) headers.set( "content-length", contentLength );
