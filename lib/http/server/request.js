...
export default class Request extends Events {
    ...
    async end ( body, close ) {
        ...
        if ( compress && writeBody && body instanceof stream.Readable ) {
    ...
    async #writeStream ( stream, size ) {
        ...
        return new Promise( resolve => {
            ...
            stream.once( "end", () => this.#res.end( "" ) );

            stream.on( "data", buffer => {
                if ( !Buffer.isBuffer( buffer ) ) buffer = Buffer.from( buffer );

                ( chunk = buffer.buffer.slice( buffer.byteOffset, buffer.byteOffset + buffer.byteLength ) ), ( lastOffset = this.#res.getWriteOffset() );
