...
export default class Request extends Events {
    ...
    // XXX this can be url if method is connect
    get path () {
        this.#path ??= path.posix.normalize( "/" + decodeURI( this.#rawUrl ).replaceAll( "\\", "/" ) );
    ...
    #isIpAddressTrusted ( address ) {
        ...
        // private
        if ( subnets.get( "private" ).includes( address ) ) return "private";

        // google
        if ( subnets.get( "google-cloud-load-balancers" ).includes( address ) ) return "google-cloud-load-balancers";

        // cloudflare
        if ( subnets.get( "cloudflare" ).includes( address ) ) return "cloudflare";
    }
}
