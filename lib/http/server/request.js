...
export default class Request extends Events {
    ...
    get stream () {
        if ( !this.#stream ) {
            ...
            const stream = ( this.#stream = new Readable( { read () {} } ) );
    ...
    async blob () {
        ...
        return new Blob( buf, { "type": this.getHeader( "content-type" ) } );
    }

    // XXX https://github.com/sifrr/sifrr/tree/master/packages/server/sifrr-server
    async formData () {
        return FormData.decode( this.stream );
    }
    ...
    writeHead ( status, headers ) {
        ...
        // resolve statusText
        if ( typeof status === "number" ) status += " " + result.getStatusText( status );
