...
export default class Request extends Events {
    ...
    constructor ( server, req, res, socketContext ) {
        ...
        // remote address
        this.#remoteAddress = Buffer.from( this.#res.getProxiedRemoteAddressAsText() || this.#res.getRemoteAddressAsText() ).toString();

        // url
        var url = "http://" + ( this.headers.get( "host" ) || this.#remoteAddress ) + req.getUrl() + ( req.getQuery() ? "?" + req.getQuery() : "" );
    ...
    get remoteAddress () {
        return ( this.#remoteIpAddress ??= new IpAddress( this.#remoteAddress ) );
    }
