...
export default class Request extends Events {
    ...
    get realRemoteAddress () {
        if ( !this.#realRemoteAddress ) {
            this.#realRemoteAddress = this.remoteAddr;

            // remote address is trusted
            if ( this.#isIpAddressTrusted( this.remoteAddr ) ) {
    ...
    #isIpAddressTrusted ( address ) {
        ...
        // private
        if ( subnets.get( "private" ).includes( address ) ) return true;

        // cloudflare
        if ( subnets.get( "cloudflare" ).includes( address ) ) return true;

        // google
        if ( subnets.get( "google" ).includes( address ) ) return true;
    }
}
