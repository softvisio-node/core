...
export default class Request extends Events {
    ...
    get urlDecoded () {
        if ( this.#urlDecoded == null ) this.#urlDecoded = path.normalize( decodeURI( this.url ) );

        return this.#urlDecoded;
    }

    // public
    // XXX url search params
    getQuery ( name ) {
        return name == null ? this.#req.getQuery() : this.#req.getQuery( name );
    }
