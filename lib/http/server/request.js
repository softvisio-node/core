    ...
    async end ( options ) {
        ...
        BODY: {
            if ( body ) {
                ...
                RANGE: if ( supportRange && status !== 206 && headers.has( "accept-ranges" ) && !headers.has( "content-range" ) ) {
                    ...
                    // check range
                    range = headers.createContentRange( range.unit, range.ranges[0], contentLength );
