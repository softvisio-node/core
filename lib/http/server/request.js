...
export default class Request extends Events {
    ...
    get remoteAddr () {
        this.#remoteAddr ??= new IpAddress( Buffer.from( this.#res.getProxiedRemoteAddressAsText() ).toString() || Buffer.from( this.#res.getRemoteAddressAsText() ).toString() );
    ...
    get realRemoteAddr () {
        if ( !this.#realRemoteAddr ) {
            this.#realRemoteAddr = this.remoteAddr;

            const xRealIp = this.#req.getHeader( "x-real-ip" );

            // x-real-ip header is trusted
            if ( xRealIp && subnets.contains( "private", this.remoteAddr ) ) {
                try {
                    this.#realRemoteAddr = new IpAddress( xRealIp );
                }
                catch ( e ) {}
            }
            ...
            if ( cloudflareConnectingIp && subnets.contains( "cloudflare", this.#realRemoteAddr ) ) {
                try {
                    this.#realRemoteAddr = new IpAddress( cloudflareConnectingIp );
                }
                catch ( e ) {}
            }
        }
