...
export default class Request extends Events {
    ...
    constructor ( server, req, res, socketContext ) {
        ...
        this.#remoteAddress = this.#remoteAddress.byteLength ? new IpAddress( Buffer.from( this.#remoteAddress ).toString() ) : localAddress;

        // url
        var url = "http://" + ( this.headers.get( "host" ) || this.#remoteAddress.toString() ) + req.getUrl() + ( req.getQuery() ? "?" + req.getQuery() : "" );
    ...
    get realRemoteAddress () {
        if ( !this.#realRemoteAddress ) {
            ...
            if ( this.#isIpAddressTrusted( this.remoteAddress ) ) {
                if ( this.#server.realIpHeader === "x-real-ip" ) {
                ...
                else if ( this.#server.realIpHeader === "x-forwarded-for" ) {
                    const forwardedFor = this.headers.get( "x-forwarded-for" )?.split( "," );

                    if ( forwardedFor ) {
                        while ( forwardedFor.length ) {
                            try {
                                const realRemoteAddress = forwardedFor.pop().trim();

                                if ( realRemoteAddress === "unix:" ) continue;

                                this.#realRemoteAddress = IpAddress.new( realRemoteAddress );
