...
export default class Request extends Events {
    ...
    // XXX http server response - always send content-length: 0 if no body
    async end ( body, close ) {
        ...
        if ( body instanceof result.Result ) {
            ...
            if ( objectIsPlain( body.data ) ) {
                ( { headers, body, lastModified, eTag, compress, zlibOptions } = body.data );
            }
            else {
                body = body.data;
            }
        }
        else if ( objectIsPlain( body ) ) {
            ( { status, headers, body, lastModified, eTag, compress, zlibOptions } = body );
        }
        else if ( objectIsPlain( close ) ) {
            ( { status, headers, lastModified, eTag, compress, zlibOptions, close } = close );
        }
