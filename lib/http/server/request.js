...
export default class Request extends Events {
    ...
    get realRemoteAddress () {
        if ( !this.#realRemoteAddress ) {
            ...
            if ( this.#server.realIpHeader && this.#server.trustedSubnets && this.#isIpAddressTrusted( this.remoteAddress ) ) {
                const addresses = this.headers.get( this.#server.realIpHeader )?.split( "," );

                if ( addresses ) {
                    while ( addresses.length ) {
                        try {
                            const realRemoteAddress = addresses.pop().trim();
    ...
    #isIpAddressTrusted ( address ) {
        for ( const subnet of this.#server.trustedSubnets ) {
