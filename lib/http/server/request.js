...
export default class Request extends Events {
    ...
    async end1 ( { status = 200, headers, body, compress, zlibOptions, close } = {} ) {
        ...
        if ( ifModifiedSince ) {
            ...
            if ( !lastModified && body instanceof File ) lastModified = body.lastModifiedDate;

            if ( lastModified ) {
                ifModifiedSince = new Date( ifModifiedSince );

                if ( !( lastModified instanceof Date ) ) lastModified = new Date( lastModified );
        ...
        if ( body ) {
            ...
            if ( body instanceof File ) {
                ...
                if ( body.lastModifiedDate && !headers.has( "last-modified" ) ) headers.set( "last-modified", body.lastModifiedDate );
