...
export const SQLITE_DECODERS = {};

export const POSTGRESQL_DECODERS = {};

for ( const type of DEFAULT_TYPES ) {
    for ( const name in type.names ) {
        const oid = type.names[name];

        if ( type.decodeSqlite || type.decode ) SQLITE_DECODERS[name] = type.decodeSqlite || type.decode;

        if ( type.decodePostgresql || type.decode ) {
            POSTGRESQL_DECODERS[name] = type.decodePostgresql || type.decode;

            if ( oid ) POSTGRESQL_DECODERS[oid] = type.decodePostgresql || type.decode;
        }
    }
}
