...
export function decodeNumber ( value, precision ) {
...
export function parseInteger ( value ) {
    if ( typeof value !== "number" ) {
        if ( Buffer.isBuffer( value ) ) {
            value = Number.parseInt( value.toString( "latin1" ) );
        }
        else {
    ...
    if ( !Number.isSafeInteger( value ) ) throw RangeError( "Number is out of range" );
...
export function decodeBigIntToInteger ( value ) {
    if ( Number.isInteger( value ) ) {
        return value;
    }
    ...
    // check range
    if ( value < MIN_NUMBER || value > MAX_NUMBER ) throw RangeError( `BigInt value is out of JS Number range` );
...
const DEFAULT_TYPES = [
    ...
    },
    {
        ...
        "decode": buffer => decodeArray( buffer, decodeInteger ),
    },
    {
        "names": {
            "int8Array": 1016,
        ...
        "decode": buffer => decodeArray( buffer, decodeBigIntToInteger ),
    },
    {
        "names": {
            "boolArray": 1000,
    ...
    },
    {
        ...
        "decode": buffer => decodeArray( buffer, decodeNumber ),
    },
    {
        "names": {
            "byteaArray": 1001,
