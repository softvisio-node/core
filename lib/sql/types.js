const DEFAULT_TYPES = [
    ...
    // number
    {
        "names": {
            ...
            "float4": 700, // real
            "float8": 701, // double
            "numeric": 1700, // decimal XXX check range
            "money": 790,
            "oid": 26,
        },
    ...
    // int8 as number
    {
        ...
        "decodePostgresql": buffer => {
            ...
            if ( number < -9007199254740991n || number > 9007199254740991n ) throw `Int8 value is out of JS Number range`;

            return Number( number );
        },

        "decodeSqlite": bigint => {
            if ( bigint < -9007199254740991n || bigint > 9007199254740991n ) throw `Int53 value is out of JS Number range`;
    ...
    // int53
    {
        ...
        "decodePostgresql": buffer => {
            ...
            if ( number < -9007199254740991n || number > 9007199254740991n ) throw `Int53 value is out of JS Number range`;
...
for ( let n = 1; n < 16; n++ ) {
    DEFAULT_TYPES.push( {
        ...
        "decodePostgresql": buffer => Number( Number( buffer.toString( "latin1" ) ).toFixed( n ) ),
