import glob from "#lib/glob";
import _url from "url";
import * as config from "#lib/config";
import CONST from "#lib/const";

const SQL_MIGRATION_TABLE_NAME = "_schema";

export default Super =>
    class DbhMigration extends Super {
        #hasSchema = false;
        #emits = new Set();

        // properties
        get emits () {
            return this.#emits;
        }

        get hasSchema () {
            return this.#hasSchema;
        }

        // public
        async loadSchema () {
            const res = await this.select( `SELECT * FROM "${SQL_MIGRATION_TABLE_NAME}"` );

            if ( !res.ok ) return res;

            if ( !res.data ) return result( 200 );

            this.#hasSchema = true;

            // apply emits
            for ( const row of res.data ) {
                if ( row.emits ) this.#emits = new Set( [...this.#emits, ...row.emits] );
            }

            return result( 200 );
        }

        async migrateSchema ( url ) {
            var res;

            const path = _url.fileURLToPath( url );

            // load schema index
            const meta = config.read( path + "/index.yaml" );

            if ( !meta.module ) throw `Database schema module is required`;

            if ( meta.patch === undefined ) throw `Database schema patch is required`;

            // check type
            if ( !meta.type ) throw `Database schema type is required`;
            const type = new Set( Array.isArray( meta.type ) ? meta.type : [meta.type] );
            if ( this.isSqlite && !type.has( "sqlite" ) ) throw `Database schema type is not compatible with SQLite`;
            if ( this.isPgsql && !type.has( "pgsql" ) ) throw `Database schema type is not compatible with PostgreSQL`;

            // XXX validate emits

            const schema = {},
                patches = {};

            // load schema
            var files = glob.sync( "*.js", { "cwd": path, "nodir": true } );

            for ( const file of files ) {
                const version = Number( file.match( /^(\d+)/g )[0] );

                if ( version in schema ) throw `Database schema patch ${version} is already exists`;

                schema[version] = await import( new URL( file, url + "/" ) );
            }

            // load patches
            files = glob.sync( "*.js", { "cwd": path + "/patch", "nodir": true } );

            for ( const file of files ) {
                const version = Number( file.match( /^(\d+)/g )[0] );

                if ( version in patches ) throw `Database patch ${version} is already exists`;

                patches[version] = await import( new URL( "patch/" + file, url + "/" ) );
            }

            // migrate
            if ( this.isSqlite ) res = this.#migrateSync( meta, schema, patches );
            else res = await this.#migrateAsync( meta, schema, patches );

            if ( res.ok ) {
                this.#hasSchema = true;

                // store emits
                if ( meta.emits ) this.#emits = new Set( [...this.#emits, ...meta.emits] );
            }

            return res;
        }

        // private
        async #migrateAsync ( meta, schema, patches ) {
            return this.lock( async dbh => {
                var res;

                // set pgsql advisory lock
                if ( this.isPgsql ) {
                    res = await dbh.selectRow( `SELECT pg_advisory_lock(${CONST.SQL_LOCKS.MIGRATION})` );

                    if ( !res.ok ) return res;
                }

                try {

                    // XXX patch, remove
                    res = await dbh.exec( `
ALTER TABLE IF EXISTS __migration RENAME COLUMN version TO patch;
ALTER TABLE IF EXISTS __migration ALTER COLUMN patch DROP NOT NULL;
ALTER TABLE IF EXISTS __migration RENAME TO __migration__;

ALTER TABLE IF EXISTS __migration__ ADD COLUMN emits json;
ALTER TABLE IF EXISTS __migration__ RENAME TO _schema;
` );
                    if ( !res.ok ) throw res;

                    // create schema table
                    res = await dbh.do( `
CREATE TABLE IF NOT EXISTS "${SQL_MIGRATION_TABLE_NAME}" (
    module text PRIMARY KEY NOT NULL,
    patch int4,
    emits json
)
            ` );

                    if ( !res.ok ) throw res;

                    // process module
                    const module = meta.module;

                    // get current module version
                    res = await dbh.selectRow( `SELECT "patch" FROM "${SQL_MIGRATION_TABLE_NAME}" WHERE "module" = ?`, [module] );
                    if ( !res.ok ) throw res;

                    let moduleVersion = res.data ? res.data.patch : undefined;

                    // apply full schema
                    if ( schema.length ) {
                        res = await dbh.begin( async dbh => {
                            for ( const version of Object.keys( schema ).sort ) {
                                const patch = schema[version];

                                // apply patch
                                if ( moduleVersion === undefined ) await this._applyPatch( dbh, module, version, patch );

                                // apply types
                                await this._applyTypes( dbh, module, version, patch );
                            }

                            if ( moduleVersion === undefined ) {
                                moduleVersion = meta.patch;

                                // update module version
                                const res = await dbh.do( `INSERT INTO "${SQL_MIGRATION_TABLE_NAME}" ("module", "patch") VALUES (?, ?)`, [module, moduleVersion] );

                                if ( !res.ok ) throw res;
                            }
                        } );

                        if ( !res.ok ) throw res;
                    }

                    // apply schema patches
                    if ( patches.length ) {
                        for ( const version of Object.keys( patches ).sort ) {
                            const patch = patches[version];

                            res = await dbh.begin( async dbh => {
                                let updated;

                                // apply patch
                                if ( moduleVersion == null || version > moduleVersion ) {
                                    await this._applyPatch( dbh, module, version, patch );

                                    moduleVersion = version;
                                    updated = true;
                                }

                                // apply types
                                await this._applyTypes( dbh, module, version, patch );

                                // update module version
                                if ( updated ) {
                                    const res = await dbh.do( `INSERT INTO "${SQL_MIGRATION_TABLE_NAME}" ("module", "patch") VALUES (?, ?) ON CONFLICT ("module") DO UPDATE SET "patch" = ?`, [module, moduleVersion, moduleVersion] );

                                    if ( !res.ok ) throw res;
                                }
                            } );

                            if ( !res.ok ) throw res;
                        }
                    }

                    // update module emits
                    const emits = new Set( meta.emits );
                    res = await dbh.do( `UPDATE "${SQL_MIGRATION_TABLE_NAME}" SET emits = ? WHERE module = ?`, [JSON.stringify( [...emits] ), module] );
                    if ( !res.ok ) throw res;

                    res = result( 200 );
                }
                catch ( e ) {
                    res = result.catch( e );
                }

                // remove pgsql advisory lock
                if ( this.isPgsql ) {
                    const res = await dbh.selectRow( `SELECT pg_advisory_unlock(${CONST.SQL_LOCKS.MIGRATION})` );
                    if ( !res.ok ) return res;
                }

                return res;
            } );
        }

        #migrateSync ( meta, schema, patches ) {
            const dbh = this;

            var res;

            try {

                // create migration schema
                res = dbh.do( `
CREATE TABLE IF NOT EXISTS "${SQL_MIGRATION_TABLE_NAME}" (
    module text PRIMARY KEY NOT NULL,
    patch int4,
    emits json
)
            ` );

                if ( !res.ok ) throw res;

                // process module
                const module = meta.module;

                // get current module version
                res = dbh.selectRow( `SELECT "patch" FROM "${SQL_MIGRATION_TABLE_NAME}" WHERE "module" = ?`, [module] );
                if ( !res.ok ) throw res;

                let moduleVersion = res.data ? res.data.patch : undefined;

                // apply full schema
                if ( schema.length ) {
                    res = dbh.begin( dbh => {
                        for ( const version of Object.keys( schema ).sort ) {
                            const patch = schema[version];

                            // apply functions
                            this._applyFunctions( dbh, module, version, patch );

                            // apply patch
                            if ( moduleVersion === undefined ) this._applyPatch( dbh, module, version, patch );

                            // apply types
                            this._applyTypes( dbh, module, version, patch );
                        }

                        if ( moduleVersion === undefined ) {
                            moduleVersion = meta.patch;

                            // update module version
                            const res = dbh.do( `INSERT INTO "${SQL_MIGRATION_TABLE_NAME}" ("module", "patch") VALUES (?, ?)`, [module, moduleVersion] );

                            if ( !res.ok ) throw res;
                        }
                    } );

                    if ( !res.ok ) throw res;
                }

                // apply schema patches
                if ( patches.length ) {
                    for ( const version of Object.keys( patches ).sort ) {
                        const patch = patches[version];

                        res = dbh.begin( dbh => {
                            let updated;

                            // apply functions
                            this._applyFunctions( dbh, module, version, patch );

                            // apply patch
                            if ( moduleVersion == null || version > moduleVersion ) {
                                this._applyPatch( dbh, module, version, patch );

                                moduleVersion = version;
                                updated = true;
                            }

                            // apply types
                            this._applyTypes( dbh, module, version, patch );

                            // update module version
                            if ( updated ) {
                                const res = dbh.do( `INSERT INTO "${SQL_MIGRATION_TABLE_NAME}" ("module", "patch") VALUES (?, ?) ON CONFLICT ("module") DO UPDATE SET "patch" = ?`, [module, moduleVersion, moduleVersion] );

                                if ( !res.ok ) throw res;
                            }
                        } );

                        if ( !res.ok ) throw res;
                    }
                }

                // update module emits
                const emits = new Set( meta.emits );
                res = dbh.do( `UPDATE "${SQL_MIGRATION_TABLE_NAME}" SET emits = ? WHERE module = ?`, [JSON.stringify( [...emits] ), module] );
                if ( !res.ok ) throw res;

                res = result( 200 );
            }
            catch ( e ) {
                res = result.catch( e );
            }

            return res;
        }
    };
