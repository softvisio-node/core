...
` );
                    ...
                    if ( meta.cron ) {
                        ...
                        if ( values.length ) {
                            res = await dbh.do( `INSERT INTO ${sqlConst.cronTableName} ( module, name, cron, timezone, query ) VALUES ${values.join( ", " )} ON CONFLICT ( module, name ) DO UPDATE SET cron = EXCLUDED.cron`, params );
                            if ( !res.ok ) throw res;
                        }
