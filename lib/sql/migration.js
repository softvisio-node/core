...
const SQL_MIGRATION_TABLE_NAME = "__migration";
...
export default Super =>
    class DBHMigration extends Super {
        ...
        async loadSchema ( url, module = SQL_MIGRATION_DEFAULT_MODULE ) {
            ...
            if ( this.isSQLite && !type.has( "sqlite" ) ) throw `DB schema type is not compatible`;
            if ( this.isPgSQL && !type.has( "pgsql" ) ) throw `DB schema type is not compatible`;
                        ...
                        if ( this.#schema[module] ) {
                            res = await dbh.begin( async dbh => {
                                ...
                                if ( moduleVersion == null ) {
                                    ...
                                    const res = await dbh.do( `INSERT INTO "${SQL_MIGRATION_TABLE_NAME}" ("module", "version") VALUES (?, ?) ON CONFLICT ("module") DO UPDATE SET "version" = ?`, [module, moduleVersion, moduleVersion] );
    ...
    "version" int4 NOT NULL
            ...
            ` );
                ...
                for ( const module of Object.keys( this.#schema ).sort() ) {
                    ...
                    // get current module version
                    res = dbh.selectRow( `SELECT "version" FROM "${SQL_MIGRATION_TABLE_NAME}" WHERE "module" = ?`, [module] );
                    ...
                    if ( this.#schema[module] ) {
                        res = dbh.begin( dbh => {
                            ...
                            if ( moduleVersion == null ) {
                                ...
                                // update module version
                                const res = dbh.do( `INSERT INTO "${SQL_MIGRATION_TABLE_NAME}" ("module", "version") VALUES (?, ?) ON CONFLICT ("module") DO UPDATE SET "version" = ?`, [module, moduleVersion, moduleVersion] );
