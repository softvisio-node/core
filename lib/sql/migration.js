...
` );
                    ...
                    if ( meta.cron ) {
                        res = await dbh.do( `UPDATE ${sqlConst.cronTableName} SET crom = NULL` );
                        if ( !res.ok ) throw res;
                        ...
                        for ( const name in meta.cron ) {
                            values.push( `( ?, ?, ?, ? )` );
                            params.push( module, name, meta.cron[name].cron, meta.cron[name].timezone );
                        }

                        res = await dbh.do( `INSERT INTO ${sqlConst.cronTableName} ( module, name, cron, timezone ) VALUES ${values.join( ", " )} ON CONFLICT ( module, name ) DO UPDATE SET cron = EXCLUDED.cron`, params );
                        if ( !res.ok ) throw res;

                        res = await dbh.do( `DELETE FROM ${sqlConst.cronTableName} WHERE crom IS NULL` );
                        if ( !res.ok ) throw res;
                    }
