...
export default Super =>
    class TransactionsSync extends ( Super || class {} ) {
        ...
        begin ( mode, callback ) {
            ...
            // start transaction
            var res = dbh.do( savepointId ? `SAVEPOINT "${ savepointId }"` : begin );
            ...
            if ( res.ok ) {
                ...
                // commit
                if ( res.ok ) {
                    const commitRes = dbh.do( savepointId ? `RELEASE SAVEPOINT "${ savepointId }"` : SQL.commit );
                ...
                // rollback
                    ...
                    dbh.do( savepointId ? `ROLLBACK TO SAVEPOINT "${ savepointId }"` : SQL.rollback );
                }
            }
