...
export default class DbhPoolPgsql extends mixins( Transactions, Main, Pool, Dbh ) {
    ...
    constructor ( url, options = {} ) {
        ...
        // maxConnectionsPerSlave
        this.#maxConnectionsPerSlave = +( options.maxConnectionsPerSlave || url.searchParams.get( "maxConnectionsPerSlave" ) || DEFAULT_MAX_CONNECTIONS_PER_SLAVE );
        if ( !Number.isInteger( this.#maxConnectionsPerSlave ) || this.#maxConnectionsPerSlave < 1 ) {
            throw TypeError( `PostgreSQL maxConnectionsPerSlave value is invalid` );
        }
    ...
    quote ( param ) {
        ...
        if ( param != null && typeof param === "object" && param[Symbol.for( "SQLType" )] ) {
            param = this.#encode[param[Symbol.for( "SQLType" )]]( param );
        }
    ...
    async select ( query, params ) {
        return ( await this._getConnection() ).select( query, params );
    }
    ...
    async read ( query, options ) {
        return ( await this._getConnection() ).read( query, options );
    }

    // types
    async addType ( name, options ) {
        return ( await this._getConnection() ).addType( name, options );
    }
