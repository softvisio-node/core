...
import { sqliteDecoders } from "#lib/sql/types";
...
export default class DbhPoolSqlite extends mixins( Transactions, Dbh ) {
    ...
    #parseFileUrl ( url, options = {} ) {
        ...
        if ( options.cache !== DEFAULT_CACHE ) url.searchParams.set( "cache", options.cache );
        if ( options.mode !== DEFAULT_MODE ) url.searchParams.set( "mode", options.mode );
        if ( options.tempStore !== DEFAULT_TEMP_STORE ) url.searchParams.set( "tempStore", options.tempStore );
        if ( options.journalMode !== DEFAULT_JOURNAL_MODE ) url.searchParams.set( "journalMode", options.journalMode );
        if ( options.synchronous !== DEFAULT_SYNCHRONOUS ) url.searchParams.set( "synchronous", options.synchronous );
        if ( options.cacheSize !== DEFAULT_CACHE_SIZE ) url.searchParams.set( "cacheSize", options.cacheSize );
        if ( options.busyTimeout !== DEFAULT_BUSY_TIMEOUT ) url.searchParams.set( "busyTimeout", options.busyTimeout );
