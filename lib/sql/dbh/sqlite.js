...
export default class DbhPoolSqlite extends mixins( Schema, Transactions, Dbh ) {
    #url;
    #database;
    #isRelative;
    ...
    constructor ( url, options = {} ) {
        ...
        this.function( "sqlite_notify", ( name, data ) => {
            ...
            // unknown event
            if ( this.schema.isLoaded && !this.emits.has( name ) ) throw Error( `SQLite event name "${name}" is unknown` );
    ...
    #subscribe ( name ) {
        ...
        if ( this.schema.isLoaded && !this.emits.has( name ) ) throw `Event name "${name}" is not emitted by the database`;
    }
