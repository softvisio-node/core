...
import { v1 as uuidv1, v4 as uuidv4 } from "#lib/uuid";
...
export default class DBHPoolSqlite extends DBHPool {
    ...
    constructor ( url, options = {} ) {
        ...
        this.#sqlite.function( "uuid_generate_v1", () => {
            return uuidv1();
        } );

        this.#sqlite.function( "uuid_generate_v1mc", () => {
            return uuidv1();
        } );
    ...
    quote ( param ) {
        ...
        if ( typeof param === "object" && param[CONST.SQL_TYPE] ) param = this.#encode[param[CONST.SQL_TYPE]]( param );
    ...
    #do ( query, params, firstRow, ignoreData ) {
        ...
        if ( !sth ) {
            ...
            // prepare sth
            try {
                sth = this.#sqlite.prepare( query[0] );
            }
            catch ( e ) {
            ...
            if ( sth.reader ) {
                ...
                for ( const column of sth.columns() ) {
                    ...
                    if ( query[3] && query[3][column.name] ) {
                        type = query[3][column.name];

                        // invalid decode type
                        if ( !this.#decode[type] ) return this._onQueryError( Error( `Invalid decode type for column "${column.name}"` ), query[0] );
                    }
                    else {
                        type = column.type;
                    }
    ...
    #prepareQuery ( query, params, toString ) {
        ...
        if ( query[1] && ( toString || query[1].length > MAX_PARAMS ) ) {
            query[0] = this.queryToString( query[0], query[1] );
            query[1] = null;
            query[2] = null;
        }
