...
const DEFAULT_BUSY_TIMEOUT = 30000; // ms, 30 sec.
...
export default class DBHPoolSQLite extends DBHPool {
    ...
    constructor ( url, options = {} ) {
        ...
        // readOnly
        this.#mode = options.mode ?? ( url.searchParams.get( "mode" ) || DEFAULT_MODE );
        ...
        this.#database = _url.fileURLToPath( url );

        // temp database
        if ( path.basename( this.#database + "temp" ) === "temp" ) {
            this.#database = "";
        }

        // in-memory database
        else if ( path.basename( this.#database ) === ":memory:" ) {
            this.#database = ":memory:";
        }

        this.#sqlite = new SQLite( this.#database, {
            "readonly": this.#mode === "ro",
            "fileMustExist": this.#mode !== "rwc",
    ...
    get url () {
        if ( !this.#url ) {
            ...
            if ( this.#mode !== DEFAULT_MODE ) url.searchParams.set( "mode", this.#mode );
            ...
            if ( this.#database === "" ) {
                this.#url = "file:" + url.search;
            }
            else if ( this.#database === ":memory:" ) {
                this.#url = "file::memory:" + url.search;
            }
            else {
                this.#url = _url.pathToFileURL( this.#database ) + url.search;
            }
        }
    ...
    attach ( name, database ) {
        database = new URL( database, _url.pathToFileURL( process.cwd() + "/" ) );

        database = _url.fileURLToPath( database );

        // temporary database
        if ( path.basename( database + "temp" ) === "temp" ) {
            database = "";
        }

        // in-memory database
        else if ( path.basename( database ) === ":memory:" ) {
            database = ":memory:";
        }

        this.do( `ATTACH DATABASE '${database}' AS "${name}"` );
