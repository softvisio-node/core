...
import * as fileUrl from "#lib/utils/file-url";
...
const publishQuery = sql`SELECT sqlite_notify( ?, ? )`.prepare();
...
export default class DbhPoolSqlite extends mixins( Transactions, Dbh ) {
    ...
    publish ( name, data ) {
        this.select( publishQuery, [name, data] );
    }

    #parseFileUrl ( url, options = {} ) {
        options = { ...options };

        const pathname = fileUrl.relativeFileUrlToPath( url );
        ...
        return [fileUrl.relativePathToFileUrl( pathname, tmpl ), options];
    }
}
