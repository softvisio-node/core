...
export default class DBHPoolSQLite extends DBHPool {
    ...
    constructor ( url, options = {} ) {
        ...
        [this.#database, this.#isRelative] = this.#parseFileURL( url );
    ...
    get url () {
        if ( !this.#url ) {
            ...
            if ( this.#database === "" ) {
                ...
                this.#url = "file:" + encodeURI( this.#database ) + url.search;
            }
            else {
    ...
    #parseFileURL ( url ) {
        var database = _url.fileURLToPath( url ),
            relative = false;

        if ( database.startsWith( "a:\\" ) ) {
            relative = true;
            database = database.substring( 3 );
        }
        else if ( database.startsWith( "/a:/" ) ) {
            relative = true;
            database = database.substring( 4 );
        }
