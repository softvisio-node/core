    ...
    class extends ( Super || Object ) {
        ...
        async _getConnection ( lock, standby ) {
            ...
            if ( standby ) {
                if ( !this.standbyHost ) {
                    ...
                    this.#dnsWatcher = new DnsWatcher( this.standbyHost ).unref().start();
                }
            }

            const connections = standby ? this.#standbyConnections : this.#primaryConnections;
        ...
        _destroy () {
            ...
            for ( const connections of [this.#primaryConnections, this.#standbyConnections] ) {
                for ( const connection of connections.idle ) connection.destroy( result( [400, `DBH destroyed`] ) );

                for ( const connection of connections.active ) connection.destroy( result( [400, `DBH destroyed`] ) );

                if ( connections.locked ) {
                    for ( const connection of connections.locked ) connection.destroy( result( [400, `DBH destroyed`] ) );
                }
        ...
        #onConnectionDestroy ( dbh, res ) {
            const connections = dbh.isPrimary ? this.#primaryConnections : this.#standbyConnections;
        ...
        #getFakeConnection () {
            return ( this.#fakeConnection ??= this._newConnection( {
                "destroyed": true,
