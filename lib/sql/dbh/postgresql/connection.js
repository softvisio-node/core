...
export default class DbhPostgreSql extends Transactions( Dbh ) {
    ...
    #connect () {
        ...
        if ( this.#pool.protocol === "postgresqls:" ) this.#socket = tls.connect( { "socket": this.#socket } );
    ...
    // transactions
    doAfterCommit ( callback ) {
        if ( !this.inTransaction ) throw Error( `Unable to use "doAfterCommit" when transaction is not started` );

        this.#doAfterCommit ||= [];

        this.#doAfterCommit.push( callback );
    }

    doAfterRollback ( callback ) {
        if ( !this.inTransaction ) throw Error( `Unable to use "doAfterRollback" when transaction is not started` );

        this.#doAfterRollback ||= [];

        this.#doAfterRollback.push( callback );
    }

    async _endTransaction ( commited ) {
        var callbacks;

        if ( commited ) {
            callbacks = this.#doAfterCommit;
        }
        else {
            callbacks = this.#doAfterRollback;
        }

        this.#doAfterCommit = null;
        this.#doAfterRollback = null;
