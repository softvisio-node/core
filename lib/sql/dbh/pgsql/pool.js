...
Slave maxConnectionsPerSlave: ${this.#pool.maxConnectionsPerSlave}, total: ${connections.slave.total}, idle: ${connections.slave.idle.size}, active: ${connections.slave.active.size}`;
    }
...
export default Super =>
    class extends ( Super || Object ) {
        ...
        async #resolveSlave () {
            if ( !this.#slaveResolveMutex.tryDown() ) return this.#slaveResolveMutex.wait();

            const records = await dns.promises.lookup( this.slaveHostname, { "all": true, "family": 0 } );

            this.#slaveResolveMutex.broadcast( records );
