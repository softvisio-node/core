...
export default Super =>
    class extends ( Super || Object ) {
        ...
        get _connections () {
            return { "master": this.#masterConnections, "standby": this.#standbyConnections };
        }

        // protected
        async _getConnection ( lock, standby ) {
            if ( standby ) {
                if ( !this.standbyHostname ) standby = false;
                else if ( !this.#dnsWatcher ) this.#dnsWatcher = new DnsWatcher( this.standbyHostname ).unref().start();
            }

            const connections = standby ? this.#standbyConnections : this.#masterConnections;
            ...
            while ( 1 ) {
                ...
                // try to get idle master / standby connection, do not lock last connection
                ...
                // standby
                    ...
                    if ( !addresses.size && !connections.active.size ) {
                        return ( this.#fakeConnection ??= this._newConnection( { "destroyed": true, "standbyAddress": "0.0.0.0" } ) );
                    }
        ...
        #onConnectionDestroy ( dbh, res ) {
            const connections = dbh.isMaster ? this.#masterConnections : this.#standbyConnections;
