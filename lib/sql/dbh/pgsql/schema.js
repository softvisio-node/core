...
WITH cte AS (
    ...
    WHERE
        id = ?
        AND pg_try_advisory_lock( ?, id ) = TRUE
        AND next_start = ?
    FOR UPDATE
)
UPDATE
...
const cronUnlockQuery = sql`UPDATE _cron SET last_finished = CURRENT_TIMESTAMP, status = ? WHERE id = ?`.prepare();
...
` );
    ...
    async #runTask ( task, cron ) {
        ...
        try {
            ...
            if ( !res.data ) throw `Cron task is already done`;

            // run command
            res = await dbh.exec( task.query );

            // unlock
            res = await dbh.do( cronUnlockQuery, [res.statusText, task.id] );
        }
        catch ( e ) {}
