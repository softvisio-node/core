...
CREATE TABLE IF NOT EXISTS _cron (
    ...
    new_connection bool NOT NULL DEFAULT TRUE,
                ...
                if ( meta.cron ) {
                    ...
                    const values = Object.keys( meta.cron ).map( name => {
                        ...
                        return {
                            ...
                            "run_missed": meta.cron[name].run_missed,
                            "new_connection": meta.cron[name].new_connection,
...
ON CONFLICT ( module, name ) DO UPDATE SET
    ...
    new_connection = EXCLUDED.new_connection,
...
` );
    ...
    async #runTask ( task, cron ) {
        ...
        if ( task.new_connection ) {
            ...
            await dbh.selectRow( runTaskQuery, [task.id, tickDate, task.next_start] );

            dbh.destroy();
        }
        else {
            this._pool.selectRow( runTaskQuery, [task.id, tickDate, task.next_start] );
        }
    }
}
