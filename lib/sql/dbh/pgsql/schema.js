    ...
    async runCron () {
        ...
        for ( const task of cron.data ) {
            ...
            if ( nextDate <= currentDate ) {
                ...
                // run missed task
                if ( task.run_missed ) this.#runTask( task, cron );

                cron.start();
            }

            // wait until next date
            else {
                setTimeout( () => {
                    cron.start();
                    this.#runTask( task, cron );
...
` );
    ...
    async #runTask ( task, cron ) {
        ...
        // XXX cron.scheduledTickDate
        task.next_start = cron.nextDate.toISOString();
