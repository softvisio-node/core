...
export default class extends Schema {
    ...
    async _migrate ( meta, schema, patches ) {
        return this._pool.lock( async dbh => {
            ...
            res = await dbh.selectRow( sql`SELECT pg_advisory_lock(${this.getAdvisoryLockId( "migration" )})` );
...
CREATE OR REPLACE FUNCTION get_advisory_lock_id ( _lock text ) RETURNS int4 IMMUTABLE AS $$
DECLARE
    _id int8;
BEGIN
...
` );
            ...
            {
                const res = await dbh.selectRow( sql`SELECT pg_advisory_unlock(${this.getAdvisoryLockId( "migration" )})` );
                if ( !res.ok ) return res;
            }
