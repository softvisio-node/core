...
WITH cte AS (
    SELECT
        id,
        pg_try_advisory_lock( ?, id ) AS locked
    FROM
        _cron
    WHERE
        id = ?
...
SET
    next_start = ?,
    last_started = CASE WHEN cte.locked THEN CURRENT_TIMESTAMP ELSE _cron.last_started END,
    last_finished = CASE WHEN cte.locked THEN NULL ELSE _cron.last_finished END,
    status = CASE WHEN cte.locked THEN NULL ELSE _cron.status END
FROM
    cte
WHERE
    _cron.id = cte.id
...
export default class extends Schema {
    ...
    // public
    async runCron () {
        if ( !this.isLoaded ) return result( [400, `Database schema is not loaded`] );

        if ( this.#cronStarted ) return result( [200, `Cron already started`] );
        ...
        if ( !cron.data ) return result( [200, `No cron tasks found`] );
        ...
        for ( const task of cron.data ) {
            ...
            const cron = ( this.#cronTasks[task.id] = new Cron( task.cron, { "timezone": task.timezone } ).on( "tick", this.#runCronTask.bind( this, task ) ).unref() );
            ...
            // run missed task
            if ( nextDate <= currentDate && task.run_missed ) this.#runCronTask( task, cron );
        }
...
WITH cte AS (
    ...
    ON CONFLICT ( module, name ) DO UPDATE SET
        ...
        next_start = CASE
            WHEN _cron.cron = EXCLUDED.cron AND coalesce( _cron.timezone, '' ) = coalesce( EXCLUDED.timezone, '' ) THEN _cron.next_start
            ELSE EXCLUDED.next_start
        END
    RETURNING id
)
DELETE FROM _cron WHERE module = ${module} AND NOT EXISTS ( SELECT FROM cte WHERE id = _cron.id )
` );
                    }
                    else {
                        res = await await dbh.do( sql`DELETE FROM _cron WHERE module = ${module}` );
                    }
    ...
    async #runCronTask ( task, cron ) {
        const tickDate = task.next_start;

        task.next_start = cron.nextDate;

        const dbh = this._pool._newConnection();

        try {

            // lock
            var res = await dbh.selectRow( cronLockQuery, [sqlConst.locks.cron, task.id, tickDate, task.next_start] );

            if ( !res.data?.locked ) throw `Unable to lock cron task`;

            // run query
            res = await dbh.exec( task.query );

            // unlock
            await dbh.do( cronUnlockQuery, [res.statusText, task.id] );
        }
        catch ( e ) {}
