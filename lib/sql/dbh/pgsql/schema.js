    ...
    async #upsateCron ( dbh, module, meta ) {
        ...
        if ( meta.cron ) {
            for ( const name in meta.cron ) {
                ...
                // update
                else if ( meta.cron[name].cron !== index[name].cron || meta.cron[name].timezone !== index[name].timezone || query !== index[name].query || meta.cron[name].runMissed !== index[name].run_missed ) {
                    res = await dbh.do( sql`UPDATE _schema_cron SET cron = ?, timezone = ?, query = ?, run_missed = ? WHERE id = ?`, [meta.cron[name].cron, meta.cron[name].timezone, query, meta.cron[name].runMissed, index[name].id] );
