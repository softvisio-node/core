    ...
    async _migrate ( meta, schema, patches ) {
        return this._pool.lock( async dbh => {
            ...
            try {
                ...
                // update cron
                res = await this.#upsateCron( dbh, module, meta );
                if ( !res.ok ) throw res;
...
` );
    ...
    // XXX
    async #upsateCron ( dbh, module, meta ) {
        var res;

        res = await dbh.select( sql`SELECT * FROM _schema_cron WHERE module = ?`, [module] );
        if ( !res.ok ) return res;
        ...
        if ( meta.cron ) {
            for ( const name in meta.cron ) {
                ...
                // insert
                if ( !index[name] ) {
                    res = await dbh.do( sql`INSERT INTO _schema_cron ( module, name, cron, timezone, query, run_missed ) VALUES ( ?, ?, ?, ?, ?, ? )`, [module, name, meta.cron[name].cron, meta.cron[name].timezone, meta.cron[name].query, meta.cron[name].runMissed] );

                    if ( !res.ok ) return res;
                }

                // update
                // meta.cron[name].cron, meta.cron[name].timezone, meta.cron[name].query, meta.cron[name].runMissed
                else if ( meta.cron[name].cron !== index[name].crpn || meta.cron[name].timezone !== index[name].timezone || meta.cron[name].query !== index[name].query || meta.cron[name].runMissed !== index[name].run_missed ) {
                    res = await dbh.do( sql`UPDATE _schema_cron SET cron = ?, timezone = ?, query = ?, run_missed = ? WHERE id = ?`, [meta.cron[name].cron, meta.cron[name].timezone, meta.cron[name].query, meta.cron[name].runMissed, index[name].id] );
...
WITH cte AS (
    INSERT INTO
        _schema_cron`.VALUES( values ).sql`
...
DELETE FROM _cron WHERE module = ${module} AND NOT EXISTS ( SELECT FROM cte WHERE id = cron.schedule.id )
` );
        }
        else {
            res = await dbh.do( sql`DELETE FROM _schema_cron WHERE module = ?'`, [module] );
        }
