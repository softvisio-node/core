...
export default class extends Schema {
    ...
    async _migrate ( meta, schema, patches ) {
        return this._pool.lock( async dbh => {
            ...
            res = await dbh.selectRow( sql`SELECT pg_advisory_lock(${constants.locks.migration})` );
...
CREATE OR REPLACE FUNCTION get_advisory_lock_id ( _local_name text ) RETURNS int4 AS $$
BEGIN
    RETURN ( SELECT id FROM _schema_lock WHERE lock = _lock_name );
END;
$$ LANGUAGE plpgsql;
` );
                ...
                // update module emits
                {
                    ...
                    const res = await dbh.do( sql`UPDATE _schema SET emits = ? WHERE module = ?`, [JSON.stringify( [...emits] ), module] );
                    if ( !res.ok ) throw res;
                }
                ...
                // update module cron schedule
                {
                    const values = Object.keys( meta.cron || {} ).map( name => {
                        return {
                            ...
                            "run_as_superuser": meta.cron[name].run_as_superuser ?? false,
...
WITH cte AS (
    ...
    ON CONFLICT ( username, module, name ) DO UPDATE SET
        ...
        run_as_superuser = EXCLUDED.run_as_superuser,
...
` );
            ...
            {
                const res = await dbh.selectRow( sql`SELECT pg_advisory_unlock(${constants.locks.migration})` );
                if ( !res.ok ) return res;
            }
