    ...
    async #upsateCron ( dbh, module, meta ) {
        ...
        if ( meta.cron ) {
            for ( const name in meta.cron ) {
                ...
                // insert
                if ( !index[name] ) {
                    res = await dbh.do( sql`INSERT INTO _schema_cron ( module, name, cron, timezone, query, run_missed ) VALUES ( ?, ?, ?, ?, ?, ? )`, [module, name, meta.cron[name].cron, meta.cron[name].timezone, query, meta.cron[name].runMissed] );
