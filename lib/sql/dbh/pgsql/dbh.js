...
export default class DbhPgSql extends Transactions( Dbh ) {
    ...
    _ON_PG_AUTHENTICATION ( data ) {
        ...
        if ( type === BACKEND.AUTHENTICATION_OK ) {
            ...
            const response = this.#sasl.initialize();

            if ( !response ) return this.destroy( result( [500, "Unsupported authentication method"] ) );

            // SASLInitialResponse
            this.#socket.write( messageBuffer.beginMsg( FRONTEND.SASL_RESPONSE ).str( "SCRAM-SHA-256" ).z( 1 ).i32( response.length ).str( response ).endMsg().get() );
        }
        else if ( type === BACKEND.AUTHENTICATION_SASL_CONTINUE ) {
            const response = this.#sasl.continue( data.slice( 4 ).toString(), this.#pool.password );
