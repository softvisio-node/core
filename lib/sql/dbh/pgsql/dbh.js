...
export default class DbhPgSql extends Transactions( Dbh ) {
    ...
    _ON_PG_AUTHENTICATION ( data ) {
        ...
        if ( type === BACKEND.AUTHENTICATION_OK ) {
            ...
            this.#sasl = new Sasl( mechanisms );

            if ( this.#sasl.mechanism ) {
                const msg = this.#sasl.initialize();

                // SASLInitialResponse
                this.#socket.write( messageBuffer.beginMsg( FRONTEND.SASL_RESPONSE ).str( "SCRAM-SHA-256" ).z( 1 ).i32( msg.length ).str( msg ).endMsg().get() );
            }
            else {
            ...
            const msg = this.#sasl.continue( data.slice( 4 ).toString(), this.#pool.password );

            if ( !msg ) return this.destroy( result( [500, "Unsupported authentication method"] ) );

            // SASLResponse
            this.#socket.write( messageBuffer.beginMsg( FRONTEND.SASL_RESPONSE ).str( msg ).endMsg().get() );
        }
        else if ( type === BACKEND.AUTHENTICATION_SASL_FINAL ) {
            const ok = this.#sasl.finalize( data.slice( 4 ).toString() );
