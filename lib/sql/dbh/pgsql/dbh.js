...
export default class DbhPgsql extends Transactions( Dbh ) {
    ...
    async _ON_PG_AUTHENTICATION ( data ) {
        ...
        if ( type === BACKEND.AUTHENTICATION_OK ) {
            ...
            this.#sasl = await Sasl.new( mechanisms, this.#pool.username, this.#pool.password );
    ...
    _ON_PG_ROW_DESCRIPTION ( data ) {
        ...
        for ( let n = 0; n < numOfCols; n++ ) {
            ...
            const column = [
                ...
                //
                data.toString( "binary", pos, idx ), // 0, column name
    ...
    _ON_PG_NOTIFICATION_RESPONSE ( data ) {
        ...
        if ( sqlConst.reservedEvents.has( name ) ) {
            if ( this.#pool.hasSchema ) throw Error( `PostgreSQL event name "${name}" is reserved` );
            else return;
        }

        // unknown event
        if ( this.#pool.hasSchema && !this.#pool.emits.has( name ) ) throw Error( `PostgreSQL event name "${name}" is unknown` );
    ...
    _checkIdle () {
        ...
        // not locked, no current query, queue is empty
        const idle = !this.isLocked && !this.#sth && !this.#queue.length;
        ...
        // idle
        if ( idle ) {
            if ( !this.#isPersistent && this.#pool.idleTimeout ) {
                this.#idleTimeout = setTimeout( () => this.destroy( result( [500, "Database connection closed on timeout"] ) ), this.#pool.idleTimeout );
            }
        }
