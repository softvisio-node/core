...
export default class DbhPgsql extends Transactions( Dbh ) {
    ...
    _ON_PG_NOTIFICATION_RESPONSE ( data ) {
        ...
        var [channel, payload] = data.slice( 4 ).toString().split( "\0" );

        // invalid event name
        // XXX ignore this event
        if ( !channel.startsWith( "/" ) ) channel = "/" + channel; // XXX return

        if ( !payload ) {
            this.#pool.emit( channel );
        }
        else {
            try {
                payload = JSON.parse( payload );

                this.#pool.emit( channel, payload );
            }
            catch ( e ) {
                console.log( `Invalid postgres notification payload for event: "${channel}"` );
            }
        }
    }
