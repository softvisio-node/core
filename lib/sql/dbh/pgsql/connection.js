...
export default class DbhPgsql extends Transactions( Dbh ) {
    ...
    #onConnect () {
        ...
        const msg = messageBuffer.beginMsg( FRONTEND.STARTUP_MESSAGE ).addBuffer( PROTOCOL_VERSION );
    ...
    async _ON_PG_AUTHENTICATION ( data ) {
        ...
        if ( type === BACKEND.AUTHENTICATION_OK ) {
            ...
            // SASLInitialResponse
            this.#socket.write( messageBuffer.beginMsg( FRONTEND.SASL_RESPONSE ).str( this.#sasl.type ).z( 1 ).ui32( response.length ).str( response ).endMsg().get() );
        }
