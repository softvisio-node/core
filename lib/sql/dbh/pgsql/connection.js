...
export default class DbhPgsql extends Transactions( Dbh ) {
    ...
    #onConnect () {
        ...
        messageBuffer.beginMsg( FRONTEND.STARTUP_MESSAGE ).addBuffer( PROTOCOL_VERSION );

        Object.keys( params ).forEach( param => messageBuffer.str( param + "\0" + params[param] + "\0" ) );
    ...
    async _ON_PG_AUTHENTICATION ( data ) {
        ...
        if ( type === BACKEND.AUTHENTICATION_OK ) {
            ...
            this.#socket.write( messageBuffer.beginMsg( FRONTEND.PASSWORD_MESSAGE ).str( hash ).z( 1 ).endMsg().done() );
        }
            ...
            // SASLInitialResponse
            this.#socket.write( messageBuffer.beginMsg( FRONTEND.SASL_RESPONSE ).str( this.#sasl.type ).z( 1 ).ui32( response.length ).str( response ).endMsg().done() );
        }
            ...
            // SASLResponse
            this.#socket.write( messageBuffer.beginMsg( FRONTEND.SASL_RESPONSE ).str( response ).endMsg().done() );
        }
