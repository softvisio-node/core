...
export default class DbhPgsql extends Transactions( Dbh ) {
    ...
    async read ( query, { summaryQuery, id, orderBy, offset, limit, defaultLimit, maxLimit, maxResults } = {} ) {
        ...
        // has summary query
        if ( summaryQuery ) {
            const preparedQuery = ( QUERIES_CACHE[summaryQuery.toString()] ??= sql( summaryQuery.toString() ).prepare().decode( { "total": "int53" } ) );
        ...
        const preparedQuery = ( QUERIES_CACHE[fullQuery.toString()] ??= sql( fullQuery.toString() ).prepare() );
