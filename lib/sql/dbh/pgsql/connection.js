...
export default class DbhPgsql extends Transactions( Dbh ) {
    ...
    #connect () {
        ...
        if ( this.#pool.protocol === "pgsqls:" ) this.#socket = tls.connect( { "socket": this.#socket } );
    ...
    #onConnect () {
        ...
        messageBuffer.beginMsg( FRONTEND.STARTUP_MESSAGE ).buf( PROTOCOL_VERSION );

        Object.keys( params ).forEach( param => messageBuffer.buf( param + "\0" + params[param] + "\0" ) );

        this.#socket.write( messageBuffer.z( 1 ).endMsg().done() );
    }
    ...
    async _ON_PG_AUTHENTICATION ( data ) {
        ...
        if ( type === BACKEND.AUTHENTICATION_OK ) {
            ...
            this.#socket.write( messageBuffer.beginMsg( FRONTEND.PASSWORD_MESSAGE ).zbuf( hash ).endMsg().done() );
        }
            ...
            // SASLInitialResponse
            this.#socket.write( messageBuffer.beginMsg( FRONTEND.SASL_RESPONSE ).zbuf( this.#sasl.type ).uint32( response.length ).buf( response ).endMsg().done() );
        }
            ...
            // SASLResponse
            this.#socket.write( messageBuffer.beginMsg( FRONTEND.SASL_RESPONSE ).buf( response ).endMsg().done() );
        }
    ...
    async read ( query, { summaryQuery, id, orderBy, offset, limit, defaultLimit, maxLimit, maxResults } = {} ) {

        // XXX how to correctly fetch record by id???
        // get by id
        if ( id ) return this.selectRow( query );

        var maxResultsLimit;

        ( { offset, limit, maxResultsLimit } = calcOffsetLimit( offset, limit, { defaultLimit, maxLimit, maxResults } ) );
        ...
        // has summary query
        if ( summaryQuery ) {
            summary = await this.selectRow( summaryQuery.decode( { "total": "int53" } ) );
    ...
    #do ( resolve, query, params, options ) {
        ...
        if ( useSimpleQuery ) {
            messageBuffer.beginMsg( FRONTEND.QUERY ).zbuf( sth.query ).endMsg();
        }

        // extended query mode
        else {
            const portalId = ""; // uuidV4(), currently we use unnamed portals
            ...
            messageBuffer
                ...
                .buf( portalId + "\0" + sth.id + "\0" )
                .uint16( 0 ) // params format text
                .uint16( query.params?.length ?? 0 ); // params length
            ...
            if ( query.params ) {
                for ( let param of query.params ) {
                    ...
                    if ( param == null ) {
                        ...
                        messageBuffer.int32( Buffer.byteLength( param ) ).buf( param );
                    }
                }
            }
            ...
            if ( !sth.id || !this.#prepared[sth.id].described ) {
                messageBuffer
                    ...
                    .zbuf( FRONTEND.DESCRIBE_PORTAL + portalId )
                    .endMsg();
            }
            ...
            if ( options.maxRows ) {
                messageBuffer
                    ...
                    .buf( FRONTEND.CLOSE_PORTAL + portalId + "\0" )
                    .endMsg();
            }
