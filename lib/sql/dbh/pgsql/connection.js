...
export default class DbhPgsql extends Transactions( Dbh ) {
    ...
    async _ON_PG_AUTHENTICATION ( data ) {
        ...
        if ( type === BACKEND.AUTHENTICATION_OK ) {
            ...
            // SASLInitialResponse
            this.#socket.write( messageBuffer.beginMsg( FRONTEND.SASL_RESPONSE ).zbuf( this.#sasl.type ).writeUInt32BE( response.length ).buf( response ).endMsg().done() );
        }
            ...
            if ( query.params ) {
                for ( let param of query.params ) {
                    ...
                    if ( param == null ) {
                        ...
                        messageBuffer.writeInt32BE( Buffer.byteLength( param ) ).buf( param );
                    }
                }
            }
