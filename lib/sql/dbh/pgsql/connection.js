...
import { Sql, sql, calcOffsetLimit } from "#lib/sql/query";
...
export default class DbhPgsql extends Transactions( Dbh ) {
        ...
        if ( !res ) {
            if ( sth.error ) {
                res = this._onQueryError( sth.error, sth.query, sth.error, {
                    "silent": sth.options?.readOnlyTest && sth.error?.code === READ_ONLY_SQL_TRANSACTION,
                } );
            }
            else {
                let columns;
    ...
    async select ( query, params, options ) {
        return new Promise( resolve => {
            this.#do( resolve, query, params, options );
        } );
    }
    ...
    async selectRow ( query, params, options ) {
        return new Promise( resolve => {
            this.#do(
                ...
                {
                    "maxRows": 1,
                    "readOnlyTest": options?.readOnlyTest,
                }
            );
        } );
    }
    ...
    async read ( query, { summaryQuery, id, orderBy, offset, limit, defaultLimit, maxLimit, maxResults } = {} ) {
        ...
        // has summary query
        if ( summaryQuery ) {
            const preparedQuery = ( PREPARED_QUERIES_CACHE[summaryQuery.toString()] ??= sql( summaryQuery.toString() ).prepare().decode( { "total": "int53" } ) );
        ...
        const preparedQuery = ( PREPARED_QUERIES_CACHE[fullQuery.toString()] ??= sql( fullQuery.toString() ).prepare() );
    ...
    #prepareQuery ( query, params, toString ) {
        ...
        if ( sth.params && ( toString || sth.params.length > MAX_PARAMS ) ) {
            sth.query = this.serializeQuery( sth.query, sth.params );
            sth.id = null;
            sth.params = null;
        }
