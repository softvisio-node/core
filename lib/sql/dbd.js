...
class DbhPool extends Dbh {
        ...
        const res = await this.begin( async dbh => {
            ...
            for ( const module of Object.keys( this.#schema ).sort() ) {
                ...
                for ( const id of Object.keys( this.#schema[module] ).sort( ( a, b ) => a - b ) ) {
                    ...
                    this.#applyFunctions( dbh, patch.functions );

                    // apply main schema
                    if ( moduleVersion == null ) await this.#applySql( dbh, patch.sql );

                    await this.#applyTypes( dbh, patch.types );
                }
                ...
                for ( const id of Object.keys( this.#patch[module] ).sort( ( a, b ) => a - b ) ) {
                    ...
                    // apply full schema
                    if ( id > moduleVersion ) {
                        await this.#applySql( dbh, patch.sql );
    ...
    #applyFunctions ( dbh, functions ) {
        if ( !this.isSqlite ) return;

        if ( !functions ) return;

        for ( const name in functions ) {
            dbh.db.function( name, functions[name] );
        }
    }

    async #applySql ( dbh, sql ) {
        var res;

        if ( typeof sql === "function" ) {
            try {
                res = result.tryResult( await sql( dbh ) );
            }
            catch ( e ) {
                res = result.catchResult( e );
            }
    ...
    async #applyTypes ( dbh, types ) {
        if ( !types ) return;

        for ( const name in types ) {
            const res = await dbh.addType( { ...types[name], name } );
