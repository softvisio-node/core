...
class Dbh {
    ...
    async begin ( func ) {
        ...
        // start transaction
        let res = await this.do( `SAVEPOINT "${name}"` );
        ...
        if ( res.ok ) {
            try {
                ...
                res = parseResult( ( await func( this ) ) || 200 );

                // release savepoint
                const tres = await this.do( `RELEASE SAVEPOINT "${name}"` );
                ...
                await this.do( `ROLLBACK TO SAVEPOINT "${name}"` );
            }
        }
