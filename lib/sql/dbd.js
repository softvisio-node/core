...
import { SQL_MIGRATION_TABLE_NAME, SQL_MIGRATION_DEFAULT_MODULE, SQL_LOCKS } from "#lib/const";
...
export class DbhPool extends Dbh {
    ...
    async loadSchema ( index, module = SQL_MIGRATION_DEFAULT_MODULE ) {
        ...
        for ( const file of files ) {
            ...
            this.#addSchema( id, module, ( await import( url.pathToFileURL( path + "/" + file ) ) ).default );
        }
        ...
        for ( const file of files ) {
            ...
            this.#addPatch( id, module, ( await import( url.pathToFileURL( path + "/patch/" + file ) ) ).default );
        }
    }
