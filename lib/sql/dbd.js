...
class DbhPool extends Dbh {
    ...
    async loadSchema ( path, module = SQL_MIGRATION_DEFAULT_MODULE ) {
        ...
        // load patches
        files = glob.sync( "*.js", { "cwd": path + "/patches", "nodir": true } );

        for ( const file of files ) {
            const id = file.match( /^(\d+)/g )[0];

            this.#addPatch( id, module, require( path + "/patches/" + file ) );
        }
    }
    ...
    async migrate () {
        const res = await this.begin( async dbh => {
            ...
            // XXX remove
            await this.#updateMigrationSchema( dbh );
            ...
            for ( const module of Object.keys( this.#schema ).sort() ) {
                ...
                for ( const id of Object.keys( this.#schema[module] ).sort( ( a, b ) => a - b ) ) {
                    const patch = this.#schema[module][id];
                ...
                for ( const id of Object.keys( this.#patch[module] ).sort( ( a, b ) => a - b ) ) {
                    const patch = this.#patch[module][id];

                    this.#applyFunctions( dbh, patch );

                    // apply full schema
                    if ( id > moduleVersion ) {
                        console.log( "appl patch: " + id );
    ...
    #preparePatch ( id, module, patch ) {
        ...
        return { module, id, sql, types, functions };
    }
    ...
    #applyFunctions ( dbh, patch ) {
        ...
        try {
            ...
            throw result( [500, `Error applying functions for module "${patch.module}", patch "${patch.id}": ` + e] );
        }
    }
    ...
    async #applySql ( dbh, patch ) {
        ...
        if ( typeof patch.sql === "function" ) {
            try {
                ...
                throw result( [500, `Error applying sql for module "${patch.module}", patch "${patch.id}": ` + res.reason] );
            }
        }
        else {
        ...
        if ( !res.ok ) throw result( [500, `Error applying sql for module "${patch.module}", patch "${patch.id}": ` + res.reason] );
    }
    ...
    async #applyTypes ( dbh, patch ) {
        ...
        for ( const name in patch.types ) {
            ...
            if ( !res.ok ) throw result( [500, `Error applying types for module "${patch.module}", patch "${patch.id}": ` + res.reason] );
        }
    }

    // XXX remove
    async #updateMigrationSchema ( dbh ) {

        // create migration schema
        var res = await dbh.do( `
            CREATE TABLE IF NOT EXISTS "${SQL_MIGRATION_TABLE_NAME}" (
                "module" text PRIMARY KEY NOT NULL,
                "version" int4 NOT NULL
            )
        ...
        res = await dbh.selectAll( `SELECT * FROM "${SQL_MIGRATION_TABLE_NAME}"` );

        if ( !res.ok ) throw res;

        if ( !res.data ) return;

        if ( !( "id" in res.data[0] ) ) return;

        const data = res.data;

        res = await dbh.do( `DROP TABLE "${SQL_MIGRATION_TABLE_NAME}"` );
        ...
        for ( const row of data ) {
            if ( modules[row.module] ) continue;

            modules[row.module] = true;

            res = await dbh.do( `INSERT INTO "${SQL_MIGRATION_TABLE_NAME}" ("module", "version") VALUES (?, ?)`, [row.module, 0] );
