...
class Dbh {
    ...
    queryToString ( query, params ) {
        ...
        query = query.replace( /(?:\?|\$\d+)/g, () => {
            if ( idx >= length ) {
                throw Error( `SQL number of passed params is less, than number of placeholders in query` );
            }
            else {
        ...
        if ( idx < length ) throw Error( `SQL number of passed params is greater, than number of placeholders in query` );
...
class DbhPool extends Dbh {
    ...
    async begin ( mode, func ) {
        ...
        if ( dbh.inTransaction() ) return this._onError( Error( "Already in transaction. Use savepoints." ), "BEGIN" );
        ...
        try {
            ...
            tres = await dbh.do( "COMMIT" );

            if ( !tres.isOk() ) result = tres;
        }
        catch ( e ) {
            tres = await dbh.do( "ROLLBACK" );
