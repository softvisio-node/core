                ...
                for ( const id of Object.keys( this.#patch[module] ).sort( ( a, b ) => {
                    ...
                    if ( this.#patch[module][id].sql ) {
                        ...
                        if ( !res.data ) {
                            ...
                            // apply patch
                            if ( typeof this.#patch[module][id].sql === "function" ) {
                                try {
                                    res = result.tryResult( await this.#patch[module][id].sql( dbh ) );
                                }
                                catch ( e ) {
                                    res = result.catchResult( e );
                                }
                    ...
                    // types
                    if ( this.#patch[module][id].types ) {
                        for ( const name in this.#patch[module][id].types ) {
                            const res = await dbh.addType( { ...this.#patch[module][id].types[name], name } );
