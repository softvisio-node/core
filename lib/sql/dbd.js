    ...
    addSchemaPatch ( id, module, sql ) {
        ...
        if ( typeof sql === "object" && !this.isQuery( sql ) ) {
            if ( this.isSqlite && sql.sqlite ) {
                if ( typeof sql.sqlite === "object" && !this.isQuery( sql.sqlite ) ) {

                    // add sqlite functions
                    if ( sql.sqlite.functions ) {
                        for ( const name in sql.sqlite.functions ) {
                            this.db.function( name, sql.sqlite.functions[name] );
                        }
                    }

                    if ( sql.sqlite.query ) sql = sql.sqlite.query;
                }
                else {
                    sql = sql.sqlite;
                }
