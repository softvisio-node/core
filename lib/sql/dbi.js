    ...
    for ( const name in types ) {
        ...
        for ( const alias of aliases ) {
            sql[alias.toUpperCase()] = function ( object ) {
                ...
                object[SQL_TYPE] = alias.toUpperCase();
        ...
        if ( types[name].pgsql ) {
            ...
            if ( type.from ) {
                for ( const alias of aliases ) {
                    TYPES.pgsql.from[alias.toUpperCase()] = type.from;
                }
            ...
            if ( type.to ) {
                for ( const alias of aliases ) {
                    TYPES.pgsql.to[alias.toUpperCase()] = type.to;
                }
            }
        }
        ...
        if ( types[name].sqlite ) {
            ...
            for ( const alias of aliases ) {
                if ( type.to ) TYPES.sqlite.to[alias.toUpperCase()] = type.to;

                if ( type.from ) TYPES.sqlite.from[alias.toUpperCase()] = type.from;
            }
        }
    }
};
