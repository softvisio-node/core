    ...
    types(columns) {
        if (columns) {
            ...
            for (const column in columns) {
                this.#types[column.toLowerCase()] = columns[column];
            }
        } else {
            this.#types = null;
        }
...
sql.addTypes = function (types) {
    for (const name in types) {
        ...
        for (const alias of aliases) {
            sql[alias.toUpperCase()] = function (object) {
                if (object == null) return object;

                object[SQL_TYPE] = alias.toLowerCase();
        ...
        if (types[name].pgsql) {
            ...
            if (type.to) {
                for (const alias of aliases) {
                    TYPES.pgsql.to[alias.toLowerCase()] = type.to;
                }
            }
        }
        ...
        if (types[name].sqlite) {
            ...
            for (const alias of aliases) {
                if (type.to) TYPES.sqlite.to[alias.toLowerCase()] = type.to;

                if (type.from) TYPES.sqlite.from[alias.toLowerCase()] = type.from;
            }
        }
    }
};
...
// |=======+===============+==============================+================================================================================|
// | ERROR | 213:18        |                              | Parsing error: Private name #typs is not defined                               |
// |       |               |                              |   211 |             }                                                          |
// |       |               |                              |   212 |         } else {                                                       |
// |       |               |                              | > 213 |             this.#typs = null;                                         |
// |       |               |                              |       |                  ^                                                     |
