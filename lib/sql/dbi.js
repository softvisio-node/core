const { OBJECT_IS_SQL_QUERY, SQL_TYPE } = require( "../const" );
...
class QueryBase {
    static [OBJECT_IS_SQL_QUERY] = true;
...
sql.addTypes = function ( types ) {
    for ( const name in types ) {
        ...
        // postgresql
        if ( types[name].postgresql ) {
            const type = types[name].postgresql;

            if ( type.from ) {
                for ( let alias of aliases ) {
                    alias = alias.toUpperCase();

                    TYPES.postgresql.from[alias] = type.from;
                }

                if ( type.oid ) {
                    for ( const oid of type.oid ) {
                        TYPES.postgresql.from[oid] = type.from;
                    }
                }
            }
            ...
            if ( type.to ) {
                for ( let alias of aliases ) {
                    ...
                    TYPES.postgresql.to[alias] = type.to;
                }
            }
        }
