...
class Query {
    ...
    getQuery ( placeHolder ) {
        if ( placeHolder === "$" ) {
            if ( !this.#queryPlaceholder["$"] ) {
                ...
                this.#queryPlaceholder["$"] = this.#query.replace( /\0/g, () => {
                    return "$" + ++idx;
                } );
            }

            return [this.#queryPlaceholder["$"], this.#params, this.#id];
        }
        else if ( placeHolder === "?" ) {
            if ( !this.#queryPlaceholder["?"] ) {
                this.#queryPlaceholder["?"] = this.#query.replace( /\0/g, "?" );
    ...
    _ ( query, ...params ) {
                ...
                if ( idx < params.length ) {
                    ...
                    if ( this.isQuery( params[idx] ) ) {
                        ...
                        if ( subQuery ) {
                            ...
                            if ( subQuery[1] ) this.#params.push( ...subQuery[1] );
                        }
                    }
        ...
        // called as sql("...")
            ...
            // trim and convert "?" placeholder
            const sql = query.trim().replace( /\?/g, "\0" );
        ...
        for ( const val of values ) {
            ...
            // object
                ...
                for ( const field in val ) {
                    ...
                    // subquery
                    if ( val[field] != null && val[field].constructor[IS_SQL] ) {
...
// |=======+===============+==============================+================================================================================|
// | ERROR | 379:67        | no-empty                     | Empty block statement.                                                         |
// +-------+---------------+------------------------------+--------------------------------------------------------------------------------+
//
