...
class Where extends SQL {
    ...
    and ( ...args ) {
        const query = new this.constructor( args ).getQuery();
...
class Query extends SQL {
    ...
    getQuery ( forPostgres ) {
        if ( forPostgres ) {
            if ( !this.#postgresQuery ) {
                ...
                this.#postgresQuery = this.#query.replaceAll( "?", () => "$" + ++n );
            }

            return [this.#postgresQuery, this.#params, this.#id, this.#decode];
        }
        else {
            return [this.#query, this.#params, this.#id, this.#decode];
        }
    }
    ...
    SET ( values, fields ) {
        ...
        if ( typeof values === "object" ) {
            if ( fields ) values = Object.pick( values, fields );
    ...
    VALUES ( rows, fields ) {
        ...
        if ( !Array.isArray( rows ) ) {
            if ( !Object.isPlain( rows ) ) throw Error( `SQL rows value is invalid` );
