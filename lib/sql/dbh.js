...
export class DbhPool extends Dbh {
    ...
    async _getDbh ( lock, slave ) {
        ...
        while ( 1 ) {
            ...
            if ( connections.idle.size && !lastConnection ) {
                dbh = connections.idle.values().next();

                if ( lock ) connections.idle.delete( dbh );
            }
            ...
            // create new connection
                ...
                if ( !lock ) connections.idle.add( dbh );
            }

            // try to get busy connection, do not lock last connection
            else if ( connections.busy.size && !lastConnection ) {
                dbh = connections.busy.values().next();

                if ( lock ) connections.busy.delete( dbh );
            }
    ...
    #onDbhIdle ( dbh ) {
        ...
        // idle
        if ( dbh.isIdle ) {
            if ( !dbh.isLocked ) connections.idle.add( dbh );
            connections.busy.delete( dbh );
        ...
        // busy
            ...
            connections.idle.delete( dbh );
            if ( !dbh.isLocked ) connections.busy.add( dbh );
        }
    }
}
