    ...
    PRIMARY KEY ("module", "id")
            ...
            for ( const module of Object.keys( this.#patch ).sort() ) {
                for ( const id of Object.keys( this.#patch[module] ).sort( ( a, b ) => {
                    ...
                    res = await dbh.selectRow( `SELECT "id" FROM "${DB_MIGRATION_TABLE_NAME}" WHERE "module" = ? AND "id" = ?`, [module, id] );
                    ...
                    // register patch
                    res = await dbh.do( `INSERT INTO "${DB_MIGRATION_TABLE_NAME}" ("module", "id") VALUES (?, ?)`, [module, id] );
