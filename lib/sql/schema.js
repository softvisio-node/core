...
class Locks {
    ...
    get ( lock ) {
        ...
        if ( !id ) throw Error( `Database lock "${lock}" is not registered` );
...
export default class {
    ...
    constructor ( pool ) {
        ...
        this.#_locks = new Locks( this.#locks );
    }
        ...
        try {
            ...
            if ( meta.locks ) {
                for ( const name of meta.locks ) {
                    ...
                    if ( this.#locks[name] ) throw `Database lock name "${name}" is already registered`;
                }
            }
