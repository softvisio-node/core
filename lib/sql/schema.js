...
export default class {
    ...
    async migrate ( url ) {
        try {
            ...
            if ( this.#pool.isPostgreSql && !type.has( "pgsql" ) ) throw `Database schema type is not compatible with PostgreSQL`;
    ...
    isEventValid ( name ) {
        return SCHEMA_EVENTS.has( name ) || this.#emits.match( name );
    }
}
