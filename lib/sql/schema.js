...
export default class {
    ...
    async migrate ( url ) {
        try {
            ...
            VALIDATOR ||= Ajv.new().compile( config.read( "#resources/schemas/sql.index.schema.yaml", { "resolve": import.meta.url } ) );

            if ( !VALIDATOR( meta ) ) throw `SQL index is not valid:\n${JSON.stringify( VALIDATOR.errors, null, 4 )}`;
            ...
            if ( meta.emits ) {
                for ( const name of meta.emits ) {
                    ...
                    if ( !isKebabCase( name, { "sep": "/" } ) ) throw `Database event name "${name}" must be in the kebab-case`;
                }
            }
