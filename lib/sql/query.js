...
export function calcOffsetLimit ( offset, limit, { maxResults, defaultLimit, maxLimit } = {} ) {
    ...
    if ( limit ) {
        if ( typeof limit !== "number" || limit < 0 ) throw Error( `SQL limit value "${limit}" is invalid` );
    }
    else {
        limit = defaultLimit || maxLimit || 0;
    }

    if ( maxLimit && limit > maxLimit ) limit = maxLimit;

    if ( maxResults ) {
        const requestResults = offset + limit;

        if ( !requestResults ) {
            limit = maxResults;
        }
        else if ( requestResults > maxResults ) {
            limit = limit - ( requestResults - maxResults );
...
export class Query extends SQL {
    ...
    OFFSET_LIMIT ( offset, limit, options ) {
        const { _offset, _limit } = calcOffsetLimit( offset, limit, options );

        if ( _offset ) {
            this.#query += " OFFSET " + PLACEHOLDER;

            this.#params.push( _offset );
        }

        if ( _limit ) {
            this.#query += " LIMIT " + PLACEHOLDER;

            this.#params.push( _limit === -1 ? 0 : _limit );
        }

        return this;
    }

    LIMIT ( value, { defaultLimit, maxLimit } = {} ) {
        if ( !value ) {
        ...
        else if ( maxLimit && value > maxLimit ) {
            value = maxLimit;
        }
