                ...
                for ( const field in arg ) {
                    ...
                    // glob
                        ...
                        const id = this.quoteId( field ),
                            patterns = new GlobPatterns( { "caseSensitive": operator.isCaseSensitive } ).add( value ),
                            conditions = [];
                        ...
                        else if ( patterns.allowedList.isMatchAll ) {
                            conditions.push( `${id} IS NOT NULL` );
                        }
                        else {
                            conditions.push( `${id} ${operator.allowed} ${PLACEHOLDER}` );
                            params.push( patterns.allowedList.pattern );
                        }

                        // ignored
                        if ( !patterns.ignoredList.hasPatterns ) {
                            if ( !patterns.allowedList.isMatchAll ) conditions.push( `${id} IS NOT NULL` );
                        }
                        else if ( patterns.ignoredList.isMatchAll ) {
                            if ( patterns.allowedList.hasPatterns ) conditions.push( `${id} IS NULL` );
                        }
                        else {
                            conditions.push( `${id} ${operator.ignored} ${PLACEHOLDER}` );
...
class Query extends Sql {
    ...
    ORDER_BY ( values ) {
        ...
        if ( values ) {
            ...
            for ( const value of values ) {
                const direction = ORDER_BY_DIRECTION[value[1]];
