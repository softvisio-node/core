    ...
    ORDER_BY ( values ) {
        ...
        if ( values ) {
            if ( typeof values === "string" ) {
                buf.push( this.quoteId( values ) );
            }
        }
        else if ( typeof values === "object" ) {
            for ( const [name, order] of Object.entries( values ) ) {
                const direction = ORDER_BY_DIRECTION[order.toLowerCase()];

                if ( direction ) {
                    buf.push( this.quoteId( name ) + " " + direction );
                }
                else {
                    throw Error( `SQL order by direction "${order}" is invalid` );
                }
            }
        }
        else {
            throw Error( `SQL order by value "${values}" is invalid` );
        }
