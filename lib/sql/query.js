                        ...
                        for ( const v of val ) {
                            if ( v instanceof Sql ) {
                                ...
                                if ( isLike && typeof v === "string" ) {
                                    ...
                                    params.push( "%" + quoteLikePattern( v ) + "%" );
                                }
                                else {
...
export class Query extends Sql {
    ...
    VALUES ( rows, { index = "full" } = {} ) {
        const [fields, _rows] = this.#processValues( rows, index );

        if ( fields ) {
            this.#query += " ( " + fields.map( field => this.quoteId( field ) ).join( ", " ) + " )";
        }

        this.#query += " VALUES " + _rows.join( ", " );
    ...
    // rows: {} || [{}]
    // index: [String], "firstRow", "fullScan". "fullScan" is default
    VALUES_AS ( alias, rows, { index = "full" } = {} ) {
        const [fields, _rows] = this.#processValues( rows, index );

        this.#query += ` ( VALUES ` + _rows.join( ", " ) + ` ) AS ` + this.quoteId( alias ) + ` ( ` + fields.map( field => this.quoteId( field ) ).join( ", " ) + ` )`;
    ...
    #processValues ( rows, index ) {
        if ( !Array.isArray( rows ) ) throw TypeError( `SQL rows value is not a array` );
