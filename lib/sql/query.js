...
class SqlCachedQuery extends Sql {
    #id;
    #readOnly;
    #params = [];
    ...
    decode ( decoders ) {
        return new SqlCachedQueryCustomized( this, {
            decoders,
        } );
    }
...
class SqlCachedQueryCustomized extends Sql {
    #query;
    #decoders;
    #params;

    constructor ( query, { decoders, params } = {} ) {
        super();
    ...
    get postgresqlQuery () {
        return this.#query.postgresqlQuery;
    }

    get sqliteQuery () {
        return this.#query.sqliteQuery;
    }
...
export class SqlQuery extends Sql {
    ...
    prepare () {
        var query = CACHE[ this.query ];

        if ( !query ) {
            query = new SqlCachedQuery( this.query, {
                "readOnly": this.#readOnly,
            } );

            CACHE[ this.query ] = query;
        }

        if ( this.#decoders || this.#params.length ) {
            query = new SqlCachedQueryCustomized( query, {
