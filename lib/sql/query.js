...
class SqlCachedQuery extends Sql {
    ...
    constructor ( query, { readOnly, decoders } = {} ) {
        super( query );
...
export class Query extends Sql {
    ...
    ID ( value ) {
        this[ queryAccessor ] += " " + quoteId( value ) + " ";
    ...
    VALUES ( rows, { index = "fullScan" } = {} ) {
        ...
        if ( fields ) {
            this[ queryAccessor ] += " ( " + fields.map( field => quoteId( field ) ).join( ", " ) + " )";
        }
    ...
    VALUES_AS ( alias, rows, { index = "fullScan" } = {} ) {
        ...
        this[ queryAccessor ] += ` ( VALUES ` + _rows.join( ", " ) + ` ) AS ` + quoteId( alias ) + ` ( ` + fields.map( field => quoteId( field ) ).join( ", " ) + ` )`;
