...
module.exports = class DbhPgsql extends Dbh {
    ...
    _ON_PG_ROW_DESCRIPTION ( data ) {
        ...
        for ( let n = 0; n < numOfCols; n++ ) {
            ...
            // get type decoder
            column.push( this.#pool._decode[type] );
    ...
    async addType ( { name, oid, sql, encode, decode } ) {
        if ( !oid || sql ) {
            let res = await this.selectRow( `SELECT "oid" FROM "pg_type" WHERE "typname" = ?`, [name] );

            if ( !res.ok ) return res;

            if ( res.data ) oid = res.data.oid;
            ...
            if ( !oid && sql ) {
                ...
                res = await this.selectRow( `SELECT "oid" FROM "pg_type" WHERE "typname" = ?`, [name] );
        ...
        if ( decode ) {
            ...
            if ( oid ) this.#pool._decode[oid] = decode;
        }
