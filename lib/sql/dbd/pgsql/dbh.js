...
export default class DbhPgsql extends Dbh {
    ...
    #finishRequest ( res ) {
        ...
        if ( sth.options.ignoreData && sth.columns ) throw Error( `Invalid usage, you need to execute query, that returns data, using "select" method` );

        if ( !sth.options.ignoreData && !sth.columns ) throw Error( `Invalid usage, you need to execute query, that returns no data, using "do" method` );
