...
export default class DbhPgsql extends Dbh {
    ...
    #connect () {
        this.#socket = this.#options.hostname ? net.connect( this.#options.port, this.#options.hostname ) : net.connect( this.#options.socket );
    ...
    #finishRequest ( res ) {
        ...
        if ( sth.options.ignoreData && sth.columns ) throw Error( `Invalid usage, you need to execute query, that returns data, using "select" method, ` + sth.query );

        if ( !sth.options.ignoreData && !sth.columns ) throw Error( `Invalid usage, you need to execute query, that returns no data, using "do" method, ` + sth.query );
    ...
    _ON_PG_ROW_DESCRIPTION ( data ) {
        ...
        for ( let n = 0; n < numOfCols; n++ ) {
            ...
            if ( types && types[column[0]] ) {
                decoder = this.#pool._decode[types[column[0]]];
            }

            // get decoder by base type
            else if ( column[5] === -1 ) {
                decoder = this.#pool._decode[column[3]];
            }

            // get decoder by type mod. or by base type
            else {
                decoder = this.#pool._decode[column[3] + "/" + column[5]] || this.#pool._decode[column[3]];
            }
            ...
            if ( query[1] ) {
                for ( let param of query[1] ) {
                    ...
                    if ( param == null ) {
                        ...
                        if ( param[SQL_TYPE] ) param = this.#pool._encode[param[SQL_TYPE]]( param );
    ...
    async addType ( { name, encode, decode } ) {
        ...
        if ( encode ) {
            ...
            this.#pool._encode[name] = encode;
        }

        if ( decode ) {
            this.#pool._decode[name] = decode;

            if ( id ) this.#pool._decode[id] = decode;
        }
