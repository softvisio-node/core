...
module.exports = class DbhPgsql extends Dbh {
    ...
    _ON_PG_ERROR_RESPONSE ( data ) {
        ...
        for ( const field of data.toString().split( "\0" ) ) {
            ...
            error[field.charAt( 0 )] = field.substr( 1 );
        }
    ...
    _ON_PG_ROW_DESCRIPTION ( data ) {
        ...
        for ( let n = 0; n < numOfCols; n++ ) {
            ...
            // column.push = data.toString("binary", pos, idx);
            // pos = idx;
    ...
    _ON_PG_DATA_ROW ( data ) {
        ...
        for ( let n = 0; n < numOfCols; n++ ) {
            let length = data.readInt32BE( pos ),
                col;
            ...
            if ( !length ) {
                ...
                // TODO encoding based on column type
...
// |=======+===============+==============================+================================================================================|
// | ERROR | 359:17        | prefer-const                 | 'length' is never reassigned. Use 'const' instead.                             |
// |-------+---------------+------------------------------+--------------------------------------------------------------------------------|
// | ERROR | 382:15        | no-unused-vars               | 'rowsTag' is assigned a value but never used.                                  |
// +-------+---------------+------------------------------+--------------------------------------------------------------------------------+
//
