...
module.exports = class DbhPgsql extends Dbh {
    ...
    async addType ( { name, sql, encode, decode } ) {
        var id;

        if ( sql ) {
            let res = await this.selectRow( `SELECT "oid", "typbasetype", "typtypmod" FROM "pg_type" WHERE "typname" = ?`, [name] );
            ...
            // register type
                ...
                res = await this.do( sql );

                if ( !res.ok ) return res;

                res = await this.selectRow( `SELECT "oid", "typbasetype", "typtypmod" FROM "pg_type" WHERE "typname" = ?`, [name] );
