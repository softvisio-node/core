...
class MessageBuffer {
    ...
    get () {
        ...
        if ( this.buf.length > this.size ) this.buf = Buffer.allocUnsafe( this.size );
...
module.exports = class DbhPgsql extends Dbh {
    ...
    _onConnect () {
        ...
        const msg = messageBuffer.msgBegin( FRONTEND.STARTUP_MESSAGE ).add( PROTOCOL_VERSION );

        Object.keys( params ).forEach( ( param ) => msg.str( param + "\0" + params[param] + "\0" ) );

        this.#socket.write( msg.z( 1 ).msgEnd().get() );
    }
    ...
    _ON_PG_AUTHENTICATION ( data ) {
        ...
        if ( type === BACKEND.AUTHENTICATION_OK ) {
            ...
            this.#socket.write( messageBuffer
                .msgBegin( FRONTEND.PASSWORD_MESSAGE )
            ...
            this.#socket.write( messageBuffer.msgBegin( FRONTEND.PASSWORD_MESSAGE ).str( hash ).z( 1 ).msgEnd().get() );
        }
        else {
            this._onFatalError( res( [500, "Unsupported authentication method"] ) );
        }
    }
