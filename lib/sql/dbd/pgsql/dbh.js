...
function loadTypes () {
    ...
    for ( const name in types ) {
        ...
        if ( type.oid && type.from ) {
            for ( const oid of type.oid ) {
                TYPES.from[oid] = type.from;
            }
        }
...
module.exports = class DbhPgsql extends Dbh {
    ...
    _ON_PG_AUTHENTICATION ( data ) {
        ...
        if ( type === BACKEND.AUTHENTICATION_OK ) {
            ...
            this._push( FRONTEND.PASSWORD_MESSAGE, Buffer.from( ( this.#options.password || "" ) + "\0", "utf8" ) );
            ...
            this._push( FRONTEND.PASSWORD_MESSAGE, Buffer.from( hash + "\0" ) );
    ...
    _exec ( resolve, query, params, maxRows ) {
        ...
        if ( useSimpleQuery ) {
            this._push( FRONTEND.QUERY, Buffer.from( query[0] + "\0", "utf8" ) );
        }
        ...
        // extended query mode
            ...
            if ( !this.#sth.parseComplete ) {
                this._push( FRONTEND.PARSE, Buffer.from( queryId + "\0" + this.#sth.query + "\0\0\0" ) );
            }
            ...
            // bind, create portal
            this._push( FRONTEND.BIND, Buffer.concat( [Buffer.from( portalId + "\0" + queryId + "\0" ), ...paramsFormatCodes, ...paramsVals, ZERO, ZERO] ) );
            ...
            // execute
            this._push( FRONTEND.EXECUTE, Buffer.concat( [Buffer.from( portalId + "\0" ), maxRows] ) );
