...
module.exports = class DbhPoolPgsql extends DbhPool {
    ...
    async _getDbh () {
        ...
        // backlog is full
        if ( this.#options.backLog && this.#poolLength >= this.#options.backLog ) return res( [500, "Dbh is not available"] );
    ...
    async do ( query, params ) {
        ...
        const result = await dbh.data.do( query, params );
    ...
    async selectAll ( query, params ) {
        ...
        const result = await dbh.data.selectAll( query, params );
    ...
    async selectRow ( query, params ) {
        ...
        const result = await dbh.data.selectRow( query, params );
