...
module.exports = class DbhPoolPgsql extends DbhPool {
    ...
    #removeListener ( event ) {
        ...
        if ( !this.listenerCount( event ) ) {
            delete this.#notifications[event];

            if ( this.#notificationsHandle ) this.#notificationsHandle.do( `UNLISTEN "${event}"` );
        }
    }
    ...
    #checkNotificationsHandle () {
        ...
        for ( const event in this.#notifications ) this.#notifications[event] = false;
    ...
    async #setupNotifications () {
        for ( const event in this.#notifications ) {
            if ( !this.#notifications[event] ) {

                // create handle if not created
                if ( !this.#notificationsHandle ) this.#notificationsHandle = new PgsqlDbh( this, this.#options );

                const res = await this.#notificationsHandle.do( `LISTEN "${event}"` );

                if ( !res.ok ) return this.#checkNotificationsHandle();
            }
        }
