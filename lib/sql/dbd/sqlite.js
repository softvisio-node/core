...
export default class DbhPoolSqlite extends DbhPool {
    ...
    #do ( query, params, firstRow, ignoreData ) {
        ...
        // ignore rows for "select" requests
        const ignoreRows = !ignoreData;
        ...
        try {
            ...
            if ( query[1] ) {
                res = sth[method]( this.#prepareParams( query[1] ) );
            }
            else {
                res = sth[method]();
            }
            ...
            if ( ignoreData ) {
                ...
                const res1 = result( 200 );

                if ( !ignoreRows ) res1.rows = res.changes;
