...
module.exports = class DbhPoolSqlite extends DbhPool {
    ...
    async do ( query, params, options ) {
        ...
        if ( query[1] || queryId ) {
            ...
            if ( queryId ) sth = this.#prepared[queryId];
            ...
            // cache sth
            if ( queryId ) this.#prepared[queryId] = sth;
            ...
            try {
                ...
                // run with params
                if ( query[1] ) {
                    res = sth.run( this._prepareParams( query[1] ) );
                }
    ...
    async _select ( query, params, selectRow ) {
        ...
        try {
            ...
            if ( query[1] ) {
                res = sth[selectRow ? "get" : "all"]( this._prepareParams( query[1] ) );
            }
            else {
                res = sth[selectRow ? "get" : "all"]();
            }

            if ( !selectRow && !res.length ) res = null;

            if ( res && decode ) {
                if ( selectRow ) res = [res];
