...
export default class DbhPoolSqlite extends DbhPool {
    ...
    #do ( query, params, firstRow, ignoreData ) {
        ...
        if ( ignoreData && sth.reader ) throw Error( `Invalid usage, you need to execute query, that returns data, using "select" method, ` + query[0] );

        if ( !ignoreData && !sth.reader ) throw Error( `Invalid usage, you need to execute query, that returns no data, using "do" method, ` + query[0] );
