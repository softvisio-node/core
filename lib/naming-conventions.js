...
// XXX
export function toKebabCase ( string, { allowProtected, strict = true } = {} ) {
    var protectedPrefix = "";
    ...
    // strict camel case
        ...
        string = string
            .replaceAll( /([A-Z])([A-Z]+)([\da-z])?/g, ( match, letter, body, postfix ) => {
                if ( postfix ) {
            ...
            .replaceAll( /([\da-z])([A-Z])/g, ( match, prefix, letter ) => prefix + "-" + letter.toLowerCase() )
            .toLowerCase();
    }

    // camel case
    else {
        string = string.replaceAll( /(?<!^)([A-Z])/g, ( match, letter ) => "-" + letter.toLowerCase() ).toLowerCase();
    }
...
// XXX
export function toSnakeCase ( string, { allowProtected, strict } = {} ) {
    return toKebabCase( string, { allowProtected, strict } ).replaceAll( "-", "_" );
}

// XXX
export function toConstantCase ( string, { allowProtected } = {} ) {
    var protectedPrefix = "";
    ...
    string = string.toUpperCase().replaceAll( /[_-]+/g, "_" );

    return protectedPrefix + string;
}

// XXX
export function toCamelCase ( string, { allowProtected, strict } = {} ) {
    return toKebabCase( string, { allowProtected, strict } ).replaceAll( /-([a-z])/g, ( match, letter ) => letter.toUpperCase() );
}

// XXX
export function toPascalCase ( string, { allowProtected, strict } = {} ) {
