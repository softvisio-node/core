...
export default class UserAgent {
    ...
    static createChromiumUserAgentString ( brand, platform, userAgentVersion ) {
        ...
        // resolve brand
        brand = BROWSER_FAMILY_BRANDS[ brand.toLowerCase() ]?.name;
        if ( !brand ) throw new Error( `Chromium brand is not valid` );

        // resolve platform
        platform = OS_FAMILY_PLATFORM[ platform.toLowerCase() ] || platform;
        if ( !CHROMIUM_PLATFORMS[ platform ] ) throw new Error( `Chromium platform is not valid` );

        // chromium
        if ( brand === "Chromium" ) {
            return `Mozilla/5.0 (${ CHROMIUM_PLATFORMS[ platform ].platform }) AppleWebKit/537.36 (KHTML, like Gecko) Chromium/${ majorVersion }.0.0.0${ CHROMIUM_PLATFORMS[ platform ].mobile
                ? " Mobile"
                : "" } Safari/537.36`;
        }
        ...
        // chrome
            ...
            return `Mozilla/5.0 (${ CHROMIUM_PLATFORMS[ platform ].platform }) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/${ majorVersion }.0.0.0${ CHROMIUM_PLATFORMS[ platform ].mobile
                ? " Mobile"
                : "" } Safari/537.36`;
        }
