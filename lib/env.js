...
class Env extends _Env.constructor {
    ...
    get root () {
        if ( this.#root == null ) {
            let _path = isMainThread ? process.argv[1] : process.env.MAIN_THREAD_ARGV1;
    ...
    applyEnv ( env, { envPrefix, trim, overwrite } = {} ) {
    ...
    loadEnv ( { path, envPrefix = "APP_", configPrefix = "env", overwrite } = {} ) {
        const env = [],
            root = path || this.root,
        ...
        this.applyEnv( env, { envPrefix, overwrite } );

        return config;
    }

    loadUserEnv ( { overwrite } = {} ) {
        if ( !this.#userEnvLoaded ) {
            ...
            if ( configPath ) {
                this.applyEnv( readConfig( url.pathToFileURL( configPath ) ), { overwrite } );
            }
        }
    }
