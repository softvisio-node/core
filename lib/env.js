...
class Env extends _Env.constructor {
    ...
    get root () {
        if ( this.#root == null ) {
            let root = fs.realpathSync( url.fileURLToPath( global[Symbol.for( "mainThreadArgv1" )] || process.argv[1] ) );
    ...
    // public
    loadEnv ( { location, mode, defaultConfig, mergeEnv = true, overwriteEnv, envPrefix = "APP_" } = {} ) {
        location ||= this.root;
        mode ||= this.mode;
        ...
        for ( const file of files ) {
            ...
            const config = readConfig( location + "/" + file );
