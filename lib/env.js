...
import _Env from "#internals/env/common";
import os from "os";
import url from "url";
...
class Env extends _Env {
    ...
    readConfig ( options = {} ) {
        ...
        if ( mode ) files.push( `.config.${mode}.yaml`, `.config.${mode}.local.yaml` );
    ...
    async getUserConfig () {
        if ( this.#userConfig == null ) {
            if ( !fs.existsSync( os.homedir() + "/.softvisio/config.js" ) ) {
                this.#userConfig = {};
            }
            else {
                this.#userConfig = ( await import( url.pathToFileURL( os.homedir() + "/.softvisio/config.js" ) ) ).default;
            }
        }
