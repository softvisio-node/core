...
class Env extends _Env.constructor {
    ...
    isGitPackageRoot ( root ) {
        return this.isPackageRoot( root ) && fs.existsSync( root + "/.git" );
    }

    findPackageRoot ( root ) {
        root = path.normalize( path.resolve( root || process.cwd() ) );

        while ( true ) {
            if ( this.isPackageRoot( root ) ) return root;
    ...
    findGitPackageRoot ( root ) {
        root = path.normalize( path.resolve( root || process.cwd() ) );

        while ( true ) {
            if ( this.isGitPackageRoot( root ) ) return root;
