...
class Env extends _Env.constructor {
    ...
    mergeEnv ( env, { target, prefix, overwrite } = {} ) {
        target ||= process.env;
    ...
    loadEnv ( { location, mode, envTarget, envPrefix = "APP_", envOverwrite } = {} ) {
        const env = [],
            configPrefix = "env",
            ...
            merge = function ( a, b ) {
                for ( const prop in b ) {
                    if ( objectIsPlain( a[prop] ) && objectIsPlain( b[prop] ) ) {
                        merge( a[prop], b[prop] );
                    }
                    else {
                        a[prop] = b[prop];
        ...
        if ( envTarget ) config.env = mergedEnv;

        return config;
    }

    loadUserEnv ( { envTarget, envOverwrite } = {} ) {
        if ( !this.#userEnv ) {
            this.#userEnv = {};
            ...
            if ( configPath ) {
                this.#userEnv = this.mergeEnv( readConfig( url.pathToFileURL( configPath ) ), {
                    "target": envTarget,
                    "overwrite": envOverwrite,
                } );
            }
