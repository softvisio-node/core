import softvisioApiRequest from "./api/Request.js";
import softvisioApiResponse from "./api/Response.js";
import softvisioApiUpload from "./api/Upload.js";

export default class softvisioApi {
    onEvent = null;
    ...
    constructor ( args ) {
        ...
        if ( args.onConnect ) {this.onConnect = args.onConnect;}

        if ( args.onDisconnect ) {this.onDisconnect = args.onDisconnect;}

        if ( args.onEvent ) {this.onEvent = args.onEvent;}

        if ( args.onRpc ) {this.onRpc = args.onRpc;}
    ...
    auth ( token ) {
        ...
        // Reset connection
        const res = new softvisioApiResponse( [1012, "Service Restart"] );
        this._reset( res );
    ...
    call () {
        ...
        if ( arguments.length > 1 ) {
            if ( typeof arguments[arguments.length - 1] === "function" ) {
                ...
                if ( arguments.length > 2 ) {args = Array.prototype.slice.call( arguments, 1, -1 );}
        ...
        if ( String.prototype.substring.call( method, 0, 1 ) !== "/" ) {method = `/${this.#version}/${method}`;}
    ...
    upload ( file, method, onProgress ) {
        return new softvisioApiUpload( this, file, method, onProgress );
    }
    ...
    _onDisconnect ( e ) {
        ...
        const res = new softvisioApiResponse( [e.code, e.reason || "Abnormal Closure"] );
    ...
    _onMessage ( e ) {
        ...
        // RPC
            ...
            if ( tx.method ) {
                ...
                if ( this.onRpc ) {
                    const req = new softvisioApiRequest( this, tx.tid, this.#connId ),
                        func = function () {
            ...
            // RPC callback, tid is required
                ...
                if ( this.#tidCallbacks[tx.tid] ) {
                    ...
                    softvisioApiResponse.create( tx.result );
    ...
    _reset ( res ) {
        ...
        for ( const tid in callbacks ) {callbacks[tid]( res );}
