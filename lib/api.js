...
class Upload extends UploadCommon {
    ...
    constructor ( api, method, file, data, onProgress ) {
        ...
        else if ( file.path ) this.#filename = path.basename( file.path );
        else this.#filename = "blob";

        // type
        if ( file.type ) this.#type = file.type;
        else {
            const type = mime.getByFilename( this.#filename );
        ...
        else if ( file.path ) this.#size = fs.statSync( file.path ).size;
        else this.#size = file.data.length;
        ...
        formData.append( "file", file.data || fs.createReadStream( file.path ), {
