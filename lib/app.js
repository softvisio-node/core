...
export default class App {
    ...
    #publish ( source, name, args ) {
        ...
        if ( name.startsWith( "api/" ) || name.startsWith( "/api/" ) ) {
            ...
            if ( !this.api?.frontend && !cluster ) return;

            if ( name.endsWith( "/" ) ) {
                targets ??= args.shift();
                if ( !Array.isArray( targets ) ) targets = [targets];

                const targetName = name.substring( 4 ),
                    cache = {};

                for ( const target of targets ) {
                    this.api?.frontend?.forwardEvent( targetName + target, args, cache, publisherId );

                    cluster?.publish( name + target, ...args );
                }
            }
            else {
                this.api?.frontend?.forwardEvent( name.substring( 4 ), args, {}, publisherId );

                cluster?.publish( name, ...args );
            }
        }
        ...
        // to rpc
            ...
            this.rpc?.frontend?.forwardEvent( name.substring( 4 ), args, {}, publisherId );
    ...
    async #run () {
        ...
        this.#shutDownLock = process.shutdown.lock( "app" );
