...
export default class App extends Events {
    ...
    async initServices ( services ) {
        ...
        // add services
        if ( !services ) this.#services.addServicesFromEnv();
        else this.#services.addServices( services );
        ...
        // link events
        this.#services.on( "connect", service => this.#publishLocal( `service/connect/${service}` ) );
        this.#services.on( "disconnect", service => this.#publishLocal( `service/disconnect/${service}` ) );
        this.#services.on( "event", ( service, name, args ) => this.#publishLocal( `service/event/${service}/${name}`, args ) );
    ...
    async createAPI ( API, backend, options ) {
        ...
        // link events
        this.#api.on( "connect", () => this.#publishLocal( "api/connect" ) );
