...
export default class App {
    ...
    constructor ( location, _const ) {
        ...
        // this.#threads.on( "publish", ( name, args ) => this.publish( name, ...args ) );
    }
    ...
    publish ( name, ...args ) {
        ...
        if ( name === "api" ) {
            ...
            const ename = args.shift();

            this.#hub.publish( "rpc/out", ename, args );
        }
        else if ( name === "service" ) {
            const ename = args.shift();

            this.#hub.publish( "service/out", ename, args );
        }
        else if ( name.startsWith( "/" ) ) {
            this.#hub.publish( "local/out", args );
            this.#hub.publish( "global/out", args );
        }
        else {
            this.#hub.publish( "local/out", args );
        }
    ...
    async _createApi ( Api, backend, options ) {
        ...
        // link events
        // this.#api.on( "connect", () => this.publish( "api/connect" ) );
        // this.#api.on( "disconnect", () => this.publish( "api/disconnect" ) );
        // this.#api.on( "remote", ( name, auth, args ) => this.publish( "api/" + name, [auth, ...args] ) );
        // this.#api.on( "client/connect", client => this.publish( "client/connect", [client] ) );
        // this.#api.on( "client/disconnect", client => this.publish( "client/disconnect", [client] ) );
    ...
    async _createRpc ( Rpc, options ) {
        ...
        // link events
        // this.#rpc.on( "remote", ( name, args ) => this.publish( "rpc/" + name, args ) );
        // this.#rpc.on( "client/connect", client => this.publish( "client/connect", [client] ) );
        // this.#rpc.on( "client/disconnect", client => this.publish( "client/disconnect", [client] ) );
