...
export default class App extends Events {
    ...
    async createAPI ( API, backend, options ) {
        ...
        // link events
        this.#api.on( "event", ( name, auth, args ) => {
            this.#publishLocal( "api/event/" + name, auth, args );
        } );
    ...
    async createRPC ( RPC, options ) {
        ...
        // link events
        this.#rpc.on( "listening", () => {
            this.#cluster.subscribe( "rpc" );
        } );

        this.#rpc.on( "event", ( name, args ) => {
            this.#publishLocal( "rpc/event/" + name, args );
        } );
