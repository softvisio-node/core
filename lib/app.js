...
export default class App {
    ...
    async run () {
        ...
        res = appConfig.validateAppConfig( this, this.#config, this.#env );
        console.log( res + "" );
        if ( !res.ok ) {
        ...
        // create threads
        if ( this.#config.threadsEnabled ) this._createThreads();
        ...
        if ( this.#privateHttpServer ) {
            ...
            res = await this.#privateHttpServer.listen( { "address": "0.0.0.0", "port": this.#config.privateHttpServerPort } );

            if ( res.ok ) console.log( `listening on 0.0.0.0:${this.#config.privateHttpServerPort}` );
            else console.log( `unable bind to the 0.0.0.0:${this.#config.privateHttpServerPort}` );
        ...
        if ( this.#publicHttpServer ) {
            ...
            res = await this.#publicHttpServer.listen( { "address": "0.0.0.0", "port": this.#config.publicHttpServerPort } );

            if ( res.ok ) console.log( `listening on 0.0.0.0:${this.#config.publicHttpServerPort}` );
            else console.log( `unable bind to the 0.0.0.0:${this.#config.publicHttpServerPort}` );
