...
export default class App {
    ...
    get locale () {
        this.#locale ??= this.locales.getAppLocale();
    ...
    async #start () {
        ...
        // XXX validate app-public-config.schema.yaml
        ...
        // load components
        res = await this.#components.load( process.cli.options.service || config.defaultService );
        if ( !res.ok ) return this.#logError( res );
        ...
        // create components
        res = await this.#components.create( config.components );
        if ( !res.ok ) return this.#logError( res );
        ...
        // post start components
        if ( !this.#isShuttingDown ) {
            res = await this.#components.postStart();
            if ( !res.ok ) return this.#logError( res );
