...
export default class App {
    ...
    #configureEvents () {
        ...
        if ( this.threads ) {
            ...
            if ( this.cluster ) {
                this.threads.link( this.cluster, {
                    "on": name => {
                        if ( name.startsWith( "/" ) ) return name;
                    },
                    "forwarder": ( name, args ) => this.threads.publish( name, ...args ),
                } );
            }
        ...
        if ( this.api ) {
            ...
            if ( this.cluster ) {
                this.api.frontend.link( this.cluster, {
                    "on": name => "api/" + name,
                    "forwarder": ( name, args ) => this.api.frontend.publish( name, ...args ),
                } );
            }
        ...
        if ( this.rpc ) {
            ...
            if ( this.cluster ) {
                this.rpc.frontend.link( this.cluster, {
                    "on": name => "rpc/" + name,
                    "forwarder": ( name, args ) => this.rpc.frontend.publish( name, ...args ),
                } );
            }
