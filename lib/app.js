...
export default class App {
    ...
    constructor ( location, config ) {
        ...
        this.#hub.forwardListener( this.#threads.hub, "out", "threads/out", ( queue, name, args ) => this.#threads.publish( name, ...args ) );
        this.#threads.hub.forward( "in", ( name, args ) => this.publish( ":threads", name, args ) );
    }
    ...
    publish ( name, ...args ) {
        ...
        if ( name === "api" ) {
            ...
            for ( const target of targets ) {
                ...
                if ( this.#cluster ) this.#cluster.publish( "api/out", target + "/" + name, [msg] );
            }
        }
        ...
        // rpc
            ...
            if ( this.#cluster ) this.#cluster.publish( "rpc/out", name, [msg] );
        }
        ...
        // from cluster
            ...
            this.#hub.publish( "threads/out", name, args );
        ...
        // from threads
            ...
            if ( this.#cluster && name.startsWith( "/" ) ) this.#cluster.publish( "local", name, args );
            this.#hub.publish( "local", name, args );
        }
    ...
    async _initCluster ( namespace, api ) {
        ...
        if ( !namespace || !api ) {
            ...
            this.#cluster = new Cluster( namespace, this, api );
