import "#lib/result";
import Components from "#lib/app/components";
import Events from "#lib/events";
import Cli from "#lib/app/cli";
import env from "#lib/env";
import { mergeObjects } from "#lib/utils";
import Ajv from "#lib/ajv";
import { readConfig } from "#lib/config";

export default class App {
    #config;
    #components;
    #localEvents = new Events();

    // static
    static get Cli () {
        return Cli;
    }

    // properties
    get location () {
        throw `App location getter is not defined`;
    }

    get components () {
        this.#components ??= new Components( this.location );

        return this.#components;
    }

    get config () {
        return this.#config;
    }

    // public
    async run () {

        // load app env
        const config = env.loadEnv();

        var res;

        // load components
        res = await this.components.load();
        if ( !res.ok ) return res;

        // merge app config
        this.#config = mergeObjects( {}, res.data, config.config );

        // validate app config
        try {
            const validate = new Ajv().compile( readConfig( "#resources/schemas/app.connfig.schema.yaml", { "resolve": this.location } ) );

            if ( !validate( this.#config ) ) {
                res = result( [400, `App config is not valid: ` + validate.errors] );

                console.log( res + "" );

                return res;
            }
        }
        catch ( e ) {}

        // create components
        res = await this.components.create( this, config.components );
        if ( !res.ok ) return res;

        // configure components
        res = await this.components.configure();
        if ( !res.ok ) return res;

        // configure app
        res = await this._configure();
        if ( !res.ok ) return res;

        // install components
        res = await this.components.install();
        if ( !res.ok ) return res;

        // initialize components
        res = await this.components.initialize();
        if ( !res.ok ) return res;

        // initialize app
        res = await this._initialize();
        if ( !res.ok ) return res;

        // configure events
        this.#configureEvents();

        // run components
        res = await this.components.run();
        if ( !res.ok ) return res;

        // run app
        res = await this._run();
        if ( !res.ok ) return res;

        return res;
    }

    // events
    on ( name, listener ) {
        this.#subscribe( "on", name, listener );
    }

    once ( name, listener ) {
        this.#subscribe( "once", name, listener );
    }

    off ( name, listener ) {
        this.#subscribe( "off", name, listener );
    }

    publish ( name, ...args ) {
        this.#publish( "local", name, args );
    }

    // protected
    async _configure () {
        return result( 200 );
    }

    async _initialize () {
        return result( 200 );
    }

    async _run () {
        return result( 200 );
    }

    // private
    #configureEvents () {

        // configure threads events
        if ( this.threads ) {

            // forward global subscriptions from the threads to the cluster
            if ( this.cluster ) {
                this.threads.forwardSubscriptions( this.cluster, {
                    "on": ( name, listener ) => {
                        if ( name.startsWith( "/" ) ) this.cluster.on( name, listener );
                    },
                    "off": ( name, listener ) => {
                        if ( name.startsWith( "/" ) ) this.cluster.off( name, listener );
                    },
                } );
            }

            // re-publish events from threads
            this.threads.on( "*", ( name, args ) => this.#publish( "threads", name, args ) );
        }

        // config rpc events
        if ( this.rpc ) {

            // forward subscriptions from the rpc to the cluster
            if ( this.cluster ) {
                this.rpc.forwardSubscriptions( this.cluster, {
                    "on": ( name, listener ) => this.cluster.on( "rpc/" + name, listener ),
                    "off": ( name, listener ) => this.cluster.off( "rpc/" + name, listener ),
                } );
            }

            // re-publish events from rpc
            this.rpc.on( "*", ( name, args ) => this.#localEvents.emit( "rpc/" + name, ...args ) );
        }

        // cpnfigure api events
        if ( this.api ) {

            // forward subscriptions from the api to the cluster
            if ( this.cluster ) {
                this.api.forwardSubscriptions( this.cluster, {
                    "on": ( name, listener ) => this.cluster.on( "api/" + name, listener ),
                    "off": ( name, listener ) => this.cluster.off( "api/" + name, listener ),
                } );
            }

            // re-publish events from api
            this.api.on( "*", ( name, args ) => this.#localEvents.emit( "api/" + name, ...args ) );
        }
    }

    #subscribe ( method, name, listener ) {
        this.#localEvents[method]( name, listener );

        if ( name.startsWith( "/" ) && this.cluster ) this.cluster[method]( name, listener );
    }

    #publish ( source, name, args ) {

        // to api
        if ( name.startsWith( "api/" ) || name.startsWith( "/api/" ) ) {
            if ( !this.api ) return;

            let global;
            if ( name.startsWith( "/" ) ) {
                name = name.substring( 1 );
                global = this.cluster;
            }

            if ( name.endsWith( "/" ) ) {
                const targets = Array.isArray( args[0] ) ? args.shift() : [args.shift()],
                    targetName = name.substring( 4 ),
                    cache = {};

                for ( const target of targets ) {
                    this.api.forwardEvent( targetName + target, args, cache );
                    if ( global ) this.cluster.publish( name + target, ...args );
                }
            }
            else {
                this.api.publish( name.substring( 4 ), ...args );
                if ( global ) this.cluster.publish( name, ...args );
            }
        }

        // to rpc
        else if ( name.startsWith( "rpc/" ) || name.startsWith( "/rpc/" ) ) {
            if ( !this.rpc ) return;

            let global;
            if ( name.startsWith( "/" ) ) {
                name = name.substring( 1 );
                global = this.cluster;
            }

            this.rpc.forwardEvent( name.substring( 4 ), args, {} );
            if ( global ) this.cluster.publish( name, ...args );
        }

        // other
        else {

            // to local emitter
            this.#localEvents.emit( name, ...args );

            // to threads, do not re-publish, if published from threads
            if ( source !== "threads" ) this.threads.publish( name, ...args );

            // global events to cluster
            if ( name.startsWith( "/" ) && this.cluster ) this.cluster.publish( name, ...args );
        }
    }
}
