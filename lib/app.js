...
export default class App {
    ...
    async #run () {
        ...
        res = this.#validateAppConfig( "public-config", config );
        if ( !res.ok ) return this.#logError( res );

        // create components
        this.#components = new Components( this.location, process.cli.options.service || config.service );
    ...
    #validateAppConfig ( schema, config ) {
        ...
        if ( !ajv ) {
            ...
            if ( schemaPath ) {
                ajv = this.#ajv = new Ajv().addSchema( readConfig( schemaPath ) );
            }
        }
