...
module.exports = class extends EventEmitter {
    ...
    // LOAD BALANCER
    installLoadBalancerConfig () {
        if ( !process.env.APP_LOADBALANCER_ID ) return;
        if ( !process.env.APP_LOADBALANCER_HOST ) return;
        if ( !process.env.APP_LOADBALANCER_PORT ) return;

        const nginx = require( "./api/nginx" );

        nginx.installLoadBalancerConfig( process.env.APP_LOADBALANCER_ID,
            process.env.APP_LOADBALANCER_PORT,
