...
export default class App {
    ...
    forwardClientEvent ( type, publisherId, name, msg ) {
        this.#localEvents.emit( `${type}/out:${name}`, name, msg, {}, publisherId );
    }
    ...
    #subscribe ( source, method, name, listener ) {
        ...
        // threads subscriptions
            ...
            this.#threadsEvents[method]( name, listener );
    ...
    #publish ( source, name, args ) {
        ...
        if ( name.startsWith( "api/in:" ) ) {
            this.#localEvents.emit( "api/" + name.substring( 4 ), ...args );
        }

        // from rpc client: rpc/in:name -> rpc/name
        else if ( name.startsWith( "rpc/in:" ) ) {
            this.#localEvents.emit( "rpc/" + name.substring( 4 ), ...args );
        }
        ...
        // shortcut, to api clients
            ...
            for ( const target of targets ) {
                this.#localEvents.emit( `api/out:${target}/${name}`, name, args, cache );
        ...
        // shortcut, to rpc clients
            ...
            this.#localEvents.emit( `rpc/out:${name}`, name, args, cache );
        ...
        // other
            ...
            if ( source !== "threads" ) this.#threadsEvents.emit( name, ...args );
