...
module.exports = class extends EventEmitter {
    ...
    emit ( name ) {
        ...
        // route "app/" events to threads
        if ( name.substr( 0, 4 ) === "app/" ) {
            sent = this.threads.emit( ...arguments );
        }

        // route "user/" events to connected clients only
        else if ( name.substr( 0, 5 ) === "user/" ) {
            return this.server.emit( ...arguments );
        }

        // route to internal emitter
        return sent && super.emit( ...arguments );
    }
