...
export default class App {
    ...
    #publish ( source, name, args ) {
        ...
        if ( name.startsWith( "api/" ) || name.startsWith( "/api/" ) ) {
            ...
            if ( name.endsWith( "/" ) ) {
                ...
                for ( const target of targets ) {
                    this.api?.frontend?.forwardEvent( targetName + target, args, cache );

                    cluster?.publish( name + target, ...args );
                }
            }
            else {
                this.api?.frontend?.publish( name.substring( 4 ), ...args );
        ...
        // to rpc
            ...
            this.rpc?.frontend?.forwardEvent( name.substring( 4 ), args, {} );
