...
module.exports = class extends EventEmitter {
    ...
    async listen () {
        ...
        // loadbalancer
        if ( process.env.APP_LOADBALANCER_ID && process.env.APP_LOADBALANCER_HOST ) {
            ip = "127.0.0.1";
            ...
            // random port
                ...
                port = await getRandomFreePort( { ip } );
            }

            // install loadbalancer config
            require( "./api/nginx" ).installLoadBalancerConfig( process.env.APP_LOADBALANCER_ID,
                port,
                process.env.APP_LOADBALANCER_HOST.split( "," ).map( host => host.trim() ) );
        }
        ...
        this.server.listen( ip, port, socket => {
