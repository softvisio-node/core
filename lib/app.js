...
export default class extends Events {
    ...
    static runCli () {
        ...
        spec.options.mode = {
            "summary": `Set application mode. Set NODE_ENV variable. Allowed values: "production", "development"`,
            "default": "production",
            "schema": {
                "type": "string",
    ...
    constructor ( options = {} ) {
        ...
        if ( process.cli && process.cli.options && process.cli.options.mode ) {
            process.env.NODE_ENV = process.cli.options.mode;
        }
    ...
    get env () {
        if ( !this.#env ) this.#env = env.readConfigs( { "mode": process.env.NODE_ENV || "production" } );
