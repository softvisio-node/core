...
export default class App {
    ...
    async run () {
        ...
        // shutdown controller
        this.#shutdownSignal = global.shutdown.signal( "application" ).on( "shutdown", this._onShutdown.bind( this ) );
        ...
        if ( this.#privateHttpServer ) {
            ...
            res = await this.#privateHttpServer.listen( { "address": this.config.privateHttpServerAddress, "port": this.#config.privateHttpServerPort } );

            this.#config.privateHttpServerPort = res.data.port;

            if ( res.ok ) console.log( `listening on ${res.data.address}:${res.data.port}` );
            else console.log( `unable bind to the ${res.data.address}:${res.data.port}` );
        ...
        if ( this.#publicHttpServer ) {
            ...
            res = await this.#publicHttpServer.listen( { "address": this.config.publicHttpServerAddress, "port": this.#config.publicHttpServerPort } );
