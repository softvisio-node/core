...
const { toMsgPack } = require( "./util/msgpack" );
...
module.exports = class extends Events {
    ...
    // API
    async _buildApi ( backend, options = {} ) {
        const Api = this.#getApiClass( backend );

        var api;

        if ( options.mixin ) {
            api = new ( options.mixin( Api ) )( this, backend, options );
        }
        else {
    ...
    async _listen () {
        ...
        if ( process.env.APP_LOADBALANCER_ID && process.env.APP_LOADBALANCER_SERVER_NAME ) {
            ...
            // random port
                ...
                port = await net.getRandomFreePort( host );
            }
