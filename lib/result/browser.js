    ...
    constructor ( status, data, props ) {
        ...
        if ( props ) {
            for ( const prop in props ) {
                if ( !RESERVED_PROPERTIES[prop] ) this[prop] = props[prop];
            }
        }
    }
...
function result ( status, data, props ) {
    ...
    if ( objectIsResult( status ) ) {
        return new Result( [status.status, status.reason], data, props );
    }

    return new Result( status, data, props );
}
...
function convertResult ( res ) {
    ...
    if ( isPlainObject( res ) ) {
        try {
            ...
            _res = new Result( [500, "Invalid api response"] );

            _res.isException = true;
        }
    }
    else {
        _res = new Result( [500, "Invalid api response"] );
