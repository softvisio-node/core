...
module.exports.confirm = async function ( text, options ) {
    const stdin = process.stdin,
        defaultAnswer = options[0].toLowerCase(),
        answers = { [defaultAnswer]: options.shift() };

    process.stdout.write( text + " [" + defaultAnswer.toUpperCase() );

    for ( const answer of options ) {
        answers[answer.toLowerCase()] = answer;

        process.stdout.write( answer.toLowerCase() );
    }
