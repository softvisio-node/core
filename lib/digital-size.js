...
export default class DigitalSize {
        ...
        if ( size ) {
            ...
            if ( typeof size === "string" ) {
                ...
                if ( bytes != null ) {
                    ...
                    if ( match[ 0 ] !== "" || match.at( -1 ) !== "" ) throw new Error( `Digital size is not valid` );

                    for ( let n = 1; n < match.length; n += 3 ) {
                        const unit = UNITS[ match[ n + 1 ] ];
                        if ( !unit ) throw new Error( `Digital size format is not valid` );

                        this.#addUnit( match[ n ], UNIT_BYTES[ unit ] );
                    }
    ...
    #addUnit ( value, unitBytes ) {
        ...
        if ( Number.isInteger( value ) ) {
            this.#bytes += value * unitBytes;
        }

        // fractional
        else {
            this.#bytes += Numeric( value ).multiply( unitBytes ).trunc().number;
        }
    }
}
