...
class MimeType {
    ...
    constructor ( mime, type, { compressible, shebangs, filenames, extnames } = {} ) {
        this.#mime = mime;
        this.#type = type;
        ...
        if ( shebangs ) {
            for ( const shebang of shebangs ) this.#shebangs.add( shebang );
        }

        if ( filenames ) {
            for ( const filename of filenames ) this.#filenames.add( filename );
        }

        if ( extnames ) {
            for ( const extname of extnames ) this.#extnames.add( extname );
        }
    }
...
class Mime {
    ...
    deleteType ( type ) {
        const mimeType = this.#types[ type ];
...
console.log( mime.get( "text/plain" ).toJSON() );
console.log( mime.getByFilename( "1.txt" )?.toJSON() );
