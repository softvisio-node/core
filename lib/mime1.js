import fs from "node:fs";
import path from "node:path";
import { readConfig } from "#lib/config";

const SHEBANG_RE = /^#!([^\n\r]+)/;

function normalizeName ( name ) {
    return name.toLowerCase();
}

function normalizeExtname ( name ) {
    name = name.toLowerCase();

    if ( !name.startsWith( "." ) ) {
        name = "." + name;
    }

    return name;
}

class MimeTypeShebangs {
    #mimeType;
    #shebangs = new Set();

    constructor ( mimeType ) {
        this.#mimeType = mimeType;
    }

    // public
    has ( shebang ) {
        return this.#shebangs.has( normalizeName( shebang ) );
    }

    add ( shebang ) {
        shebang = normalizeName( shebang );

        if ( !this.#shebangs.has( shebang ) ) {
            this.#shebangs.add( shebang );

            this.#mimeType.mime?.addShebang( this.#mimeType, shebang );
        }

        return this;
    }

    delete ( shebang ) {
        shebang = normalizeName( shebang );

        if ( this.#shebangs.has( shebang ) ) {
            this.#shebangs.delete( shebang );

            this.#mimeType.mime?.deleteShebang( shebang );
        }

        return this;
    }

    toJSON () {
        return [ ...this.#shebangs ];
    }

    [ Symbol.iterator ] () {
        return this.#shebangs.values();
    }
}

class MimeTypeFilenames {
    #mimeType;
    #filenames = new Set();

    constructor ( mimeType ) {
        this.#mimeType = mimeType;
    }

    // public
    has ( filename ) {
        return this.#filenames.has( normalizeName( filename ) );
    }

    add ( filename ) {
        filename = normalizeName( filename );

        if ( !this.#filenames.has( filename ) ) {
            this.#filenames.add( filename );

            this.#mimeType.mime?.addFilename( this.#mimeType, filename );
        }

        return this;
    }

    delete ( filename ) {
        filename = normalizeName( filename );

        if ( this.#filenames.has( filename ) ) {
            this.#filenames.delete( filename );

            this.#mimeType.mime?.deleteFilename( filename );
        }

        return this;
    }

    toJSON () {
        return [ ...this.#filenames ];
    }

    [ Symbol.iterator ] () {
        return this.#filenames.values();
    }
}

class MimeTypeExtnames {
    #mimeType;
    #extnames = new Set();

    constructor ( mimeType ) {
        this.#mimeType = mimeType;
    }

    // public
    has ( extname ) {
        return this.#extnames.has( normalizeExtname( extname ) );
    }

    add ( extname ) {
        extname = normalizeExtname( extname );

        if ( !this.#extnames.has( extname ) ) {
            this.#extnames.add( extname );

            this.#mimeType.mime?.addExtname( this.#mimeType, extname );
        }

        return this;
    }

    delete ( extname ) {
        extname = normalizeExtname( extname );

        if ( this.#extnames.has( extname ) ) {
            this.#extnames.delete( extname );

            this.#mimeType.mime?.deleteExtname( extname );
        }

        return this;
    }

    toJSON () {
        return [ ...this.#extnames ];
    }

    [ Symbol.iterator ] () {
        return this.#extnames.values();
    }
}

class MimeType {
    #mime;
    #type;
    #category;
    #subtype;
    #compressible;
    #shebangs;
    #filenames;
    #extnames;

    constructor ( mime, type, { compressible } = {} ) {
        this.#mime = mime;
        this.#type = type;
        this.#compressible = Boolean( compressible );

        [ this.#category, this.#subtype ] = this.#type.split( "/", 2 );

        this.#shebangs = new MimeTypeShebangs( this );
        this.#filenames = new MimeTypeFilenames( this );
        this.#extnames = new MimeTypeExtnames( this );
    }

    // public
    get mime () {
        return this.#mime;
    }

    get type () {
        return this.#type;
    }

    get category () {
        return this.#category;
    }

    get subtype () {
        return this.#subtype;
    }

    get compressible () {
        return this.#compressible;
    }

    get shebangs () {
        return this.#shebangs;
    }

    get filenames () {
        return this.#filenames;
    }

    get extnames () {
        return this.#extnames;
    }

    // public
    toString () {
        return this.#type;
    }

    toJSON () {
        return {
            "type": this.#type,
            "compressible": this.#compressible,
            "shebangs": this.#shebangs.toJSON(),
            "filenames": this.#filenames.toJSON(),
            "extnames": this.#extnames.toJSON(),
        };
    }

    setCompressible ( value ) {
        this.#compressible = Boolean( value );

        return this;
    }

    delete () {
        this?.mime.deleteType( this.type );

        this.mime = null;

        return this;
    }
}

class Mime {
    #types = {};
    #shebangs = {};
    #filenames = {};
    #extnames = {};

    // public
    get ( type ) {
        if ( type instanceof MimeType ) {
            type = type.type;
        }

        return this.#types[ type ];
    }

    // XXX data -> content
    getByFilename ( filename, { useShebang, content } = {} ) {
        var mimeType;

        if ( filename ) {
            mimeType = this.#filenames[ path.basename( filename ).toLowerCase() ];

            mimeType ??= this.#extnames[ path.extname( filename ).toLowerCase() ];
        }

        if ( !mimeType && useShebang ) {
            if ( content ) {
                mimeType = this.getByShebang( content );
            }
            else if ( filename && fs.existsSync( filename ) ) {
                const fd = fs.openSync( filename );

                content = Buffer.alloc( 50 );

                // read first 50 bytes
                fs.readSync( fd, content, 0, 50, 0 );

                content = content.toString( "latin1" );

                mimeType = this.getByShebang( content );
            }
        }

        return mimeType;
    }

    // XXX
    getByShebang ( content ) {

        // find shebang
        const match = content.match( SHEBANG_RE );

        if ( match ) {
            const shebang = match[ 0 ].toLowerCase();

            for ( const name in this.#shebangs ) {
                if ( shebang.indexOf( name ) > -1 ) return this.#shebangs[ name ];
            }
        }
    }

    addType ( { type, compressible, shebangs, filenames, extnames } ) {
        var mimeType = this.get( type );

        if ( mimeType ) {
            mimeType.delete();
        }

        mimeType = new MimeType( this, type, {
            compressible,
        } );

        this.#types[ mimeType.type ] = mimeType;

        if ( shebangs ) {
            for ( const shebang of shebangs ) mimeType.shebangs.add( shebang );
        }

        if ( filenames ) {
            for ( const filename of filenames ) mimeType.filenames.add( filename );
        }

        if ( extnames ) {
            for ( const extname of extnames ) mimeType.extnames.add( extname );
        }

        return this;
    }

    deleteType ( type ) {
        const mimeType = this.get( type );

        if ( mimeType ) {
            delete this.#types[ type ];

            mimeType.delete();

            for ( const value of type.shebangs ) {
                delete this.#shebangs[ value ];
            }

            for ( const value of type.filenames ) {
                delete this.#filenames[ value ];
            }

            for ( const value of type.extnames ) {
                delete this.#extnames[ value ];
            }
        }

        return this;
    }

    addShebang ( type, shebang ) {
        shebang = normalizeName( shebang );

        const mimeType = this.get( type );

        if ( !mimeType ) throw new Error( `MIME type not registered` );

        if ( !mimeType.shebangs.has( shebang ) ) {
            mimeType.shebangs.add( shebang );
        }
        else {
            const currentMimeType = this.#shebangs[ shebang ];

            if ( currentMimeType && currentMimeType.type !== mimeType.type ) {
                currentMimeType.shebangs.delete( shebang );
            }

            this.#shebangs[ shebang ] = mimeType;
        }

        return this;
    }

    deleteShebang ( shebang ) {
        shebang = normalizeName( shebang );

        const mimeType = this.#shebangs[ shebang ];

        if ( mimeType ) {
            delete this.#shebangs[ shebang ];

            mimeType.shebangs.delete( shebang );
        }

        return this;
    }

    addFilename ( type, filename ) {
        filename = normalizeName( filename );

        const mimeType = this.get( type );

        if ( !mimeType ) throw new Error( `MIME type not registered` );

        if ( !mimeType.filenames.has( filename ) ) {
            mimeType.filenames.add( filename );
        }
        else {
            const currentMimeType = this.#filenames[ filename ];

            if ( currentMimeType && currentMimeType.type !== mimeType.type ) {
                currentMimeType.filenames.delete( filename );
            }

            this.#filenames[ filename ] = mimeType;
        }

        return this;
    }

    deleteFilename ( filename ) {
        filename = normalizeName( filename );

        const mimeType = this.#filenames[ filename ];

        if ( mimeType ) {
            delete this.#filenames[ filename ];

            mimeType.filenames.delete( filename );
        }

        return this;
    }

    addExtname ( type, extname ) {
        extname = normalizeExtname( extname );

        const mimeType = this.get( type );

        if ( !mimeType ) throw new Error( `MIME type not registered` );

        if ( !mimeType.extnames.has( extname ) ) {
            mimeType.extnames.add( extname );
        }
        else {
            const currentMimeType = this.#extnames[ extname ];

            if ( currentMimeType && currentMimeType.type !== mimeType.type ) {
                currentMimeType.extnames.delete( extname );
            }

            this.#extnames[ extname ] = mimeType;
        }

        return this;
    }

    deleteExtname ( extname ) {
        extname = normalizeExtname( extname );

        const type = this.#extnames[ extname ];

        if ( type ) {
            delete this.#extnames[ extname ];

            type.extnames.delete( extname );
        }

        return this;
    }

    *[ Symbol.iterator ] () {
        for ( const mimeType of Object.values( this.#types ) ) {
            yield mimeType;
        }
    }
}

const mime = new Mime();

const DATA = readConfig( "mime-db/db.json", {
    "resolve": import.meta.url,
} );

for ( const type in DATA ) {
    mime.addType( {
        type,
        "compressible": DATA[ type ].compressible,
        "extnames": DATA[ type ].extensions,
    } );
}

export default mime;

// register custom types
// mime.registerType( "application/x-vue", {
//     "extnames": [ ".vue" ],
//     "compressible": true,
// } );

// mime.registerShebang( "bash", "application/x-sh" );
// mime.registerShebang( "sh", "application/x-sh" );
// mime.registerShebang( "node", "application/node" );

// mime.registerFilename( ".gitignore", "application/x-sh" );
// mime.registerFilename( ".dockerignore", "application/x-sh" );
// mime.registerFilename( ".lintignore", "application/x-sh" );

// mime.registerExtname ( ".dockerfile", "application/x-sh" )
// mime.registerFilename( "dockerfile", "application/x-sh" );
