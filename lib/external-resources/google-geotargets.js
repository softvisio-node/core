...
export default class GoogleGeotargets extends ExternalResourceBuilder {
    ...
    async _build ( location ) {
        ...
        const dbh = await sql.new( url.pathToFileURL( location + "/google-geotargets.sqlite" ) );
...
` );
    ...
    async #prepare () {
        if ( !this.#version ) {
            const res = await fetch( "https://developers.google.com/adwords/api/docs/appendix/geotargeting?csw=1&hl=en-US" );
            if ( !res.ok ) return res;
            ...
            this.#url = new URL( href, "https://developers.google.com/" );
            this.#url.search = "";

            this.#version = path.basename( this.#url.href ).replace( "geotargets-", "" ).replace( ".csv", "" );
        }
