...
export default class Http extends ExternalRecourceBuilder {
    ...
    async #build () {
        ...
        for ( const brand in UserAgent.chromiumBrands ) {
            if ( !UserAgent.chromiumBrands[ brand ].chromiumPlatform ) continue;

            for ( const [ platform, config ] of Object.entries( UserAgent.chromiumPlatforms ) ) {
                data[ brand + "-" + platform ] = {
                    "userAgent": UserAgent.createChromiumUserAgentString( brand, platform, this.#majorVersion ),
                    "http:": {
