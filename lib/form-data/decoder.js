...
export default class extends Events {
    ...
    async decode () {
        ...
        // no boundary
        if ( !this.#boundary ) return this.#complete( result( [400, `No multipart/form-data boundary found`] ) );

        // read first boundary
        const start = await this.#stream.readChunk( this.#boundary.length + 2 ); // "--" + boundary

        // boundary is invalid
        if ( !start || start.toString() !== "--" + this.#boundary ) return this.#complete( result( [400, `Invalid multipart/form-data`] ) );

        this.#split = new StreamSplit( { "eol": "\r\n--" + this.#boundary } );

        this.#split.on( "chunk", this.#onChunk.bind( this ) );

        this.#split.once( "error", e => this.#complete( e ) );

        this.#split.once( "complete", e => this.#complete( e ) );
