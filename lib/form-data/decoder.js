...
export default class extends Events {
    ...
    async #onChunk ( stream ) {
        ...
        // validate headers
        try {
            if ( !contentDisposition ) throw result( [500, `Content-Disposition header is missed`] );

            if ( contentDisposition.type !== "form-data" || !contentDisposition.name ) throw result( [500, `Invalid multipart/form-data`] );

            // decode name
            if ( contentDisposition.name ) contentDisposition.name = Buffer.from( contentDisposition.name, "binary" ).toString().replaceAll( "%22", `"` );

            // decode filename, encode to utf8, truncate to base name
            if ( contentDisposition.filename ) contentDisposition.filename = path.basename( Buffer.from( contentDisposition.filename, "binary" ).toString().replaceAll( "%22", `"` ) );
        }
        catch ( e ) {
            return stream.cancel( result.catch( e ) );
        }

        this.emit( "field", contentDisposition.name, stream, headers );
    }
