...
export default class extends Events {
    ...
    async decode () {
        ...
        // no boundary
        if ( !this.#boundary ) return this.#error( result( 500 ) );

        // read first boundary
        const start = await this.#stream.readChunk( this.#boundary.length + 2 ); // "--" + boundary

        // boundary is invalid
        if ( !start || start.toString() !== "--" + this.#boundary ) return this.#error( result( 500 ) );
    ...
    async #onChunk ( stream ) {
        ...
        if ( !chunk ) return stream.destroy( result( 500 ) );
        ...
        if ( !headers ) return stream.destroy( result( 500 ) );
        ...
        headers = headers.split( "\r\n" ).reduce( ( headers, header ) => {
            ...
            if ( header === "content-disposition" ) {
                ...
                if ( !parsed[0] || parsed[0][""] !== "form-data" || !parsed[0].name ) return stream.destroy( result( 500 ) );
        ...
        if ( !headers.name ) return stream.destroy( result( 500 ) );
