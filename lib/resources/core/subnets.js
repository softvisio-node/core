...
export default class TLD extends Resources.Resource {
    ...
    async build ( location ) {
        ...
        data.google.push( ...( await res.json() ).prefixes.map( prefix => prefix.ipv4Prefix || prefix.ipv6Prefix ).sort() );

        fs.writeFileSync( location + "/" + this.files[0], JSON.stringify( data, null, 4 ) );
