...
export default async function fetch ( url, options = {} ) {
    ...
    // referer
    options.referrer ||= options.headers?.referer || url;
    if ( !( options.referrer instanceof URL ) ) options.referrer = new URL( options.referrer );
...
async function request ( url, options ) {
    ...
    if ( options.referrerPolicy === "no-referrer" ) {
        ...
        return Response.error( result( [400, `Unsupported referrerPolicy option`] ) );
    }
    else if ( options.referrerPolicy === "same-origin" ) {
        if ( url.origin === options.referrer.origin ) req.setHeader( "Referer", options.referrer.href );
        else req.removeHeader( "referer" );
    }
    else if ( options.referrerPolicy === "origin" ) {
        req.setHeader( "Referer", options.referrer.origin );
    }
        ...
        if ( url.origin === options.referrer.origin ) req.setHeader( "Referer", options.referrer.href );
        else req.setHeader( "Referer", options.referrer.origin );
    }
    else if ( options.referrerPolicy === "strict-origin-when-cross-origin" ) {
        if ( url.origin === options.referrer.origin ) req.setHeader( "Referer", options.referrer.href );
        else req.setHeader( "Referer", options.referrer.origin );
    }
    else if ( options.referrerPolicy === "unsafe-url" ) {
        req.setHeader( "Referer", options.referrer.href );
    }
    else {
