...
export default async function fetch ( url, options = {} ) {
    ...
    // referer
    options.referrer = new URL( options.referrer || options.headers?.referer || url );
    options.referrer.username = "";
    options.referrer.password = "";
    options.referrer.hash = "";
    options.origin = options.referrer.origin;
...
async function request ( url, options ) {
    ...
    // credentials
    if ( options.credentials === "omit" || ( options.credentials === "same-origin" && options.origin !== url.origin ) ) {
    ...
    if ( options.referrerPolicy === "no-referrer" ) {
        ...
        if ( url.origin === options.origin ) req.setHeader( "Referer", options.referrer );
        else req.removeHeader( "referer" );
    }
    else if ( options.referrerPolicy === "origin" ) {
        req.setHeader( "Referer", options.origin );
    }
        ...
        if ( url.origin === options.origin ) req.setHeader( "Referer", options.referrer );
        else req.setHeader( "Referer", options.origin );
    }
