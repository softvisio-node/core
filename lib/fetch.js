...
var resources;
var RESOURCES = config.readConfig( "#resources/http.yaml", { "resolve": import.meta.url } );

const REDIRECT_STATUSES = new Set( [301, 302, 303, 307, 308] );

function loadResources ( resource ) {
    if ( resource.id !== "http" ) return;

    if ( !resource.isAvailable ) return;

    RESOURCES = config.readConfig( resources.location + "/" + resource.files[0] );
}
...
export default async function fetch ( url, options = {} ) {
    ...
    // init resources
    if ( !resources ) {
        const module = await import( "#lib/resources/core" );

        if ( !resources ) {
            resources = module.default;

            resources.on( "update", resource => loadResources( resource ) ).startUpdate();

            loadResources( resources.get( "http" ) );
        }
    }
    ...
    options.browser ??= "chrome-windows";
    ...
    if ( options.body != null && ( options.method === "GET" || options.method === "HEAD" ) ) {
        return Response.error( result( [400, `Request with GET/HEAD method cannot have body`] ), url, options );
    }
...
async function request ( url, options ) {
    ...
    if ( url.protocol === "data:" ) {
        ...
        if ( idx === -1 ) {
            return new Response( 200, url, options );
        }
        else {
    ...
    // invalid protocol
        ...
        return Response.error( result( [400, `Invalid URL protocol`] ), url, options );
    }
    ...
    // add default headers
    if ( options.browser ) {
        const browser = RESOURCES[options.browser];

        if ( !browser ) return Response.error( result( [400, `Browser option is invalid`] ), url, options );

        const defaultHeaders = browser?.[url.protocol];

        if ( defaultHeaders ) {
            for ( const [header, value] of Object.entries( defaultHeaders ) ) {
                if ( !req.getHeader( header ) ) req.setHeader( header, value );
            }
        }

        // user-agent
        if ( !req.getHeader( "user-agent" ) ) req.setHeader( "User-Agent", browser.userAgent );
    }
    ...
    CREDENTIALS: {
        if ( options.credentials === "same-origin" ) {
            ...
            return Response.error( result( [400, `Invalid credentials option`] ), url, options );
        }
    ...
    if ( options.referrerPolicy === "strict-origin-when-cross-origin" ) {
        ...
        return Response.error( result( [400, `Invalid referrerPolicy option`] ), url, options );
    }
    ...
    return new Promise( resolve => {
        req.once( "error", e => {
            ...
            resolve( Response.error( result.catch( e, { "silent": true, "keepError": true } ), url, options ) );
        } );
        ...
        if ( options.body != null ) {
            ...
            else stream.pipeline( options.body, req, () => {} );
        }
