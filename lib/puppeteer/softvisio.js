...
class SoftvisioPlugin extends PuppeteerExtraPlugin {
    ...
    async beforeLaunch ( options ) {
        ...
        if ( options.headless && options.profile && options.profile.windowWidth && options.profile.windowHeight ) {
            options.args.push( `--window-size=${options.profile.windowWidth},${options.profile.windowHeight}` );
        }
    ...
    async afterLaunch ( browser, options ) {
        browser.profile = options.options.profile;
    ...
    async onPageCreated ( page ) {
        ...
        if ( browser.profile && browser.profile.viewportWidth && browser.profile.viewportHeight ) {
            await page.setViewport( {
                "width": browser.profile.viewportWidth,
                "height": browser.profile.viewportHeight,
            } );
        }
        ...
        if ( browser.timezone ) {
            if ( typeof browser.timezone === "string" ) {
                ...
                if ( ip && ip.geo ) await page.emulateTimezone( ip.geo.location.time_zone );
            }
        }
