...
class SoftvisioPlugin extends PuppeteerExtraPlugin {
    ...
    async beforeLaunch ( options ) {
        ...
        if ( !( "headfulMaximized" in options ) ) {
            options.headfulMaximized = true;
        }
        ...
        if ( options.device ) {
            ...
            const maximized = !options.headless && options.headfulMaximized ? true : false;
            ...
            if ( maximized && options.device.viewport ) {
                options.device.viewport.width = 0;
                options.device.viewport.height = 0;
            }
        }

        if ( options.proxy ) {
            options._proxyServer = new ProxyServer( { "proxy": options.proxy === true ? null : options.proxy } );

            await options._proxyServer.listen();

            options.args.push( `--proxy-server=${options._proxyServer.chromeConnectUrl}` );
        }
    ...
    async afterLaunch ( browser, options ) {
        ...
        // store device config
        browser.device = options.options.device;

        // close proxy server
        if ( options.options._proxyServer ) {
            browser.proxyServer = options.options._proxyServer;

            delete options.options._proxyServer;

            browser.once( "disconnected", () => {
                browser.proxyServer.close();

                browser.proxyServer = null;
            } );
        }

        // resolve timezone
        if ( options.options.timezone ) {
            if ( typeof options.options.timezone === "string" ) {
                ...
                if ( ip && ip.geo ) browser.timezone = ip.geo.location.time_zone;
            }
            else {
                browser.timezone = await getDefaultTimezone();
            }
        }
    }
    ...
    async onPageCreated ( page ) {
        ...
        // emulate timezone
        if ( browser.timezone ) {
            await page.emulateTimezone( browser.timezone );
        }
