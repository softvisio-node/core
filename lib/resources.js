...
export default class Resources extends Events {
    ...
    constructor ( options ) {
        ...
        for ( const resource of options.resources ) {
            ...
            if ( resource.id.length > this.#maxLength ) this.#maxLength = resource.id.length;
        }
    }
    ...
    async update ( options = {} ) {
        ...
        for ( const resource of Object.values( this.#resources ) ) {
            ...
            // build
            if ( options.build ) {
                process.stdout.write( `Building "${resource.id}" `.padEnd( 12 + this.#maxLength, " " ) );
                ...
                if ( !githubToken ) {
                    ...
                    // not modified
                    if ( updated.ok && remoteIndex[resource.id]?.etag === updated.data ) {
                        updated = result( 304 );
                    }
                    ...
                    if ( updated.ok ) {
                        ...
                        remoteIndex[resource.id].etag = updated.data;
            ...
            // update
                ...
                process.stdout.write( `Updating "${resource.id}" `.padEnd( 12 + this.#maxLength, " " ) );

                localIndex[resource.id] ||= {};
                localIndex[resource.id].checked = new Date();

                // exists and not modified
                if ( localIndex[resource.id]?.etag === remoteIndex[resource.id]?.etag && resource.isConsistent( this ) ) {
                    updated = result( 304 );
                }
                else {
