...
export default class EventsClient {
    ...
    link ( hub, { send, receive } ) {
        ...
        if ( this.#remoteOut.size ) this.#remoteOut.forEach( name => this.#hubSetListener( name ) );
    }
    ...
    _onRemoteSubscribe ( name ) {
        ...
        // already subscribed
        if ( this.#remoteOut.has( name ) ) return;

        this.#remoteOut.add( name );
    ...
    _onRemoteUnsubscribeAll () {
        this.#remoteOut.forEach( name => this._onRemoteUnsubscribe( name ) );
    }
