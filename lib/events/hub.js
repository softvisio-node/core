...
export default class EventsHub {
    ...
    // forward: threads -> cluster
    // threads.on( "subscribe", name ) -> cluster.on( "name", data => threads.publish( "name", data ) );
    // threads.on( "unsubscribe", name ) -> cluster.off( "name", subscriber );
    ...
    forward ( hub ) {
        ...
        const subscribe = names => {
            for ( const name of names ) {
                ...
                // XXX filter event name using provided prefix

                this.on( name, forwarder );
