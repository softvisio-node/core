...
export default class CLICommands {
    ...
    getCommand ( command ) {
        ...
        // find matching commands
            ...
            if ( possibleCommands.length === 1 ) return this.#items[possibleCommands];
            else return possibleCommands;
        }
    }
    ...
    getHelp () {
        ...
        for ( const command of Object.values( commands ) ) {
            const spec = command.findSpec();

            if ( !spec ) {
                console.log( `Spec for command "${command.name}" not found.` );

                process.exit( 2 );
            }

            if ( this.#hasShortNames ) {
                help.push( "  " + ( command.short ? command.short + ", " : "   " ) + command.name.padEnd( maxLength, " " ) + " ".repeat( 4 ) + spec.title );
            }
            else {
                help.push( "  " + command.name.padEnd( maxLength, " " ) + " ".repeat( 4 ) + spec.title );
            }
        }
