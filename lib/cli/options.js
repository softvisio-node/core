...
module.exports = class {
    ...
    constructor ( options ) {
        ...
        for ( const name in options ) {
            ...
            if ( reservedNames[name] ) this._throwSpecError( `Option name "${name}" is reserved.` );

            if ( option.short !== false ) {
                if ( reservedNames[option.short] ) this._throwSpecError( `Short option name "${option.short}" is reserved.` );

                // short option name is already defined
                if ( this.short[option.short] != null ) {
                    this._throwSpecError( `Short option name "${option.short}" is not unique.` );
                }
                else {
        ...
        if ( this.#help == null ) {
            ...
            if ( !maxLength ) {
                ...
                for ( const name in this.items ) {
                    ...
                    help.push( "  " + spec.padEnd( maxLength, " " ) + " ".repeat( 4 ) + desc );
                }
