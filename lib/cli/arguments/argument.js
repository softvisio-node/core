...
module.exports = class {
    ...
    constructor ( name, option, startPosition ) {
        ...
        // compile json schema
        this.#validator = new Ajv( { "coerceTypes": true, "strict": false } ).compile( {
            "type": "object",
            "properties": {
                "value": this.schema,
            },
    ...
    validate () {
        ...
        if ( this.isArray ) {
            if ( this.minItems && this.value.length < this.minItems ) errors.push( `Argument "${this.name}" must be specified at least ${this.minItems} time(s).` );

            if ( this.maxItems && this.value.length > this.maxItems ) errors.push( `Argument "${this.name}" must be specified not more than ${this.maxItems} time(s).` );
        }
        else if ( this.required && this.value == null ) {
            errors.push( `Argument "${this.name}" is required.` );
        }
