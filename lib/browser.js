...
// XXX .check( { install: false } )
const CHROME_HEADLESS_SHELL = await externalResources.add( `softvisio-node/core/resources/google-chrome-headless-shell-stable-${ process.platform }-${ process.arch }`, {
        "autoUpdate": false,
    } ),
    CHROME_FOR_TESTING = await externalResources.add( `softvisio-node/core/resources/google-chrome-for-testing-stable-${ process.platform }-${ process.arch }`, {
        "autoUpdate": false,
    } );
