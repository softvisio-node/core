...
class Glob {
    ...
    find ( patterns ) {
        ...
        this.#searchList.add( patterns, { "root": this.#searchPrefix } );
        this.#ignoreList.add( this.#ignore, { "root": this.#searchPrefix } );
    ...
    #findIgnoreFiles () {
        ...
        this.#searchPrefix = path.posix.relative( root, this.#cwd );
    ...
    #readDir ( root ) {
        ...
        const searchPrefix = path.posix.join( this.#searchPrefix, root );

        this.#processIgnoreFiles( directory, searchPrefix );
        ...
        for ( const file of fs.readdirSync( directory, { "withFileTypes": true } ) ) {
            ...
            if ( file.isFile() ) {
                ...
                if ( this.#searchList.match( file.name, { "root": searchPrefix } ) && !this.#ignoreList.match( file.name, { "root": searchPrefix } ) ) {
            ...
            // directory
                ...
                if ( this.#ignoreList.match( file.name, { "root": searchPrefix } ) ) continue;

                const foundPath = path.posix.join( root, file.name );

                if ( this.#directories && this.#searchList.match( file.name, { "root": searchPrefix } ) ) {
