...
class Glob {
    ...
    constructor ( { cwd, files = true, directories, markDirectories, ignore, ignoreServiceFiles = true, ignoreFile, caseSensitive, sort, depth } = {} ) {
    ...
    find ( patterns ) {
        ...
        if ( this.#searchList.depth < this.#depth ) {
            this.#depth = this.#searchList.depth;
        }

        if ( this.#depth ) this.#readDir( "", 1 );
    ...
    #readDir ( cwdRelativeDirectory, depth ) {
        ...
        for ( const name of directories ) {
            ...
            if ( depth < this.#depth ) subDirectories.push( foundPath );
