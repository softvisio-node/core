...
module.exports = class Cluster extends Events {
    ...
    async connect ( url, rpc ) {
        ...
        if ( process.env.APP_CLUSTER_GROUPS ) this.#groups.push( ...process.env.APP_CLUSTER_GROUPS.split( /,\s*/ ).map( group => PREFIX + ":" + group + "/*" ) );

        this.#redis.once( "connect", () => this.#redis.psubscribe() );
    ...
    publish ( name, ...args ) {
        ...
        const event = Event.parseEvent( name, args );

        if ( !event.group ) return false;

        // resolve event group name
        const group = process.env["APP_CLUSTER_GROUP_" + event.group.toUpperCase()];
