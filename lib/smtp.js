...
class SmtpConnection {
    ...
    constructor ( hostname, port, tls, proxy ) {
    ...
    async _AUTH ( auth, username, password ) {
        ...
        // cram-md5
        if ( auth["CRAM-MD5"] ) {
            this.#socket.write( "AUTH CRAM-MD5\r\n" );

            res = await this._read();
            if ( res.status !== 334 ) return res;

            const challenge = Buffer.from( res.data[0], "base64" ).toString( "ascii" ),
                hmac_md5 = crypto.createHmac( "md5", password );

            hmac_md5.update( challenge );

            this.#socket.write( Buffer.from( username + " " + hmac_md5.digest( "hex" ) ).toString( "base64" ) + "\r\n" );
        ...
        // plain
            ...
            this.#socket.write( this._authPlain );
        ...
        // login
            ...
            this.#socket.write( Buffer.from( username ).toString( "base64" ) + "\r\n" );
            res = await this._read();
            if ( res.status !== 334 ) return res;
            if ( res.data[0] !== "UGFzc3dvcmQ6" ) return result( [503, STATUS_REASON[503]] );

            this.#socket.write( Buffer.from( password ).toString( "base64" ) + "\r\n" );
            res = await this._read();
        }
    ...
    async _MAIL_FROM () {
        this.#socket.write( "MAIL FROM:<" + this.#username + ">\r\n" );

        return this._read();
    }

    async _RCPT_TO ( addresses ) {

        // to
        for ( let n = 0; n < addresses.to.length; n++ ) {
            this.#socket.write( "RCPT TO:<" + addresses.to[n] + ">\r\n" );
        ...
        for ( let n = 0; n < addresses.cc.length; n++ ) {
            this.#socket.write( "RCPT TO:<" + addresses.cc[n] + ">\r\n" );
        ...
        for ( let n = 0; n < addresses.bcc.length; n++ ) {
            this.#socket.write( "RCPT TO:<" + addresses.bcc[n] + ">\r\n" );
    ...
    async _DATA ( options, addresses ) {
        this.#socket.write( "DATA\r\n" );

        var res = await this._read();
...
const Smtp = class {
    ...
    static async verifyEmail ( email, proxy ) {
        ...
        res = conn.mail_from( "test@vasyns.com" );
        if ( !res.ok ) return res;

        res = conn.rcpt_to( email.username + "@" + email.hostname );
        if ( !res.ok ) return res;
    ...
    async test () {
        ...
        try {
            socket = await this._connect( this.#hostname, this.#port, this.#tls, this.#proxy );
        }
        catch ( e ) {
