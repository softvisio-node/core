...
async function connect ( hostname, port, useTls, proxy ) {
    var socket;
...
const Smtp = class {
    ...
    static async verifyEmail ( email, proxy ) {
        ...
        try {
            socket = await connect( smtpHostname, 25, false, proxy );
        }
        catch ( e ) {
        ...
        var res = await read( socket );
        console.log( res );

        socket.write( "MAIL FROM:<" + email.username + "@" + email.hostname + ">\r\n" );
        res = await read( socket );
        console.log( res );

        socket.write( "RCPT TO:<" + email.username + "@" + email.hostname + ">\r\n" );
        res = await read( socket );
        console.log( res );
    ...
    async _connect ( hostname, port, useTls, proxy ) {
        return connect( hostname, port, useTls, proxy );
    }
