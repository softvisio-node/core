...
export function readConfig ( path, options = {} ) {
    ...
    if ( ext === ".json" ) {
        if ( options.json5 ) {
            return JSON5.parse( options.content ?? fs.readFileSync( path ) );
        }
        else {
            return JSON.parse( options.content ?? fs.readFileSync( path ) );
        }
    }

    // yaml
    else if ( ext === ".yaml" || ext === ".yml" ) {
        const data = YAML.loadAll( options.content ?? fs.readFileSync( path, "utf8" ) );
