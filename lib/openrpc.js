...
const defaultJsonSchema = "http://json-schema.org/draft-07/schema#";
...
function loadSpecValidator ( path ) {
    ...
    spec.schema["$schema"] = spec.schema["$schema"] || defaultJsonSchema;
    spec.schema.$id = "/";
...
module.exports = class OpenRpc extends Base {
    ...
    _loadSchema ( ajv, namespace, path, filename ) {
        ...
        for ( const componentClass in spec.components ) {
            for ( const componentName in spec.components[componentClass] ) {
                ...
                component.$schema = component.$schema || defaultJsonSchema;
    ...
    generate ( namespace, basePath ) {
        basePath = basePath || "/wiki/api";

        var refs = {};

        // serialize schema components
        for ( const ref in this.refs ) {
            delete this.refs[ref]["$schema"];
            delete this.refs[ref].ref;

            refs[ref] = JSON.stringify( this.refs[ref] );
        }
        ...
        for ( const path in this.specs[namespace] ) {
            const spec = this.specs[namespace][path];
...
* Methods
            ...
            for ( const method of spec.methods ) {
                md += `
    * [${method.name}](#${method.name})${method.summary ? ` - ${method.summary}` : ""}
`;
            }
...
\`\`\`javascript
let res = await $api.call( "${method.id}", ... );
\`\`\`
                `;
...
### Parameters:
                ...
                for ( const param of method.params ) {
                    // prepare schema JSON
                    let schema = JSON.stringify( param.schema );
                    ...
                    schema = YAML.stringify( JSON.parse( schema ), { "indent": 4 } );

                    md += `
* __${param.name}__ - ${param.description}

    ${param.required ? "REQUIRED" : ""} ${param.deprecated ? "DEPRECATED" : ""}
