...
var openrpcSpecValidator, openrpcComponentsValidator;

loadSpecValidator( __dirname + "/../resources/schemas/openrpc.meta.schema.yaml" );

function loadSpecValidator ( path ) {
    var ajv = new Ajv();

    var spec = fs.config.read( path );
    ...
    openrpcSpecValidator = ajv.compile( spec.openrpcSpec, "/openrpcSpec" );

    openrpcComponentsValidator = ajv.compile( spec.openrpcComponents, "/openrpcComponents" );
}

module.exports = class OpenRpc {
    specs = {};
    refs = {};
    methods = {};
    ...
    async _load ( ajv, namespace, path1 ) {
        ...
        if ( stat.isFile() ) {
            ...
            var files = await fs.readTree( path1 );
    ...
    _processSpec ( ajv, namespace, path, filename ) {
        ...
        // read spec
        var spec = fs.config.read( filename );
    ...
    _processComponentsSpec ( ajv, namespace, path, spec ) {
        ...
        // validate spec
        if ( !openrpcComponentsValidator( spec ) ) {
            throw openrpcComponentsValidator.errors;
        }
    ...
    _compileMethods ( ajv ) {
        for ( const methodId in this.methods ) {
            ...
            try {
                ...
                throw `Unable to compile "schema //${method.path}#/methods/${method.name}" ${e}`;
            }
        }
    }
    ...
    validate ( method, data ) {
        ...
        // method not found
        if ( !methodSpec ) {
            return result( [404, "Method not found"] );
        }

        // method is deprecated
        if ( methodSpec.deprecated ) {
            return result( [400, "Method is deprecated"] );
        }
        ...
        if ( methodSpec.validator( data ) ) {
            ...
            return result( 400, methodSpec.validator.errors );
        }
    }
    ...
    async generate ( outputDir ) {
        try {
            ...
            var resolveRef = function ( match, ref ) {
                ...
                if ( !refs[componentUri] ) throw `Unknown component ${componentUri}`;
            ...
            for ( const id in this.specs ) {
                ...
                for ( const method of spec.methods ) {
                    method.templateParams = method.params
                        ? ", " +
                          method.params
                              .map( ( param ) => {
                    ...
                    for ( const param of method.params ) {
                        ...
                        schema = YAML.safeDump( JSON.parse( schema ), { "indent": docIndent } );

                        // indent with 8 spaces
                        param.templateSchema = schema.replace( /^/gm, "        " );
                    }
                }
