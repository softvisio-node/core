...
const { toMsgPack, fromMsgPack } = require( "../util/msgpack" );
...
if ( isMainThread ) {
    ...
    module.exports = class ThreadsPool extends Events {
        ...
        async run ( threads ) {
            ...
            for ( const threadName in threads ) {
                ...
                for ( let n = 1; n <= num; n++ ) {
                    ...
                    const worker = new Worker( __filename, { "workerData": { "path": thread.path, "constructor": thread.constructor } } );
...
// worker thread
    ...
    const Worker = require( workerData.path );
