...
const pack = require( "notepack.io" );
            ...
            for ( const threadName in threads ) {
                ...
                for ( let n = 1; n <= num; n++ ) {
                    ...
                    worker.on( "message", async ( msg ) => {
                        msg = pack.decode( Buffer.from( msg ) );
                        ...
                        if ( msg.type === "event" ) {
                            ...
                            if ( msg.method ) {
                                if ( this.#onRpc ) {
                                    ...
                                    // regular call
                                    if ( msg.id ) {
                                        const result = parseRes( await this.#onRpc( msg.method, msg.args ) );

                                        worker.postMessage( pack.encode( {
                                            "type": "rpc",
                                            "id": msg.id,
                                            result,
                                        } ) );
    ...
    class Host {
        async call ( method, ...args ) {
            ...
            return new Promise( ( resolve ) => {
                ...
                parentPort.postMessage( pack.encode( {
                    "type": "rpc",
                    "id": id,
                    method,
                    args,
            ...
            if ( msg.method ) {
                ...
                if ( msg.id ) {
                    ...
                    if ( !worker[method] ) {
                        ...
                        try {
                            result = parseRes( await worker[method]( ...msg.args ) );
                        }
                        catch ( e ) {
                            result = res( [500, e] );
                        }
