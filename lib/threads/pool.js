...
export default class ThreadsPool {
    ...
    #setMaxThreads ( value ) {
        if ( !Number.isInteger( value ) || value <= 0 ) throw TypeError( `Semaphore maxThreads value must be integer > 0` );
    ...
    async #runThread ( method, args, highPriority ) {
        ...
        if ( this.#freeThreads && !this.#isPaused ) {
            this.#runningThreads++;
        }
    ...
    #runWaitingThreads () {
        ...
        // run pending threads
        while ( this.waitingThreads && this.#freeThreads && !this.#isPaused ) {
            this.#runningThreads++;
