...
if ( isMainThread ) {
    ...
    module.exports = class ThreadsPool extends EventEmitter {
        ...
        // EVENTS
        // @name - to all threads in group, false - to all threads in pool
        emit ( threadName, eventName, ...args ) {
            ...
            // to group only
            else if ( threadName.charAt( 0 ) === "@" ) {
                const group = this.#threads[threadName.substr( 1 )];
...
// worker thread
    ...
    parentPort.on( "message", async function ( msg ) {
        ...
        // event
        if ( msg.type === "event" ) {
            global.host.emit( msg.name, ...msg.args );
        }
