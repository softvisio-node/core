...
class SemaphoreSet extends Events {
    ...
    get ( id ) {
        ...
        if ( !semaphore ) {
            semaphore = this.#semaphores[id] = new this._Semaphore( {
                id,
                "maxThreads": this.#maxThreads,
                "maxWaitingThreads": this.#maxWaitingThreads,
                "destroyOnDone": this.#destroyOnDone,
            } );

            semaphore.on( "free", semaphore => this.emit( "free", semaphore ) );

            semaphore.on( "done", semaphore => this.emit( "done", semaphore ) );
...
export default class Semaphore extends Events {
    ...
    constructor ( { id, maxThreads, maxWaitingThreads, destroyOnDone } = {} ) {
        super();
    ...
    #runWaitingThreads () {
        ...
        if ( !this.#paused && this.totalFreeThreads ) {
            ...
            if ( !this.#runningThreads ) {
                ...
                if ( this.#destroyOnDone ) this.destroy();
            }
        }
    }
