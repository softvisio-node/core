...
export default class Semaphore extends Events {
    ...
    constructor ( { id, defaultMaxThreads, defaultMaxWaitingThreads, maxThreads, maxWaitingThreads } = {} ) {
        super();

        this.#id = id;

        defaultMaxThreads = Number( defaultMaxThreads );
        if ( isNaN( defaultMaxThreads ) || defaultMaxThreads <= 0 ) defaultMaxThreads = Infinity;
        this.#defaultMaxThreads = defaultMaxThreads;

        defaultMaxWaitingThreads = Number( defaultMaxWaitingThreads );
        if ( isNaN( defaultMaxWaitingThreads ) || defaultMaxWaitingThreads < 0 ) defaultMaxWaitingThreads = Infinity;
        this.#defaultMaxWaitingThreads = defaultMaxWaitingThreads;
    ...
    set maxThreads ( maxThreads ) {
        ...
        if ( isNaN( maxThreads ) || maxThreads <= 0 || maxThreads === Infinity ) maxThreads = this.#defaultMaxThreads;
    ...
    set maxWaitingThreads ( maxWaitingThreads ) {
        ...
        if ( isNaN( maxWaitingThreads ) || maxWaitingThreads < 0 || maxWaitingThreads === Infinity ) maxWaitingThreads = this.#defaultMaxWaitingThreads;
