...
class SemaphoreSet {
    ...
    // public
    get ( id, options ) {
        const getSemaphore = this.#getSemaphore.bind( this, id, options );

        const proxy = new Proxy( this, {
            get ( object, property ) {
                const semaphore = getSemaphore(),
                    value = semaphore[property];

                if ( typeof value === "function" ) {
                    return value.bind( semaphore );
                }
                else {
                    return value;
                }
            ...
            set ( object, property, value ) {
                ...
                return Reflect.set( semaphore, property, value );
            },
        } );
    ...
    // private
    #getSemaphore ( id, options ) {
        const semaphoreData = this.#semaphores[id];

        if ( !semaphoreData.semaphore ) {
            semaphoreData.semaphore = new this.Semaphore( {
                ...( options || {} ),
                id,
            } );

            semaphoreData.semaphore.on( "finish", () => this.#deleteSemaphore( id ) );
        }

        return semaphoreData.semaphore;
    }
