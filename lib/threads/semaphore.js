...
class SemaphoreSet1 extends Events {
    #maxThreads;
    #maxWaitingThreads;
    #destroyOnDone;
    #semaphores = {};

    constructor ( { maxThreads, maxWaitingThreads, destroyOnDone = true } = {} ) {
        super();
    ...
    async startThread ( unshift ) {
        ...
        if ( !this.freeThreads ) {
            await new Promise( resolve => {
                if ( unshift ) this.#waitingThreads.unshift( resolve );
                else this.#waitingThreads.push( resolve );
            } );
        }
        else {
            this.#runningThreads++;
