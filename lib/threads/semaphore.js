...
class SemaphoreSet {
    ...
    get ( id, options ) {
        ...
        if ( !this.#semaphores[id] ) {
            ...
            semaphore.signal.on( "finish", this.#deleteSemaphore.bind( this, id ) );
...
export default class Semaphore {
    ...
    set maxWaitingThreads ( value ) {
        if ( value == null ) {
            ...
            if ( !Number.isFinite( value ) ) {
                throw Error( `maxWaitingThreads value is invalid` );
            }
            else {
                value = Math.floor( value );

                if ( value < 0 ) throw Error( `maxWaitingThreads value is invalid` );
            }
        }
    ...
    // public
    tryDown () {
        if ( !this.freeThreads ) return false;
    ...
    up () {
        if ( !this.#runningThreads ) throw Error( `Semaphore value is negative` );
    ...
    #setMaxThreads ( value ) {
        if ( value == null ) {
            ...
            throw Error( `maxThreads value is invalid` );
        }
        else {
            value = Math.floor( value );

            if ( value <= 0 ) throw Error( `maxThreads value is invalid` );
        }
