...
module.exports = class Semaphore extends Events {
    ...
    // end current thread, wakeup one waiting thread from the waiting threads queue
    wakeup ( res ) {
        this.#runningThreads--;

        if ( this.#waitingThreads.length ) this.#waitingThreads.shift()( res );

        if ( !this.#waitingThreads.length ) this.emit( "drain" );
    }

    // end current thread, wakeup all waiting threads from the waiting threads queue
    wakeupAll ( res ) {
        this.#runningThreads--;
