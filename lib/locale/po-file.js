...
export default class PoFile {
    ...
    #readPoFile ( content ) {
        ...
        while ( lines.length ) {
            ...
            // msgctxt
                ...
                const string = this.#readPoString( line.substring( 8 ).trim(), lines, message.disabled );

                if ( string ) message.msgCtxt = string;
            }
    ...
    #writePoFile () {
        ...
        for ( const [msgId, message] of Object.entries( this.#messages ) ) {
            ...
            // msgctxt
            if ( message.msgCtxt ) text += this.#writePoString( "msgctxt", message.msgCtxt, disabled );

            // msgid
            text += this.#writePoString( "msgid", msgId, disabled );
