...
export default class PoFile {
    ...
    static loadDomains ( locales, poFilesLocation, { locale, currency } = {} ) {
        locale ||= new Locale();
    ...
    toLocale ( id ) {
        ...
        if ( this.#messages ) {
            for ( const message of Object.values( this.#messages ) ) {
                ...
                if ( !message.domains.size || message.domains.has( "~" ) ) {
                    locale.messages ??= {};
                    locale.messages[message.id] = message.translations;
                }
                ...
                for ( const domain of message.domains ) {
                    ...
                    locale.domains ??= {};
                    locale.domains[domain] ??= { id, "messages": {} };
                    locale.domains[domain].messages[message.id] = message.translations;
                }
            }
        }
