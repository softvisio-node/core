...
export default class PoFile {
    ...
    toLocale ( id ) {
        ...
        if ( this.#messages ) {
            MESSAGES: for ( const [id, message] of Object.entries( this.#messages ) ) {

                // skip disabled message
                if ( message.disabled ) continue;

                // skip fuzzy messages
                if ( message.flags?.includes( "fuzzy" ) ) continue;

                // message not translated
                if ( !message.translations[0] ) continue;

                // check plural forms translated
                if ( message.pluralId ) {
                    if ( message.translations.length !== this.nplurals ) continue;

                    for ( let n = 0; n < message.translations.length; n++ ) {
                        if ( !message.translations[n] ) continue MESSAGES;
                    }
                }
