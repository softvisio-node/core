...
export default class PoFile {
    ...
    // static
    static createLanguageDomains ( locales, poFilesLocation ) {
        const locale = new Locale();

        for ( const id of locales ) {
            const domain = new Locale( id );

            const path = poFilesLocation + "/" + domain.language + ".po";

            if ( fs.existsSync( path ) ) {
                const poFile = new PoFile( fs.readFileSync( path, "utf8" ) );

                domain.add( poFile.locale );
            }
