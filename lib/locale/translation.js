...
export default class Translation {
    ...
    constructor ( locale, single, { localeDomain, plural, pluralNumber, data } = {} ) {
        this.#locale = locale;
        this.#localeDomain = localeDomain;
        this.#pluralNumber = pluralNumber;

        if ( single?.[ Symbol.for( "ejs-template" ) ] ) {
            this.#translator = single;
            this.#data = data;

            this.#isEjs = true;
        }
        else if ( typeof single === "function" ) {
            this.#translator = single;
            this.#data = data;
        }
        else {
    ...
    clone ( { locale, localeDomain, pluralNumber, data } = {} ) {
        if ( this.#translator ) {
            ...
            return new Translation( locale ?? this.#locale, this.#single, {
                "plural": this.#plural,
                "pluralNumber": pluralNumber ?? this.#pluralNumber,
                "localeDomain": localeDomain ?? this.#localeDomain,
            } );
    ...
    #translate ( { locale, localeDomain, pluralNumber, data } = {} ) {
        ...
        if ( this.#translator ) {
            ...
            return locale.l10n( this.#single, this.#plural, pluralNumber );
        }
    }
