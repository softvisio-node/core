export default class PoFileMessage {
    ...
    get singleFormIndex () {
        return this.#poFile.singleFormIndex;
    }

    get singleFormTranslation () {
        return this.#translations?.[ this.singleFormIndex ];
    }

    get isTranslated () {

        // message not translated
        if ( !this.#translations?.[ this.singleFormIndex ] ) return false;
    ...
    toString () {
        ...
        // single
            ...
            text += this.constructor.createPoString( "msgstr", this.#translations?.[ this.singleFormIndex ], currentPrefix );
        }
    ...
    updateExtractedMessage ( { pluralId, references, flags, extractedComments, context } = {} ) {

        // plural id
        if ( pluralId ) {
            if ( this.#pluralId && this.#pluralId !== pluralId ) {
                throw Error( `Plural form conflict found for message:
...
` );
    ...
    setTranslations ( translations ) {
        if ( !translations ) {
            ...
            this.setSingleFormTranslation( translations[ this.singleFormIndex ] );
        }
        else {
    ...
    setSingleFormTranslation ( string ) {
        if ( !string ) {
            if ( !this.#translations ) return;

            this.#translations[ this.singleFormIndex ] = "";
            ...
            this.#translations[ this.singleFormIndex ] = string;
        }
    }
