...
export default class PoFileMessage {
    ...
    static mergeExtractedCommnets ( oldExtractedComments, newExtractedComments ) {
        var extractedComments;

        const domains = [...new Set( [...( oldExtractedComments?.domains || [] ), ...( newExtractedComments?.domains || [] )] )].filter( domain => domain );

        if ( domains.length ) {
            extractedComments ??= {};
            extractedComments.domains = [...domains].sort();
        }

        const comments = [...new Set( [...( oldExtractedComments || [] ), ...( newExtractedComments?.comments || [] )] )].filter( comment => comment );

        if ( comments.length ) {
            extractedComments ??= {};
            extractedComments = [...comments].sort();
        }
    ...
    toString () {
        ...
        if ( this.#extractedComments ) {
            text += this.constructor.createPoString( "#.", yaml.stringify( this.#extractedComments ), currentPrefix );
        }
    ...
    update ( { pluralId, references, flags, extractedComments, context } = {} ) {
        ...
        if ( extractedComments ) {
            this.#extractedComments = this.constructor.mergeExtractedCommnets( this.#extractedComments, extractedComments );
        }
