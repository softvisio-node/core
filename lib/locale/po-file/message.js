...
export default class PoFileMessage {
    ...
    update ( { pluralId, references, flags, extractedComments, context } = {} ) {
        ...
        if ( extractedComments ) {
            if ( extractedComments.domains ) {
                const domains = [...new Set( [...extractedComments.domains, ...( this.#extractedComments?.domains || [] )] )].filter( domain => domain );

                if ( domains.length ) {
                    this.#extractedComments ??= {};
                    this.#extractedComments.domains = [...domains].sort();
                }
            }

            if ( extractedComments.comments ) {
                const comments = [...new Set( [...extractedComments.comments, ...( this.#extractedComments?.comments || [] )] )].filter( comment => comment );

                if ( comments.length ) {
                    this.#extractedComments ??= {};
                    this.#extractedComments.comments = [...comments].sort();
                }
            }
        }
