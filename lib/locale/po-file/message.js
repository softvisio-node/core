export default class PoFileMessage {
    ...
    constructor ( poFile, id, { disabled, references, flags, domains, tags, translatorComments, extractedComments, context, contextPrevious, idPrevious, pluralId, pluralIdPrevious, translations } = {} ) {
        this.#poFile = poFile;
        this.#id = id;
    ...
    toString () {
        ...
        if ( this.#domains.size ) {
            text += `#. !domains: ${[...this.#domains].sort().join( "," )}\n`;
        }

        // tags
        if ( this.#tags.size ) {
            text += `#. !tags: ${[...this.#tags].sort().join( "," )}\n`;
        }
    ...
    update ( { pluralId, references, flags, domains, tags, extractedComments, context } = {} ) {
        ...
        if ( extractedComments ) {
            this.#extractedComments = new Set( [...this.#extractedComments, ...extractedComments] );
        }
    ...
    merge ( { pluralId, references, flags, domains, tags, extractedComments, context } = {} ) {
        ...
        // domains
        if ( domains ) this.#domains = new Set( [...domains] );

        // tags
        if ( tags ) this.#tags = new Set( [...tags] );

        // extracted comments
        if ( extractedComments ) this.#extractedComments = new Set( [...extractedComments] );
