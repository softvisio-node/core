...
const DEFAULT_PRECISION = 32,
    ...
    ROUNDING_MODES = freezeObjectRecursively( {
        "DEFAULT": DECIMAL.ROUND_HALF_UP,
        "MATH": DECIMAL.ROUND_HALF_CEIL, // like Math.round()
        "ROUND_UP": DECIMAL.ROUND_UP, // rounds away from zero
        "ROUND_DOWN": DECIMAL.ROUND_DOWN, // rounds towards zero
        "ROUND_CEIL": DECIMAL.ROUND_CEIL, // rounds towards Infinity
        "ROUND_FLOOR": DECIMAL.ROUND_FLOOR, // rounds towards -Infinity
    ...
    ROUNDING_MODES_VALUES = new Set( Object.values( ROUNDING_MODES ) ),
    MODULO_MODES = freezeObjectRecursively( {
        "DEFAULT": ROUNDING_MODES.ROUND_DOWN,
        "ROUND_UP": ROUNDING_MODES.ROUND_UP,
        "ROUND_DOWN": ROUNDING_MODES.ROUND_DOWN,
        "ROUND_FLOOR": ROUNDING_MODES.ROUND_FLOOR,
        "ROUND_HALF_EVEN": ROUNDING_MODES.ROUND_HALF_EVEN,
        "EUCLID": DECIMAL.EUCLID,
    } ),
    MODULO_MODES_VALUES = new Set( Object.values( MODULO_MODES ) ),
    NUMERICS = {};
...
function getDecimal ( precision, roundingMode, moduloMode ) {
    ...
    roundingMode ??= ROUNDING_MODES.DEFAULT;
    moduloMode ??= MODULO_MODES.DEFAULT;
...
export default class Numeric {
    ...
    // static
    static get ROUNDING_MODES () {
        return ROUNDING_MODES;
    }

    static get MODULO_MODES () {
        return MODULO_MODES;
    }
    ...
    trunc ( scale ) {
        if ( scale ) {
            return this.#clone( this.#DECIMAL.toDecimalPlaces( scale, ROUNDING_MODES.ROUND_DOWN ) );
        }
        else {
    ...
    floor ( scale ) {
        if ( scale ) {
            return this.#clone( this.#DECIMAL.toDecimalPlaces( scale, ROUNDING_MODES.ROUND_FLOOR ) );
        }
        else {
    ...
    ceil ( scale ) {
        if ( scale ) {
            return this.#clone( this.#DECIMAL.toDecimalPlaces( scale, ROUNDING_MODES.ROUND_CEIL ) );
        }
        else {
    ...
    #toPrecision ( decimal, precision, scale, roundingMode, moduloMode ) {
        ...
        else if ( !ROUNDING_MODES_VALUES.has( roundingMode ) ) {
        ...
        else if ( !MODULO_MODES_VALUES.has( moduloMode ) ) {
