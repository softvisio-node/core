...
export default class Numeric {
    ...
    static randomInteger ( { min, max, "crypto": useCrypto } = {} ) {
        ...
        if ( range.isZero ) {
            ...
            const random = Decimal.random().mul( range[ NUMERIC_ACCESSOR ].add( 1 ) ).trunc().add( min[ NUMERIC_ACCESSOR ] );
    ...
    // XXX
    static randomInteger1 ( { min, max, "crypto": useCrypto } = {} ) {
        min = BigInt( min );
        max = BigInt( max );
        ...
        if ( min === max ) {
            ...
            const range = max - min,
                random =
                    BigInt( Decimal.random( range.toString().length * 10 )
