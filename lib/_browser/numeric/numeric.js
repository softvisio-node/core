...
export default class Numeric {
    ...
    static randomInt ( { min, max, "crypto": useCrypto, precision } = {} ) {
        min = BigInt( min ?? 0 );
        max = BigInt( max ?? 0 );
    ...
    // XXX
    getRandomInt8 ( { signed, min, max, crypto, precision } = {} ) {
    ...
    // XXX
    getRandomInt16 ( { signed, min, max, crypto, precision } = {} ) {
        if ( min == null ) {
            if ( max == null ) {
                min = -32_768n;
                max = 32_767n;
    ...
    getRandomInt32 ( { signed, min, max, crypto, precision } = {} ) {
        return this.randomInt( {
            signed,
            min,
            max,
    ...
    getRandomInt53 ( { signed, min, max, crypto, precision } = {} ) {
        return this.randomInt( {
            signed,
            min,
            max,
    ...
    getRandomInt64 ( { signed, min, max, crypto, precision } = {} ) {
        return this.randomInt( {
            signed,
            min,
            max,
    ...
    getRandomInt128 ( { signed, min, max, crypto, precision } = {} ) {
        return this.randomInt( {
            signed,
            min,
            max,
