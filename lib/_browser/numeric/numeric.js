...
export default class Numeric {
    ...
    // XXX
    static randomInteger ( { min, max, "crypto": useCrypto, precision, scale } = {} ) {
        if ( !isNumeric( min ) ) {
            min = new this( min );
        }

        if ( !min.isFinite ) {
            throw TypeError( "Min value is not valid" );
        }
        ...
        if ( !max.isFinite ) {
            throw TypeError( "Max value is not valid" );
        }
        ...
        const randomScale = max.subtract( min );

        if ( randomScale.isZero ) {
            return new this( min, {
                precision,
                scale,
            } );
            ...
            const random = Decimal.random( scale || undefined )
                .mul( randomScale[ NUMERIC_ACCESSOR ].add( 1 ) )
                .trunc();

            return new this( random.add( min[ NUMERIC_ACCESSOR ] ), {
                precision,
                scale,
            } );
        }
    ...
    sqrt () {
        return this.#clone( this.#numeric.sqrt() );
    }

    cbrt () {
        return this.#clone( this.#numeric.cbrt() );
    }

    exp () {
        return this.#clone( this.#numeric.exp() );
    }
