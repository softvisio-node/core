...
export default class extends Events {
    ...
    async #call ( method, args, voidCall ) {
        ...
        if ( method.charAt( 0 ) !== "/" ) {
            if ( this.#url.href.endsWith( "/" ) ) url = this.#url.href + `${this.version}/${method}`;
            else url = this.#url.href + `/${this.version}/${method}`;
        }
        else {
            if ( this.#url.href.endsWith( "/" ) ) url = this.#url.href + method.substr( 1 );
            else url = this.#url.href + method;
        }
