...
export default class extends WebSocket {
    ...
    constructor ( url, { token, persistent, version, maxConnections, pongInterval, onRpc, cache, cacheMaxSize, cacheMaxAge, clearCacheOn } = {} ) {
        super();
        ...
        // maxConnections
        this.#maxConnections = parseInt( maxConnections ?? url.searchParams.get( "maxConnections" ) );
        if ( isNaN( this.#maxConnections ) || this.#maxConnections <= 0 ) this.#maxConnections = DEFAULT_MAX_CONNECTIONS;

        // pongInterval
        this.#pongInterval = parseInt( pongInterval ?? url.searchParams.get( "pongInterval" ) );
        ...
        // cache
        cacheMaxSize = parseInt( cacheMaxSize ?? url.searchParams.get( "cacheMaxSize" ) );
