...
export default class extends Options {
    ...
    cacheResult ( res, key, maxAge ) {
        ...
        if ( key && res.ok && !res["cache-control-no-cache"] ) {
            if ( res["cache-control-expires"] ) {
                const _maxAge = Date.parse( res["cache-control-expires"] ) - new Date();
            ...
            if ( res["cache-control-max-age"] ) {
                const _maxAge = res["cache-control-max-age"];
