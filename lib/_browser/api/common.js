...
export default class extends WebSocket {
    ...
    constructor ( url, options = {} ) {
        ...
        this.#json = this.#json === "true" || this.#json === true;
        ...
        // cache drop events
        new Set( options.dropCacheOn ?? url.searchParams.getAll( "dropCacheOn" ) ).forEach( name => {
            name.split( "," ).forEach( name => {
                name = name.trim();

                if ( this.#dropCacheOn.has( name ) ) return;

                this.#dropCacheOn.add( name );

                this.on( name, () => this.#cache.reset() );
            } );
        } );
    }
    ...
    #buildUrl () {
        ...
        for ( const name of this.#dropCacheOn ) url.searchParams.append( "dropCacheOn", name );
