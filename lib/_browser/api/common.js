...
export default class extends WebSocket {
    ...
    constructor ( url, { token, locale, persistent, version, maxConnections, onRpc, onAuthorization, cache, cacheMaxSize, cacheMaxAge, clearCacheOn } = {} ) {
        super();
        ...
        cacheMaxAge = cacheMaxAge ?? url.searchParams.get( "cacheMaxAge" || DEFAULT_CACHE_MAX_AGE );
    ...
    get websocketsUrl () {
        if ( !this.#websocketsUrl ) {
            ...
            if ( this.#maxConnections !== DEFAULT_MAX_CONNECTIONS ) {
    ...
    async cachedCall ( method, ...args ) {
        ...
        if ( typeof method === "object" ) {
            ( { method, "argumants": args, key, maxAge, signal } = method );
        }
    ...
    cacheResult ( res, key, maxAge ) {
        ...
        if ( key && res.ok && !res.meta["cache-control-no-cache"] ) {
            if ( maxAge ) maxAge = Duration.new( maxAge ).toMilliseconds();
            ...
            if ( res.meta["cache-control-max-age"] ) {
                const remoteMaxAge = new Duration( res.meta["cache-control-max-age"] ).toMilliseconds();
    ...
    #buildUrl () {
        ...
        if ( this.#cache.maxAge !== DEFAULT_CACHE_MAX_AGE ) url.searchParams.set( "cacheMaxAge", new Duration( this.#cache.maxAge ) + "" );
