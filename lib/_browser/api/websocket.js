...
export default class extends Http {
    ...
    async ping () {
        if ( !this.persistent ) return super._pingHTTP();
    ...
    async healthcheck () {
        if ( !this.persistent ) return super._healthcheckHTTP();
    ...
    async publish ( name, ...params ) {
        if ( !this.persistent ) return super._publishHTTP( name, params );

        const connection = await this.getConnection();

        if ( !connection ) return false;

        return connection.publish( name, ...params );
    }
