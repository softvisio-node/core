...
export default class extends Http {
    ...
    async publish ( name, ...params ) {
        if ( !this.persistent ) return super.publish( name, params );

        const connection = await this.getConnection();

        if ( connection ) connection.publish( name, ...params );
    }

    async call ( method, ...params ) {
        if ( !this.persistent ) return super.call( method, params );
    ...
    callVoid ( method, ...params ) {
        if ( !this.persistent ) return super.callVoid( method, params );
