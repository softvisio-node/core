...
export default class extends Http {
    ...
    // public
    async ping () {
        if ( !this.isPersistent ) return super.ping();
    ...
    async healthcheck () {
        if ( !this.isPersistent ) return super.healthcheck();
    ...
    async _connectWebSocket () {
        ...
        for ( const address of addresses ) {
            ...
            connection.on( "authorizationRequired", () => this._emit( "authorizationRequired" ) );
    ...
    #onDisconnect ( connection, res ) {
        ...
        if ( this.#activeConnections.has( connection ) ) {
            ...
            // invalid token
            else if ( res.status === 4401 ) this._emit( "signout" );
        }
