...
export default class extends Websocket {
    ...
    set cacheMax ( value ) {
        ...
        if ( isNaN( value ) ) value = DEFAULT_CACHE_MAX_SIZE;
        else if ( value < 0 ) value = DEFAULT_CACHE_MAX_SIZE;

        // not changed
        if ( this.#cache.max === value ) return;

        this.#cache.max = value;
    ...
    #buildURL () {
        ...
        if ( this.cacheMax !== DEFAULT_CACHE_MAX_SIZE ) url.searchParams.set( "cacheMax", this.cacheMax );
