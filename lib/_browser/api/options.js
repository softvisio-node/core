...
export default class extends Websocket {
    ...
    init ( url, options = {} ) {
        ...
        this.cacheMaxSize = options.cacheMaxSize ?? url.searchParams.get( "cacheMaxSize" );
    ...
    set cacheMaxSize ( value ) {
        if ( value == null || value === "" ) value = DEFAULT_CACHE_MAX_SIZE;
        else value = +value;

        if ( typeof value !== "number" || value < 0 ) value = DEFAULT_CACHE_MAX_SIZE;

        // not changed
        if ( this.#cache.maxSize === value ) return;

        this.#cache.maxSize = value;
    ...
    set cacheMaxAge ( value ) {
        ...
        if ( typeof value !== "number" ) value = DEFAULT_CACHE_MAX_AGE;

        // not changed
        if ( this.#cache.maxAge === value ) return;

        this.#cache.maxAge = value;
    ...
    #buildURL () {
        ...
        if ( this.cacheMaxSize !== DEFAULT_CACHE_MAX_SIZE ) url.searchParams.set( "cacheMaxSize", this.cacheMaxSize );
