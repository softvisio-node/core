...
export default class ApiClientConnection extends Events {
    ...
    constructor ( api, url, servername, ref ) {
        super();
        ...
        this.#api = api;
        this.#url = url;
        this.#servername = servername || this.#url.hostname;
        this.#ref = ref;
    ...
    async #connect () {
        ...
        if ( this.#url.hostname !== this.#servername ) {
            this.#ws = new WebSocket( this.#url, protocol, {
                "servername": this.#servername,
                "headers": { "Host": this.#servername },
            } );
        }
        else {
            this.#ws = new WebSocket( this.#url, protocol );
    ...
    #onClose ( status, statusText ) {
        ...
        this.emit( "disconnect", result( [status, statusText] ) );
