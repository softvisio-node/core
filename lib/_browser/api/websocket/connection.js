    ...
    async #onMessage ( msg ) {
        ...
        // response
            ...
            if ( callback ) {
                ...
                const res = result.parseRpc( msg );

                // context is disabled
                if ( res.status === -32813 ) this.emit( "contextDisabled" );

                // context deleted
                if ( res.status === -32815 ) this.emit( "contextDeleted" );

                // authorization
                if ( res.status === -32812 ) {
                    if ( this.#api.onAuthorization && ( await this.#api.onAuthorization() ) ) {
