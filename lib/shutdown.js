...
class ShutdownController {
    ...
    constructor () {
        this.#finalization = new FinalizationRegistry( id => this.#onLockDone.bind( this, true ) );
    ...
    #onProcessExit ( code ) {
        if ( this.#locks.size ) {
            for ( const lock of this.#locks.values() ) {
                console.log( `Process component forcibly terminated: ${lock.name}` );
            }
        }
    ...
    #onLockDone ( finalization, id ) {
        ...
        if ( this.#isShuttingDown ) this.#checkLocks();
    }
}
