...
class MIME {
    registerContentType ( id, extnames, compressible ) {
        if ( ID_IDX[id] ) throw `MIME content type "${id}" is already registered.`;
        ...
        if ( extnames ) {
            ...
            extnames = extnames.map( extname => {
                ...
                if ( EXTNAME_IDX[extname] ) throw `Extension "${extname}" is already registered.`;
    ...
    registerShebang ( shebang, id ) {
        if ( SHEBANGS[shebang] ) throw `Shebang "${shebang}" is already registered.`;
    ...
    getByFilename ( filename ) {
        return EXTNAME_IDX[path.extname( filename ).toLowerCase()];
    }
