...
export class GlobPattern {
    ...
    #compile () {
        ...
        // static pattern
            ...
            this.#matchPattern = quoteMeta( this.#matchPattern );
        }

        this.#regExp = new RegExp( this.#matchPattern, this.#caseSensitive ? "" : "i" );
...
class GlobPatternsList {
    ...
    get hasPatterns () {
        return this.hasStaticPatterns || this.hasGlobPatterns;
    }
            ...
            if ( !this.hasPatterns ) {
                ...
                for ( const pattern of this.#patterns.values() ) {
                    ...
                    // static pattern
                        ...
                        parts.push( "(?:" + quoteMeta( pattern.matchPattern ) + ")" );
                    }
                }

                this.#regExp = new RegExp( "^(?:" + parts.sort().join( "|" ) + ")$", this.#caseSensitive ? "" : "i" );
            }
        }
    ...
    #createRegExp () {
        ...
        for ( const pattern of this.#patterns.values() ) {
            ...
            if ( pattern.isMatchAlll ) continue;

            parts.push( "(?:" + pattern.matchPattern + ")" );
        }

        if ( parts.length ) {
            return new RegExp( "^(?:" + parts.sort().join( "|" ) + ")$" );
        }
    }
}
