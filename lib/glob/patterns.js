...
export default class GlobPatterns {
    ...
    constructor ( { caseSensitive = true, allowNegated = true, allowGlobstar = true, allowBraces = true, allowExtglob = true } = {} ) {
    ...
    // public
    has ( pattern, { prefix, caseSensitive, allowNegated, allowGlobstar, allowBraces, allowExtglob } = {} ) {
        if ( !pattern ) return false;

        var list;

        [ pattern, list ] = this.#createPattern( pattern, { prefix, caseSensitive, allowNegated, allowGlobstar, allowBraces, allowExtglob } );

        return list.has( pattern, { prefix } );
    }

    add ( patterns, { prefix, caseSensitive, allowNegated, allowGlobstar, allowBraces, allowExtglob } = {} ) {
    ...
    delete ( patterns, { prefix, caseSensitive, allowNegated, allowGlobstar, allowBraces, allowExtglob } = {} ) {
        if ( !Array.isArray( patterns ) ) patterns = [ patterns ];
    ...
    // private
    #createPattern ( pattern, { prefix, caseSensitive, allowNegated, allowGlobstar, allowBraces, allowExtglob } ) {
