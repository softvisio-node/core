...
export default class GlobPatternsLise {
    ...
    constructor ( { caseSensitive = true, allowBraces = true, allowBrackets = true, allowExtglob = true } = {} ) {
    ...
    // public
    has ( pattern, { prefix, caseSensitive, allowBraces, allowBrackets, allowExtglob } = {} ) {
        if ( !pattern ) return false;

        pattern = this.#createPattern( pattern, { prefix, caseSensitive, allowBraces, allowBrackets, allowExtglob } );

        return this.#patterns.has( pattern.id );
    }

    add ( patterns, { prefix, caseSensitive, allowBraces, allowBrackets, allowExtglob } = {} ) {
    ...
    delete ( patterns, { prefix, caseSensitive, allowBraces, allowBrackets, allowExtglob } = {} ) {
        if ( !Array.isArray( patterns ) ) patterns = [ patterns ];
    ...
    // private
    #createPattern ( pattern, { prefix, caseSensitive, allowBraces, allowBrackets, allowExtglob } ) {
