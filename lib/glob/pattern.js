...
export default class GlobPattern {
    ...
    constructor ( pattern, { caseSensitive = true, allowBraces = true, allowBrackets = true, allowExtglob = true, prefix } = {} ) {
        ...
        // negated pattern
        if ( this.constructor.isNegated( this.#pattern, { allowExtglob } ) ) {
            this.#isNegated = true;
        ...
        // XXX
            ...
            if ( !allowExtglob || !this.#pattern.startsWith( "!(" ) ) {
                this.#pattern = "\\" + this.#pattern;
            }
        }
    ...
    static isNegated ( pattern, { allowExtglob } = {} ) {
        if ( pattern instanceof GlobPattern ) {
            return pattern.isNegated;
        }
        else if ( pattern.startsWith( "!" ) ) {
            if ( allowExtglob && pattern.startsWith( "!(" ) ) {
                return false;
            }
            else {
                return true;
