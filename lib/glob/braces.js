...
export default class GlobBraces {
    ...
    #processBraces () {
        ...
        for ( this.#n; this.#n < this.#pattern.length; this.#n++ ) {
            ...
            // braces end
                ...
                return this.#processPatterns( state.patterns );
            }
        ...
        // incomplete pattern
        var pattern = "{" + state.patterns.join( "," );
    ...
    #processNunericSequence ( start, end, step, prefix ) {
        ...
        if ( step === 1 ) {
            let pattern = toRegexRange( prefix + start, prefix + end, {
                "capture": false,
                "shorthand": false,
                "relaxZeros": false,
            } );
...
console.log( new GlobBraces( "{-1234671243123..100}" ).expand() );
process.exit();
