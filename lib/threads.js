...
export default class Threads {
    ...
    forwardSubscriptions ( target, { on, off } = {} ) {
        this.#out.forwardSubscriptions( target, {
            on,
            off,
    ...
    async #onMessage ( thread, msg ) {
        ...
        if ( msg.method ) {
            ...
            if ( msg.method === "/subscribe" ) {
                ...
                thread._listeners[name] = ( msg, cache = {}, threadId ) => {
                    if ( threadId === thread.threadId ) return;

                    thread.postMessage( ( cache.encoded ??= msgpack.encode( msg ) ) );
                };

                this.#out.on( name, thread._listeners[name] );
            }
            ...
            // publish
                ...
                this.#publish( name, msg.params, thread.threadId );
                this.#in.publish( name, msg.params );
            }
    ...
    #onExit ( thread, code ) {
        ...
        for ( const name in thread._listeners ) {
            this.#out.off( name, thread._listeners[name] );

            delete thread._listeners[name];
        }
    }

    #publish ( name, args, threadId ) {
        if ( !this.#out.hasListeners( name ) ) return;
        ...
        this.#out.publish( name, [msg, {}, threadId] );
    }
}
